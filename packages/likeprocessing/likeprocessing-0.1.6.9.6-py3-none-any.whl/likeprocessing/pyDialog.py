...
class Dialog(Boite):
    ...
    def center(self, objet_name: [str, list[str]], groupe=False, **kwargs):
        ...
        if groupe:
            ...
            w = maxi - mini
            decalage = (self.width - w) // 2
            for o in objet_name:
                self.objet[o].setX(decalage + self.objet[o].x)
        else:
            for o in objet_name:
                if isinstance(o, list):
                    self.alligne_h(o, **kwargs)
                else:
    ...
    def same_height(self, objects_names: list[str], height):
        for o in objects_names:
            self.objet_by_name(o).height = height
    ...
    def pack(self, objet_name: [str, list[str]], **kwargs):
        ...
        for o in objet_name:
            if isinstance(o, list):
                self.alligne_h(o, posy=y + pady, **kwargs)
    ...
    def visibled(self, name: [str, list[str]]):
        if name == "all":
            for o in self.objet.values():
                o.visible = True
    ...
    def unvisibleb(self, name: [str, list[str]]):
        if name == "all":
            for o in self.objet.values():
                o.visible = False
        elif isinstance(name, list):
        ...
        for i in range(l):
            for j in range(c):
                ...
                if 0 < n < len(list_item):
                    self.addObjet(
                        Radio(self, (1 + j * self.width, i * h, self.width - 15, h), self.liste_item[n],
                              (selected == n),
                              **kwargs))
