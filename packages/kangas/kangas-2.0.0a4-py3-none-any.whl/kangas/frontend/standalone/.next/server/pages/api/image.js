...
/* harmony import */ var _config__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(39884);


const handler = async (req, res)=>{
    const { endpoint , ...query } = req.query;
    const queryString = new URLSearchParams(query).toString();
    const result = await fetch(`${_config__WEBPACK_IMPORTED_MODULE_1__/* ["default"].apiUrl */ .Z.apiUrl}${endpoint}?${queryString}`, {
        next: {
            revalidate: 100000
        }
    });
    ...
    stream__WEBPACK_IMPORTED_MODULE_0___default().pipeline(image, passthrough, (err)=>console.error(err));
...
var __webpack_exports__ = (__webpack_exec__(73829));
