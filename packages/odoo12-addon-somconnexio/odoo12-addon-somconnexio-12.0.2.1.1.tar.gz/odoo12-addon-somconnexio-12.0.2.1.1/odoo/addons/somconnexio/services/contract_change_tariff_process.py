...
class ContractChangeTariffProcess:
    ...
    def run_from_api(self, **params):
        ...
        if params['parent_pack_contract_id']:
            self._vinculate_parent_pack_contract(
                params['contract_id'], params['parent_pack_contract_id']
            )
        return self.to_dict(wiz)

    def _vinculate_parent_pack_contract(
        self, contract_id, parent_pack_contract_id
    ):
        contract = self.env['contract.contract'].browse(contract_id)
        parent_pack_contract = self.env['contract.contract'].browse(
            parent_pack_contract_id
        )
        contract.parent_pack_contract_id = parent_pack_contract
    ...
    def _prepare_create(self, params):
        ...
        if not (mobile_phone or contract_code):
            raise UserError(
               _("Either a contract_code (fiber) or a phone_number (mobile) is required to search contracts")  # noqa
            )

        if mobile_phone:
            contract = self.env['contract.contract'].sudo().search([
                ("mobile_contract_service_info_id.phone_number", "=", mobile_phone),
                '|',
            ...
            fiber_templ = self.env["product.template"].sudo().search([
                ("categ_id", '=', self.env.ref('somconnexio.broadband_fiber_service').id)  # noqa
            ])
            product = self.env["product.product"].sudo().search([
        ...
        if params.get('parent_pack_contract_id'):
            parent_pack_contract = self.env['contract.contract'].search(
                [('code', '=', params['parent_pack_contract_id'])]
            )
        else:
    ...
    def _extract_date_from_string(self, string_date):
        try:
            ...
            try:
                ...
                raise UserError(
                    _("Date with unknown format: {}".format(string_date))
                )
        return date
