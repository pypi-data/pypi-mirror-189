...
class BeamClient(beam.DoFn):
    ...
    def run(self):
        ...
        [self.run_step_pipeline(self.options, infile.name, data, step) for step in data["steps"]] 
        ...
        [self.get_step_results(self.options, infile.name, results, step) for step in data["steps"]] 

        with open(f'{infile.name}.results', "a+") as f:
            f.write(json.dumps(results))


    def run_step_pipeline(self, options=None, filename=None, data=None, step=None):  
        logger = logging.getLogger(__name__)
    ...
    def get_step_results(self, options=None, filename=None, data=None, step=None):
        logger = logging.getLogger(__name__)
