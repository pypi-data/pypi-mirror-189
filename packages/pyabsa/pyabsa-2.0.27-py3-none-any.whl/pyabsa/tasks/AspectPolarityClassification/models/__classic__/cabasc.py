...
class Cabasc(nn.Module):
    ...
    def context_attention(self, x_l, x_r, memory, memory_len, aspect_len):
        ...
        context_l, (_, _) = self.rnn_l(
            x_l, left_len
        )  # left, right context : (batch size, max_len, embedds)
    ...
    def forward(self, inputs):
        ...
        v_a = torch.div(aspect, nonzeros_aspect.unsqueeze(1)).unsqueeze(
            1
        ...
        v_s = torch.div(v_s, nonzeros_memory.unsqueeze(1)).unsqueeze(
            1
        )  # batch_size x 1 x embed_dim

        # position attention module
        if self.type == "c":
            memory = self.locationed_memory(
                memory, memory_len
        ...
        for memory_chunk in memory_chunks:  # batch_size x 1 x embed_dim
            c_i = self.linear1(
                torch.cat([memory_chunk, v_a, v_s], dim=1).view(
                    memory_chunk.size(0), -1
                )
            )
