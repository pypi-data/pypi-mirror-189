...
class JobManager:
    ...
    def add_docker_build_job(
        ...
        *,
        ...
        branch_match: tp.Optional[tp.List[tp.Tuple[str, str, str]]] = None,
    ) -> T:
        slug = self.env["slug"]
...
class DockerBuildMixin(DockerShared, _DockerBuildMixinState, Job):
    ...
    def is_up_to_date(self, env: tp.Mapping[str, tp.Any]) -> bool:
        ...
        url = f"{protocol}://{self.registry_host}/v2/{self.get_image_name()}/tags/list"
...
class _DockerBuildBranchMatchMixinState(_DockerBuildMixinState):
    branch_match: tp.List[tp.Tuple[str, str, str, tp.List[str]]]


@dc.dataclass(frozen=True)
class DockerBuildBranchMatchMixin(
    ConfigAccess, DockerBuildMixin, _DockerBuildBranchMatchMixinState
):
    def _get_tag_parts(self) -> tp.Iterator[tp.Union[bytes, str]]:
        yield from super()._get_tag_parts()
        ...
        for pkg, var, url, skip in self.branch_match:
            assert branch
            if branch in skip:
            ...
            if exists:
                print(f"found matching branch '{branch}' for '{pkg}'")
                yield sha
...
@dc.dataclass(frozen=True)
class DockerBuild(
    DockerBuildBranchMatchMixin, CommonJob, _DockerBuildBranchMatchMixinState, Job
):
    pass
