...
class DataFrameSource(DataSourceBase):
    def __init__(self, df: pd.DataFrame, max_rows=100, timestamp_col="timestamp"):
    ...
    def get_data(self) -> (pd.DataFrame, int):
        # print("get_data-------------- current_row {}".format(self._current_row))
        ...
        # print("Starting time for chunl: {}".format(chunk_start_datetime))
        if self._current_row >= self._df.shape[0]:
            # print("done with df - only 1 row left")
        ...
        for loc in range(search_start_row, search_end_row):
            ...
            # print("loc timestamp: {}".format(loc_datetime))

            if check_if_in_same_time_bucket(
                chunk_start_datetime, loc_datetime, self._time_bucket
            ):
                # print("In same time bucket {}".format(loc))
                chunk_df = chunk_df.append(self._df.iloc[self._current_row])
                self._current_row += 1
            else:
                # print("Found another time bucket: {}".format(loc))
                self._current_chunk_id += 1
                break
        # print("chunk_df {}".format(chunk_df))
