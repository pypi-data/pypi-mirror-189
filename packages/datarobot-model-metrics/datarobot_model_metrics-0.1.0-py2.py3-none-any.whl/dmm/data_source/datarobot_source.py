...
from dmm.utils import (
    hour_rounder_down,
    hour_rounder_up,
    wait_for_result_from_status_url,
    wait_for_result_raw,
)
...
class DataRobotSource(DataSourceBase):
    ...
    def get_data(self) -> (pd.DataFrame, int):
        ...
        if not self._association_id:
            raise Exception(
                "Cannot import actuals - deployment association id not found"
            )
        ...
        df = (
            pd.merge(
                prediction_df,
                self._format_acuals_df_before_merge(actuals_df),
    ...
    def _get_actuals_data_provider(self):
        """
    ...
    def _get_prediction_data_provider(self):
        """
    ...
    def _get_training_data_provider(self):
        """
    ...
    @staticmethod
    def _get_corresponding_actuals(actuals_data_provider) -> pd.DataFrame:
        """
    ...
    def _format_prediction_df(self, pred_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self._deployment.target_column in pred_df.columns:
            rename_columns.update(
                {self._deployment.target_column: ColumnName.TARGET_VALUE}
            )
    ...
    @staticmethod
    def _format_acuals_df_before_merge(actuals_df: pd.DataFrame) -> pd.DataFrame:
        """
...
class DataRobotExportBase:
    ...
    def _get_time_bucket_chunks(self):
...
class PredictionDataExportProvider(DataRobotExportBase):
    ...
    def get_last_prediction_timestamp(self, prediction_df: pd.DataFrame) -> datetime:
        last_timestamp = prediction_df[self._timestamp_col].iloc[-1]
        return parse(last_timestamp)

    def get_chunk_timestamp(self) -> datetime:
...
class Deployment:
    ...
    def __init__(self, api: DataRobotApiClient, deployment_id: str):
        ...
        self._info = api.get_deployment_info(deployment_id)
        if self._info["isTimeSeries"]:
    ...
    def type(self) -> DeploymentType:
        ...
        if target_type == "Binary":
            ...
            raise Exception(f"Unsupported model target type {target_type}")
    ...
    def positive_class_label(self) -> str:
        ...
        return self._info["positiveClassLabel"]
    ...
    def prediction_threshold(self) -> float:
        ...
        return self._info["predictionThreshold"]
...
class PushBackFrameIterator:
    ...
    def push_back(self, df: pd.DataFrame):
        """
...
class DataRobotApiClient:
    ...
    def get_export_id_sync(self, start_export_response: requests.Response) -> str:
        ...
        return json.loads(export_id_response)["id"]
    ...
    def get_actuals_export_dataset_ids(
        ...
        if data:
            ...
            logger.info("No saved rows for requested time frame")
            return []
    ...
    def get_deployment_info(self, deployment_id: str) -> dict:
        """
        Fetches deployment info (is time series, prediction threshold etc.)

        Parameters
        ----------
        deployment_id: str
        """
        return self._client.get(f"deployments/{deployment_id}/info").json()
...
class DataRobotChunksIterator:
    ...
    def _fetch_subset(
        ...
        logger.info(f"polling next dataframe... {start_dt} - {start_dt + interval}")
        ...
        try:
            ...
            logger.info(
                f"Failed to export data for {start_dt} {start_dt + interval} interval",
                e,
            )
