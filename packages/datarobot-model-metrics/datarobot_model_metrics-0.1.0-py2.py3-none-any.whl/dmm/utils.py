...
def wait_for_result_raw(client, response):
...
def wait_for_result_from_status_url(client, status_url):
    result = wait_for_async_resolution(client, status_url)
    resp = client.get(result)
    if resp.status_code != 200:
        raise Exception(f"Couldn't fetch export data response={resp.content}")
...
def hour_rounder_up(datetime):
    datetime = datetime + timedelta(hours=1)
    return datetime.replace(second=0, microsecond=0, minute=0)


def hour_rounder_down(datetime):
    return datetime.replace(microsecond=0, second=0, minute=0)
