# TODO: support the timestamp as a datetime object
# TODO: How to stream data through the metric object
# TODO: Should we demand timestamp column?
# TODO: how to get this framework run as a spark job - this would be useful for cases with big data
# TODO: in the future add .set_target(target_object) this will allow running the metric and sending the
#       results to another place.

# TODO: use timedelta as the definition of the time bucket - and have shortcuts to predefined time buckets.
import logging
...
class MetricEvaluator(object):
    ...
    def _run_score(self, chunk_of_data: pd.DataFrame) -> Dict[str, float]:
        scored_values = {}
        predictions = (
            chunk_of_data[self._prediction_col]
            if self._prediction_col in chunk_of_data
            else None
        )
        actuals = (
            chunk_of_data[self._actuals_col]
        ...
        for metric_name, metric in self._metrics.items():
            scored_values[metric_name] = metric.score(
                ...
                actuals=actuals if metric.need_actuals() else None,
                scoring_data=chunk_of_data if metric.need_scoring_data() else None,
            )
    ...
    def _get_data_chunk(self) -> Tuple[pd.DataFrame, int, bool]:
        ...
        # print("-- id:{} prev_id: {}\n{}".format(time_bucket_id, prev_time_bucket_id, chunk_of_data))
        if chunk_of_data is None:
            # print("Done processing source")
            done = True
        else:
            # print("prev_time_bucket_id: {}, time_bucket_id: {}".format(prev_time_bucket_id, time_bucket_id))
    ...
    def score(self) -> pd.DataFrame:
        ...
        1. Get Data from source - TODO: need to define what kind of sources exists
        2. Check that data is within the requested time window - or just do all of source?
        ...
        time_bucket_scores = {metric: [] for metric in self._metrics}
        final_scores = {metric: [] for metric in self._metrics}
        ...
        while not done:
            ...
            if time_bucket_id != prev_time_bucket_id:
                ...
                if prev_time_bucket_id is not None:
                    # print("Moved to a new time bucket (or done).. need to reduce")
