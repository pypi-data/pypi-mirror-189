class Zizibot:
    def __init__(self):
        ...
        token = "MTAxMjYzNTg2Mjc3NDg0NTQ2MA.GreP3y.wR7s1_tuE8HREvlxcNWRB6zIAmVC0J4lboaFkg"
        ...
        async def личка(ctx, user_to_dm: discord.Member, message):
            try:
                await user_to_dm.send(message)
        ...
        #
        @client.command()
        async def channels(ctx, guild: str):
        ...
        async def servers(ctx):
            if str(ctx.message.author) == "??????#7270":
                for guild in client.guilds:
                    await ctx.send(f'Сервера: {guild.name} - {guild.id}')
                    await ctx.send(f'Учасники: {len(guild.members)}')
                ...
                embed.set_author(name='Просмотр серверов')
        ...
        async def invite(ctx, server_id: int):
            if str(ctx.message.author) == "??????#7270":
                ...
                invite = await guild.channel.create_invite()
                ...
                embed.set_author(name='Приглашение')
                embed.add_field(name='Вы не создатель бота')
                await ctx.send(embed=embed)
        #
        ...
        async def занят(ctx, help='Перевод в режим Занят. Доступен только создателю бота'):
            if str(ctx.message.author) == "??????#7270":
                await client.change_presence(status=discord.Status.dnd, activity=discord.Game("Python"))
            else:
                embed = discord.Embed(
                    title = "Неудача",
                    color = discord.Colour.red()
