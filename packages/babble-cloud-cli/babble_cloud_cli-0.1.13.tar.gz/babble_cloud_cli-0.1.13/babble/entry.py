...
if not args.action == "help":
    check_if_built = subprocess.run("docker image inspect babble", shell=True, capture_output=True)
    if check_if_built.returncode != 0:
        print()
        spinner = yaspin(text = "building image...").simpleDots
        spinner.start()
        build_image = subprocess.run(f"docker build -t babble {path}", shell=True, capture_output=True)
        if build_image.returncode == 0:
            spinner.write("> image built")
            spinner.stop()
        else:
            spinner.stop()
            print("ERROR: Please check your docker installation and try again.\n")
            exit()
