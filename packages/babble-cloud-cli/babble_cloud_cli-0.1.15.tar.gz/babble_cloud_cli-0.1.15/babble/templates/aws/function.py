...
def handler(event, context):
    try:
        return get_output(event["httpMethod"], event["headers"], event["path"], event["queryStringParameters"], json.loads(event.get("body", "").decode("utf-8")))
    except Exception as e:
        return get_output(event["httpMethod"], event["headers"], event["path"], event["queryStringParameters"], {})