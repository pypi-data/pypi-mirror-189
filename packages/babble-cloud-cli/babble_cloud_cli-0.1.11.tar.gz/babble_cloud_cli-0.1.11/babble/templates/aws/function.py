...
def get_output(method, headers, path, query, body):
    ...
    if endpoint_id == "":
        ...
        try:
            return {
                "statusCode": 200,
                "body": json.dumps(sys.modules[endpoint_id].request(headers, path_parameters, query, body))
            }
...
def handler(event, context):
    return get_output(event["httpMethod"], event["headers"], event["path"], event["queryStringParameters"], json.loads(event["body"]))