...
def count_estimate(qs):
    ...
    try:
        ...
        cursor.execute("SELECT django_pbx_admin_count_estimate(%s)", [str(qs.query)])
        return cursor.fetchone()[0]
    except ProgrammingError:
        log.warning("django_pbx_admin_count_estimate function not found in PostgreSQL")
        return qs.count()
