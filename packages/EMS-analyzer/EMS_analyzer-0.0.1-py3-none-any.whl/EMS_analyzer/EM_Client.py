...
def port_split(hub,range,name_var_list):
    if hub==0:
...
def img_split(image_orign, img_name_list,quadrant):
    ...
    if quadrant ==0:
        ...
        =image_orign[0:int(len(image_orign)/2),0:int(len(image_orign[0])/2)]
    if quadrant ==1:
        globals()[f"{img_name_list[quadrant]}"]\
        =image_orign[0:int(len(image_orign)/2),int(len(image_orign[0])/2):]
    if quadrant ==2:
        globals()[f"{img_name_list[quadrant]}"]\
        =image_orign[int(len(image_orign)/2):,0:int(len(image_orign[0])/2)]
    if quadrant ==3:
        globals()[f"{img_name_list[quadrant]}"]\
        =image_orign[int(len(image_orign)/2):,int(len(image_orign[0])/2):]
    if quadrant >=4:
...
try:
    while True:  # send images as stream until Ctrl-C
        ...
        if ret:
            ...
            img_split(image_orign, img_, 2)
            img_split(globals()[img_[2]],img3_ , 2)
            img_split(globals()[img3_[2]],img3_2_ , 3)
            img_split(globals()[img3_2_[3]],img3_2_1_ , 4)
            #prediction_img(img_,s0,h0)
            #prediction_img(img3_,s0,h0)
            prediction_img(img3_2_1_,s0,h0)
