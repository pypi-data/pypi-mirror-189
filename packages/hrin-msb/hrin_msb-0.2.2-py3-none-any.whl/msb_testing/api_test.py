...
class ApiTest(APILiveServerTestCase, ApiTestConfig):
	...
	def __make_api_request(self, method='post', endpoint: str = '', data: dict | list = None, _func=None, **opt) -> ApiTestResult:
		...
		try:
			...
			if api_response.headers.get('Content-Type') == self.request_content_type:
