...
from nptying import NDArray
from multiprocessing import Pool
from odb_plotter.odb_visualizer import OdbVisualizer
from odb_plotter.util import confirm
...
def main():
    ...
    while main_loop:
        try:
            ...
            elif user_input in ("melt", "meltpoint", "point"):
                set_meltpoint(state)

            elif user_input in ("title", "label"):
                set_title_and_label(state, user_options)

            elif user_input in ("dir", "dirs", "directories", "directory"):
                set_directories(user_options)

            elif user_input in ("run", "process"):
                load_hdf(state)
...
def set_title_and_label(state: OdbVisualizer, user_options: UserOptions):
    default_title: Union[str, None] = None
    ...
    while True:
        try:
            ...
            if confirm(f"Time Sample: {time_sample}", "yes"):
                ...
                print(f"Time Sample set to: {state.set_time_sample}")
                break
...
def set_time(state: OdbVisualizer) -> None:
    lower_time: Union[int, float]
    upper_time: Union[int, float]
    while True:
        values: list[tuple[str, Union[int, float], str]] = [("lower time", 0, "0"), ("upper time", np.inf, "infinity")]
        i: int
...
def process_input(state: OdbVisualizer) -> UserOptions:
    ...
    if args.title:
        ...
        if hasattr(state, "hdf_file"):
            ...
            image_title = "DEFAULT VALUE: Please Set Image Title"
...
def print_state(state: OdbVisualizer) -> None:
    print(
        f"""X Range:                 {state.x.low} to {state.x.high - state.mesh_seed_size if state.x.high is not None and state.mesh_seed_size is not None else "not set"}
Y Range:                 {state.y.low} to {state.y.high - state.mesh_seed_size if state.y.high is not None and state.mesh_seed_size is not None else "not set"}
Z Range:                 {state.z.low} to {state.z.high - state.mesh_seed_size if state.z.high is not None and state.mesh_seed_size is not None else "net set"}
Time Range:              {state.time_low} to {state.time_high}
Seed Size of the Mesh:   {state.mesh_seed_size}
View Angle:              {state.angle}
View Elevation:          {state.elev}
View Azimuth:            {state.azim}
View Roll:               {state.roll}

Data loaded into memory: {'Yes' if state.loaded else 'No'}

Is each time-step being shown in the matplotlib interactive viewer? {'Yes' if state.show_plots else 'No'}"""
    )
...
sample                 -- Set the Time Sample for the hdf5 file
melt, meltpoint, point --  Set the Melting Point for the hdf5 file
run, process           -- Actually load the selected data from the file set in select
...
def plot_time_range(state: OdbVisualizer, user_options: UserOptions):
    ...
    times: NDArray = state.out_nodes["Time"]
...
def plot_time_slice(time: float, state: OdbVisualizer, user_options: UserOptions) -> None:
    ...
    fig = state.plot_time_3d(time, user_options.image_label, user_options.image_title)
