...
from nptying import NDArray, Float, Size
from .odb import Odb


ViewsDict: TypeAlias = dict[str, tuple[int, int, int]]
FloatArray: TypeAlias = NDArray[Any, Float]
...
class OdbVisualizer(Odb):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.views: ViewsDict = self.load_views_dict(os.path.join(self.static_data_dir, "views.json"))
    ...
    def plot_time_3d(self, time: float, label: str, title: str)-> Any:
        ...
        face: str
        for face in faces:
            X: NDArray[Size[self.z.size, self.y.size], Float]
            Y: NDArray[Size[self.z.size, self.x.size], Float]
            Z: NDArray[Size[self.y.size, self.x.size], Float]
            ...
            temp_nodes: NDArray = curr_nodes[temp_mask]
            first_dim: NDArray = temp_nodes[indices[0]]
            first_offset: float = first_dim.min()
            second_dim: NDArray = temp_nodes[indices[1]]
            second_offset: float = second_dim.min()
            first_color_dim: int
            second_color_dim: int
            first_color_dim, second_color_dim = face_shape

            colors: NDArray = np.zeros((first_color_dim, second_color_dim, 3))
            ...
            illuminated_colors: NDArray = ls.shade_rgb(colors, np.full(shape=(first_color_dim, second_color_dim), fill_value=1))
            ax.plot_surface(X, Y, Z, facecolors=illuminated_colors)
