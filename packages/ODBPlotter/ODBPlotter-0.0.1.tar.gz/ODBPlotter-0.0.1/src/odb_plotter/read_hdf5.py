...
def main():
    ...
    time_sample: int = 1
    target_csv: str = ""
    nodes: pd.DatafFrame = get_coords(hdf5_filename)
    args: list[tuple[str, int, int]] = [(hdf5_filename, node, time_sample) for node in nodes]
    with Pool() as pool:
        results: pd.DataFrame = pool.starmap(read_node_data, args)

    final: pd.DataFrame = pd.concat(results)
...
def get_coords(hdf5_filename: str) -> pd.DataFrame:
    ...
    with h5py.File(hdf5_filename, "r") as hdf5_file:
        coords: pd.DataFrame = hdf5_file["node_coords"][:]
        # TODO type hints
        node_labels, x, y, z = np.transpose(coords)
    return pd.DataFrame.from_dict({"Node Labels": node_labels.astype(int), "X": x, "Y": y, "Z": z})


def read_node_data(hdf5_filename: str, node_label: int, time_sample: int) -> pd.DataFrame:
    hdf5_file: h5py.File
    with h5py.File(hdf5_filename, "r") as hdf5_file:
        coords: pd.DataFrame = hdf5_file["node_coords"][:]
        node_coords: pd.DataFrame = coords[np.where(coords[:, 0] == node_label)[0][0]][1:]
        temps: list[float] = list()
        times: list[int] = list()
        # TODO type hints (next 4 lines)
