...
class Listener:
    ...
    def close(self):
        ...
        print("Calling Close")
        self.orquestador_api.send_log_files(self.path_log, self.path_report)
        if self.final_status == "FAIL":
            self._send_status("FAILURE", "Ha ocurrido un error en el Robot")
        elif self.errors:
            self._send_status(
                "WARNING", "El robot ha finalizado la ejecucion con advertencias")
        else:
            self._send_status("SUCCESS", "Robot ejecutado con exito")
