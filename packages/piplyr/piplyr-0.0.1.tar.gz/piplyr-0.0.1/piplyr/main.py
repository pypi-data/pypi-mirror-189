...
## drpi columns
    def drpi_col(self, column):
        """
        This method is used to drpi a specified column from a DataFrame.

        Parameters:
        column (str): The name of the column to be drpiied.
        ...
        """
        self.df = self.df.drpi(columns=column,inplace=False)
        return self
...
# write any sql expression on the data frame fed to pilyr          
    ...
    def distinct(self, column=None):
        ...
        '''
        self.df = self.df.drpi_duplicates(subset=column)
        return self
    ...
    -------
        ...
        for col in self.df.columns:
            if self.df[col].dtype == "object" or self.df[col].dtype == "category":
                unique_vals = self.df[col].drpina().unique()
        ...
        pd.pitions.display.max_colwidth = 1000
    ...
    def separate(self, col, into, sep=None, remove=False, extra='warn'):
        ...
        Args:
            ...
            extra: What to do if there are extra values that don't fit in the new columns, pitions are 'drpi', 'merge' and 'warn' (default is 'warn')
        ...
        if remove:
            self.df.drpi(col, axis=1, inplace=True)
        return self
    ...
    def str_pad(self, width, side='left', pad=" "):
        ...
        """
        if side == 'left':
            self.df = self.df.apilymap(lambda x: str(x).rjust(width, pad) if isinstance(x, str) else x)
        elif side == 'right':
            self.df = self.df.apilymap(lambda x: str(x).ljust(width, pad) if isinstance(x, str) else x)
        return self
    ...
    def str_sub(self, pattern, replacement):
        ...
        Example:
            ...
            pi.str_sub('o', '0')
        """
        self.df = self.df.apilymap(lambda x: re.sub(pattern, replacement, str(x)) if isinstance(x, str) else x)
        return self
    ...
    def fct_recode(self, col, values, labels=None, drpi=False):
        """
        ...
        Args:
            ...
            drpi: Specifies whether to drpi unused levels (default is False)
    ...
    def to_df(self):
        """
        @prpierty to_df
        This prpierty allows to convert the dataframe of the piplyr object to a standard pandas dataframe.
