...
def send_message(url: str, data: list[dict]):
...
class PyLogger:
    ...
    def error(self, message: str, status_code: Optional[int] = None, data=None, alert: bool = True):
    ...
    def send_alert(self, message: str, status_code: Optional[int] = None, data=None):
        ...
        try:
            ...
            if response.status_code != 200:
                self.warning(message="Warning request POST AlertManager error", data=response.json(), status_code=response.status_code)
        except Exception as error:
            self.warning(message="Not passing data for send alert in alertmanager", data=error, status_code=500)