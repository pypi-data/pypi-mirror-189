...
class DataFrame():
    ...
    def _get_dataframe_aggregate(self, operation, **kwargs):
        ...
        try:
            ...
            if (operation in ["sum", "csum", "mean"] and
                self._check_numeric_overflow(agg_df)):
                # As numeric overflow occured, get a new DataFrame with columns of higher
    ...
    def _promote_dataframe_types(self):
        ...
        """
        new_cols = {}
        # Dictionary to map next higher type.
    ...
    def pivot(self,
        ...
        # If user do not pass values for FOR clause, retrieve it.
            ...
            # TODO: Return a generator instead of records. Will be implemented with ELE-5308.
            # Get the values from drop_duplicates DataFrame. get_values gets data in numpy array.
            # Convert it to tuple and return it.
            distinct_values = (tuple(rec) for rec in self.drop_duplicate(
                column_names=participating_columns).get_values())
...
class _TDUAF(DataFrame):
    """
    Base class for TD_Series and TD_Matrix classes.
    Initializes common parameters and performs validations.
    This class is used to represent the UAF functions SERIES_SPEC and Matrix_SPEC inputs.
    """
    ...
    def _get_sql_repr(self, render=False):
        """
        DESCRIPTION:
            Internal function to generate the SQL Clause for Series Spec in UAF Function.
    ...
    def _create_validate_dataframe_from_node(self, nodeid, metaexpr, index_label, undropped_columns=None):
        ...
        # TDSeries over an ART table. As the new DataFrame is internally
...
class TDSeries(_TDUAF):
    ...
    Objects of this class can be used directly in UAF functions in place of
    'SERIES_SPEC'.
    """
    ...
    def __init__(self, data, id, row_index, row_index_style="TIMECODE", id_sequence=None,
        ...
        DESCRIPTION:
            1. Create a TDSeries object from a teradataml DataFrame
               ...
               and output units of UAF functions.
               A series is identified by its series ID, i.e., "id" argument,
               and indexed by its "row_axis" argument.
               ...
               and ordered on the "row_axis" argument.
            ...
            interval:
                ...
                "interval" is categorised in to 4 types:
                    ...
                    * sequence-zero:
                        ...
                        Used when ROW_AXIS is SEQUENCE.
                Allowed Values:
        ...
        EXAMPLES:
            # Example 1: Creating TDSeries (Series_Spec).
...
class TDMatrix(_TDUAF):
    ...
    Objects of this class can be used directly in UAF functions in place of 'MATRIX_SPEC'.
    """
    ...
    def __init__(self, data, id, row_index, column_index, row_index_style="TIMECODE", column_index_style="TIMECODE",
        ...
        DESCRIPTION:
            ...
            and is indexed by its "row_axis" and "column_axis" arguments.
            A matrix can be a one of the following types:
                * Row-major matrix: Each row is a wavelet that is grouped by
                  its matrix id and "row_axis", and ordered by its "column_axis".
                * Column-major matrix: Each column is a wavelet that is grouped
                  by its matrix id and "column_axis", and ordered by its "row_axis".
...
class TDGenSeries():
    ...
    Objects of this class can be used directly in UAF functions in place of 'GenSeries_SPEC'.
    """
    def __init__(self, instances, data_types, start, offset, num_entries):
        """
        TDGenSeries allows to pass a generated series to a UAF function rather than using a
        pre-existing series instance. It contains all the information that would have been derivable
