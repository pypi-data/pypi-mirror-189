...
def case(whens, value=None, else_=None):
    ...
    # whens can be a list of 2-tuples
        ...
        for when in whens:
            ...
            if isinstance(when[0], ColumnExpression):
                new_whens.append((when[0].expression, when[1]))
            else:
                new_whens.append((when[0], when[1]))
    ...
    return _SQLColumnExpression(case_when(new_whens, value=value, else_=else_))
