...
from .md_content_processor import OutputOrganizerSingleDir, ImageProcessorCopy, LinkProcessorTex, MDContentProcessor
...
class PublicationVisitorTeX(PublicationVisitor, TaskVisitor):
    def __init__(self, yaml_specification, task_repo, publication_repo, dst,
                 tex=False, pdf=False, langs=[], header=None, babel=None, translit=lambda x: x, standalone=True, tex_template=None):
        ...
        link_processor = LinkProcessorTex(self._yaml_specification, self._task_repo)
    ...
    # first time the task occurs, we print its full statement and next times only reference to the original statement
