...
from .md_content_processor import MDContentProcessor, OutputOrganizerHierarchy, LinkProcessorHTML, ImageProcessorCopy
from . import logger
...
class PublicationVisitorHTML(PublicationVisitor, TaskVisitor):
    ...
    def __init__(self, yaml_specification, task_repo, publication_repo, dst, langs,
                 ...
                 css=None, header=None, translit=lambda x: x, babel=None):
        ...
        link_processor = LinkProcessorHTML(self._yaml_specification, self._task_repo, self._output_organizer)
        self._md_processor = MDContentProcessor(link_processor, image_processor)
    ...
    def read_and_process_md(self, md_file_name, section_path, level):
        ...
        # read the content
        title, content = self._publication_repo.read_md_file(md_file_path)
        ...
        if markdown_magic_comments.contains_block(md, "lang"):
            ...
            md = self._md_processor.process(section_path, md_file_path, md, level=level)
        ...
        def process_javascript(javascript_spec_lines):
            def copy(file_name):
                logger.info("Copy:", file_name)
    ...
    def create_toc(self, section_path, md_files, subsections, tasks):
        ...
        for md_file in md_files:
            ...
            title, _ = self._publication_repo.read_md_file(md_file_path)
    ...
    def md_file(self, section_path, level, md_file_name, unnumbered=False):
        ...
        self._md_serializer.write(path, md, title=title)
    ...
    def task_sol(self, task, sols):
        ...
        self._sol_md = self._md_processor.process(self.task_output_dir(task.id()), task.sol_path(), sol_md, langs=self._langs)
    ...
    def task_end(self, task):
        ...
        if not self._inline_task:
            ...
            self._md_serializer.write(st_path, self._st_md, title=task.title(), md_time=task.modification_time())
            ...
            if "solution" in self._what_to_print:
                ...
                if not self._join_langs:
                    ...
                    for lang in self._sol_md:
                        ...
                        self._md_serializer.write(sol_path, sol_md, title=title, md_time=task.modification_time())
                else:
                    # add title
                    ...
                    self._md_serializer.write(sol_path, joined_sol_md, title=title, md_time=task.modification_time())
