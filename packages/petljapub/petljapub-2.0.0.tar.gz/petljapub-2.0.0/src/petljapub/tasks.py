import os, sys, glob, shutil, getpass
import re
import json, yaml
...
def set_language(ctx, lang):
    ...
    """
    if not lang:
        logger.error("Please specify language (en, sr-Cyrl, sr-Latn, ...)")
        return
...
def clean(ctk):
    ...
    if os.path.isdir(Task.build_dir_name()):
        try:
            shutil.rmtree(Task.build_dir_name())
        except:
...
@task(help = {
    ...
    'timeout': "Set a timeout in seconds"
})
def run(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1, verbosity=3, timeout=None):
    """
    ...
    if testcase:
        status = task.run(solution(sol), lang, testcase=testcase, output="stdout")
        if status != "OK":
...
@task
def r(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1, verbosity=3):
    """
    Run a solution (shortcut for 'petljapub run')
    """
    run(ctx, lang, sol, testcase, example, generated, crafted, verbosity)
...
@task(help={
    ...
    'recompile': "Recompile the solution (and the custom checker if it exists) before checking",
    'timeout': "Timeout in seconds for each testcase (1 seconds is the default value)",
...
def test(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1, recompile=True, timeout=None, outputs=False, verbosity=3):
    """
    ...
    if example >= 1:
        ...
        output = input[:-2] + "out"
        status, time = task.test_on_testcase(solution(sol), lang, input, output, timeout=timeout, save_output_dir=task.example_test_output_dir() if outputs else None)
        logger.info(status)
        logger.info("Time: ", round(time * 1000), "ms")
    elif generated >= 1:
        input = task.testcase_path("generated", generated)
        output = input[:-2] + "out"
        status, time = task.test_on_testcase(solution(sol), lang, input, output, timeout=timeout, save_output_dir=task.generated_test_output_dir() if outputs else None)
        logger.info(status)
        logger.info("Time: ", round(time * 1000), "ms")
    elif crafted >= 1:
        ...
        print(input)
        output = input[:-2] + "out"
        status, time = task.test_on_testcase(solution(sol), lang, input, output, timeout=timeout, save_output_dir=task.crafted_test_output_dir() if outputs else None)
        logger.info(status)
        logger.info("Time: ", round(time * 1000), "ms")
    else:
        task.test_on_all_testcases("ex" + str(sol), lang, force_recompile=recompile, timeout=timeout, save_outputs=outputs)

@task
def t(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1, recompile=True, timeout=None, outputs=False, verbosity=3):
    """
    Test the given solution for the current task (shortcut for 'petljapub test')
    """
    test(ctx, lang, sol, testcase, example, generated, crafted, recompile, timeout, outputs, verbosity)
    
    
@task(help={'lang': "Test just this language",
            ...
            'verbosity': "Level of messages printed to the user (from 0/quiet to 5/full)"})
def test_all(ctx, lang=None, recompile=True, timeout=None, verbosity=3):
    """
    ...
    if lang:
        langs = [lang]
    task.test_all(langs=lang, force_recompile=recompile, timeout=timeout)
...
def runtime(ctx, force_run=False, timeout=1.0, repeat=3, plot=False, sol=None, lang=None):
    """
...
@task(iterable=["langs"],
      help={
      'langs': "Languages that should be included",
      'css': "CSS stylesheet",
...
def html_preview(ctx, langs=None, css="pandoc.css", header="header.md", lat=False, babel=None):
    """
    ...
    if not babel:
        babel = read_config("lang") or read_config("lang")
...
def yaml_test(ctx, yaml, tasks_dir=None, pub_dir=None, recompile=False, timeout=None, verbosity=3):
    """
    ...
    yaml_specification.traverse(PublicationVisitorTest(yaml_specification, task_repo, recompile, timeout))
...
def yaml_runtime(ctx, yaml, tasks_dir=None, pub_dir=None, force_run=False, timeout=1.0, repeat=3):
    """
...
@task(help={'yaml': "YAML specification of the publication",
            ...
            'lat': "If this flag is set, all content ist transliterated to latin alphabet"})
def yaml_html(ctx, yaml, dst, tasks_dir=None, pub_dir=None, standalone=True, join_langs=True, css=None, header="header.md", babel=None, lat=False, verbosity=3):
    """
...
def yaml_petlja_package(ctx, yaml, dst, tasks_dir=None, pub_dir=None, join_langs=True, header="header.md", lat=False, generate_tests=False):
    """
...
def yaml_tex(ctx, yaml, dst, tasks_dir=None, pub_dir=None, header='header.md', babel=None, tex_template='default.latex', standalone=True, lat=False):
    """
    ...
    visitor = PublicationVisitorTeX(yaml, task_repo, pub_repo, dst, langs=langs, tex=tex, pdf=pdf, header=header, babel=babel, standalone=standalone, tex_template=tex_template, translit=translit)
    yaml.traverse(visitor)
