...
class Motilal(Broker):
    ...
    def positions(self):
        ...
        if positions is None or len(positions) == 0:
            return pd.DataFrame(columns=['ltp', 'last_price', 'tradingsymbol', 'pnl'])

        positions = pd.DataFrame(positions)
        positions.loc[:, 'tradingsymbol'] = positions.symbol

        self.add_ltp(positions)
        positions.loc[:, 'm2m'] = 0
        positions.loc[:, 'm2m'] = np.where(positions.cfsellquantity > 0,
                                           positions.cfsellamount - positions.cfsellquantity * positions.ltp,
                                           positions.m2m)
        positions.loc[:, 'm2m'] = np.where(positions.cfbuyquantity > 0,
                                           positions.cfbuyquantity * positions.ltp - positions.cfbuyamount,
                                           positions.m2m)

        positions.loc[:, 'pnl'] = positions.bookedprofitloss + positions.m2m
        return positions
    ...
    def orders(self, tag=None):
        ...
        orders = pd.merge(orders, positions[['symbol', 'ltp']],
                          how="left",
                          left_on=['symbol'],
                          right_on=['symbol'])
