...
from Nanylibrary.encryption import encryption
from Nanylibrary.Ny_Wb import SetClines,Server
from re import findall
from pathlib import Path
from random import randint, choice
from json import loads, dumps
from Nanylibrary.Cliob import chup
...
class Nany_Robot:
	...
	def sendMessage(self, chat_id,text,metadata=[],message_id=None):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Nany_Robot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
	...
	def getMessagefilter(self, chat_id, filter_whith):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Nany_Robot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("messages")
				break
			except: continue
	...
	def setMembersAccess(self, chat_id, access_list):
		...
		while 1:
			try:
				return loads(request.urlopen(request.Request(Nany_Robot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read())
				break
			except: continue
	...
	def getGroupLink(self, chat_id):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Nany_Robot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("join_link")
				break
			except: continue
	...
	def getChatsUpdate(self):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Nany_Robot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("chats")
				break
			except: continue
	...
	def see_GH_whith_Linkes(self,link_gh):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Nany_Robot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
				break
			except: continue
	...
	def _requestSendFile(self, file):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Nany_Robot._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
				break
			except: continue

	def _uploadFile(self, file):
		if not "http" in file:
			REQUES = Nany_Robot._requestSendFile(self, file)
	...
	def uploadAvatar(self,myguid,main,thumbnail=None):
		mainID = str(Nany_Robot._uploadFile(self, main)[0]["id"])
		thumbnailID = str(Nany_Robot._uploadFile(self, thumbnail or main)[0]["id"])
		inData = {
			"method":"uploadAvatar",
			"input":{
				"object_guid":myguid,
	...
	def sendDocument(self, chat_id, file, caption=None, message_id=None):
		uresponse = Nany_Robot._uploadFile(self, file)
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Nany_Robot._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
	...
	def sendPhoto(self, chat_id, file, size=[], thumbnail=None, caption=None, message_id=None):
		...
		elif "." in thumbnail:thumbnail = str(Nany_Robot._getThumbInline(open(file,"rb").read() if not "http" in file else get(file).content))

		if size == []: size = Nany_Robot._getImageSize(open(file,"rb").read() if not "http" in file else get(file).content)
