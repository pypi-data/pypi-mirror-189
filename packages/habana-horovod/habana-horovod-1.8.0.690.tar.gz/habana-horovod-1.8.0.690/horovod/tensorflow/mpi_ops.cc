...
const char* const DEVICE_HPU = "HPU";
...
TFOpContext::AllocateZeros(int64_t num_elements, common::DataType dtype,
  ...
  Status status = context_->allocate_temp(
      tf_data_type, ::tensorflow::TensorShape({num_elements}),
      zero_tensor.get(), tf_attribute);
  ...
  if (device_ != CPU_DEVICE_ID) {
    ...
    auto stream =
        (device_context != nullptr)
            ? stream_executor::gpu::AsGpuStreamValue(device_context->stream())
            : 0;
    ...
    memset((void*)zero_tensor->tensor_data().data(), 0,
           zero_tensor->tensor_data().size());
  }
  if (status.ok()) {
...
public:
  explicit HorovodAllreduceOp(OpKernelConstruction* context)
    ...
    // Ordering
    if (horovod::common::ShouldUseOrderedHccl()) {
      context->GetAttr("_order_id", &order_index_);
      context->GetAttr("_group_order_ids", &group_order_indices_);
    }
  }
  ...
  void ComputeAsync(OpKernelContext* context, DoneCallback done) override {
    ...
    horovod::common::ReduceOp reduce_op =
        static_cast<horovod::common::ReduceOp>(reduce_op_);
    Tensor* output;
    OP_REQUIRES_OK_ASYNC(context,
                         context->forward_input_or_allocate_output(
                             {0}, 0, tensor.shape(), &output),
                         done);
    ...
    auto enqueue_result = EnqueueTensorAllreduce(
        ...
        process_set_id_,
        order_index_, group_order_indices_);
...
private:
  ...
  std::vector<int64_t> group_order_indices_;
  int order_index_{-1};
...
REGISTER_KERNEL_BUILDER(Name("HorovodAllreduce").Device(DEVICE_HPU),
                        HorovodAllreduceOp);
#endif
...
REGISTER_KERNEL_BUILDER(Name("HorovodAllgather").Device(DEVICE_HPU),
                        HorovodAllgatherOp);
#endif
...
REGISTER_KERNEL_BUILDER(
    Name("HorovodBroadcast").Device(DEVICE_HPU).TypeConstraint<float>("T"),
    HorovodBroadcastOp);
REGISTER_KERNEL_BUILDER(
    Name("HorovodBroadcast").Device(DEVICE_HPU).TypeConstraint<bfloat16>("T"),
    HorovodBroadcastOp);
#endif
...
REGISTER_KERNEL_BUILDER(Name("HorovodReducescatter").Device(DEVICE_HPU).TypeConstraint<float>("T"),
                        HorovodReducescatterOp);
REGISTER_KERNEL_BUILDER(Name("HorovodReducescatter").Device(DEVICE_HPU).TypeConstraint<bfloat16>("T"),
                        HorovodReducescatterOp);
#endif
