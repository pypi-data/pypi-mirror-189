...
from habana_frameworks.tensorflow.internal.allcompare import AllCompare
...
def _allreduce(tensor, name=None, op=Sum, prescale_factor=1.0, postscale_factor=1.0,
    ...
    reduced_tensor = MPI_LIB.horovod_allreduce(tensor, name=name, reduce_op=op,
    ...
    if allcompare is not None:
        assert op == Sum, f"AllCompare for HorovodAllreduce supports only op=Sum, but is: {op}"
        err_tensor, _ = allcompare.expect_similar(reduced_tensor / float(size()), tensor, name_prefix="AllCompare/HorovodAllreduce")

        from tensorflow import control_dependencies
        with control_dependencies([err_tensor]):
            reduced_tensor = tf.identity(reduced_tensor)
...
def _reducescatter(tensor, name=None, op=Sum, ignore_name_scope=False,
    ...
    tensor_first_dim_size = tensor.shape[0]

    if process_set.size() > tensor_first_dim_size:
        raise ValueError(
            f"First dimension of provided tensor has to be greater than process set size. Process set size: {process_set.size()}, tensor shape: {tensor.shape}")
