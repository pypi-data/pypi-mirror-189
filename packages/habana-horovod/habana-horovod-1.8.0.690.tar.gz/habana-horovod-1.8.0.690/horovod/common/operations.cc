...
#if HAVE_HCCL
  hccl_context.hvd_global_ptr(&horovod_global);
#if HOROVOD_HPU_ALLREDUCE

#if HAVE_MPI
  if (global_mpi_context.IsEnabled()) {
    allreduce_ops.push_back(
        std::shared_ptr<AllreduceOp>(new HCCLHierarchicalAllreduce(
            &hccl_context, &state)));
  }
#endif // HAVE_MPI
  if (ShouldUseOrderedHccl()) {
    allreduce_ops.push_back(std::shared_ptr<AllreduceOp>(
        new HCCLDummySignaledAllreduce(&hccl_context, &state)));
    allreduce_ops.push_back(std::shared_ptr<AllreduceOp>(
        new HCCLSignaledAllreduce(&hccl_context, &state)));
  }
  allreduce_ops.push_back(std::shared_ptr<AllreduceOp>(
      new HCCLDummyAllreduce(&hccl_context, &state)));
  allreduce_ops.push_back(
      std::shared_ptr<AllreduceOp>(new HCCLAllreduce(&hccl_context, &state)));
#endif // HOROVOD_HPU_ALLREDUCE
#if HOROVOD_HPU_BROADCAST
#if HAVE_MPI
  broadcast_ops.push_back(
      std::shared_ptr<BroadcastOp>(new HCCLHierarchicalBroadcast(
          &hccl_context, &state)));
#endif
  broadcast_ops.push_back(
      std::shared_ptr<BroadcastOp>(new HCCLBroadcast(&hccl_context, &state)));
#endif // HOROVOD_HPU_BROADCAST
#if HOROVOD_HPU_ALLGATHER
  if (ShouldUseOrderedHccl()) {
    allgather_ops.push_back(std::shared_ptr<AllgatherOp>(
        new HCCLSignaledAllgather(&hccl_context, &state)));
  }
  allgather_ops.push_back(
      std::shared_ptr<AllgatherOp>(new HCCLAllgather(&hccl_context, &state)));
#endif // HOROVOD_HPU_ALLGATHER
#if HOROVOD_HPU_REDUCE_SCATTER
  reducescatter_ops.push_back(std::shared_ptr<ReducescatterOp>(
      new HCCLReduceScatter(&hccl_context, &state)));
#endif // HOROVOD_HPU_REDUCE_SCATTER
#if HOROVOD_HPU_ALLTOALL
  alltoall_ops.push_back(
      std::shared_ptr<AlltoallOp>(new HCCLAlltoall(&hccl_context, &state)));
#endif // HOROVOD_HPU_ALLTOALL
#endif // HAVE_HCCL
...
void PerformOperation(Response response, ProcessSet& process_set) {
  ...
  if (!status.in_progress()) {
    // After calling callback TF (or other framework for that matter) might
    // close the device and invalidate memory allocator before
    // PerformOperations(...) returns, and local tensor reference,
    // that is stored in 'entries' vector will be deleted.
    // In that case we observe race condition - tensor is deleted after the
    // device containing it is closed and pointer to memory allocator becomes
    // invalid.
    for (auto& e : entries) {
...
#if !(HAVE_HCCL) && HOROVOD_GPU_ALLREDUCE != 'N' && HOROVOD_GPU_ALLREDUCE != 'D'
...
int horovod_hccl_built() {
#if HAVE_HCCL
  return HCCL_VERSION_CODE;
#else
  return 0;
#endif
}
...
Status EnqueueTensorAllreduce(std::shared_ptr<OpContext> context,
                              ...
                              int order_index,
                              std::vector<int64_t> group_order_indices) {
  ...
  return EnqueueTensorAllreduces(
      ...
      reduce_op, prescale_factor, postscale_factor, process_set_id, order_index,
      group_order_indices);
}

Status EnqueueTensorAllreduces(
    std::vector<std::shared_ptr<OpContext>>& contexts,
    std::vector<std::shared_ptr<Tensor>>& tensors,
    std::vector<std::shared_ptr<Tensor>>& outputs,
    std::vector<ReadyEventList>& ready_event_lists,
    std::vector<std::string>& names, const int device,
    std::vector<StatusCallback>& callbacks, ReduceOp reduce_op,
    double prescale_factor, double postscale_factor, int32_t process_set_id,
    int order_index, std::vector<int64_t> group_order_indices) {
  ...
  for (int n = 0; n < (int)tensors.size(); ++n) {
    ...
    for (auto index : group_order_indices) {
      e.group_order_indices.insert(index);
    }
  ...
  const bool handshake_skip{(device != CPU_DEVICE_ID) &&
                            ShouldSkipHandshakeForAllreduce() &&
                            AreEntriesEnabledForSignaling(entries)};

  if (handshake_skip) {
    Response resp;
    status = op_manager->ExecuteOperation(entries, resp, process_set);
