...
#elif HAVE_HCCL
#include <hccl/hccl.h>
// Do not define NCCL_P2P_SUPPORTED, as HCCL does not support Point-to-Point communication (hcclSend, hcclRecv).
#endif
...
struct NCCLContext {
  ...
  void hvd_global_ptr(HorovodGlobalState* ptr) {
      if (ptr == nullptr) throw std::logic_error("NCCLContext::hvd_global_ptr() failed: HorovodGlobalState* ptr cannot be null.");
      hvd_global_ptr_ = ptr;
    }
