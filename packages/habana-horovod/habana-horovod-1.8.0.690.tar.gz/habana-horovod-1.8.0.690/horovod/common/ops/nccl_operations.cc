...
#if HAVE_HCCL
  // Added by Habana.
  if (any_comm_destroyed) {
    if (hvd_global_ptr_ == nullptr) throw std::logic_error("NCCLContext::ShutDown() failed: HorovodGlobalState* must be set with hvd_global_ptr() during NCCLContext instance initialization.");
    hvd_global_ptr_->fusion_buffer.FreeDeviceBuffers(0);
  }
#endif
}
