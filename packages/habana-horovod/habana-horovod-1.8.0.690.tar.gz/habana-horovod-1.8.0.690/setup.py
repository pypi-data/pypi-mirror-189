...
# Set default env for habana-horovod
os.environ['HOROVOD_HPU'] = "HCCL"
os.environ['HOROVOD_WITHOUT_GLOO'] = "1"
os.environ['HOROVOD_WITHOUT_PYTORCH'] = "1"
os.environ['HOROVOD_WITHOUT_MXNET'] = "1"
...
def get_cmake_bin():
    ...
    if cmake_installed_version < LooseVersion("3.13.0"):
        ...
        try:
            _ = subprocess.check_output([sys.executable, "-m", "pip", "install", "--target", cmake_temp_dir.name, "cmake~=3.13.0"])
        except Exception:
...
def read_package_long_description():
    this_directory = Path(__file__).parent
    long_description = (this_directory / "habana_package_long_description.md").read_text()
    return long_description


setup(name='habana-horovod',
      version=get_package_version(),
      packages=find_packages(),
      description="This package contains fork of Horovod (distributed training framework for TensorFlow and others) adjusted to work with Habana® TensorFlow bridge on Habana® Gaudi® accelerator.",
      author='Habana Labs, Ltd. an Intel Company',
      author_email='support@habana.ai',
      license='Apache 2.0',
      long_description=read_package_long_description(),
