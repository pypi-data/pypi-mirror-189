...
def 删除目录树(path,显示错误=True):
    ...
    try:
        ...
        if 显示错误:
            print(f'删除目录树错误 {os.path.abspath(path)}\n{e}')
def 获取_页数(总数, 分页数):
        ...
        for i,d in enumerate(列表):
            if d and 子字段 in d and d[子字段]:
                if 加编号:
                    ...
                    name_ = d[目录字段]
                yield from 递归目录(列表=d[子字段], 子字段=子字段, 目录字段=目录字段, names=names + [name_], 加编号=加编号)
            else:
                if 加编号:
                    name_ = f'{i+1}--{d[目录字段]}'
                else:
...
def curl_bash(curl_bash:str,code:bool=False)->requests.Response:
    ...
    import uncurl
    if code:
        return uncurl.parse(curl_bash)
    else:
        return eval(uncurl.parse(curl_bash))
...
def 下载文件(文件路径: str = None, 网址: str = '', headers=None,params=None, proxies=None, verify=False, allow_redirects=True, cookies=None, try_num=3) -> int:
    ...
    if os.path.exists(文件路径):
        ...
        fdir, _ = os.path.split(文件路径)
        if not os.path.exists(fdir):
            raise Exception('目录不存在:{}'.format(fdir))
...
def 获取_网络文件(文件网址: str, headers=None,params=None, proxies=None, cookies=None, verify=False,allow_redirects=True, try_num=3) -> bytes:
    ...
    for num in range(try_num):
        try:
            ...
            if num == (try_num - 1):
                raise Exception('{},{}'.format(e.args, e.__traceback__.tb_lineno))
def 获取_本地IP(proxies=None, timeout=30, try_num=3) -> json:
    proxies = proxies if proxies else default_proxies
    for num in range(try_num):
        try:
            url = 'http://pv.sohu.com/cityjson?callback='
            res = requests.get(url, timeout=timeout, proxies=proxies, verify=False).text.strip(
...
class SESSION():
    ...
    def post(self, *args, **kwargs):
        ...
        return resp
class PYPP():
    def __init__(self, headless=False, executablePath=None, width=1200, height=800, userDataDir=None, 启用拦截器=False,timeout=20):
        '''
        :param headless:
