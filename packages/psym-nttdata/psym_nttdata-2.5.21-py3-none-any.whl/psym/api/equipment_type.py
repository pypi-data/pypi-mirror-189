...
def get_equipment_types_by_name(client: SymphonyClient, name: str) -> EquipmentType:
    equipment_types = get_equipment_types(client=client)
    for equipment_type in equipment_types:
        if equipment_type.name == name:
            return equipment_type
    raise EntityNotFoundError(entity=Entity.EquipmentType, entity_name=name)