...
def _map_and_wrap(name, docstring):
    @forbid_nonstring_types(["bytesb"], name=name)
    def wrapper(self):
        # result = getattr(self._data.array, f"_str_{name}")()
        result = getattr(np.char.array(self._data.array), f"{name}")()
...
class StringMethods(NoNewAttributesMixin):
    ...
    def _wrap_result(
        ...
        if not hasattr(result, "ndim") or not hasattr(result, "dtype"):
            if isinstance(result, ABCDataFrame):
                result = result.__finalize__(self._orig, name="bytes")
            return result
    ...
    @forbid_nonstring_types(["bytesb"])
    def partition(self, sep=b" ", expand=True):
    ...
    @forbid_nonstring_types(["bytesb"])
    def rpartition(self, sep=b" ", expand=True):
    ...
    def get(self, i):
        ...
        return self._wrap_result(result).apply(chr)
    ...
    def join(self, sep):
        ...
        return self._data.apply(lambda x: b"".join([chr(y).encode() + sep for y in x]))
        # return self._wrap_result(result)
    ...
    def replace(
        ...
        if not (isinstance(repl, (str, bytes)) or callable(repl)):
    ...
    @forbid_nonstring_types(["bytesb"])
    def pad(self, width, side="left", fillchar=b" "):
        """
        ...
        dtype: object
        """
        if not isinstance(fillchar, (str, bytes)):
    ...
    def slice_replace(self, start=None, stop=None, repl=None):
        ...
        dtype: object
        """
        result = self._data.apply(lambda x: x[:start] + repl + x[stop:])
    ...
    def wrap(self, width, **kwargs):
        ...
        dtype: object
        """
        twrap = textwrap.TextWrapper(width)
        return self._data.apply(
            lambda x: [
                convert_utf8_to_ascii(y)
                for y in twrap.wrap(x.decode("unicode_escape"), **kwargs)
            ]
        )
    ...
    def translate(self, table):
        ...
        Series or Index
        """
        # df5.Name2.s_str().translate(bytes.maketrans(b"\x0A\x0B\x0C\x0D\x04", b"\x00\x00\x00\x00\x00"))
        #  bytes.maketrans(b"\x0A\x0B\x0C\x0D\x04", b"\x00\x00\x00\x00\x00"
        # result = self._data.array._str_translate(table)
        # return self._wrap_result(result)
        # return self._data.apply(lambda x:b''.join([struct_pack_trans(g) if (g:=table.get(y)) else struct_pack_trans(y) for y in x]))
        result = np.char.array(self._data.array).translate(table)
        return self._wrap_result(result)
    ...
    def startswith(
        ...
        dtype: bool
        """
        if not isinstance(pat, (str, tuple, bytes)):
        ...
        if returns_df:
            ...
            if obj.array.size == 0:
                ...
                result_list = _str_extract(
                    self._data.array, pat, flags=flags, expand=returns_df
                )
    ...
    def find(self, sub, start=0, end=None):
        if not isinstance(sub, (str, bytes)):
    ...
    def normalize(self, form):
        ...
        result = self._data.apply(
            lambda x: convert_utf8_to_ascii(
                unicodedata.normalize(form, x.decode("unicode_escape"))
            )
        )
    ...
    def index(self, sub, start=0, end=None):
        ...
        result = np.char.array(self._data.array).index(sub, start=start, end=end)
        return self._wrap_result(result, returns_string=False)
    ...
    def rindex(self, sub, start=0, end=None):
        ...
        result = np.char.array(self._data.array).rindex(sub, start=start, end=end)
        return self._wrap_result(result, returns_string=False)
    ...
    @forbid_nonstring_types(["bytesb"])
    def lower(self):
        result = np.char.array(self._data.array).lower()
        return self._wrap_result(result)

    @Appender(_shared_docs["casemethods"] % _doc_args["upper"])
    @forbid_nonstring_types(["bytesb"])
    def upper(self):
        result = np.char.array(self._data.array).upper()
        return self._wrap_result(result)

    @Appender(_shared_docs["casemethods"] % _doc_args["title"])
    @forbid_nonstring_types(["bytesb"])
    def title(self):
        result = np.char.array(self._data.array).title()
        return self._wrap_result(result)

    @Appender(_shared_docs["casemethods"] % _doc_args["capitalize"])
    @forbid_nonstring_types(["bytesb"])
    def capitalize(self):
        result = np.char.array(self._data.array).capitalize()
        return self._wrap_result(result)

    @Appender(_shared_docs["casemethods"] % _doc_args["swapcase"])
    @forbid_nonstring_types(["bytesb"])
    def swapcase(self):
        result = np.char.array(self._data.array).swapcase()
        return self._wrap_result(result)
    ...
    def casefold(self):
        result = self._data.apply(
            lambda x: convert_utf8_to_ascii(x.decode("unicode_escape").casefold())
        )
    ...
    def s_to_ascii(
        ...
        return convert_to_unicode_ascii(
            self._data,
    ...
    def s_to_utf8(
        ...
        return convert_unicode_ascii_to_utf8(
            self._data,
        )

    def s_uniascii_equal(
        self,
        stri,
        ensure_ascii=True,
        encode_html_chars=True,
        ...
        return search_unicode_ascii_equal(
            self._data,
            stri,
            ensure_ascii=ensure_ascii,
            encode_html_chars=encode_html_chars,
    ...
    def s_uniascii_contains(
        ...
        return search_unicode_ascii_contains(
            self._data,
            stri,
            ensure_ascii=ensure_ascii,
            encode_html_chars=encode_html_chars,
    ...
    for subject_key, subject in arr.items():
        if isinstance(subject, (str, bytes)):
            ...
            for match_i, match_tuple in enumerate(regex.findall(subject)):
                if isinstance(match_tuple, (str, bytes)):
                    match_tuple = (match_tuple,)
                na_tuple = [np.NaN if group == b"" else group for group in match_tuple]
...
def fastloc(df, queries, default=False, convertstr=True):
    keya = {}
    aldax = "".join([x[0] for x in queries])

    for col in df.columns:
        if col in aldax:
            keya[col] = df[col].__array__()
            if convertstr:
                if df[col].dtype in ["object", "string"]:
                    keya[col] = keya[col].astype("S")

    conditions = np.array([numexpr.evaluate(x[0], local_dict=keya) for x in queries])
    choices = np.array([x[1] for x in queries])

    indextruefalse = np.select(conditions, choices, default=default)
    return df.loc[indextruefalse]


def convert_to_unicode_ascii(
    df,
    ensure_ascii=True,
    encode_html_chars=True,
    escape_forward_slashes=False,
...
def convert_unicode_ascii_to_utf8(df):
    return df.apply(lambda x: x.decode("unicode_escape"))


def convert_utf8_to_ascii(
    stri,
    ensure_ascii=True,
    encode_html_chars=True,
    escape_forward_slashes=False,
...
def search_unicode_ascii_equal(
    df,
    stri,
    ensure_ascii=True,
    encode_html_chars=True,
    ...
    b = df.__array__()
    return numexpr.evaluate(f"(b=={repr(enc)})")


def search_unicode_ascii_contains(
    df,
    stri,
    ensure_ascii=True,
    encode_html_chars=True,
    ...
    b = df.__array__()
    return numexpr.evaluate(f"contains(b,{repr(enc)})")


def _s_str(df):
    return StringMethods(df)


def pd_add_fastloc():
    from pandas.core.frame import DataFrame, Series

    DataFrame.d_floc = fastloc
    Series.s_str = _s_str
...
def get_byte_regex(
    ...
    va = regex.sub(rb"\\+", b"\\\\", va)
    va = regex.sub(
        rb"\\u([abcdef0-9]{4})\b", rb"\\\\\bu\g<1>", va, flags=regex.I
    ).replace(b"\x08", b"")
    vax = re.compile(va)
    return vax


def get_compiled_regex_from_wordlist(
    wordlist,
    ...
    **kwargs,
    ...
    return re.compile(tt.pattern(), *args, **kwargs)
...
class Trie:
    ...
    def add(self, word: str | list):
        for keys in word:
            ...
            reduce(operator.getitem, keysn[:-1], self.data)[keysn[-1]][b""] = 1
    ...
    def _pattern(self, pData):
        ...
        for char in sorted(data.keys()):
            if isinstance(data[char], dict):
                try:
                    ...
                    if len(char) == 1:
                        alt.append(regex.escape(char) + recurse)
                    elif rb"\\u" in char:
                        alt.append(regex.escape(char) + recurse)
                    else:
        ...
        if len(alt) == 1:
            ...
            # print(alt)
            alt = [b"(?:" + x.replace(b"(?:)?", b"") + b")" for x in alt]
    ...
    def pattern(self):
        return self._pattern(self.dump()).replace(b"\\u", b"\\\\u")
