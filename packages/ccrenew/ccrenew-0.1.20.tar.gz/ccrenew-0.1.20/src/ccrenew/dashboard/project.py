...
class Project:
    ...
    def __init__(self, proj_init_dict: dict):
        ...
        # TODO: Update this once we get a static date set in df_keys
        try:
            self.static_date = self.df_proj_keys['static_date']
        except:
            self.static_date = datetime(2022, 12, 31).date()
        ...
        self.powertrack_filepath = self._find_powertrack_file()
        self.powertrack_csv = Path(self.powertrack_filepath).parent / f"{self.year}_hourly_{self.project_name}.csv"
    ...
    def run_bluesky(self, start:Union[str, datetime]=None, end:Union[str, datetime]=None, tran_ghi:str='add', plot:bool=True) -> DataFrame:
        """Pulls irradiance & weather data from Solcast for the Project & date range.

        Args:
            start (str or datetime): The start date for Solcast data. Defaults
                to None, which will set it to the first day of the current month.
            end (str or datetime): The end date for Solcast data. Defaults
                to None, which will set it to the day before the current date.
                Known by some as yesterday.
        ...
        Returns:
            The Solcast irradiance & weather data.
        """
        df_cats = utils.run_bluesky(self.project_name, start, end, tran_ghi, plot)
        return df_cats
    ...
    def _disable_faulty_sensors(self):
        """
        ...
        for i, row in self.sensor_OFF.query("Var_ID not in @ignore_ids").iterrows():
            ...
            if str(row['#']).lower() == 'eval':
                ...
                self.df_sensor_ON.loc[self.df_sensor_ON.eval(row['description']) & (self.df_sensor_ON.index >= s) & (self.df_sensor_ON.index <= e), sensor] = False
            else:
    ...
    def generate_bool(self):
        ...
        """
        # Get bool file from disk
        try:
            self.df_bool = pd.read_csv(self.bool_filepath, index_col=0, parse_dates=True)
        except:
            pass

        # Read raw powertrack data
        raw_df = pd.read_csv(self.powertrack_csv, index_col=0, parse_dates=True)
        raw_df = raw_df.query("index.dt.date > @self.static_date")
        ...
        wind_cols = get_col_locs(raw_df.columns, 'Wind_speed')
        ...
        self.df_sensors = raw_df[weather_cols].copy()

        # Apply data determination steps
        comms = det.comms(self.df_sensors)
        frozen = det.frozen(self.df_sensors, window=3)
        band_pass = det.band_pass(self.df_sensors, [(temp_cols, (-30, 120)),
                                               (wind_cols, (0, 67))])
        # We'll come back to implement some more complex logic later
        # spline_filter = det.spline_filter(self.df_sensors)
        df_bool = comms+frozen+band_pass
        
        # Set `self.df_bool` equal to calculated bool if empty. Update if exists.
        if self.df_bool.empty:
            self.df_bool = ~df_bool
        else:
            self.df_bool.update(~df_bool)
        pass
    ...
    def _calculate_sensor_average_data_sub(self, neighbor_sensor_avgs: dict = {}, get_solcast=False):
        """
        ...
        while any_nulls:
            ...
            if get_solcast:
                ...
                df_solcast = wr.athena.read_sql_query(
                    sql="""SELECT local_datetime
                                ,air_temp as "Tamb_avg"
                                ,wind_speed_10m as "Wind_speed"
                            FROM solcast_hourly
                            WHERE project=:project;
                            AND YEAR(local_datetime)=:year;
                        ORDER BY local_datetime""",
                    params={"project":f"'{self.project_name}'",
                            "year":f"{self.year}"
                            },
                    database='solcast'
                    )

                df_solcast = df_solcast.set_index('local_datetime')
                self.df_solcast = df_solcast.rename(columns={'tamb_avg': 'Tamb_avg', 'wind_speed': 'Wind_speed'})

                df_sensors_avg = df_sensors_avg.fillna(self.df_solcast)
            
            any_nulls = False

        self.df = self.df.join(df_sensors_avg)
    ...
    def __calculate_sensor_averages(self):
        ...
        # Calculating Temperature & Wind averages above in `calculate_bool_sensor_averages()`
