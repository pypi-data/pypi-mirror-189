            ...
            for file_name in file:
                ...
                if "whole_img_boot2.bin" in file_name:
                    if file_size > pt_parcel['pt_addr0']:
                        ...
                        raise Exception("Error: boot2 bin size is overflow with partition table")
                        return False
                for name in name_list:
                    ...
                    if new_name == "factory":
                        ...
                        if "whole_img.bin" in file_name:
                            if file_size > pt_parcel['fw_len']:
                                ...
                                raise Exception(
                                    "Error: fw bin size is overflow with partition table")
                                return False
                        continue
                    if "whole_img_" + new_name + ".bin" in file_name or new_name + ".bin" in file_name:
                        if file_size > pt_parcel[new_name + '_len']:
                            ...
                            raise Exception("Error: %s bin size is overflow with partition table" %
                                            new_name)
                            return False
        except Exception as e:
            bflb_utils.printf(e)
        ...
        try:
            while i < len(file):
                ...
                if "whole_img.bin" in file_name:
                    if file_size > pt_parcel['fw_len']:
                        #bflb_utils.printf("Error: fw bin size is overflow with partition table")
                        raise Exception("Error: fw bin size is overflow with partition table")
                        return False
                if "whole_img_mfg.bin" in file_name:
                    if file_size > pt_parcel['mfg_len']:
                        ...
                        raise Exception("Error: mfg bin size is overflow with partition table")
                        return False
                if "media.bin" in file_name:
                    if file_size > pt_parcel['media_len']:
                        ...
                        raise Exception("Error: media bin size is overflow with partition table")
                        return False
                if "whole_img_d0fw.bin" in file_name:
                    if file_size > pt_parcel['fw_d0_len']:
                        ...
                        raise Exception("Error: d0fw bin size is overflow with partition table")
                        return False
                if "imtb.bin" in file_name:
                    if file_size > pt_parcel['imtb_len']:
                        ...
                        raise Exception("Error: imtb bin size is overflow with partition table")
                        return False
                if "kv.bin" in file_name:
                    if file_size > pt_parcel['kv_len']:
                        #bflb_utils.printf("Error: kv bin size is overflow with partition table")
                        raise Exception("Error: kv bin size is overflow with partition table")
                        return False
                if "dtb.bin" in file_name:
                    if file_size > pt_parcel['dtb_len']:
                        ...
                        raise Exception("Error: dtb bin size is overflow with partition table")
                        return False
                i += 1
        except Exception as e:
            bflb_utils.printf(e)
...
class BflbIotTool(object):
    ...
    def bl60x_image_gen_cfg(self,
        ...
        if cfg_ini in [None, '']:
            if chiptype == "bl60x":
                f_org = os.path.join(chip_path, chipname, "img_create_iot",
                                     "img_create_cfg_dp.conf")
            else:
        ...
        if bintype == "fw":
            if group_type is None or group_type == "Group0":
                ...
                bootinfo_file = os.path.join(chip_path, chipname, "img_create_iot",
                                             "bootinfo_{0}.bin".format(group_type.lower()))
                img_file = os.path.join(chip_path, chipname, "img_create_iot",
    ...
    def bl60x_mfg_ota_header_haier(self, fw_ota_bin, chipname="bl60x", version="", dir_ota=None):
        ...
        fw_ota_bin_haier = bytes.fromhex(fw_ota_bin_ver_haier.encode('utf-8').hex() + \
                           '00'*7 + fw_ota_bin_sn_haier + fw_ota_bin_md5.encode('utf-8').hex() + \
                           version.encode('utf-8').hex() + '00'*(48-len(version)))
        if dir_ota:
    ...
    def bl60x_mfg_ota_xz_gen(self,
        ...
        if len(fw_ota_bin_xz) > fw_len:
            bflb_utils.printf(
                "Warn: fw1 xz bin size is overflow with partition table, don't create ota bin")
            return False
    ...
    def bl60x_mfg_ota_bin_gen(self,
                              chipname="bl60x",
                              chiptype="bl60x",
                              pt_parcel=None,
                              cpu_type=None,
        ...
        if cpu_type is None or cpu_type == "Group0":
            ...
            bootinfo_fw_path = os.path.join(chip_path, chipname, "img_create_iot",
        ...
        if cpu_type is None or cpu_type == "Group0":
            ...
            img_fw_path = os.path.join(chip_path, chipname, "img_create_iot",
        ...
        if len(fw_ota_bin) > pt_parcel[pt_fwlen_name]:
            ...
            raise Exception(
                "Error: ota bin size is overflow with partition table, don't create ota bin")
            return False
        ...
        #    bflb_utils.printf("Error: fw ota bin size is overflow with partition table, don't create ota bin")
        #    shutil.rmtree(ota_path)
        #    return False
        bflb_utils.printf("FW OTA bin is Done. Len is %d" % len(fw_ota_bin))
        if self.bl60x_mfg_ota_xz_gen(ota_path, chipname, chiptype, cpu_type,
                                     pt_parcel[pt_fw1len_name]) == False:
            bflb_utils.printf("Warning: remove OTA directory")
            shutil.rmtree(ota_path)
    ...
    def bl60x_image_gen(self,
        ...
        f = self.bl60x_image_gen_cfg(chipname, chiptype, raw_bin_name, bintype, key, iv, publickey,
        ...
        if key or (publickey and privarekey):
            ...
            self.efuse_load_en = False
        return img_create.create_sp_media_image_file(f, chiptype, cpu_type, security_save_efuse)
    ...
    def bl_write_flash_img(self, d_addrs, d_files, flash_size):
        ...
        whole_img_file = os.path.join(chip_path, self.chipname, "img_create_iot",
                    ...
                    if name + '_addr' in pt_parcel:
                        ...
                        if bind_bootinfo is True:
                            if name == "fw":
                                ...
                                bootinfo_filedata = bl_get_file_data(
                                    ["chips/" + chipname + "/img_create_iot/bootinfo.bin"])[0]
                                filedata = bl_get_file_data(
                                    ["chips/" + chipname + "/img_create_iot/img.bin"])[0]
                                if bootinfo_filedata[0:4] == filedata[0:4]:
                                    whole_img_data = filedata
                                else:
                                ...
                                fp.write(whole_img_data)
                                fp.close()
                                d_files.append("chips/" + chipname +
                                               "/img_create_iot/whole_img.bin")
                            else:
                                whole_img = chipnamedir + "/img_create_iot/whole_img_" + name + ".bin"
                                bootinfo_filedata = bl_get_file_data([
                                    "chips/" + chipname + "/img_create_iot/bootinfo_" + name +
                                    ".bin"
                                ])[0]
                                filedata = bl_get_file_data([
                                    "chips/" + chipname + "/img_create_iot/img_" + name + ".bin"
                                ])[0]
                                if bootinfo_filedata[0:4] == filedata[0:4]:
                                    whole_img_data = filedata
                                else:
                                    whole_img_len = offset + os.path.getsize(
                                        chipnamedir + "/img_create_iot/img_" + name + ".bin")
                                    whole_img_data = bl_create_flash_default_data(whole_img_len)
                                    whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                                    whole_img_data[offset:offset + len(filedata)] = filedata
                                fp = open(whole_img, 'wb+')
                                fp.write(whole_img_data)
                                fp.close()
                                d_files.append("chips/" + chipname + "/img_create_iot/whole_img_" +
                                               name + ".bin")
                            d_addrs.append(hex(pt_parcel[name + '_addr'])[2:])
                        else:
                            bootinfo_filedata = bl_get_file_data([
                                "chips/" + chipname + "/img_create_iot/bootinfo_" + name + ".bin"
                            ])[0]
                            filedata = bl_get_file_data(
                                ["chips/" + chipname + "/img_create_iot/img_" + name + ".bin"])[0]
                            if bootinfo_filedata[0:4] == filedata[0:4]:
                                d_files.append("chips/" + chipname + "/img_create_iot/img_" +
                                               name + ".bin")
                                d_addrs.append(hex(pt_parcel[name + '_addr'])[2:])
                            else:
                                d_files.append("chips/" + chipname + "/img_create_iot/bootinfo_" +
                                               name + ".bin")
                                d_addrs.append(hex(pt_parcel[name + '_addr'])[2:])
                                d_files.append("chips/" + chipname + "/img_create_iot/img_" +
                                               name + ".bin")
    ...
    def bl60x_mfg_flasher_eflash_loader_cfg_old(self,
                                                chipname,
                                                chiptype,
                                                bin_file,
                                                boot2,
        ...
        if bin_file is True:
            if 'fw_cpu0_addr' in pt_parcel:
                if bind_bootinfo is True:
                    ...
                    bootinfo_filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/bootinfo_cpu0.bin"])[0]
                    filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/img_cpu0.bin"])[0]
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        whole_img_data = filedata
                    else:
                        whole_img_len = 0x1000 + os.path.getsize(chipnamedir +
                                                                 "/img_create_iot/img_cpu0.bin")
                        whole_img_data = bl_create_flash_default_data(whole_img_len)
                        whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                ...
                if bind_bootinfo is True:
                    ...
                    bootinfo_filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/bootinfo_group0.bin"])[0]
                    filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/img_group0.bin"])[0]
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        whole_img_data = filedata
                    else:
                        whole_img_len = 0x2000 + os.path.getsize(chipnamedir +
                                                                 "/img_create_iot/img_group0.bin")
                        whole_img_data = bl_create_flash_default_data(whole_img_len)
                        whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                        whole_img_data[0x2000:0x2000 + len(filedata)] = filedata
                    fp = open(whole_img, 'wb+')
                    ...
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        ...
                        d_addrs.append(hex(pt_parcel['fw_group0_addr'])[2:])
                        d_files.append("chips/" + chipname + "/img_create_iot/img_group0.bin")
                        d_addrs.append(hex(pt_parcel['fw_group0_addr'] + 0x2000)[2:])
                ...
                if bind_bootinfo is True:
                    ...
                    bootinfo_filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/bootinfo.bin"])[0]
                    filedata = bl_get_file_data(["chips/" + chipname + "/img_create_iot/img.bin"
                                                ])[0]
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        whole_img_data = filedata
                    else:
                        whole_img_len = offset + os.path.getsize(chipnamedir +
                                                                 "/img_create_iot/img.bin")
                        whole_img_data = bl_create_flash_default_data(whole_img_len)
                        whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                        whole_img_data[offset:offset + len(filedata)] = filedata
                    fp = open(whole_img, 'wb+')
        ...
        if d0fw is True:
            if 'fw_d0_addr' in pt_parcel:
                ...
                if bind_bootinfo is True:
                    ...
                    bootinfo_filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/bootinfo_d0fw.bin"])[0]
                    filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/img_d0fw.bin"])[0]
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        whole_img_data = filedata
                    else:
                        whole_img_len = offset + os.path.getsize(chipnamedir +
                                                                 "/img_create_iot/img_d0fw.bin")
                        whole_img_data = bl_create_flash_default_data(whole_img_len)
                        whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                        whole_img_data[offset:offset + len(filedata)] = filedata
                    fp = open(whole_img, 'wb+')
                ...
                bflb_utils.printf("Warning: D0FW is not in partition table")
        ...
        if yocboot is True:
            if 'fw_yocboot_addr' in pt_parcel:
                ...
                if bind_bootinfo is True:
                    ...
                    bootinfo_filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/bootinfo_yocboot.bin"])[0]
                    filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/img_yocboot.bin"])[0]
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        whole_img_data = filedata
                    else:
                        whole_img_len = offset + os.path.getsize(chipnamedir +
                                                                 "/img_create_iot/img_yocboot.bin")
                        whole_img_data = bl_create_flash_default_data(whole_img_len)
                        whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                        whole_img_data[offset:offset + len(filedata)] = filedata
                    fp = open(whole_img, 'wb+')
                    ...
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        ...
                        d_files.append("chips/" + chipname +
                                       "/img_create_iot/bootinfo_yocboot.bin")
                        d_addrs.append(hex(pt_parcel['fw_yocboot_addr'])[2:])
                        d_files.append("chips/" + chipname + "/img_create_iot/img_yocboot.bin")
                        d_addrs.append(hex(pt_parcel['fw_yocboot_addr'] + offset)[2:])
            else:
                bflb_utils.printf("Warning: yocboot is not in partition table")
        ...
        if dtb is True:
            if 'dtb_addr' in pt_parcel:
                ...
                if bind_bootinfo is True:
                    ...
                    bootinfo_filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/bootinfo_dtb.bin"])[0]
                    filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/img_dtb.bin"])[0]
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        whole_img_data = filedata
                    else:
                        whole_img_len = offset + os.path.getsize(chipnamedir +
                                                                 "/img_create_iot/img_dtb.bin")
                        whole_img_data = bl_create_flash_default_data(whole_img_len)
                        whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                        whole_img_data[offset:offset + len(filedata)] = filedata
                    fp = open(whole_img, 'wb+')
                ...
                bflb_utils.printf("Warning: dtb is not in partition table")
        ...
        if imtb is True:
            if 'imtb_addr' in pt_parcel:
                ...
                bflb_utils.printf("Waning: imtb is not in partition table")
        ...
        if kv is True:
            if 'kv_addr' in pt_parcel:
                ...
                bflb_utils.printf("Warning: kv is not in partition table")

        if ro_params is not None and len(ro_params) > 0:
            if 'factory_addr' in pt_parcel:
                bl_ro_device_tree.bl_ro_params_device_tree(
                    ro_params, chipnamedir + "/device_tree/ro_params.dtb")
                d_files.append("chips/" + chipname + "/device_tree/ro_params.dtb")
                d_addrs.append(hex(pt_parcel['factory_addr'])[2:])
            else:
                bflb_utils.printf("Warning: factory is not in partition table")
        ...
        if media is True:
            if 'media_addr' in pt_parcel:
                ...
                bflb_utils.printf("Warning: media is not in partition table")
        ...
        if mfg is True:
            if 'mfg_addr' in pt_parcel:
                ...
                if bind_bootinfo is True:
                    ...
                    bootinfo_filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/bootinfo_mfg.bin"])[0]
                    filedata = bl_get_file_data(
                        ["chips/" + chipname + "/img_create_iot/img_mfg.bin"])[0]
                    if bootinfo_filedata[0:4] == filedata[0:4]:
                        whole_img_data = filedata
                    else:
                        whole_img_len = offset + os.path.getsize(chipnamedir +
                                                                 "/img_create_iot/img_mfg.bin")
                        whole_img_data = bl_create_flash_default_data(whole_img_len)
                        whole_img_data[0:len(bootinfo_filedata)] = bootinfo_filedata
                        whole_img_data[offset:offset + len(filedata)] = filedata
                    fp = open(whole_img, 'wb+')
                ...
                bflb_utils.printf("Warning: mfg is not in partition table")
    ...
    def bl60x_mfg_uart_flasher(self, uart, baudrate='57600', cfg_ini=None):
        ...
        exe_genitor([
            'python',
            os.path.join(chip_path, "libs/bflb_eflash_loader.py"), '--write', '--flash', '-c',
    ...
    def flasher_download_cfg_ini_gen(self, chipname, chiptype, cpu_type, config):
        ...
        if chiptype == "bl702" or chiptype == "bl702l":
            if 'boot2_download' in config[
                    "check_box"] and config["check_box"]['boot2_download'] is True:
                boot2_en = False
            else:
                boot2_en = True
        ...
        if 'factory_download' in config[
                "check_box"] and config["check_box"]['factory_download'] is True:
                ...
                if 'boot2_bin_input' in config['input_path']:
                    ...
                    if temp_bin is not None and len(temp_bin) > 0:
                        try:
                            shutil.copyfile(
                                temp_bin,
                                os.path.join(chip_path, chipname, "img_create_iot", "boot2.bin"))
                        except Exception as e:
                            bflb_utils.printf(e)
                    boot2_d = True
                ...
                if boot2 is not None and len(boot2) > 0:
                    ...
                    if config["check_box"]['sign']:
                        if "boot2_security" in pt_parcel:
                            ...
                            temp_sign = True
                    self.bl60x_fw_boot_head_gen(True, config['param']["chip_xtal"],
                                                self.bh_cfg_file, temp_encrypt, temp_sign,
        ...
        for i in range(len(name_list)):
            ...
            if len(name) > 7:
                bflb_utils.printf(
                    "Warning: the length of {} is exceeds the number 7 allowed".format(name))
                continue
            ...
            if name == "boot2":
                ...
                if 'factory_len' in pt_parcel and pt_parcel['factory_len'] == 0:
                    ...
                    continue
            if pt_parcel[name + "_header"] == 1:
                if name + '_download' in config["check_box"] and name + '_bin_input' in config[
                        'input_path']:
                    media_sign = 0
                    if name == "media":
                        ...
                        if config["check_box"]['media_download'] is True:
                            ...
                            if media_bin is not None and len(media_bin) > 0:
                                try:
                                    shutil.copyfile(
                                        media_bin,
                                        os.path.join(chip_path, chipname, "img_create_iot",
                                                     "media.bin"))
                                except Exception as e:
                                    bflb_utils.printf(e)
                    ...
                    if media_sign == 1 or media_sign == 2 or config["check_box"][
                            name + '_download'] is True:
                        if name + '_addr' in pt_parcel:
                            if chiptype == "bl808":
                                if media_sign == 2:
                                    media_path = os.path.join(chip_path, chipname,
                                                              "img_create_iot", "media.bin")
                                    temp_obj = "%s|UNUSED|UNUSED|" % media_path
                                else:
                                    temp_obj = "%s|UNUSED|UNUSED|" % config['input_path'][
                                        name + '_bin_input']
                                cfg_t.read(self.bh_cfg_file)
                                cfg_t.set(bootheader_section_name, 'group_image_offset',
                            ...
                            if temp_obj is not None and len(temp_obj) > 0:
                                ...
                                if config["check_box"]['sign']:
                                    if name + '_security' in pt_parcel:
                                        ...
                                        temp_sign = True
                                self.bl60x_fw_boot_head_gen(False, config['param']["chip_xtal"],
                                                            self.bh_cfg_file, temp_encrypt,
                                                            temp_sign, chipname, chiptype,
                                                            cpu_type, boot2_en)
                                ...
                                if name == "fw":
                                    ...
                                    if chiptype == "bl616":
                                        ...
                                        if cfg_flasher.has_option("THIRD_CFG", "amazon_header_enable"):
                                            if (cfg_flasher.get("THIRD_CFG", "amazon_header_enable") == "true"):
                                                ...
                                                shutil.copyfile(os.path.join(chip_path, chipname) + \
                                                               "/img_create_iot/img.bin", out_encrypt_file)
                                    ...
                                    if in_signed_header != "":
                                        with open(in_signed_header, 'rb') as fp:
                                            ...
                                            fp_header = open("chips/" + chipname + \
                                                            "/img_create_iot/bootinfo.bin", \
                                                            'wb+')
                                            fp_header.write(header_data)
                                            fp_header.close()
                                    if False == self.bl60x_mfg_ota_bin_gen(
                                            chipname, chiptype, pt_parcel, cpu_type,
                                            config["param"]["version"], config["param"]["ota"]):
                                        bflb_utils.set_error_code("0082")
                                        return bflb_utils.errorcode_msg()
                                # 注释部分是mfg的单独处理
                                """elif name == "mfg":
                ...
                if name == "media":
                    if 'media_download' in config[
                            "check_box"] and config["check_box"]['media_download'] is True:
                        media_bin = config['input_path']['media_bin_input']
                        if media_bin is not None and len(media_bin) > 0:
                            try:
                                shutil.copyfile(
                                ...
                                bflb_utils.printf(e)
                        bin_d_list[i] = True
                    if 'romfs_download' in config[
                            "check_box"] and config["check_box"]['romfs_download'] is True:
                        romfs_dir = config['input_path']['romfs_dir_input']
                        if romfs_dir is not None and len(romfs_dir) > 0:
                            ret = generate_romfs_img(
                                romfs_dir,
                    ...
                    if config["check_box"][name + '_download'] is True:
                        ...
                        if temp_bin is not None and len(temp_bin) > 0:
                            try:
                                ...
                                bflb_utils.printf(e)
                        bin_d_list[i] = True
        return self.bl60x_mfg_flasher_eflash_loader_cfg(chipname, chiptype, boot2_d, pt_parcel,
                                                        bin_d_list, name_list)
    ...
    def flasher_download_cfg_ini_gen_old(self, chipname, chiptype, cpu_type, config):
        ...
        if config["check_box"]['bin_download'] is True:
            if 'fw_addr' in pt_parcel:
                ...
                if bin is not None and len(bin) > 0:
                    ...
                    if False == self.bl60x_mfg_ota_bin_gen(chipname, chiptype, pt_parcel, cpu_type,
                ...
                bflb_utils.printf("Warning: FW is not in partition table")
            ...
            if config["check_box"]['d0_download'] is True:
                if 'fw_d0_addr' in pt_parcel:
                    if chiptype == "bl808":
                        ...
                        cfg_t.read(self.bh_cfg_file)
                        cfg_t.set(bootheader_section_name, 'group_image_offset',
                                  "0x%x" % (pt_parcel['fw_d0_addr'] + 0x1000))
                        cfg_t.write(self.bh_cfg_file)
                    else:
                    ...
                    if d0_bin is not None and len(d0_bin) > 0:
                        ...
                        self.bl60x_fw_boot_head_gen(False, config['param']["chip_xtal"],
                                                    self.bh_cfg_file,
                                                    config["check_box"]['encrypt'],
                                                    config["check_box"]['sign'], chipname,
                                                    chiptype, cpu_type, boot2_en)
                        self.bl60x_image_gen(chipname, chiptype, cpu_type, "d0fw", d0_bin,
                                             config["param"]['aes_key'], config["param"]['aes_iv'],
                                             config["input_path"]["publickey"],
                                             config["input_path"]["privatekey"])
                        d0_bin_d = True
        ...
        if 'yocboot_download' in config["check_box"] and 'yocboot_bin_input' in config[
                'input_path']:
            if config["check_box"]['yocboot_download'] is True:
                if 'fw_yocboot_addr' in pt_parcel:
                    if chiptype == "bl808":
                        yocboot_bin = "%s|UNUSED|UNUSED|" % config['input_path'][
                            'yocboot_bin_input']
                        cfg_t.read(self.bh_cfg_file)
                        cfg_t.set(bootheader_section_name, 'group_image_offset',
                                  "0x%x" % (pt_parcel['fw_yocboot_addr'] + 0x1000))
                        cfg_t.write(self.bh_cfg_file)
                    else:
                    ...
                    if yocboot_bin is not None and len(yocboot_bin) > 0:
                        ...
                        self.bl60x_fw_boot_head_gen(False, config['param']["chip_xtal"],
                                                    ...
                                                    config["check_box"]['sign'], chipname,
                                                    chiptype, cpu_type, boot2_en)
                        self.bl60x_image_gen(chipname, chiptype, cpu_type, "yocboot", yocboot_bin,
                                             config["param"]['aes_key'], config["param"]['aes_iv'],
                                             config["input_path"]["publickey"],
                                             config["input_path"]["privatekey"])
                        yocboot_bin_d = True
                ...
                if 'dtb_addr' in pt_parcel:
                    ...
                    if dtb_bin is not None and len(dtb_bin) > 0:
                        ...
                        self.bl60x_fw_boot_head_gen(False, config['param']["chip_xtal"],
                                                    ...
                                                    config["check_box"]['sign'], chipname,
                                                    chiptype, cpu_type, boot2_en)
                        self.bl60x_image_gen(chipname, chiptype, cpu_type, "dtb", dtb_bin,
                                             config["param"]['aes_key'], config["param"]['aes_iv'],
                                             config["input_path"]["publickey"],
                                             config["input_path"]["privatekey"])
                        dtb_bin_d = True
            ...
            if config["check_box"]['imtb_download'] is True:
                ...
                if imtb_bin is not None and len(imtb_bin) > 0:
                    try:
                        shutil.copyfile(
                            imtb_bin,
                            os.path.join(chip_path, chipname, "img_create_iot", "imtb.bin"))
                    except Exception as e:
                        bflb_utils.printf(e)
                imtb_bin_d = True
        ...
        if 'kv_download' in config["check_box"] and 'kv_bin_input' in config['input_path']:
            if config["check_box"]['kv_download'] is True:
                ...
                if kv_bin is not None and len(kv_bin) > 0:
                    try:
                        shutil.copyfile(
                            kv_bin, os.path.join(chip_path, chipname, "img_create_iot", "kv.bin"))
                    except Exception as e:
                        bflb_utils.printf(e)
                kv_bin_d = True
        ...
        if config["check_box"]['media_download'] is True:
            ...
            if media_bin is not None and len(media_bin) > 0:
                try:
                    shutil.copyfile(
                        media_bin, os.path.join(chip_path, chipname, "img_create_iot",
                                                "media.bin"))
                except Exception as e:
                    bflb_utils.printf(e)
            media_bin_d = True
        ...
        if config["check_box"]['romfs_download'] is True:
            ...
            if romfs_dir is not None and len(romfs_dir) > 0:
                ret = generate_romfs_img(
                    romfs_dir, os.path.join(chip_path, chipname, "img_create_iot", "media.bin"))
                if ret != 0:
                    bflb_utils.printf('ERROR, ret %s.' % ret)
                    error = 'ERROR, ret %s.' % ret
                    bflb_utils.printf(error)
        ...
        if config["check_box"]['mfg_download'] is True:
            if 'mfg_addr' in pt_parcel:
                ...
                bflb_utils.printf("Warning: mfg is not in partition table")
        return self.bl60x_mfg_flasher_eflash_loader_cfg(chipname, chiptype, bin_d, boot2_d,
                                                        ro_params_d, pt_parcel, media_bin_d,
                                                        mfg_bin_d, d0_bin_d, imtb_bin_d, kv_bin_d,
                                                        yocboot_bin_d, dtb_bin_d)
    ...
    def flasher_download_thread(self, chipname, chiptype, act, config, callback=None):
        ...
        if act != "build" and act != "download" and act != "erase":
        ...
        try:
            ...
            if act == "build":
                ...
                error = self.eflash_loader_t.efuse_flash_loader(args, self.eflash_loader_cfg,
                                                                eflash_loader_bin, callback, None,
                                                                None)
            ...
            # generation partition table/boot2/fw bin files and rf files
                ...
                if act == "erase":
                    ...
                    if not config["param"]["start"] and not config["param"]["end"]:
                        ...
                        if config["param"]["end"]:
                            options.extend(["--end=" + config["param"]["end"]])
                if "erase_only" in config[
                        "check_box"] and config["check_box"]["erase_only"] is True:
                    bflb_utils.printf("erase only")
                else:
                    error = self.flasher_download_cfg_ini_gen(chipname, chiptype, cpu_type, config)
                    if error is True:
                        ...
                        if  config["check_box"]["encrypt"] is True or\
                            ...
                            if  config["check_box"]["encrypt"] is True or\
                                config["check_box"]["sign"] is True:
                                cfg_file = os.path.join(chip_path, chipname, "img_create_iot",
                                                        "img_create_cfg.ini")
...
def iot_download_cmd(args):
    ...
    if not firmware or not os.path.exists(firmware):
        ...
        config["check_box"]["erase_only"] = True
    ...
    if chiptype == "bl60x":
        if not args.boot2:
            config["input_path"]["boot2_bin_input"] = os.path.join(chip_path, chipname,
                                                                   "builtin_imgs",
                                                                   "blsp_boot2.bin")
        else:
