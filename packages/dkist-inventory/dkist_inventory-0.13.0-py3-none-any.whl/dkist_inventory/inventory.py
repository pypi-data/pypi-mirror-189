...
def _get_world_coordinates(wcs) -> dict[str, u.Quantity]:
    """
    Return the world coordinates of all the edge pixels for all the dimensions correlated to a spatial dimension.
    """
    ## Compute the pixel indices of all edges of the higher-dimensional cube spanned by the axes correlated to a
    #  spatial dimension.
    ...
    return {name: world_coords[idx] << u.Unit(wcs.world_axis_units[idx])
...
def _compute_edge_pixels(wcs) -> list[np.ndarray]:
    ...
    # `slice_combos` contains all permutations of [0, -1, None] over all axes. For example, 2 axes will produce
    # [(0, 0), (0, -1), (0, None), (-1, 0), (-1, -1), (-1, None), (None, 0), (None, -1), (None, None)]
    # We'll use `None` to denote `slice(None)` (i.e., the whole axis) for now
    #
    # We need to use (a - 1) as the "end" edge pixel because wcs.array_index_to_world_values doesn't interpret negative
    # indices as starting in from the end of the array, but rather as being on the opposite side of 0. Freaky.
    slice_combos = product(*[[0, a - 1, None] for a in wcs_shape])
    # Edges are defined as only varying along a single axis at a time. So we use filter to grab slices that only
    # have a single `None` value.
...
def _inventory_from_wcs(wcs):
    ...
    known_fields = set(("custom:pos.helioprojective.lat",
                        "custom:pos.helioprojective.lon",
                        "em.wl",
                        "time",
                        "phys.polarization.stokes"))

    required_fields = set(("custom:pos.helioprojective.lat",
                           "custom:pos.helioprojective.lon",
                           "time"))
    ...
    inventory = {'boundingBox': ((_closest_angle_to_zero(min_dict["custom:pos.helioprojective.lon"].to_value(u.arcsec)),
                                  _closest_angle_to_zero(min_dict["custom:pos.helioprojective.lat"].to_value(u.arcsec))),
                                 (_closest_angle_to_zero(max_dict["custom:pos.helioprojective.lon"].to_value(u.arcsec)),
                                  _closest_angle_to_zero(max_dict["custom:pos.helioprojective.lat"].to_value(u.arcsec)))),
    ...
    if "phys.polarization.stokes" in present_world_types:
        ...
        # TODO: Related to todo above, remove this np.unique call when NDCube has been updated
        stokes_components = stokes_converter(np.unique(world_coords["phys.polarization.stokes"]))
...
def _closest_angle_to_zero(angle: float) -> float:
    """
...
def _inventory_from_headers(headers: Table):
    ...
    inventory["contributingExperimentIds"] = (_get_keys_matching(headers, r"EXPRID\d\d$", expected_type=str))
