...
from .awss3bucketobjectkey import KeyDecodeException, encode_key, decode_key, is_folder, join, split
from heaserver.service.heaobjectsupport import new_heaobject_from_type
from heaserver.service.db.awss3bucketobjectkey import replace_parent_folder
...
from ..wstl import add_run_time_action
...
async def get_account_by_id(request: web.Request) -> tuple[DesktopObjectDict | None, str | None]:
    """
    ...
    account, volume_id = next((a for a in await gather(
        *[get_account_and_volume_id(_get_account(request, v.id), v.id) async for v in
          request.app[HEA_DB].get_volumes(request, AWSFileSystem)])
                               if
                               a[0]['id'] == request.match_info['id']), None)
...
async def account_opener_by_id(request: web.Request) -> web.Response:
    ...
    representing possible choices for opening the HEA desktop object; or Not Found.
    """
    result, _ = await get_account_by_id(request)
    if result is None:
        return response.status_not_found()
    return await response.get_multiple_choices(request, result)
...
def _handle_client_error(e) -> Response:
    logger = logging.getLogger(__name__)
    error_code = e.response['Error']['Code']
