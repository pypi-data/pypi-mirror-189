...
class dailyTimeManager():
    ...
    def init(self):
        ...
        if self.start_time.__gt__(self.end_time):
            raise Exception('Bad config:end_time {} is not later than start_time {}!'.format(end_time, start_time))
    ...
    # def get_nxt_trading_time(self):
    #     #Daily
    #     if self.period == 0:
    #         return utils.getNextTradingDate(self.now, self.period)      
    #     else:
    #         return utils.getNextTradingBar(self.now, self.period)
    ...
    def get_between_date(self):
        ...
        if init_date.__gt__(final_date) :
            raise Exception('Bad config: init_date {} is not later than final_date {}'.format(init_date,final_date))        
