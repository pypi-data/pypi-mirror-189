...
def winrm(ctx, bucket, key, password=None, profile=None):
    ...
    from System.Security.Cryptography.X509Certificates import X509FindType
    from System.Security.Cryptography.X509Certificates import X509Store
    from System.Security.AccessControl import FileSystemAccessRule
    from System.Security.AccessControl import FileSystemRights
    from System.Security.AccessControl import AccessControlType
    from Security.Cryptography.X509Certificates import RSACertificateExtensions
    from System.IO import File
    thumbprint = install_cert_on_windows(
        ctx, bucket, key, profile, password, 'my')

    store = StoreName.My
    root_store = StoreLocation.LocalMachine
    store = X509Store(store, root_store)
    store.Open(OpenFlags.ReadWrite | OpenFlags.OpenExistingOnly)
    try:
        certs = store.Find(X509FindType.Thumbprint, thumbprint, False)
        for cert in certs:
            filename = None
            try:
                filename = cert.PrivateKey.CspKeyContainerInfo.UniqueKeyContainerName
                ...
                pass
            if not filename:
                filename = RSACertificateExtensions.GetRSAPrivateKey(cert)
            root_path = r'c:\programdata\microsoft\crypto\rsa\machinekeys'
            ...
            if os.path.isfile(filename):
                ...
                print(f'modifying acl on {filename}')
                acl = File.GetAccessControl(filename)
                acl.AddAccessRule(rule)
