...
def request_cert(ctx, filename, profile, defaults):
    ...
    for x in buckets:
        ...
        for namespace in namespaces:
            save_to_s3(
                ctx, bucket, namespace, values['hostname'], certificate,
...
def install_cert_on_windows(
    ...
    if pfx_password:
        ...
        password = NetworkCredential('', pfx_password)
    ...
    pfx.Import(pfx_data, password, flags)
    p = X509Store(store, root_store)
    p.Open(OpenFlags.ReadWrite | OpenFlags.OpenExistingOnly)
    exists = False
    t = pfx.Thumbprint
    for x in p.Certificates:
        if x.Thumbprint == t:
            print('cert already exists in cert store')
            exists = True
            break
    if not exists:
        p.Add(pfx)
