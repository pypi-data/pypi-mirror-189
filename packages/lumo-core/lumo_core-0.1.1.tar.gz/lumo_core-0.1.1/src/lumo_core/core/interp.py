...
class ABCContinuous(Interpolate, ABC):
...
class ABCPeriod(Interpolate, ABC):
    """
    period
    """

    def __init__(self, start=0, end=1, left=0, period=1, *args, **kwargs):
        super().__init__(start=start, end=end, left=left, period=period)
        self.left = left
        self.period = period
        self.start = start
        self.end = end
...
class Cos(ABCContinuous):
    ...
    def interp(cls, cur, start=0., end=1., left=0., right=1., *args, **kwargs):
        ...
        cos_ratio = 0.5 * (1 + math.cos(ratio * math.pi))
...
class Exp(ABCContinuous):
    ...
    def interp(cls, cur, start=0., end=1., left=0., right=1., *args, **kwargs):
        ...
        exp_ratio = math.exp((ratio - 1) * 5) - residual * (1 - ratio)
...
class Log(ABCContinuous):
    ...
    def interp(cls, cur, start=0., end=1., left=0., right=1., *args, **kwargs):
        ...
        log_ratio = 1 - math.exp(-ratio * 5) + residual * ratio
        return start * (1 - log_ratio) + end * log_ratio


class Constant(ABCContinuous):
    """Always return start value."""
...
class PeriodCos(ABCPeriod):
    ...
    def interp(cls, cur, start=0., end=1., left=0., period=1., *args, **kwargs):
        ...
        cos_ratio = 0.5 * (1 + math.cos(ratio * math.pi * 2))
        return start * cos_ratio + end * (1 - cos_ratio)


class PeriodHalfCos(ABCPeriod):
    """
    Half periodic cosine scheduler, period is (right-left)
    """
...
class PowerDecay(Interpolate):
    ...
    def __call__(self, cur):
        rate = math.power(self.decay_rate, cur // self.decay_steps)
        res = self.start * rate
...
class InterpolateList:
    ...
    def __init__(self, schedules: List[ABCContinuous]):
        super().__init__()
        self.schedules = schedules
        if len(schedules) > 1:
