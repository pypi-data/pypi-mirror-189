...
class ComplianceColumn(tables.TemplateColumn):
    """
    Render a colored column. Column is colored red when compliance is false, else is green
    """
    template_code = """
  {% load helpers %}
  {% if value is True %}
...
class NetosSoftwareReportTable(DeviceTable):
    actions = (
        'changelog',
    )
    ...
    class Meta(NetBoxTable.Meta):
        ...
        default_columns = ('pk', 'name', 'status', 'device_type',
                           'serial', 'compliance') + software_fields
        fields = (
            ...
            'vc_priority', 'comments', 'contacts', 'tags', 'created', 'last_updated', 'compliance') + software_fields
