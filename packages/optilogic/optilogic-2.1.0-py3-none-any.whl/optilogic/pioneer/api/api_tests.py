...
class TestApi(unittest.TestCase):
    ...
    def setUpClass(self) -> None:
        ...
        self.dir_local_current: str = os.path.dirname(__file__)
        self.dir_testdata_local: str = os.path.join(self.dir_local_current, 'quick_tests')
        assert(os.path.exists(self.dir_testdata_local))
        assert(len(os.listdir(self.dir_testdata_local)) >= 1)
        self.dir_testdata_remote: str = 'quick_tests'
        self.files_testdata_local: list[str] = []
        self.files_testdata_remote: list[str] = []
        self.py_run_me: str = ''
        self.py_run_me_quick: str = ''
        ...
        for f in os.listdir(self.dir_testdata_local):
            local: str = os.path.join(self.dir_testdata_local, f)
            if os.path.isfile(local) is False:
                continue
            elif os.path.getsize(local) == 0:
                continue
            dest: str = os.path.join(self.dir_testdata_remote, f)
    ...
    def test_000_prereqs(self) -> None:
        ...
        res: bool = self.API.util_job_monitor(self.WORKSPACE, self.API._job_start_recent_key, stop_when='done', secs_max=300)
    ...
    def test_account_jobs_active(self) -> None:
        ...
        resp = self.API._account_jobs(max_jobs=200) # BUG submitted counts as active therefore must account for excessive submitted jobs 
        for job in resp['jobs']:
        ...
        with self.subTest():
            for d in resp['storages']:
                ...
                if d['type'] == 'azure_workspace':
                    ...
                    self.assertTrue(d['bytesUsed'] is None or isinstance(d['bytesUsed'], int))
                    self.assertTrue(d['bytesUsedLastUpdated'] is None or isinstance(d['bytesUsedLastUpdated'], int))
                    self.assertIsInstance(d['created'], int)
                    ...
                    if d['defaultSchema'].startswith('anura_2_'):
                        ...
                        self.assertTrue(d['schemaStatus'] in ['error', 'invalid', 'valid'] or len(d['schemaStatus']) == 0) 
                        self.assertTrue(d['schemaStatusLastUpdated'] is None or isinstance(d['schemaStatusLastUpdated'], str))
    ...
    def test_wksp_file_copy(self) -> None:
        ...
        src_result: str = f"{resp['sourceFileInfo']['directoryPath']}/{resp['sourceFileInfo']['filename']}"
        dest_result: str = f"{resp['targetFileInfo']['directoryPath']}/{resp['targetFileInfo']['filename']}"
    ...
    def test_wksp_file_download_crash(self) -> None:
        ...
        resp: str = self.API.wksp_file_download(self.WORKSPACE, file_path='I_DONT_EXIST')
    ...
    def test_wksp_file_download_meta(self) -> None:
        ...
        self.assertEqual(resp['result'], 'success')
        keys: Tuple[str, ...] = ('result', 'workspace', 'filename', 'directoryPath', 'filePath', 'lastModified', 'contentLength', 'date', 'fileCreatedOn', 'fileLastWriteOn', 'fileChangeOn')
    ...
    def test_wksp_job_start(self) -> None:
        ...
        self.assertEqual(resp['result'], 'success')
        self.assertEqual(len(resp['jobKey']), 36)
        job_info_keys: Tuple[str, ...] = ('workspace', 'directoryPath', 'filename', 'command', 'resourceConfig', 'tags', 'timeout')
    ...
    def test_wksp_job_status(self) -> None:
        ...
        job_info_keys: Tuple[str, ...] = ('workspace', 'directoryPath', 'filename', 'command','errorFile', 'resultFile', 'resourceConfig', 'tags', 'timeout')
        for key in resp['jobInfo'].keys():
            self.assertIn(key, job_info_keys)
    ...
    def test_wksp_jobs(self) -> None:
        ...
        filter_keys: Tuple[str, ...] = ('command', 'history', 'runSecsMax', 'runSecsMin', 'status', 'tags')
        for filter in resp['filters']:
            self.assertIn(filter, filter_keys)

        self.assertGreaterEqual(len(resp['jobs']), 1)
        job_keys: Tuple[str, ...] = ('jobKey', 'submittedDatetime', 'startDatetime', 'endDatetime',
    ...
    def test_wksp_share_file(self) -> None:
        ...
        f: str = f"{resp['sourceFileInfo']['directoryPath']}/{resp['sourceFileInfo']['filename']}"
