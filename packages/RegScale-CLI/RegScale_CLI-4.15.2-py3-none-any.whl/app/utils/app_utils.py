...
def str_to_date(date_str: str) -> datetime:
    ...
    :param str date_str:
    :return: datetime object of provided string date
    :rtype: datetime
    """
...
def save_data_to(file_name: str, file_type: str, data) -> None:
    """
    ...
    if file_type.lower() == ".csv":
        ...
        try:
            # try save the data as a xlsx file
...
def days_between(vuln_time: str) -> int:
    ...
    :rtype: int
    """
    a = datetime.strptime(vuln_time, "%Y-%m-%dT%H:%M:%SZ")
    today = datetime.strftime(datetime.now(), "%Y-%m-%dT%H:%M:%SZ")
    b = datetime.strptime(today, "%Y-%m-%dT%H:%M:%SZ")
