...
def get_active_users(save_output_to: Path, file_type: str):
    ...
    if is_valid(app=app):
        ...
        with job_progress:
            ...
            fetching_users = job_progress.add_task(
                "[#f8b737]Fetching active users from Okta...", total=1
            )
...
def analyze_okta_users(
    user_list: list, key: str, filter_value: any, user_type: str
) -> list:
    """
            ...
            if isinstance(filter_value, datetime) and isinstance(data_filter, str):
                ...
                try:
                    data_filter = datetime.strptime(
                        data_filter, "%Y-%m-%dT%H:%M:%S.%fZ"
                    )
                # if an error is encountered, make it an old date
                except (TypeError, KeyError, AttributeError):
                    error_and_exit(
                        "Incorrect date format. Please follow this format: '%Y-%m-%dT%H:%M:%S.%fZ'"
                    )
...
def check_and_save_data(
    data: list, file_name: str, file_path: Path, file_type: str, data_desc: str
) -> None:
    """
    ...
    if len(data) >= 1:
        ...
        with job_progress:
            ...
            clean_data_task = job_progress.add_task(
                "[#21a5bb]Cleaning data from Okta...", total=1
            )
...
def get_all_okta_users(api) -> list:
    """
    ...
    # add task for fetching users from Okta
    fetching_users = job_progress.add_task(
        "[#f8b737]Fetching all users from Okta...", total=1
    )
...
def get_okta_data(api, task, url: str, headers: dict, params: Tuple = None) -> list:
    """
    Function to use the Okta core API to get data, also handles pagination for Okta requests
    :param api: API object
    :param task: task to update to show user live progress
...
def clean_okta_output(data: list, skip_keys: list = None) -> dict:
    ...
    list data from Okta API response and returns a clean dictionary
    without any nested dictionaries
...
def authenticate_with_okta(app, api, type: str) -> None:
    """
    ...
    if type.lower() == "ssws":
        ...
        if verify_response.status_code in requests.codes.ok:
...
def parse_url_for_pagination(raw_string: str) -> str:
    """
    Function to parse the provided string and get the URL for pagination in Okta
