...
def _init_low_pass_filter (box, simulation_params) -> None :
...
def _function_low_pass_filter (box, event_) -> List :
    
    v = 0
    current_u_derivatives = []
    current_y_derivatives = []
    input_value = box.get_input(0).value
    LogManager().debug(f'low pass input value : {input_value}')
    ...
    if box['order'] > 0 :
        ...
        for j in range(box['order']).__reversed__() :
            LogManager().debug(f'time : {event_.timing: .3f} - order : {j} - derivative : {current_y_derivatives[j]}')
            v *= event_.step_time
