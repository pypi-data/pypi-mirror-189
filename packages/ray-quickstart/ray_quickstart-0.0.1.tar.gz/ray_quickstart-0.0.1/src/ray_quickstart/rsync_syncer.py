...
class RsyncSyncer(Syncer):
    ...
    def sync_up(self, local_dir: str, remote_dir: str, exclude: Optional[List] = None) -> bool:
        ...
        try:
            ...
            remote_dir = expand_user_home_path(remote_dir, self.driver_username, self.driver_platform)
            sync_cmd = f'rsync -avz -e "ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no" {local_dir}/ {self.driver_username}@{self.driver_hostname}:{remote_dir}/'
    ...
    def sync_down(self, remote_dir: str, local_dir: str, exclude: Optional[List] = None) -> bool:
        ...
        try:
            ...
            if remote_dir.startswith('file://'):
                remote_dir = remote_dir[7:]
            remote_dir = expand_user_home_path(remote_dir, self.driver_username, self.driver_platform)
            sync_cmd = f'rsync -avz -e "ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no" --delete --ignore-errors {remote_dir}/ {self.worker_username}@{self.worker_hostname}:{local_dir}/'
    ...
    def delete(self, local_dir: str) -> bool:
        ...
        worker_dir = expand_user_home_path(local_dir, self.worker_username, self.worker_platform)
