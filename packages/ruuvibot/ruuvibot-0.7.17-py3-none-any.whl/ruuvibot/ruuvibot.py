...
def get_ruuvi_data():
    ...
    for ruuvi_data_point in ruuvi_data:
        ...
        ruuvi_data[ruuvi_data_point]["temperature_calibrated"] = (
...
async def temperature(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ruuvi in SETTINGS["ruuvitags"]:
        ...
        time_since_update = datetime.datetime.now() - datetime.datetime.strptime(data[0], '%Y-%m-%d %H:%M:%S')
        text += f"{data[1]}: {data[3]} °C  ({ time_since_update / datetime.timedelta(minutes=1) } minuuttia sitten) \n" 
