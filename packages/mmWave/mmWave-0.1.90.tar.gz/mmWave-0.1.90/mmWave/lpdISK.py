...
# v0.0.1 : 2019/12/03 release
# v0.0.2 : 2023/02/03 release
#
...
class LpdISK:
	...
	def __init__(self,port):
		...
		#print("V7 structure: [(tid,posX,posY,velX,velY,accX,accY,posZ,velZ,accZ),....]") #v0.0.1
		print("V7 structure: [(tid,posX,posY,posZ,velX,velY,velZ,accX,accY,accZ,EC[16],g,confi),....]") #v0.0.2
	...
	def tlvTypeInfo(self,dtype,count,dShow):
		...
		if dtype == 6:
			...
			sbyte = 8 	   #tlvHeader Struct = 8 bytes
			#dataByte = 40  #target struct 40 (v0.0.1) => bytes:(tid,posX,posY,posZ,velX,velY,velZ,accX,accY,accZ) 
			dataByte = 112  #target struct 40 (v0.0.2) => bytes:(tid,posX,posY,posZ,velX,velY,velZ,accX,accY,accZ) + ec[16] + g + confi = 40 + 64 + 4 + 4 = 112
			...
			if lstate == 'idle':
				...
				if idx == 8:
					...
					try:
						...
						#print("\n\n(TL)numTLVs({:d}): tlvCount({:d})-------ttype:tlvLength:{:d}:{:d}\n".format(self.hdr.numTLVs,tlvCount,ttype,self.tlvLength))
				...
				if (idx%dataBytes == 0):
					...
					try:
						#(tid,posX,posY,velX,velY,accX,accY,posZ,velZ,accZ) = struct.unpack('I9f', sbuf) #v0.0.1
						 
						(tid,posX,posY,velX,velY,accX,accY,posZ,velZ,accZ,
						ec0,ec1,ec2,ec3,ec4,ec5,ec6,ec7,ec8,ec9,ec10,ec11,
						ec12,ec13,ec14,ec15,g,cfd) = struct.unpack('I27f', sbuf) 
						v7.append((tid,posX,posY,velX,velY,accX,accY,posZ,velZ,accZ))    #v0.0.2
