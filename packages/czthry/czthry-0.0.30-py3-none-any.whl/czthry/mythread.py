...
class MyProcess(object):
    ...
    def begin(self, data, func, each_callback=None, finish_callback=None):
        ...
        while True:
            if not data:
                break
            sub = data[:self._batch]
            data = data[self._batch:]
            threads = []
            for i in range(len(sub)):
                arg = sub[i]
                thread = self._threads_pool.get(i)
                if thread:
                    thread.reset(func, args=(arg,))
                else:
                    thread = MyThread(func, args=(arg,), semaphore=self._semaphore)
                    self._threads_pool[i] = thread
                thread.start()
    ...
    def begin_v2(self, data, func, each_callback=None, finish_callback=None):
        ''' v2 - func处理函数的参数为 index, item
        :param data: 需要处理的数据，数组
        :param func: 处理数据的方法
        :param each_callback: 每处理一个数据，回调方法
        :param finish_callback: 处理完所有数据，回调方法
...
if __name__ == '__main__':
    ...
    while True:
        ...
        ret = process.begin_v2(arr, go_v2, each, finish)
        time.sleep(3)
    pass
