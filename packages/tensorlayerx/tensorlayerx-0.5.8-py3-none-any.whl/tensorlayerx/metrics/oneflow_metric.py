...
class Accuracy(Metric):
    ...
    def update(self, y_pred, y_true):
        ...
        y_pred = flow.argsort(y_pred, dim=-1, descending=True)
        y_pred = y_pred[:, :self.topk]
        if (len(y_true.shape) == 1) or (len(y_true.shape) == 2 and y_true.shape[-1] == 1):
            y_true = flow.reshape(y_true, (-1, 1))
        elif y_true.shape[-1] != 1:
            y_true = flow.argmax(y_true, dim=-1, keepdim=True)
        correct = y_pred == y_true
        correct = correct.to(flow.float32)
...
def acc(predicts, labels, topk=1):
    y_pred = flow.argsort(predicts, dim=-1, descending=True)
    y_pred = y_pred[:, :topk]
    if (len(labels.shape) == 1) or (len(labels.shape) == 2 and labels.shape[-1] == 1):
        y_true = flow.reshape(labels, (-1, 1))
    elif labels.shape[-1] != 1:
        y_true = flow.argmax(labels, dim=-1, keepdim=True)
    correct = y_pred == y_true
    correct = correct.to(flow.float32)
    correct = correct.cpu().numpy()
    num_samples = np.prod(np.array(correct.shape[:-1]))
