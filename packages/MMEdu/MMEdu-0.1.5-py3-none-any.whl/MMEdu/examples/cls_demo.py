...
def only_infer_demo():
	...
	result = model.inference(image=img, show=True, class_path=class_path,checkpoint = checkpoint, device='cuda')
...
def fast_infer_demo():
	...
	# class_path = '../dataset/cls/cats_dogs_dataset/classes.txt'
	# 推理，“show=True”表示不弹出识别结果窗口
	model.load_checkpoint(device='cuda',checkpoint=checkpoint, class_path=class_path)
	import time
	begin = time.time()
	for i in range(10):
