...
class workflow_lambda_host:
    ...
    def create_view(self, vf : voxelfarmclient.rest, workflow_request : request, name : str, view_type : str, view_lambda : str, inputs : dict, props:dict):
            ...
            if view_type == None:
                ...
                if view_lambda.startswith(script_dir):
                    self.lambda_framework.log('view_lambda is an absolute path')
                else:
                    self.lambda_framework.log('view_lambda is a relative path')
                    view_lambda = os.path.join(script_dir, view_lambda)
