...
class SvmSchema(ResourceSchema, metaclass=ResourceSchemaMeta):
    ...
    anti_ransomware_default_volume_state = fields.Str(
...
* dry_run"""
    ...
    auto_enable_activity_tracking = fields.Boolean(
    ...
    auto_enable_analytics = fields.Boolean(
...
Example: svm1"""
    ...
    ndmp = fields.Nested("netapp_ontap.models.svm_ndmp.SvmNdmpSchema", data_key="ndmp", unknown=EXCLUDE)
...
Example: 02c9e252-41be-11e9-81d5-00a0986138f7"""
    ...
    """links,aggregates,aggregates_delegated,anti_ransomware_default_volume_state,auto_enable_activity_tracking,auto_enable_analytics,certificate.links,certificate.name,certificate.uuid,cifs.links,cifs.ad_domain,cifs.allowed,cifs.enabled,cifs.name,comment,dns,fc_interfaces,fcp,ip_interfaces,ipspace.links,ipspace.name,ipspace.uuid,is_space_enforcement_logical,is_space_reporting_logical,iscsi,language,ldap.ad_domain,ldap.base_dn,ldap.bind_dn,ldap.enabled,ldap.servers,max_volumes,name,ndmp,nfs,nis,nsswitch,nvme,qos_policy.links,qos_policy.max_throughput_iops,qos_policy.max_throughput_mbps,qos_policy.min_throughput_iops,qos_policy.min_throughput_mbps,qos_policy.name,qos_policy.uuid,s3.links,s3.certificate,s3.default_unix_user,s3.default_win_user,s3.enabled,s3.is_http_enabled,s3.is_https_enabled,s3.name,s3.port,s3.secure_port,snapmirror,snapshot_policy.links,snapshot_policy.name,snapshot_policy.uuid,state,subtype,uuid,"""
    ...
    """aggregates,anti_ransomware_default_volume_state,auto_enable_activity_tracking,auto_enable_analytics,certificate.name,certificate.uuid,cifs.allowed,comment,fcp,ip_interfaces,is_space_enforcement_logical,is_space_reporting_logical,iscsi,language,max_volumes,name,ndmp,nfs,nsswitch,nvme,qos_policy.max_throughput_iops,qos_policy.max_throughput_mbps,qos_policy.min_throughput_iops,qos_policy.min_throughput_mbps,qos_policy.name,qos_policy.uuid,snapmirror,snapshot_policy.name,snapshot_policy.uuid,state,"""
    ...
    """aggregates,anti_ransomware_default_volume_state,auto_enable_activity_tracking,auto_enable_analytics,cifs.ad_domain,cifs.allowed,cifs.enabled,cifs.name,comment,dns,fc_interfaces,fcp,ip_interfaces,ipspace.name,ipspace.uuid,is_space_enforcement_logical,is_space_reporting_logical,iscsi,language,ldap.ad_domain,ldap.base_dn,ldap.bind_dn,ldap.enabled,ldap.servers,max_volumes,name,ndmp,nfs,nis,nsswitch,nvme,routes,s3.certificate,s3.default_unix_user,s3.default_win_user,s3.enabled,s3.is_http_enabled,s3.is_https_enabled,s3.name,s3.port,s3.secure_port,snapmirror,snapshot_policy.name,snapshot_policy.uuid,subtype,"""
...
"""
    ...
    if RECLINE_INSTALLED:
        ...
        def svm_show(
            fields: List[Choices.define(["aggregates_delegated", "anti_ransomware_default_volume_state", "auto_enable_activity_tracking", "auto_enable_analytics", "comment", "is_space_enforcement_logical", "is_space_reporting_logical", "language", "max_volumes", "name", "state", "subtype", "uuid", "*"])]=None,
        ) -> ResourceTable:
