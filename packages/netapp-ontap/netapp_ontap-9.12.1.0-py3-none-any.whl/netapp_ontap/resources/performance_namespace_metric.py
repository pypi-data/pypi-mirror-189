...
__pdoc__ = {
    ...
    "PerformanceNamespaceMetric.performance_namespace_metric_show": False,
    "PerformanceNamespaceMetric.performance_namespace_metric_create": False,
    "PerformanceNamespaceMetric.performance_namespace_metric_modify": False,
    "PerformanceNamespaceMetric.performance_namespace_metric_delete": False,
}
...
def _get_field_list(field: str) -> Callable[[], List]:
    def getter():
        return [getattr(r, field) for r in PerformanceNamespaceMetric.get_collection(fields=field)]
    return getter
...
async def _wait_for_job(response: NetAppResponse) -> None:
    ...
    while True:
        ...
        if job.state == "failure":
            raise NetAppRestError("PerformanceNamespaceMetric modify job failed")
...
class PerformanceNamespaceMetric(Resource):
    ...
    def get_collection(
        ...
        **kwargs
        ...
        r"""Retrieves historical performance metrics for an NVMe namespace."""
        return super()._get_collection(*args, connection=connection, max_records=max_records, **kwargs)

    get_collection.__func__.__doc__ += "\n\n---\n" + inspect.cleandoc(Resource._get_collection.__doc__)

    if RECLINE_INSTALLED:
        @recline.command(name="performance namespace metric show")
        def performance_namespace_metric_show(
            uuid,
            ...
            """Fetch a list of PerformanceNamespaceMetric resources
            ...
            return PerformanceNamespaceMetric.get_collection(
                uuid,
                **kwargs
            )
