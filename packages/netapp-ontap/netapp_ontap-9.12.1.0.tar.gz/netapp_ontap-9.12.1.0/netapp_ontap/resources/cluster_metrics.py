...
__pdoc__ = {
    ...
    "ClusterMetrics.cluster_metrics_show": False,
    "ClusterMetrics.cluster_metrics_create": False,
    "ClusterMetrics.cluster_metrics_modify": False,
    "ClusterMetrics.cluster_metrics_delete": False,
}
...
def _get_field_list(field: str) -> Callable[[], List]:
    def getter():
        return [getattr(r, field) for r in ClusterMetrics.get_collection(fields=field)]
    return getter
...
async def _wait_for_job(response: NetAppResponse) -> None:
    ...
    while True:
        ...
        if job.state == "failure":
            raise NetAppRestError("ClusterMetrics modify job failed")
...
class ClusterMetrics(Resource):
    ...
    if RECLINE_INSTALLED:
        @recline.command(name="cluster metrics show")
        def cluster_metrics_show(
            fields: List[Choices.define(["duration", "status", "timestamp", "*"])]=None,
        ) -> ResourceTable:
            """Fetch a list of ClusterMetrics resources
            ...
            return ClusterMetrics.get_collection(
                **kwargs
            )
