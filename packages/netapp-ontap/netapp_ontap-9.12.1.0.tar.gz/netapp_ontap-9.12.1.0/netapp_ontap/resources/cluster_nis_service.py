...
class ClusterNisServiceSchema(ResourceSchema, metaclass=ResourceSchemaMeta):
    ...
    binding_details = fields.List(fields.Nested("netapp_ontap.models.binding_details.BindingDetailsSchema", unknown=EXCLUDE), data_key="binding_details")
...
* [`DOC /security/authentication/cluster/nis`](#docs-security-security_authentication_cluster_nis)"""
    ...
    if RECLINE_INSTALLED:
        @recline.command(name="cluster nis service show")
...
* [`DOC /security/authentication/cluster/nis`](#docs-security-security_authentication_cluster_nis)"""
    ...
    if RECLINE_INSTALLED:
        @recline.command(name="cluster nis service create")
        async def cluster_nis_service_create(
            ...
            Args:
                links: 
                binding_details: An array of objects where each object represents the NIS server and it's status for a given NIS domain. It is an advanced field.
                bound_servers: 
                domain: The NIS domain to which this configuration belongs. 
                servers: A list of hostnames or IP addresses of NIS servers used by the NIS domain configuration. 
            """
            ...
            if binding_details is not None:
                kwargs["binding_details"] = binding_details
            if bound_servers is not None:
                kwargs["bound_servers"] = bound_servers
            if domain is not None:
                kwargs["domain"] = domain
            ...
            try:
                ...
                raise ReclineCommandError("Unable to create ClusterNisService: %s" % err)
            return [resource]
...
* [`DOC /security/authentication/cluster/nis`](#docs-security-security_authentication_cluster_nis)"""
    ...
    if RECLINE_INSTALLED:
        @recline.command(name="cluster nis service modify")
        async def cluster_nis_service_modify(
            ...
            try:
                ...
                raise ReclineCommandError("Unable to modify ClusterNisService: %s" % err)
...
* [`DOC /security/authentication/cluster/nis`](#docs-security-security_authentication_cluster_nis)"""
    ...
    if RECLINE_INSTALLED:
        @recline.command(name="cluster nis service delete")
        async def cluster_nis_service_delete(
            ...
            try:
                ...
                raise ReclineCommandError("Unable to delete ClusterNisService: %s" % err)
