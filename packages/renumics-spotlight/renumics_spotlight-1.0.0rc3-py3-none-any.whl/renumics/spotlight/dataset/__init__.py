...
def _get_column_type_name(column_type: Type[ColumnType]) -> str:
    """
...
def _get_column_type(
    ...
    if isinstance(x, str):
        try:
            ...
            raise exceptions.InvalidDTypeError(f"Unknown column type: {x}.") from e
    if isinstance(x, h5py.Dataset):
        return _get_column_type(x.attrs["type"])
    if isinstance(x, h5py.AttributeManager):
        return _get_column_type(x["type"])
    raise TypeError(
...
class Dataset:
    ...
    def _default_default(cls, column_type: Type[ColumnType]) -> Any:
        ...
        raise exceptions.InvalidAttributeError(
            ...
            f"{_get_column_type_name(column_type)} should be set, but `None` received."
        )
    ...
    def get_column_type(
        ...
        if as_string:
            return type_name
        return _get_column_type(type_name)
    ...
    def _pretty_table(self) -> prettytable.PrettyTable:
        ...
        for column in columns:
            ...
            column_type = _get_column_type(type_name)
            optional_keys = set(
    ...
    def _append_column(
        ...
        **attrs: Any,
        ...
        try:
            ...
            column.attrs["type"] = _get_column_type_name(column_type)
    ...
    def _encode_ref_value(
        ...
        if issubclass(column_type, Mesh) and isinstance(value, trimesh.Trimesh):
            value = column_type.from_trimesh(value)
        elif issubclass(column_type, _BaseFileBasedData) and isinstance(
