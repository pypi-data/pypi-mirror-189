...
if __name__ == "__main__":
    ...
    if mode == "service": # start no gui, only run service
        ...
        applogic.on_app_exited.attach_observer(qt_app.stop)
        qt_app.run()
        ...
        guiservice.start(block=False)
        chromeapp = PyHtmlChromeApp(url=guiservice.get_url())
        applogic.on_app_exited.attach_observer(chromeapp.close)
        chromeapp.show()
        chromeapp.join() # wait for chrome to exit
        ...
        if sys.platform == "darwin":         # on OSX, overwrite the default menu bar.
            window.addMenuButton(["exit"],applogic.exit)  # this will overwrite the default osx menus "quit <application>" button, for other names that have special meaning on osx see https://www.riverbankcomputing.com/static/Docs/PyQt4/qmenubar.html#qmenubar-on-mac-os-x
            window.addMenuButton(["View", "Show"], window.show)
        ...
        if sys.platform == "darwin":
            ...
            window.addMenuButton(["View", "Show"], window.show)
            window.addMenuButton(["View", "Hide"], window.hide)
            window.on_closed_event.attach_observer(qt_app.hide_osx_dock) # hide dock item if window is closed, ala "minimize to tray"
        ...
        if sys.platform == "darwin":
            ...
            window.addMenuButton(["View", "Show"], window.show)
            window.addMenuButton(["View", "Hide"], window.hide)
            window.on_closed_event.attach_observer(qt_app.hide_osx_dock)
            window.on_show_event.attach_observer(qt_app.show_osx_dock)
            qt_app.on_activated_event.attach_observer(window.show)
