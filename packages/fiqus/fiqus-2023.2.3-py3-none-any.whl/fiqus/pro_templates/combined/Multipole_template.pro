...
Group {
  ...
  <<rm.air_far_field.vol.name>> = Region[ <<rm.air_far_field.vol.number>> ];  // AirInf
...
{% endfor %}
  ...
  <<nc.omega>><<nc.air_far_field>> = Region[ <<rm.air_far_field.vol.name>> ];
  <<nc.omega>><<nc.conducting>> = Region[ {<<nc.omega>><<nc.powered>>, <<nc.omega>><<nc.iron>>, <<nc.omega>><<nc.induced>>} ];
  <<nc.omega>> = Region[ {<<rm.air.vol.name>>, <<rm.air_far_field.vol.name>>, <<nc.omega>><<nc.powered>>, <<nc.omega>><<nc.iron>>, <<nc.omega>><<nc.induced>>} ];
...
Function {
  ...
  nu [ Region[{<<rm.air.vol.name>>, <<nc.omega>><<nc.powered>>, <<rm.air_far_field.vol.name>>, <<nc.omega>><<nc.induced>>}] ]  = 1. / mu0;
