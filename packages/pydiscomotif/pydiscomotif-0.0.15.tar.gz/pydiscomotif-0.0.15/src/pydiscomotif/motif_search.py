...
from pydiscomotif.constants import (AMINO_ACID_RELAXED_GROUPS_MAP,
...
def dataframes_within_tolerance_ranges_generator(
    ...
    for bin_combination in itertools.product(C_alpha_distance_bins_list, sidechain_CMR_distance_bins_list, vector_angle_bins_list):
        ...
        index_file = index_folder_path / 'index' / f'{pair_of_residues}_{C_alpha_distance_bin}_{sidechain_CMR_distance_bin}_{vector_angle_bin}.{compression}' # e.g /home/user_name/database_folder/pyDiscoMotif_index/index/AG_4_4_5.bz2
...
def update_map_of_PDBs_with_all_residue_pairs(
    ...
    for PDB_ID in list(map_of_PDBs_with_all_residue_pairs.keys()):
        if PDB_ID not in updated_set_of_PDBs_with_all_residue_pairs:
            ...
            map_of_PDBs_with_all_residue_pairs[PDB_ID].update(new_residue_pair_data[PDB_ID]) # We update the set of residue pairs of the PDBs that have all the residue pairs

    return

def find_PDBs_with_all_residue_pairs(
        motif_MST: nx.Graph, index_folder_path: Path, distance_delta_thr: float, angle_delta_thr: float, compression: str, concurrent_executor: ProcessPoolExecutor
    ) -> Dict[str, Set[Tuple[str,str]]]:
    """
    """
    ...
    submitted_futures: List[Future[Dict[str, Set[Tuple[str,str]]]]] = [
    ...
    map_of_PDBs_with_all_residue_pairs: Dict[str, Set[Tuple[str,str]]]
    ...
    for future in as_completed(submitted_futures):
        ...
        if map_of_PDBs_with_all_residue_pairs_initialized_flag == False:
            map_of_PDBs_with_all_residue_pairs = defaultdict(set, PDBs_that_contain_the_residue_pair)
...
def filter_out_PDBs_with_unconnected_residue_pairs(
        PDBs_with_all_residue_pairs: Dict[str, Set[Tuple[str, str]]], motif_MST: nx.Graph, concurrent_executor: ProcessPoolExecutor, 
...
def get_PDBs_with_similar_motifs(
    ...
    with ProcessPoolExecutor(max_workers=n_cores) as concurrent_executor:
        ...
        for motif_MST in get_all_motif_MSTs_generator(motif_MST, max_n_mutated_residues, residue_type_policy, motif_residues_data):
            ...
            # After finding PDBs that contain all the residue pairs of the given motif_MST in their correct geometric arrangement, 
            # we need to make sure these pairs are connected and not just scattered. PDBs with connected pairs correspond to 
            # those that contain a similar motif(s). 
...
def search_index_for_PDBs_with_similar_motifs(
    ...
    if not index_folder_exists(index_folder_path):
        raise ValueError("Could not find all the index folders and files. Did you previously create the index with the 'create-index' command ? Does the index path point towards a pyDiscoMotif_index folder ?")
