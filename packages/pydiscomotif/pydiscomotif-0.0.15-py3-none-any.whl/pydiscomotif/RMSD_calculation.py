...
def calculate_RMSD_between_motif_and_similar_motifs(
    ...
    pyDiscoMotif_results_dict: Dict[str, List[Any]] = {
        'matched_motif':[], 'PDB_ID':[], 'similar_motif_found':[], 'n_mutations':[], 'RMSD':[], 'header_description':[]
    }
    for future in concurrent.futures.as_completed(submitted_futures):
        ...
        pyDiscoMotif_results_dict['matched_motif'].append(' '.join(residue_mapping_dict.values()))
        pyDiscoMotif_results_dict['PDB_ID'].append(PDB_ID)
        pyDiscoMotif_results_dict['similar_motif_found'].append(' '.join(residue_mapping_dict.keys()))
        pyDiscoMotif_results_dict['n_mutations'].append(n_mutations)
        pyDiscoMotif_results_dict['RMSD'].append(RMSD)
        pyDiscoMotif_results_dict['header_description'].append(header_description)
    
    pyDiscoMotif_results_df = pd.DataFrame(pyDiscoMotif_results_dict)
    return pyDiscoMotif_results_df[pyDiscoMotif_results_df.RMSD <= RMSD_threshold].sort_values(by=['n_mutations', 'RMSD'], ignore_index=True)
