...
from pydiscomotif.constants import (INDEX_MAX_ANGLE_VALUE,
                                    INDEX_MAX_DISTANCE_VALUE)
from pydiscomotif.indexing import create_index_folder, update_index_folder
from pydiscomotif.motif_search import search_index_for_PDBs_with_similar_motifs
from pydiscomotif.RMSD_calculation import calculate_RMSD_between_motif_and_similar_motifs


@click.group(help='pyDiscoMotif: a tool for the discovery of similar 3D structural motifs across proteins.', context_settings={'max_content_width':2000})
...
def check_index_path_option(ctx: Any, param: Any, value: Union[None, Path]) -> Path:
    ...
    default_index_folder_path: Path = ctx.params['database_path'] / 'pyDiscoMotif_index'
...
@click.option('--index_path', default=None, show_default=False, type=click.Path(file_okay=False, dir_okay=True, readable=True, writable=True, path_type=Path), 
              callback=check_index_path_option, help="Full path of the directory that will contain the index files. Defaults to <database_path>/pyDiscoMotif_index.")
...
def create_index(database_path: Path, index_path: Path, pattern: str, compression: str, n_cores: int) -> None:
    """
    Command to create the index needed for the rapid search of similar 3D structural motifs with pyDiscoMotif. If the --index_path option is not provided, a directory 
    named 'pyDiscoMotif_index' will be created in the given database path. Once the indexing is completed, the directory can be renamed or moved 
...
def update_index(database_path: Path, pattern: str, index_path: Path, n_cores: int) -> None:
    """
    Command to update an existing pyDiscoMotif index with new PDB files. The new PDB files are determined by comparing the name 
    ...
    INDEX_PATH  Full path of the pyDiscoMotif index directory to update (e.g: /home/user/Downloads/PDB/pyDiscoMotif_index).
    """
...
def check_results_output_path_option(ctx: Any, param: Any, value: Union[None, Path]) -> Path:
    if value:
        ...
        return Path(os.getcwd()) / f'pyDiscoMotif_result_{now_timestamp}.csv'
...
@click.option('--results_output_path', type=click.Path(path_type=Path), default=None, callback=check_results_output_path_option,
              help='Full path of the csv file where the results will be saved (e.g: /home/user/Downloads/similar_motifs.csv). If not given, the results will be saved in the current working directory in a file named with the current timestamp (e.g: pyDiscoMotif_result_14230214102022.csv)')
@click.option('--residue_type_policy', default='strict', callback=check_residue_type_policy_option,
               help='''\b\nOption to control whether mutated versions of the query motif should also be tested. There are 3 possibilities:\n 1) strict (DEFAULT): only similar motifs with strictly identical residues to the query motif are searched.\n 2) relaxed: finds PDBs with motifs that are similar but with potential mutated residues by allowing residues to mutate according to the residue group they are part of, such as polar, positively charged, aromatic, etcetc (see our Github for the exact grouping of the residues).\n 3) custom position specific exchange: specify possible residue mutations for the desired positions in your motif using a JSON formated string (e.g: '{"A1":"KL", "A2":"YW", "A5":"E"}'. Note the use of single and double quotes).\n To control the maximum number of simultaneous mutations, see the max_n_mutated_residues option.''')
@click.option('--max_n_mutated_residues', type=click.IntRange(min=1), default=1, show_default=True,
               help='Maximum number of simultaneous mutations that should be allowed when generating mutated motifs through the residue_type_policy option. For example, when set to 1, versions of the query motif with 1 residue mutated according to the given residue_type_policy will be generated and checked, in addition to the reference query motif itself. Note that very high values for this option will result in longer computation time given all the possible combinations of mutated residues will generated and searched for in the index. For most use cases, we recommend to use a value between 1 and 3.')
...
@click.option('--RMSD_threshold', type=click.FloatRange(min=0, min_open=True), default=1.0, show_default=True, 
...
def motif_search(
    ...
    Arguments
    ---------
    INDEX_PATH  Full path of the pyDiscoMotif index directory to use for the motif search (e.g: /home/user/Downloads/PDB/pyDiscoMotif_index).
    ...
    pyDiscoMotif_results_df = calculate_RMSD_between_motif_and_similar_motifs(
        motif_MST=motif_MST, 
        PDB_file=pdb_file, 
    ...
    pyDiscoMotif_results_df.to_csv(results_output_path)
    if len(pyDiscoMotif_results_df) == 0:
