...
class FirstSeat():
    ...
    def update_manager(self):
        ...
        if suit:
            ...
            if suit_strategy == 'missing_ace':
                if Card('A', suit.name) not in player.total_unplayed_cards[suit.name]:
                    manager.set_suit_strategy(player.seat, suit.name, '')

            if suit_strategy == 'missing_king':
                if Card('K', suit.name) not in player.total_unplayed_cards[suit.name]:
                    manager.set_suit_strategy(player.seat, suit.name, '')

            if suit_strategy == 'missing_queen':
                if Card('Q', suit.name) not in player.total_unplayed_cards[suit.name]:
                    manager.set_suit_strategy(player.seat, suit.name, '')

            if suit_strategy == 'ruff_in_void' and player.trump_suit:
                if (manager.voids[player.seat][player.trump_suit.name] or
                        manager.voids[player.partner_seat][player.trump_suit.name]):
    ...
    def _best_suit(self, score_reasons: Dict[str, int]) -> Suit:
        ...
        suit = Suit(random.choice(candidate_suits))
        return suit
