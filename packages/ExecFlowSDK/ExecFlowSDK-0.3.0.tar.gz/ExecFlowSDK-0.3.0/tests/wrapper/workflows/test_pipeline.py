...
def test_run_pipeline(entry_point: str, node_input: "Union[Path, str, dict]") -> None:
    ...
    from execflow.wrapper.workflows.pipeline import OTEPipeline
...
def test_result_pipeline(samples: "Path") -> None:
    ...
    # Have to add `sorted()` here, since the mapping strategy uses an (unsorted) `set`
    # for "triples" in its implementation.
    assert sorted(result["session"]["triples"]) == sorted(
        declarative_pipeline_file["strategies"][1]["triples"]
    )
