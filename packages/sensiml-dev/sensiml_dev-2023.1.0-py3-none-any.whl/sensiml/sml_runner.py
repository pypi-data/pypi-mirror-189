...
def compile_libsensiml_shared_library(path):
    ...
    try:
        ...
        if os.path.exists("libtensorflow-microlite.a"):
            print("calling: ar -x libtensorflow-microlite.a")
            subprocess.call(["ar", "-x", "libtensorflow-microlite.a"])
...
class struct_decision_tree_model_result(ctypes.Structure):
    __slots__ = ["number_of_trees", "classification_counts", "class_percentages"]

    _fields_ = [
        ("number_of_trees", ctypes.c_ubyte),
        ("classification_counts", ctypes.POINTER(ctypes.c_uint8)),
        ("class_percentages", ctypes.POINTER(ctypes.c_float)),
    ]
...
class SMLRunner(object):
    ...
    def __init__(self, path, class_map=None):
        self._run_type = None
        ...
        if path[-3:] == "zip":
            ...
            with zipfile.ZipFile(path, "r") as zip_ref:
                ...
                path = os.path.join(unzip_dir, "knowledgepack", "libsensiml")

        if os.name == "nt":
            if not os.path.exists(os.path.join(path, "libsensiml.dll")):
                print("ERROR: libsensiml.dll as not found in {}.".format(path))
                raise ImportPathException(
                    "ERROR: libsensiml.dll as not found in {}".format(path)
                )

            clf_lib = CDLL(os.path.join(path, "libsensiml.dll"))

        if os.name == "posix":
            if not os.path.exists(os.path.join(path, "libsensiml.so")):
                print(
                    "Warning: libsensiml.so as not found in {}. Attempting".format(path)
                )
                if not os.path.exists(os.path.join(path, "libsensiml.a")):
                    print("ERROR: libsensiml.a as not found in {}.".format(path))

                print("Attempting to compile libsensiml.so")

                compile_libsensiml_shared_library(path)
        ...
        self._kb_get_classification_result_info.restype = None
    ...
    def retrain_model(self, model_index):
        """
        After a model has been scored, this will restraing the model according
        to the models supported reinforcement learning approach
    ...
    def run_model(
        self, data_sample, model_index, debug_log=True, model_api="run_model"
    ):
        """
    ...
    def get_segment_data(self, model_index: int):
    ...
    def get_model_framelen(self, model_index: int):
    ...
    def get_feature_vector(self, model_index, feature_vector_buffer_size=4096):
    ...
    def get_segment_length(self, model_index):
    ...
    def get_feature_bank_number(self, model_index):
    ...
    def get_model_result(self, model_index: int, model_type: str):
        """
        Get the model results object for the most recent classification of a model type


        Args:
            model_index (int): the model index to use
            model_type (str): tensorflow lite for microcontrollers or decision tree ensemble
        ...
        if model_type.lower() == "tensorflow lite for microcontrollers":
            model_index_ctype = ctypes.c_int(model_index)
        ...
        elif model_type.lower() == "decision tree ensemble":
            model_index_ctype = ctypes.c_int(model_index)
            model_result = struct_decision_tree_model_result()
            classification_counts = (ctypes.c_uint8 * 128)()
            class_percentages = (ctypes.c_float * 128)()
            model_result.classification_counts = ctypes.POINTER(classification_counts)
            model_result.class_percentages = ctypes.POINTER(class_percentages)
        else:
