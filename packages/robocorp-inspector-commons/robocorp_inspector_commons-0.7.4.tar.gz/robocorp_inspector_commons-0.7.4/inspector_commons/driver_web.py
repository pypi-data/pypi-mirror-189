...
class WebDriver:
    ...
    def _init_driver(self, preferred_browser: Optional[str] = None):  # noqa: C901
        browsers = (
            [preferred_browser]
        ...
        for browser in browsers:
            ...
            try:
                ...
                download = (
                    webdriver.download(browser) if browser.lower() != "safari" else None
                )
                if download:
    ...
    def start(self, preferred_browser: Optional[str] = None):
        ...
        self.selenium = self._init_driver(preferred_browser)
    ...
    def stop(self):
        ...
        try:
            if self.selenium:
                self.logger.debug(
                    "[FORCE] Stopping browser: %s", self.selenium.service.process.pid
                )
                browser_proc = psutil.Process(self.selenium.service.process.pid)
