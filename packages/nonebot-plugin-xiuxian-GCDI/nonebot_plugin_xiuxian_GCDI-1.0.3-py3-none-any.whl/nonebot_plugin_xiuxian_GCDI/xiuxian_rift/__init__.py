...
create_rift = on_command("生成秘境", priority=5, permission= GROUP | GUILD and (SUPERUSER | GUILD_SUPERUSER|GUILD_ADMIN))
        ...
        for group_id in groups:
            ...
            if XiuConfig().img:
                ...
                if isinstance(group_id, int):
                    ...
                    await bot.send_group_forward_msg(group_id=int(group_id), messages=pic)
                else:
                    ...
                    await bot.send_guild_channel_forward_msg(guild_id=int(guild_id), channel_id=channel_id, messages=pic)
            else:
                if isinstance(group_id, int):
                    #await bot.send_group_msg(group_id=int(group_id), message=msg)
                    await bot.send_group_forward_msg(group_id=int(group_id), messages=msg)
                else:
                    guild_num = group_id.split('@')
                    guild_id = guild_num[0]
                    channel_id = guild_num[1]
                    #await bot.send_guild_channel_msg(guild_id=int(guild_id), channel_id=channel_id, message=msg)
                    await bot.send_guild_channel_forward_msg(guild_id=int(guild_id), channel_id=channel_id, messages=msg)
