...
携带灵石：{boss['stone']}
        ...
        i = 1
        for boss in bosss:
            bossisms += f"编号{i}、{boss['jj']}Boss:{boss['name']} \n"
            i += 1
        msg = bossisms
        if XiuConfig().img:
            pic = await get_msg_pic(msg)
...
async def _(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    if isinstance(event, GuildMessageEvent):
        ...
        n =''
    l_msg = []
    l_msg.append(f"道友目前拥有的世界积分：{user_boss_fight_info['boss_integral']}点{n}")
    if boss_integral_shop != {}:
        for k, v in boss_integral_shop.items():
            msg = f"{n}编号:{k}\n"
            msg += f"描述：{v['desc']}\n"
            msg += f"所需世界积分：{v['cost']}点{n}"
            l_msg.append(msg)
    else:
...
async def data_check(bot: Bot, event: MessageEvent):
    """
    判断用户信息是否存在
    """
    if isinstance(event, GroupMessageEvent):
        ...
        user_qq = event.get_user_id()
        group_id = 0
        msg = sql_message.get_user_message2(user_qq)
        if msg:
            pass
        else:
            await bot.send(event=event, message=f"没有您的信息，输入【我要修仙】加入！")
