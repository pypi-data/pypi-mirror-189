...
def ridge_latitude_of_wpsh(hgt,u):
    temp1=hgt.sel(lon=[110,120,130]).where(hgt>586,drop=True)
            ...
            for idt in temp2['dtime'].values:
                ...
                for ilon in temp2['lon'].values:
                    if(temp2.sel(member=imdl,time=it,dtime=idt,lon=ilon).dropna(dim='lat',how='all').size > 0):
                        ...
                        temp4.append(temp3)
                temp5.append(pd.DataFrame({'id':1,'time':it,'dtime':idt,'lon':['110,120,130'],'lat':np.mean(temp4),imdl:np.mean(temp4)}))
        if(rp_wsph is None):
...
if __name__=='__main__':
    ...
    v500=metdig.io.era5.get_model_grids(init_times=init_times,var_name='v',level=500)
    rp_wsph=ridge_latitude_of_wpsh(hgt500,u500,v500)
    print(rp_wsph)