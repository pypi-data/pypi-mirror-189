...
from autoregressive import decode,batch_decode
from tokenization import FullTokenizer

tokenizer = FullTokenizer(vocab_file=r'D:\onnx\vocab.txt',do_lower_case=False)


seq_length = 256
sdk_inf = csdk_object(config)
if sdk_inf.valid():
    def fn_predict_call(fn_args, b_input_ids: np.ndarray,
                      b_input_mask: np.ndarray,
                      b_seg_ids: np.ndarray,
                      b_temperature: np.ndarray,
                      b_top_k: np.ndarray,
                      b_top_p: np.ndarray):
        net_stage = 0
        ret, outputs = sdk_inf.process(net_stage, b_input_ids.astype(np.int64), b_input_mask.astype(np.int64))
        logits = outputs[0]
        return logits
    result = decode(text='[unused4]',
                    tokenizer=tokenizer,
                    end_tokens=['$','[SEP]'],
                    max_length=100,
                    num=10,
