...
CONFIGS_PATH = "/".join(LIB_FOLDER.split("/")[:-5])


def QuikBaseCreate():
    if os.path.exists(f"{CONFIGS_PATH}/configs.tuser"):
    ...
    keys = data.keys()
    if data['res']:
        configFile = open(f"{CONFIGS_PATH}/configs.tuser", 'w')
        for i in keys:
            if i != "res":
...
def GetBaseStruct():
    if os.path.exists(f"{CONFIGS_PATH}/configs.tuser"):
        configs = open(f"{CONFIGS_PATH}/configs.tuser")
...
def DropBase():
    if os.path.exists(f"{CONFIGS_PATH}/configs.tuser"):
        if input("Are you sure? [Y/N] ").lower() == "y":
            os.remove(f"{CONFIGS_PATH}/configs.tuser")
            print ("Base deleted")
        else:
            print ("Dropping canceled")
    else:
        print ("There is no Tuser configs")

def GetConfigs():
    if os.path.exists(f"{CONFIGS_PATH}/configs.tuser"):
        data = open(f"{CONFIGS_PATH}/configs.tuser").readlines()
        for d in data:
            print (d)
