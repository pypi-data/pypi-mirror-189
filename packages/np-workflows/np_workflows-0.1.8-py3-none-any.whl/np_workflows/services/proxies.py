...
class ScriptCamstim(Camstim):
    ...
    def pretest(cls) -> None:
        ...
        logger.info('%s | Pretest: running %s with MTrain stage params for mouse %s', cls.__name__, cls.script, pretest_mouse)
        cls.initialize()
        cls.test()
        cls.start()
        while not cls.is_ready_to_start():
...
class SessionCamstim(Camstim):
    ...
    def pretest(cls) -> None:
        ...
        logger.info('%s | Pretest with mouse %s, user %s', cls.__name__, cls.labtracks_mouse_id, cls.lims_user_id)
        super().pretest()
...
class PlatformJson(JsonRecorder, pydantic.BaseModel):
    ...
    foraging_id_list: list[str] = pydantic.Field(default_factory=lambda: [''], regex=foraging_id_re)
    stimulus_name: str = ''
    script_name: str = ''
    mouse_weight_pre_float: float = .0
    mouse_weight_post: str = ''
    rig_id: str = Rig.ID or ''
    operatorID: str = ''
    ...
    ExperimentNotes: dict[str, dict[str, Any]] = dict(BleedingOnInsertion={}, BleedingOnRemoval={})
    DiINotes: dict[str, str]= dict(EndTime='', StartTime='', dii_description='', times_dipped='')
    InsertionNotes: dict = pydantic.Field(default_factory=dict)
    water_supplement: float = .0
    workflow_start_time: str = ''
    platform_json_save_time: str = ''
    manifest_creation_time: str = ''
    workflow_complete_time: str = ''
    mouse_weight_pre: str = ''
    water_calibration_heights: list[float] = [.0]
    water_calibration_volumes: list[float] = [.0]
    wheel_height: str = ''
