...
class Address(db.Base):  # type: ignore
    ...
    users = orm.relationship("User", secondary=users_to_addresses, backref="addresses")
...
class TokenBalance(db.Base):
    ...
    amount = Column(Numeric(precision=10, scale=10), nullable=False)
    price = Column(Numeric(precision=10, scale=10), nullable=True)
