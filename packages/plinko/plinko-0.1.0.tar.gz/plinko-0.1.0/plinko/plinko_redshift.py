...
def plinko_redshift(environment, 
                    database = None, server = None):
    '''
    Connect to AWS Redshift databases in Kortex

    TODO: Add support for additional databases and servers, currently
    default to REDSHIFT_default_database when connecting (limited service account
    ...
    if environment.upper() == "DEV":
        server_id   = "REDSHIFT_DEV_server"
        uid_id      = "REDSHIFT_DEV_uid"
        pwd_id      = "REDSHIFT_DEV_pwd"

    # PROD
    elif environment.upper() == "PROD":
        server_id   = "REDSHIFT_PROD_server"
        uid_id      = "REDSHIFT_PROD_uid"
        pwd_id      = "REDSHIFT_PROD_pwd"

    # QA
    elif environment.upper() == "QA":
        server_id   = "REDSHIFT_QA_server"
        uid_id      = "REDSHIFT_QA_uid"
        pwd_id      = "REDSHIFT_QA_pwd"

    else:
        print("Please specify PROD, DEV, or QA as your environment.")
        return None

    # Establish connection
    cnxn = pyodbc.connect(Driver = 'redshift',
                          servername = zoltar_ask(server_id),
                          database = zoltar_ask("REDSHIFT_default_database") if database is None else database,
                          UID = zoltar_ask(uid_id),
                          PWD = zoltar_ask(pwd_id),
                          Port = zoltar_ask("REDSHIFT_default_port"))
