...
__all__ = ['Driver', 'CategoryScrapper', 'get_category_df_from_page', 'update_page_num', 'run', 'SingleItemScrapper',
           ...
           'ShopstatScrapper', 'authorize', 'get_single_item_df', 'run']
...
def authorize(self):
    ...
    logger.info(f"SUCCESS: authorized at shopstat.ru")

    # wait until enter the main page:
    while driver.current_url!='https://app.shopstat.ru/':
        logger.info("Waiting 0.1 s to enter main page")
        sleep(0.1)
    logger.info(f"SUCCESS: entered main page at shopstat.ru")

    # wait main page to upload:
    sleep(0.5)

# Cell
@fcb.patch_to(ShopstatScrapper)
def get_single_item_df(self, code):
...
def run(self, code_list, postgres_creds, save_to_postgres=False):
    ...
    """
    METHOD FOR DOWNLOADING MULTIPLE ITEMS' SOLD_QTY.
    ...
    # for code in codes - scrap its sold_qty:
    dfs = []
    for code in code_list:
        # if something goes wrong - retry to scrap item for 3 times:
        for i in [1, 2, 3]:
            try:
                df = self.get_single_item_df(code)
                dfs.append(df)
                break
            except:
                if i==3:
                    logger.info(f"FAIL: can't scrap code {code} at shopstat.ru")

    multiple_items_df = pd.concat(dfs)

    logger.info(f"MULTIPLE_ITEMS_DF: {multiple_items_df.head(3)}") ## delete me

    if save_to_postgres:
        pc = PostgresConnector(**postgres_creds)
        pc.check_connection()
        pc.save(df=multiple_items_df, table_name='shopstat_table', schema='main_schema', if_table_exists='append')
        logger.info(f"SUCCESS: saved to shopstat_table in Postgres")

    return multiple_items_df