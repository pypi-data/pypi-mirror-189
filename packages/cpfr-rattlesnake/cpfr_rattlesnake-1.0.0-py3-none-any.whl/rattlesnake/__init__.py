...
def run():
            ...
            if not file_path.startswith("_"):
                ...
                if file_path.lower().endswith(".html"):
                    ...
                    if "_template" in front_matter.keys():
                        ...
                        rendered_html = render_template(source_path, {**config, **metadata, "_content": rendered_markdown}, front_matter["_template"], dependencies[file_path])
                    else:
    ...
    if args.watch:
        ...
        print("Starting HTTP server on Port %d..." % args.port)
        http_server = http.server.HTTPServer(('', args.port), lambda *_: http.server.SimpleHTTPRequestHandler(*_, directory=args.output_dir))
