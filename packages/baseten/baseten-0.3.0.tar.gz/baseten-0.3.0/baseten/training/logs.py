...
# Needed to aboid circular dependencies
if TYPE_CHECKING:
...
_PENDING_STATE_DELAY_SECONDS = 1.0
_RUNNING_STATE_DELAY_SECONDS = 0.2
...
def _typed_and_ordered_training_logs(training_run_id: str,
    ...
    return sorted(
        [
            LogLine(**d)
            for d in inner_logs
        ], key=lambda logline: int(logline.ts)
    )
...
def _dt_from_nanos_str(timestamp: int) -> datetime:
    return datetime.fromtimestamp(float(timestamp) / 1e9).replace(tzinfo=pytz.UTC)
...
class TrainingLogsConsumer:
    ...
    def _refresh(self):
        self._run.refresh()
        self._start_time: Optional[datetime] = datetime.fromisoformat(
            self._run.started).replace(tzinfo=pytz.UTC) if self._run.started else None
        self._stop_time: Optional[datetime] = datetime.fromisoformat(
            self._run.stopped).replace(tzinfo=pytz.UTC) if self._run.stopped else None

    def _get_lastest_time(self) -> datetime:
        return self._stop_time if self._stop_time else datetime.now()
    ...
    def _consume_logs_period(
        ...
        while query_end <= end_time:
            logs.extend(_typed_and_ordered_training_logs(
                self._entity_id,
    ...
    def _get_next_start_time(self, last_logs: List[LogLine], last_start_time: datetime) -> datetime:
        if len(last_logs) > 0:
            ...
            # NOTE: this is slightly error prone due to the discrepency of
            #       loki working in nanos and graphql taking millis
            return _dt_from_nanos_str(last_logs[-1].ts) + timedelta(milliseconds=1)
    ...
    def stream_logs(self):
        ...
        while self._run.is_running:
            retrieved_logs = self._consume_logs_period(period_start_time, self._get_lastest_time())
