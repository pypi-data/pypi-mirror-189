    ...
    if model_info is not None and not publish:
        ...
        if resp.succeeded or not resp.needs_full_deploy:
            if resp.succeeded:
                ...
                logger.error(f'Failed to patch: `{resp.error_message}`. Model left in original state.')
...
def try_patch_draft_truss(
    ...
    if model_deployment_status != "MODEL_READY":
        return TryPatchDraftTrussResponse(
            succeeded=False,
            needs_full_deploy=True,
            model_version_info=dev_model_version_info,
            error_message=f"model {model_name} is not ready.",
        )
...
def build_and_deploy_truss(
    ...
    if isinstance(model, TrussHandle):
        return deploy_truss(model, model_name, version_bump, is_trusted, publish, external_model_version_id)
    truss = build_truss(model)
    return deploy_truss(truss, model_name, version_bump, is_trusted, publish, external_model_version_id)
