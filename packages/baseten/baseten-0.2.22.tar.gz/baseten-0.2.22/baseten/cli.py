...
from baseten.client_commands import (baseten_cli, finetuning_cli, models_cli,
                                     pretrained_cli, upload_cli)
from baseten.common.settings import (get_server_url, read_config,
                                     set_config_value, set_server_url)
from baseten.common.util import setup_logger

logger = logging.getLogger(__name__)

DEFAULT_NETWORK_ERROR_MESSAGE = "Sorry, something went wrong on our end. Please try again in 5 minutes."
...
@cli_group.command()
@click.option('--email', prompt='Email Address', hide_input=False)
def signup(email):
    """
    The sign-up process:
...
cli_group.add_command(upload_cli.upload)
cli_group.add_command(finetuning_cli.finetune)
