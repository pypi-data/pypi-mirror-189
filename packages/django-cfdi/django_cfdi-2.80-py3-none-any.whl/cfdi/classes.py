...
class CfdiPago():
    ...
    def generar_cfdi(self, configuracion, timbrado_prueba=None, pac=None):
            ...
            for imp_p in impuestos_p.values():
                ...
                if imp_p['impuesto_p'] == "002":
                    ...
                    if str(imp_p['tasa_o_cuota_p']) == "0.000000":
                        ...
                        base_iva_total_0 = base
                        impuesto = to_precision_decimales(imp_p['importe_p'], 2)
                        impuesto_iva_0 = to_precision_decimales(imp_p['importe_p'], 2)
                    
                    if str(imp_p['tasa_o_cuota_p']) == "0.080000":
                        base = to_precision_decimales(imp_p['base_p'], 2)
                        base_iva_total_8 = base
                        impuesto = to_precision_decimales(imp_p['importe_p'], 2)
                        impuesto_iva_8 = to_precision_decimales(imp_p['importe_p'], 2)
                    
                    if str(imp_p['tasa_o_cuota_p']) == "0.160000":
                        base = to_precision_decimales(imp_p['base_p'], 2)
                        base_iva_total_16 = base
                        impuesto = to_precision_decimales(imp_p['importe_p'], 2)
                        impuesto_iva_16 = to_precision_decimales(imp_p['importe_p'], 2)

                    if str(imp_p['tasa_o_cuota_p']) == "Exento":
                        base = to_precision_decimales(imp_p['base_p'], 2)
                        base_iva_total_excento = base
                        impuesto = None
        ...
        if base_iva_total_16:
            cp.TotalTrasladosBaseIVA16 = to_decimal_prodiga(base_iva_total_16 * tc)
            cp.TotalTrasladosImpuestoIVA16 = to_decimal_prodiga(impuesto_iva_16 * tc) 

        if base_iva_total_8:
            cp.TotalTrasladosBaseIVA8 = to_decimal_prodiga(base_iva_total_8 * tc)
            cp.TotalTrasladosImpuestoIVA8 = to_decimal_prodiga(impuesto_iva_8 * tc) 
        
        if base_iva_total_0:
            cp.TotalTrasladosBaseIVA0 = to_decimal_prodiga(base_iva_total_0 * tc)
            cp.TotalTrasladosImpuestoIVA0 = to_decimal_prodiga(impuesto_iva_0 * tc) 

        if base_iva_total_excento:
            cp.TotalTrasladosBaseIVAExento = to_decimal_prodiga(base_iva_total_excento * tc) 
