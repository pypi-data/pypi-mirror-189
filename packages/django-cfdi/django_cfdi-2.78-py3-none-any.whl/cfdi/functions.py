...
def to_decimal_prodiga(valor):
    valor_split = str(valor).split(".")[-1] 
    if int(valor_split[2]) <= 5:
        return to_precision_decimales((math.floor(Decimal(valor) * 100) / 100), 2)
    else:
        return to_precision_decimales(valor, 2)
