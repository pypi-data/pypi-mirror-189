...
import autoai_ts_libs.deps.tspy.time_series as TimeSeries
from autoai_ts_libs.deps.tspy.functions import interpolators as autoai_ts_libs.deps.tspy_interpolators

# try:
#     #from autoai_ts_libs.deps.tspy import TSContext
#     # import autoai_ts_libs.deps.tspy
#     os.environ['autoai_ts_libs.deps.tspy_LAZY_JVM'] = "true"
#     # import autoai_ts_libs.deps.autoai_ts_libs.deps.tspy as autoai_ts_libs.deps.tspy
#     #import autoai_ts_libs.deps.tspy
#     from autoai_ts_libs.deps.tspy.functions import interpolators as autoai_ts_libs.deps.tspy_interpolators
#     #from autoai_ts_libs.deps.autoai_ts_libs.deps.tspy.functions import interpolators as autoai_ts_libs.deps.tspy_interpolators
#
...
#
#     except Exception as e:
#         logger.warning("Exception importing autoai_ts_libs.deps.tspy twice")
#         logger.warning(e)
...
class WatForeInterpolator(TransformerMixin, BaseEstimator):
    ...
    def __init__(self, interpolator_type ='nearest',append_tranform=False, ts_icol_loc=-1,ts_ocol_loc=-1
        ...
        # This will be not be needed once new version of autoai_ts_libs.deps.tspy is release
        # try:
        #      import os
        ...
        self.autoai_ts_libs.deps.tspy_interpolators = autoai_ts_libs.deps.tspy_interpolators
        self.debug = debug
        ...
        if 'imputer_fill_type' in kwargs.keys() and self.interpolator_type=='fill':
            ...
            if self.imputer_fill_type not in ['mean','median','value']:
                ...
                raise Exception("Invalid 'Fill' imputer type specified. Supported types are: 'mean', 'median' and 'value'.")
                ...
                if 'imputer_fill_value' in kwargs.keys():
                    ...
                    if not isinstance(self.imputer_fill_value, (int, float)):
                        ...
                        raise Exception("Invalid value for setting 'imputer_fill_value'. Value must be numerical. Correct the input and try again.")
    ...
    def _getTransform(self,kwargs):
        ...
        if interpolator_type.lower() == 'linear':
            interp = self.autoai_ts_libs.deps.tspy_interpolators.linear()
        if interpolator_type.lower() == 'cubic':
            interp = self.autoai_ts_libs.deps.tspy_interpolators.cubic(fill_value=float(self.default_value))
        if interpolator_type.lower()== 'nearest':
            interp = self.autoai_ts_libs.deps.tspy_interpolators.nearest()
        if interpolator_type.lower() == 'next':
            ...
            interp = self.autoai_ts_libs.deps.tspy_interpolators.next(default_value=self.default_value)
        ...
        if interpolator_type.lower() == 'prev':
            #print ('prev')
                ...
                #kwargs['default_value'] = float(0)
            interp = self.autoai_ts_libs.deps.tspy_interpolators.prev(default_value=self.default_value)
        if interpolator_type.lower() == 'fill':
            ...
            #kwargs['default_value'] = float(0)
            interp = self.autoai_ts_libs.deps.tspy_interpolators.fill(self.imputer_fill_value)
    ...
    def __getstate__(self):
        ...
        state["model"] = None
        state["autoai_ts_libs.deps.tspy_interpolators"] = None
        # print('After copy')
        #state['model'] = None
        #print(state['autoai_ts_libs.deps.tspy_interpolators'])
        #state['autoai_ts_libs.deps.tspy_interpolators'] = None
        #print(state['autoai_ts_libs.deps.tspy_interpolators'])
        # #print()
        #return self
    ...
    def transform(self, X):
        ...
        if (self.ts_icol_loc == -1):
            ...
            if ts_col < 0 or ts_col >= X.shape[1]:
                ...
                #self.stop_context()
                raise AssertionError("Improper timestamp column(<0 or >#featurs) for input data. Input timestamp col={0}.")
        ...
        for val_ind in range(0, self.n_features_):
            if val_ind != ts_col:
               ...
               # vals_df_array.append(dataframe)
                ...
                #TODO: renable this once autoai_ts_libs.deps.tspy is fixed
                #self._timeseries = autoai_ts_libs.deps.tspy.time_series(dataframe, ts_column="timestamp", value_column="value")
                self._timeseries = TimeSeries(list(dataframe['value'])) #fix for autoai_ts_libs.deps.tspy 2.0.x error
                #print('Time series',self._timeseries)
                #print(self.model)
                trans_x = self._timeseries.fillna(self.model, self.missing_val_identifier).to_df()
                #trans_x = self._timeseries.fillna(autoai_ts_libs.deps.tspy_interpolators.cubic(fill_value=0.0),np.nan).to_df()
