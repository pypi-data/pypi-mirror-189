...
class Client:
    ...
    def _get_secret_token(self):
        try:
            s = requests.Session()
            s.mount('http://', HTTPAdapter(max_retries=3))  # 请求失败重试3次
            s.mount('https://', HTTPAdapter(max_retries=3))
            r = requests.post(url='https://' + EndPoint.GetSecretToken.value,
                          data={'secret_id': self.auth.secret_id, 'secret_key': self.auth.secret_key}, timeout=(5, 6))
