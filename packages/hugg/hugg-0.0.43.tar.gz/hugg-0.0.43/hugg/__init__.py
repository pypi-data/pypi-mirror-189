...
class mem(object):     
    ...
    def frameload_sqlite(self,file):
        if file not in self.files():
            print("FILE IS NOT AVAILABLE")
            return None
...
class face(mem):
    ...
    def upload(self, path=None,path_in_repo=None, auto_accept_all_pull_requests=False,use_pull_request = False):
        if not self.opened:
            self.login()
