...
from argus_api.lib.customers.v1.customer import get_customer
...
def datastore_data(data: str, csv_key: str = "key") -> Dict:
...
def csv_to_json(csv_file_path: Path, csv_key: str = "key") -> dict:
    ...
    with open(csv_file_path, "r") as csv_in:
        ...
        for row in DictReader(csv_in):
            ...
            csv_data[row[csv_key]] = dumps(value)
...
def update(
    ...
    entries = [
        {csv_key: key, "value": value or default_value} for key, value in data.items()
    ]
    ...
    print(f"Successfully updated {response['size']} entries")
