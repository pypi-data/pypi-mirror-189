...
def init(api_key: str, verbose: bool = False):
    global HEADERS
    global models
    global incantations
...
class Incantation:
    ...
    def delete(self, incantation_id: str):
        ...
        #del incantations[incantation_id] TODO: 'NoneType' object does not support item deletion
...
class Model:
    ...
    # convenience method
    def create_instance(self, ai_name: str = "AI"):
        instance_obj = ModelInstance(model_id=self.model_id, ai_name=ai_name)
        resp_body = instance_obj.create()
        return instance_obj


class ModelInstance:
    def __init__(self, ai_name: str, model_id: str = None, supp_id: str = None, description: str = None, instance_id = None):
        self.supp_id = supp_id
        self.description = description
...
def model_spec_preprocessing(model_spec: dict):
    ...
    # if temperature is in the model_spec, convert it to a string
    if "temperature" in model_spec:
        model_spec["temperature"] = str(model_spec["temperature"])
...
def create_model(model_spec: dict):
    #if model_spec["model_title"] in models:
    #    return get_model(model_spec["model_title"])
...
def list_instances(model_title: str, verbose=False):
    ...
    if verbose:
        for instance_obj in instances.values():
            print (instance_obj.instance_id, instance_obj.chat_history[-2:])
    return instances
...
We probably need to make users aware of the Incantation, Model, and ModelInstance classes, but we generally want to discourage them from using them directly.
