...
def get_incantations():
    ...
    for incantation in resp['body']:
        ...
        incantations[incantation['id']] = incantation
...
class Incantation:
    ...
    def create(self):
        ...
        url = self.base_url + 'incantations'
        payload = {
            'title': self.title,
            'text': self.text,
        ...
        resp = _format_http_response(requests.post(url, headers=self.headers, json=payload))
    ...
    def update(self):
        ...
        url = self.base_url + f'incantations/{self.incantation_id}'
        payload = {}
        ...
        resp = _format_http_response(requests.put(url, headers=self.headers, json=payload))
        return resp['body']

    def delete(self, incantation_id: str):
        url = self.base_url + f'incantations/{incantation_id}'
        _format_http_response(requests.delete(url, headers=self.headers))
        del incantations[incantation_id]


class Model:
    def __init__(self, model_spec: dict, supp_id: str = None, description: str = None, model_id: str = None):
        self.model_instances = self.get_model_instances()
    ...
    def update(self):
        ...
        url = self.base_url + f'models/{self.model_id}'
        payload = {}
    ...
    def get_instances(self):
        url = self.base_url + f'models/{self.model_id}/instances'
        resp = _format_http_response(requests.get(url, headers=self.headers))
        instances = {}
...
class ModelInstance:
    ...
    def get_chat_history(self):
        ...
        url = self.base_url + f'models/{self.model_id}/instances/{self.instance_id}/chat_history'
        response = _format_http_response(requests.get(url, headers=self.headers))
        self.chat_history.extend(response['body']['interactions'])
        while 'next_page_token' in response:
            response = _format_http_response(requests.get(url, headers=self.headers, params={'next_page_token': response['next_page_token']}))
            self.chat_history.extend(response['body']['interactions'])
    ...
    def update(self):
        ...
        url = self.base_url + f'models/{self.model_id}/instances/{self.instance_id}'
        payload = {}
        if self.supp_id is not None:
            payload['supp_id'] = self.supp_id
        if self.description is not None:
    ...
    def delete(self):
        url = self.base_url + f'models/{self.model_id}/instances/{self.instance_id}'
        resp = _format_http_response(requests.delete(url, headers=self.headers))
        del models[self.instance_id]
        return resp['body']

    def get_response(self, human_input: list):
        url = self.base_url + f'models/{self.model_id}/instances/{self.instance_id}/get_response'
        payload = {
