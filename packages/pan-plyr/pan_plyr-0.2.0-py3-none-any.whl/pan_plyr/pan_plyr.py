...
# write any sql expression on the data frame fed to pplyr          
    ...
    def pivot_longer(self, cols, id_vars=None, values_to='value', names_to='name'):
        ...
        Returns:
            self: the pan_plyr object with the pivoted dataframe
        
        Example:
            df = pd.DataFrame({'A': [1, 2, 3, 4], 'B': [5, 6, 7, 8], 'C': [9, 10, 11, 12]})
            pp = pan_plyr(df)
            op.pivot_longer(cols=['B', 'C'], id_vars='A', values_to='value', names_to='name')
        """
        if id_vars is None:
    ...
    def pivot_wider(self, names_from, values_from, names_repair=None, id_vars=None):
        ...
        Example:
            ...
            pp = pan_plyr(df)
            pp.pivot_wider(names_from='variable', values_from='value')
        """
        if id_vars is None:
    ...
    def separate(self, col, into, sep=None, remove=False, extra='warn'):
        ...
        Returns:
            self: the pan_plyr object with the separated dataframe
        
        Example:
            df = pd.DataFrame({'A': [1, 2, 3, 4], 'B': ['a_/*b', 'c_d', 'e_f', 'g*_h']})
            pp = pan_plyr(df)
            pp.separate(col='B', into=['B1', 'B2'], sep='_', remove=True)
        """
    ...
    def str_pad(self, width, side='left', pad=" "):
        ...
        Returns:
            self: the pan_plyr object with the padded strings in the dataframe

        Example:
            df = pd.DataFrame({'A': ['hello', 'world', 'foo', 'bar']})
            pp = pan_plyr(df)
            pp.str_pad(10, side='right', pad='*')
        """
        if side == 'left':
    ...
    def str_sub(self, pattern, replacement):
        ...
        Returns:
            self: the pan_plyr object with the replaced strings in the dataframe
    ...
    def str_extract(self, pattern, col=None):
        ...
        Args:
            ...
            col: The column name to extract the substring from. If None, applies to all columns with object dtype.
        
        Returns:
            self: The pan_plyr object with the extracted substrings added as a new column

        Example:
            df = pd.DataFrame({'col1': ['abc123', 'def456', 'ghi789']})
            pp = pan_plyr(df)
            pp.str_extract(pattern='[a-z]+', col='col1')
        """
        if col:
    ...
    def str_detect(self, col, pattern):
        ...
        Returns:
            self: the pan_plyr object with the modified dataframe
        
        Example:
            df = pd.DataFrame({'col1': ['abc', 'def', 'ghi', 'jkl']})
            pp = pan_plyr(df)
            pp.str_detect(col='col1', pattern='a')
        """
    ...
    def fct_lump(self, column, n=10, other_level='Other'):
        """
        Lumps levels of a factor column into 'Other' level for levels that appear less than n times

        Args:
            column: The name of the column containing the factor levels
            n: The number of times a level should appear to avoid being lumped (default is 10)
            other_level: The name of the new level for the lumped levels (default is 'Other')
        
        Returns:
            self: the pan_plyr object with the lumped column
        
        Example:
            df = pd.DataFrame({'A': [1, 2, 3, 4], 'B': ['A', 'B', 'C', 'A']})
            pp = pan_plyr(df)
            pp.fct_lump(column='B', n=2, other_level='Rare')
        """
    ...
    def fct_infreq(self, column, frac=0.01, other_level='Other'):
        """
        Lumps levels of a factor column into 'Other' level for levels that appear less than a fraction of total

        Args:
            column: The name of the column containing the factor levels
            frac: The fraction of the total number of times a level should appear to avoid being lumped (default is 0.01)
            other_level: The name of the new level for the lumped levels (default is 'Other')
        ...
        Example:
            df = pd.DataFrame({'A': [1, 2, 3, 4], 'B': ['A', 'B', 'C', 'A']})
            pp = pan_plyr(df)
            pp.fct_infreq(column='B', frac=0.5, other_level='Rare')
        """
    ...
    def fct_relevel(self, col, ref, after=True):
        ...
        Returns:
            self: the pan_plyr object with the releveled factor column
        
        Example:
            df = pd.DataFrame({'col1': [1, 2, 3, 4], 'col2': ['A', 'B', 'C', 'D']})
            pp = pan_plyr(df)
            pp.fct_relevel(col='col2', ref='B', after=False)
        """
    ...
    def fct_recode(self, col, values, labels=None, drop=False):
        ...
        Returns:
            self: The pan_plyr object with the recoded dataframe
        """
        self.df[col] = self.df[col].astype('category')
