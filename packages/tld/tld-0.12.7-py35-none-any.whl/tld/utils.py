...
__all__ = (u'BaseMozillaTLDSourceParser', u'get_fld', u'get_tld', u'get_tld_names', u'get_tld_names_container', u'is_tld', u'MozillaTLDSourceParser', u'MozillaPublicOnlyTLDSourceParser',
           u'parse_tld', u'pop_tld_names_container', u'process_url', u'reset_tld_names', u'Result', u'tld_names', u'update_tld_names', u'update_tld_names_cli', u'update_tld_names_container')
tld_names = {
...
def update_tld_names_cli():
    ...
    parser = argparse.ArgumentParser(description=u'Update TLD names')
    parser.add_argument(u'parser_uid', nargs=u'?', default=None,
                        help=u'UID of the parser to update TLD names for.')
    parser.add_argument(u'--fail-silently', dest=u'fail_silently',
                        default=False, action=u'store_true', help=u'Fail silently')
...
class BaseMozillaTLDSourceParser(BaseTLDSourceParser):
    ...
    def get_tld_names(cls, fail_silently=False, retry_count=0):
        ...
        try:
            ...
            local_file = codecs_open(local_path, u'r', encoding=u'utf8')
            trie = Trie()
            trie_add = trie.add
...
class MozillaTLDSourceParser(BaseMozillaTLDSourceParser):
    ...
    uid = u'mozilla'
    source_url = u'https://publicsuffix.org/list/public_suffix_list.dat'
    local_path = u'res/effective_tld_names.dat.txt'


class MozillaPublicOnlyTLDSourceParser(BaseMozillaTLDSourceParser):
    u'Mozilla TLD source.'
    uid = u'mozilla_public_only'
    source_url = u'https://publicsuffix.org/list/public_suffix_list.dat?publiconly'
    local_path = u'res/effective_tld_names_public_only.dat.txt'
...
def process_url(url, fail_silently=False, fix_protocol=False, search_public=True, search_private=True, parser_class=MozillaTLDSourceParser):
    ...
    if domain_name.endswith(u'.'):
        domain_name = domain_name.rstrip(u'.')
...
def get_fld(url, fail_silently=False, fix_protocol=False, search_public=True, search_private=True, parser_class=None, **kwargs):
    ...
    if (not parser_class):
        parser_class = (
            MozillaTLDSourceParser if search_private else MozillaPublicOnlyTLDSourceParser)
...
def get_tld(url, fail_silently=False, as_object=False, fix_protocol=False, search_public=True, search_private=True, parser_class=None):
    ...
    if (non_zero_i < 0):
        subdomain = u''
        domain = u''
        assert (parsed_url.hostname is not None), u'No hostname in URL'
...
def parse_tld(url, fail_silently=False, fix_protocol=False, search_public=True, search_private=True, parser_class=None):
...
def is_tld(value, search_public=True, search_private=True, parser_class=None):
