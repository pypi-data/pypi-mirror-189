...
from ..utils import BaseMozillaTLDSourceParser, MozillaTLDSourceParser, get_fld, get_tld, get_tld_names, get_tld_names_container, is_tld, parse_tld, reset_tld_names, update_tld_names, update_tld_names_cli
...
class TestCore(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.good_patterns = [{
            ...
            u'url': u'https://github.com....../barseghyanartur/tld/',
            u'fld': u'github.com',
            u'subdomain': u'',
            u'domain': u'github',
            u'suffix': u'com',
        ...
        self.invalid_tlds = {u'v2.www.google.com', u'tld.doesnotexist',
                             u'2001:0db8:0000:85a3:0000:0000:ac1f', u'192.169.1.1', u'localhost', u'google.com'}
    ...
    def get_custom_parser_class(self, uid=u'custom_mozilla', source_url=None, local_path=u'tests/res/effective_tld_names_custom.dat.txt'):
        parser_class = type(u'CustomMozillaTLDSourceParser', (BaseMozillaTLDSourceParser,), {
            u'uid': uid,
    ...
    def test_5_tld_good_patterns_pass_parsed_object(self):
        ...
        for data in self.good_patterns:
            ...
            self.assertEqual(str(_res).encode(u'utf8'),
    ...
    def test_7_public_private(self):
        res = get_fld(u'http://silly.cc.ua', fail_silently=True,
                      search_private=False, parser_class=MozillaTLDSourceParser)
        self.assertEqual(res, None)
        res = get_fld(u'http://silly.cc.ua',
                      fail_silently=True, search_private=False)
        self.assertEqual(res, u'cc.ua')
        res = get_fld(u'http://silly.cc.ua',
                      fail_silently=True, search_private=True)
        self.assertEqual(res, u'silly.cc.ua')
        res = get_fld(u'mercy.compute.amazonaws.com', fail_silently=True,
                      search_private=False, fix_protocol=True, parser_class=MozillaTLDSourceParser)
        self.assertEqual(res, None)
        res = get_fld(u'mercy.compute.amazonaws.com',
                      fail_silently=True, search_private=False, fix_protocol=True)
        self.assertEqual(res, u'amazonaws.com')
    ...
    def test_14_fail_update_tld_names(self):
        ...
        parser_class = self.get_custom_parser_class(
            uid=u'custom_mozilla_2', source_url=u'i-do-not-exist')
    ...
    def test_15_fail_get_tld_names(self):
        u'Test fail `update_tld_names`.'
        parser_class = self.get_custom_parser_class(
            uid=u'custom_mozilla_3', source_url=u'i-do-not-exist', local_path=u'/srv/tests/res/effective_tld_names_custom_3.dat.txt')
        reset_tld_names()
        for params in self.good_patterns:
            kwargs = {
                u'url': params[u'url'],
            }
            kwargs.update(params[u'kwargs'])
            kwargs[u'fail_silently'] = False
            kwargs[u'parser_class'] = parser_class
            with self.assertRaises(TldIOError):
                get_tld(**kwargs)
    ...
    def test_18_get_tld_names_and_reset_tld_names(self):
        ...
        parser_class = self.get_custom_parser_class(
            source_url=u'i-do-not-exist', local_path=tmp_filename)
        reset_tld_names()
        with self.subTest(u'Assert raise TldIOError'):
            with self.assertRaises(TldIOError):
                get_tld_names(fail_silently=False, parser_class=parser_class)
        tmp_filename = join(gettempdir(), u''.join(
            [u'{}'.format(self.faker.uuid4()), u'.dat.txt']))
        parser_class_2 = self.get_custom_parser_class(
            source_url=u'i-do-not-exist-2', local_path=tmp_filename)
        reset_tld_names()
        with self.subTest(u'Assert get None'):
    ...
    def test_20_parse_tld_custom_tld_names_good_patterns(self):
        ...
        for data in self.good_patterns_custom_parser:
            kwargs = copy.copy(data[u'kwargs'])
            kwargs[u'parser_class'] = self.get_custom_parser_class()
    ...
    def test_26_case(self):
        res = get_tld(u'https://MyDomain.com/AsDrFt?QUeRY=12aA',
                      fail_silently=True, search_private=False, as_object=True)
        self.assertEqual(res.tld, u'com')
        self.assertEqual(res.domain, u'mydomain')
        self.assertEqual(res.subdomain, u'')
        self.assertEqual(res.fld, u'mydomain.com')
        self.assertEqual(res.parsed_url, SplitResult(
            scheme=u'https', netloc=u'MyDomain.com', path=u'/AsDrFt', query=u'QUeRY=12aA', fragment=u''))

    @log_info
    def test_27_tld_fail_silently_pass(self):
        u"Test `get_tld` bad URL patterns that would raise exception\n        if `fail_silently` isn't `True`.\n        "
        res = []
        bad_url = [u'https://user:password[@host.com',
                   u'https://user[@host.com']
        for url in bad_url:
            _res = get_tld(url, fail_silently=True)
            self.assertEqual(_res, None)
            res.append(_res)
