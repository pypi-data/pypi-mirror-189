...
def parse_result(response: Union[TxResponse, dict],
                 contract_result_type: Union[ContractResultType, str] = ContractResultType.STRING):
    """
    解析交易响应中的result
    :param response: 交易响应结构体或MessageToDict后的字典
    :param contract_result_type: 'str'/ 'int' / 'json' 转为字符串、整型、字典格式
    :return: 如果响应中不存在result则返回愿响应，否则按指定格式返回转换后的result信息
    """
    if isinstance(contract_result_type, str):
        contract_result_type = ContractResultType(contract_result_type)
    ...
    if isinstance(response, dict):
        ...
        if not result:
            return response
        if contract_result_type == ContractResultType.STRING:
            return base64_to_str(result)
        if contract_result_type == ContractResultType.INT:
            return base64_to_int(result)
        if contract_result_type == ContractResultType.JSON:
