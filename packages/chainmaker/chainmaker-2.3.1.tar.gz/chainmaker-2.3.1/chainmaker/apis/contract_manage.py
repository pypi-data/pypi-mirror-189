...
from chainmaker.protos import (Contract, Payload, TxRequest, EndorsementEntry, TxResponse, TxStatusCode)
from chainmaker.sdk_config import DefaultConfig
from chainmaker.utils import common, file_utils
from chainmaker.utils.evm_utils import calc_evm_params
from chainmaker.utils.result_utils import parse_result
...
class ContractQueryMixIn(BaseClient):
    ...
    def get_contract_info(self, contract_name: str) -> Union[Contract, None]:
        ...
        """
        self._debug("begin to GetContractInfo, [contract_name:%s]" % contract_name)
    ...
    def get_contract_list(self) -> List[Contract]:
        ...
        :raise: 当数据不是JSON格式时，抛出json.decoder.JSONDecodeError
        """
        self._debug("begin to GetContractList")
        payload = self._create_contract_query_payload(ContractManageMethod.GET_CONTRACT_LIST.name)
    ...
    def get_disabled_native_contract_list(self) -> List[str]:
        ...
        :return: 禁用合约名称列表
        """
        self._debug("begin to GetDisabledNativeContractList")
    ...
    def _create_contract_query_payload(self, method: str, params: dict = None):
        """
        创建合约查询待签名Payload
        :param method: 合约查询方法
        :param params: 方法参数
...
class UserContractMixIn(BaseClient):
    ...
    def invoke_contract_with_limit(self, contract_name: str, method: str, params: dict = None, tx_id: str = None,
                                   timeout: int = None,
                                   with_sync_result: bool = None, gas_limit: int = None) -> TxResponse:  # todo remove
        """
        带Gas限制调用合约
        :param contract_name: 合约名
        :param method: 调用合约方法名
        ...
        payload = self.attach_gas_limit(payload, gas_limit)
...
class SystemContractMixIn(BaseClient):
    ...
    def invoke_system_contract(self, contract_name: str, method: str, params: Dict[str, Union[str, int, bool]] = None,
        ...
        :return: 交易响应信息
        """
        self._debug("begin to InvokeSystemContract, [contract_name:%s]/[method:%s]/[params:%s]" % (
            contract_name, method, params))
        if timeout is None:
    ...
    def query_system_contract(self, contract_name: str, method: str, params: Dict[str, Union[str, int, bool]] = None,
        ...
        :return: 交易响应信息
        """
        self._debug("begin to QuerySystemContract, [contract_name:%s]/[method:%s]/[params:%s]" % (
            contract_name, method, params))
        if timeout is None:
            timeout = DefaultConfig.rpc_send_tx_timeout
        payload = self._payload_builder.create_query_payload(contract_name, method, params)
    ...
    def create_native_contract_access_grant_payload(self, grant_contract_list: List[str]) -> Payload:
        ...
        :return: 待签名Payload
        """
        self._debug("begin to create [NativeContractAccess] to be signed payload")
    ...
    def create_native_contract_access_revoke_payload(self, revoke_contract_list: List[str]) -> Payload:
        ...
        :return: 待签名Payload
        """
        self._debug("begin to create [NativeContractRevoke] to be signed payload")
    ...
    def create_native_contract_access_verify_payload(self, contract_list: List[str]) -> Payload:
        ...
        :return: 待签名Payload
        """
        self._debug("begin to create [NativeContractVerify] to be signed payload")
...
class ContractManageWithEndorsers(BaseClient):
    ...
    def create_contract(self, contract_name: str, byte_code_or_file_path: str, runtime_type: Union[RuntimeType, str],
        ...
        :raises: RequestError: 请求失败
        """
        payload = self.create_contract_create_payload(contract_name, version, byte_code_or_file_path, runtime_type,
    ...
    # 05-01 升级合约
    def upgrade_contract(self, contract_name: str, byte_code_or_file_path: str, runtime_type: Union[RuntimeType, str],
                         params: dict = None, version: str = None, gas_limit: int = None, tx_id: str = None,
                         timeout: int = None, with_sync_result: bool = None) -> TxResponse:
        """
        升级合约
        ...
        :raises: RequestError: 请求失败
        """
        payload = self.create_contract_upgrade_payload(contract_name, version, byte_code_or_file_path, runtime_type,
                                                       params, gas_limit, tx_id)
