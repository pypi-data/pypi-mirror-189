...
class ConnectionPool:
    def __init__(self, user, nodes: List[ClientNode], rpc_client_config=None, logger=None):
        self.user = user
        self.nodes = nodes
    ...
    def get_channel(self, node: ClientNode) -> grpc.Channel:
        ...
        :return: Node桩
        """
        max_send_message_size = self.rpc_client_config.max_send_message_size or DefaultConfig.rpc_max_send_message_length
        max_receive_message_size = self.rpc_client_config.max_receive_message_size or DefaultConfig.rpc_max_receive_message_length
        opts = [
    ...
    def get_random_channel(self) -> (ClientNode, grpc.Channel):
        conn_node = random.randint(0, len(self.nodes) - 1)
        return self.nodes[conn_node], random.choice(self.pool[conn_node])

    def get_node_random_channel(self, conn_node: int) -> (ClientNode, grpc.Channel):
        if not 0 <= conn_node < len(self.pool):
            raise ValueError('conn_node 应大于0并小于节点长度')
        return self.nodes[conn_node], random.choice(self.pool[conn_node])
    ...
    def _get_channel(self, conn_node: int = None) -> grpc.Channel:
        if conn_node is None:
            ...
            self._node, channel = self.get_node_random_channel(conn_node)
        return channel
