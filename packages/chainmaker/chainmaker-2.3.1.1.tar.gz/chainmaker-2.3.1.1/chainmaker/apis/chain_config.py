...
from chainmaker.protos import (TxResponse, Payload, EndorsementEntry, ChainConfig, Policy, ResourcePolicy)
from chainmaker.utils.common import (ensure_enum, ensure_str, ensure_list_str)
from chainmaker.utils.result_utils import msg_to_dict
...
class _PermissionMixIn(BaseClient):
    ...
    def create_policy(rule: Union[PolicyRule, str], org_list: List[str] = None, role_list: List[PolicyRole] = None):
        ...
        role_list = ensure_list_str(role_list)
...
class ChainConfigWithEndorsers(BaseClient):
    ...
    def chain_config_trust_root_check(self, org_id: str, trust_root_crts: List[str] = None):
        """
        检查信任组织根证书是否上链
        :param org_id: 组织Id
    ...
    def chain_config_trust_root_add(self, org_id: str, trust_root_crts: List[str],
        ...
        :raises RequestError: 请求出错
        """
        self._info("begin to add trust root")
    ...
    # 00-07 添加共识节点地址
    def chain_config_node_addr_add(self, org_id: str, node_addrs: List[str], timeout: int = None,
                                   with_sync_result: bool = None) -> TxResponse:
        """
        添加节点地址
        <00-07-CHAIN_CONFIG-NODE_ADDR_ADD>
    ...
    # 00-08 更新共识节点地址
    def chain_config_node_addr_update(self, org_id: str, node_old_addr: str, node_new_addr: str,
                                      timeout: int = None, with_sync_result: bool = None) -> TxResponse:
        """
        更新节点地址
        <00-08-CHAIN_CONFIG-NODE_ADDR_UPDATE>
    ...
    # 00-09 删除共识节点地址
    def chain_config_node_addr_delete(self, org_id: str, node_addr: str,
    ...
    def chain_config_node_org_check(self, org_id: str) -> bool:
    ...
    ## 00-10 添加共识节点组织
    def chain_config_node_org_add(self, org_id: str, node_ids: List[str],
                                  timeout: int = None, with_sync_result: bool = None) -> TxResponse:
        """
        添加共识节点组织
        :param org_id: 节点组织Id
    ...
    ## 00-11 更新共识节点组织
    def chain_config_node_org_update(self, org_id: str, node_ids: List[str],
                                     timeout: int = None, with_sync_result: bool = None) -> TxResponse:
        """
        更新共识节点组织
        :param org_id: 节点组织Id
        ...
        :raises RequestError: 请求出错
        """
        self._info('begin to update consensus org')
        payload = self.create_chain_config_consensus_node_org_update_payload(org_id, node_ids)
        return self._send_chain_config_update_request(payload, timeout=timeout, with_sync_result=with_sync_result)

    ## 00-12 删除共识节点组织
    def chain_config_node_org_delete(self, org_id: str,
                                     timeout: int = None, with_sync_result: bool = None) -> TxResponse:
        """
        删除共识节点组织
        :param org_id: 节点组织Id
    ...
    def chain_config_consensus_ext_check(self, key: str, value: str = None) -> bool:
        ...
        consensus_ext = msg_to_dict(self.chain_config.consensus.ext_config)
        if key in self.consensus_ext.keys():
            if value is None:
                return True
            else:
                return value == self.consensus_ext.get(key)
        return False
    ...
    def chain_config_node_id_check(self, org_id: str, node_id: str) -> bool:
        consensus_nodes = self.chain_config.consensus.nodes
        consensus_orgs = [item.org_id for item in consensus_nodes]
    ...
    # 00-19 添加共识节点Id
    def chain_config_node_id_add(self, org_id: str, node_ids: List[str],
                                 timeout: int = None, with_sync_result: bool = None) -> TxResponse:
        """
        添加共识节点Id
    ...
    # 00-20 更新共识节点Id
    def chain_config_node_id_update(self, org_id: str, node_old_id: str, node_new_id: str,
                                    timeout: int = None, with_sync_result: bool = None) -> TxResponse:
        """
        更新共识节点Id
        :param str org_id: 节点组织Id
    ...
    # 00-21 删除共识节点Id
    def chain_config_node_id_delete(self, org_id: str, node_id: str,
                                    timeout: int = None, with_sync_result: bool = None) -> TxResponse:
        """
        删除共识节点Id
        :param str org_id: 节点组织Id
    ...
    def get_permission_list(self) -> ResourcePolicy:
        ...
        :return: 权限列表
        """
        return super().get_chain_config_permission_list()
