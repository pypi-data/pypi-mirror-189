...
class ChainClient(SystemContractMixIn, ChainQueryMixIn, UserContractMixIn, ContractQueryMixIn, ChainConfigMixIn,
                  ...
                  ConsensusMixIn, DPosErc20MixIn, DPosStakeMixIn, DPosSlashingMixIn):
    ...
    def __init__(self, chain_id, user: ClientUser, nodes: List[ClientNode], conn_node: int = None,
        ...
        self._pool = ConnectionPool(user, nodes, logger=self._logger)
    ...
    def _get_client(self, conn_node: int = None) -> RpcNodeStub:
        ...
        :return:
        """
        return self._pool.get_client(conn_node)

    def _create_endorsers(self, payload: Payload, endorse_users: List[ClientUser] = None) -> List[EndorsementEntry]:
        """
        根据背书用户创建背书
    ...
    def create_endorsers(self, payload: Payload, endorsers_config: List[dict]) -> List[EndorsementEntry]:
        ...
        :return: 背书列表
        """
        endorse_users = [ClientUser.from_conf(**item) for item in endorsers_config]
    ...
    def _get_sync_result_with_timeout(self, tx_id: str, retry_timeout: int, retry_interval: int) -> Result:
        ...
        raise TimeoutError(
            f'[Sdk] get tx_id="%s" transaction info %ss timeout: %s' % (tx_id, retry_timeout, err_msg))
    ...
    def _get_sync_result_with_limit(self, tx_id: str, retry_limit: int, retry_interval: int) -> Result:
        ...
        for i in range(retry_limit):
            try:
                ...
                time.sleep(retry_interval // 1000)
        raise TimeoutError(
            f'[Sdk] get tx_id="%s" transaction info timeout when retry %s times: %s' % (tx_id, retry_limit, err_msg))
    ...
    def _send_request(self, payload: Payload, endorsers: List[EndorsementEntry] = None,
        ...
        if timeout is None or timeout <= 0:
            if payload.tx_type == TxType.QUERY_CONTRACT:
    ...
    def _set_config(cc: "ChainClient", chain_client_config: ChainClientConfig):
        ...
        if endorsers_config:
            cc.endorsers = [ClientUser.from_conf(**item.__dict__) for item in endorsers_config]
    ...
    def from_conf(cls, sdk_config: Union[dict, str, Path], conn_node: int = None,
        ...
        user = ClientUser.from_conf(**user_conf)

        nodes = [ClientNode.from_conf(**item.__dict__) for conn_node, item
    ...
    # @property
    # def endorsers(self):
    #     warnings.warn('请使用ChainManager.endorsers')
    #
    #     if not hasattr(self, '_endorsers'):
    #         setattr(self, '_endorsers', DefaultConfig.endorsers)
    #     return getattr(self, '_endorsers')
    #
    # @endorsers.setter
    # def endorsers(self, users: List[ClientUser]):
    #     warnings.warn('请使用ChainManager.endorsers')
    #     setattr(self, '_endorsers', users)

    # @property
    # def request_timeout(self) -> int:
    #     warnings.warn('请使用cc.timeout')
    ...
    # def change_signer(self, sign_key_bytes: bytes = None, sign_cert_bytes: bytes = None):
    #     """更新用户证书及密钥"""
