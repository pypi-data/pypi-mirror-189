...
def create_crl_bytes(cert_bytes_or_file_path: Union[Path, str, bytes],
                     ca_key_bytes_or_file_path: Union[Path, str, bytes],
                     ca_crt_bytes_or_file_path: Union[Path, str, bytes]) -> bytes:
    """
    创建吊销证书列表文件二进制数据
    ...
    """
    revocation_date_timestamp = 1711206185  # todo arg
    now = datetime.now()
    ...
    revoked_crt = load_pem_cert(cert_bytes_or_file_path)
