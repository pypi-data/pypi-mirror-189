...
def initMyredis():
    ...
    try:
        ...
        print(f'连接Redis数据库pool失败,请检查数据库是否存在,redis服务是否启动，IP/端口是否正确:{e}')         
        return False
...
def get_bar_data(context):
    try:
        mydb = mysql.connector.connect(
          ...
          user = 'mysql',
          passwd = '_Qs21user',
          database = 'futures_bar',  
          )
        
        mycursor = mydb.cursor()
        #limitstr = ""
        #if count and isinstance(count, int):
        #    limitstr = " limit "+ str(count);
        
        dfDict = {}
        #print(cols2fields(cols))
        tday = context.tradingday
        for inst in context.instid:
            tname = inst2tablename(inst)
            mycursor.execute("SELECT * FROM "+tname+" where tradingday="+tday)
            #print("SELECT "+ cols2fields(cols)  +" FROM "+secu+" WHERE time >= "+str(start_dt)+" AND time <= "+str(end_dt))
            df = pd.DataFrame(mycursor.fetchall())
            if len(df) == 0:
                dfDict[inst] = pd.DataFrame()
            else:
                df.columns = ["time", "open", "close", "high", "low", "volume", "money", "position", \
                               "presett", "preclose", "lowerlimit", "upperlimit", "tradingday"]
                dfDict[inst] = df
        mydb.close()
        return dfDict
