...
from .qestratmarket import save_trades, writeStratStat,  writeStratPosition,update_stratCard,writeStratPnl,clearStratTrades,readStratPosition,update_stratCard_append,readFullStratStat,clearStratStat,clearStratPosition
...
class qeContextBase:
    ...
    def updateProdData(self, curday):
        writeStratStat(self.stratname, curday.replace('-',''),self.prodMaxmarg,self.prodTurnover) 
        day = datetime.datetime.strptime(curday, "%Y-%m-%d")
    ...
    def updateDailyPnl(self, curday):
        ...
        try:
            ...
            if self.record_strat:
                for prod in prodPnl:
                    writeStratPnl(self.stratname, curday.replace('-',''), prod, prodPnl[prod])    
            day = datetime.datetime.strptime(curday, "%Y-%m-%d")
    ...
    def crossDay(self):
        try:
            ...
            ## on trades
            if self.record_strat:
                writeStratPosition(self.stratname,curday.replace('-',''),self.position,self.instCloseProf)
...
#                         print(tempcolumns) 
        ...
        if record_strat:
            ...
            if len(RTrades) > 0:
                ...
                save_trades(strat.name, trade_m, context.tradingday)
                print('save trades into stratgy market sucessfully')
            
            
            ## Write statistics to db for last day
            writeStratPosition(context.stratname,context.tradingday,context.position,context.instCloseProf)
            ...
            update_stratCard(strat.name,card_indicators, context.prodTMaxmarg)
        if showchart:
...
def dynamicBacktest(user, datamode, instids, start_date, end_date, strat, printlog=True, \
    ...
    if record_strat:
        ...
        if not strat._append_mode:
            clearStratTrades(strat.name)
            clearStratStat(strat.name, start_date)
            clearStratPosition(strat.name, start_date)            
        else:
            clearStratTrades(strat.name, start_date)
            clearStratStat(strat.name, start_date)
            clearStratPosition(strat.name, start_date)            
            context.position, tmp = readStratPosition(strat.name, start_date.strftime('%Y%m%d'))
            tmpstat = readFullStratStat(strat.name)
    ...
    try:
        ...
        if record_strat:
            ...
            #print(date_pos,'??')
            writeStratPosition(context.stratname,date_pos,context.position,context.instCloseProf)
            curday = context.curday
            context.updateProdData(curday)
            ...
            if not strat._append_mode:
                ...
                update_stratCard_append(strat.name, min(initloss,0))
