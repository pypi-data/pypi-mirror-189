...
import time
import os
from .qeglobal import getExemode
...
try:
    ...
    logger_path = os.path.abspath(os.path.dirname(__file__)) +'/qelog'
...
def initBacktestLogger(printlog=True):
    ...
    fh = TimedRotatingFileHandler(filename=f"{logger_path}/backtestLog", when="D", interval=1, backupCount=5)
...
def initLogger(userid,real=False, printlog=True):
    ...
    try:
        ...
        if real:
            ...
            fh = TimedRotatingFileHandler(filename=f'{logger_path}/qelog_real_{str(userid)}', when="D", interval=1, backupCount=5)
        else:
            #if os.path.isfile("/var/log/qelog/qelog_"+str(userid)):
            #    os.system('rm -f /var/log/qelog/qelog_'+str(userid))
            fh = TimedRotatingFileHandler(filename=f'{logger_path}/qelog_{str(userid)}', when="D", interval=1, backupCount=5)
...
def initTableLogger():    
    ...
    if not os.path.exists(logger_path):
        os.mkdir(logger_path)
    fh = TimedRotatingFileHandler(filename=f"{logger_path}/tabLog", when="D", interval=1, backupCount=5)
    fh.setLevel(logging.DEBUG)  # 输出到file的log等级的开关
    ch = logging.StreamHandler()
    ch.setLevel(logging.WARNING)  # 输出到console的log等级的开关
    logging.captureWarnings(True)
