...
import random
from .qestratmarket import readStratPosition, writeStratPosition,writeStratStat,writeStratTrade
from .qestratmarket import readStratStat,writeContract_messages,writeContractTable
            ...
            for oid in context.orders:
                ...
                if 'autocancel' in corder:
                    ...
                    if corder['leftvol'] > 0 and corder['autocancel'][:2] != [0 , 0] and corder['autocancel'][statusloc] == 'wait':
                        ...
                        if canceltime > 0 and (context.curtime - starttime).seconds >= canceltime: 
                ...
                remake = False
                statusloc = 5
                if corder['instid']==inst and 'autoremake' in corder and len(corder['autoremake']) == statusloc+1 \
                    ...
                    if mode == 1: # FAK track mode
                        ...
                        if corder['leftvol'] > 0 and rmcount > 0:
                            if rmtime > 0 and (context.curtime - rmstart).seconds >= rmtime and status=='wait':
                                context.orders[oid]['autoremake'][statusloc] = 'cancel'
                                cancel_order(context, corder['orderid'])
                                print('cancel order by time',rmtime,'left count',rmcount-1)
                            elif rmticks > 0 and abs(price - corder['price'])/ticksize >= rmticks and status=='wait':
                                context.orders[oid]['autoremake'][statusloc] = 'cancel'
                                cancel_order(context, corder['orderid'])
                                print('cancel order by ticks',rmticks,'left count',rmcount-1)
                        
                        elif corder['leftvol'] == 0: 
                            if rmtime > 0 and rmcount > 0 and status =='cancel':      
                                autorm = [mode, rmtime, rmticks, rmcount-1]
                                remake = True
                            elif rmticks > 0 and rmcount > 0 and status =='cancel':      
                                autorm = [mode, rmtime, rmticks, rmcount-1]
                                remake = True
                            else:
                                context.orders[oid]['autoremake'][statusloc] ='finished'
                    ...
                    if remake:
                        if 'autocancel' in corder and  corder['autocancel'][:2] != [0 , 0]:
                            [canceltime, canceltick, starttime,status] =  corder['autocancel']
                            if (context.curtime - starttime).seconds < canceltime:
                                lefttime = canceltime - (context.curtime - starttime).seconds
            ...
            for strat in self.strats:
                if tempinstid in strat.instid: # or self.recmode:
                    ...
                    if self.recmode and not strat.posLoaded:
                        ...
                        pos,instPnl = readStratPosition(strat.name, strat.context.tradingday)
                        if pos == {} and instPnl == {}:
                            prev_date = self.getPreviousTradingDay() 
                            strat.context.position, tmp = readStratPosition(strat.name, prev_date.strftime('%Y%m%d'))
                        ...
                        strat.context.prodMaxMarg,strat.context.prodTurnover = readStratStat(strat.name,strat.context.tradingday)
                        ## write back to database
                        strat.context.clearExpirePosition()
                        writeStratPosition(strat.name, strat.context.tradingday,strat.context.position,strat.context.instClosePnl)
                        writeContractTable(strat.name, strat.context.tradingday,strat.context.position)
                        writeStratStat(strat.name,strat.context.tradingday, strat.context.prodMaxMarg, strat.context.prodTurnover)
    ...
    def crossDay(self):
        ...
        if self.recmode:
            ...
            for strat in self.strats:
                ...
                strat.context.clearExpirePosition()
                writeStratPosition(strat.name, lastday,context.position,context.instClosePnl)
                writeContractTable(strat.name, lastday,context.position)
                writeStratStat(strat.name,lastday, context.prodMaxMarg, context.prodTurnover)
        self.dataslide = {}
        self.presett = {}
        ...
        try:
            for strat in self.strats:
                if strat.name == trade['stratName']:
                    ...
                    context.updateData() 
                    if self.recmode:
                        writeStratPosition(strat.name, context.tradingday,context.position,context.instClosePnl)
                        writeContractTable(strat.name, context.tradingday,context.position)
                        writeStratStat(strat.name,context.tradingday, context.prodMaxMarg, context.prodTurnover)
                        writeStratTrade(strat.name,context.tradingday, trade)
                        writeContract_messages(strat.name,context.tradingday, trade)
        except Exception as e:
