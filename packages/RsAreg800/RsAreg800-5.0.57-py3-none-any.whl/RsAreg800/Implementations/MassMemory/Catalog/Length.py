...
class LengthCls:
	...
	def get(self, path: str = None) -> int:
		...
		Snippet: value: int = driver.massMemory.catalog.length.get(path = '1') \n
		Returns the number of files in the current or in the specified directory. \n
			:param path: string String parameter to specify the directory. If the directory is omitted, the command queries the content of the current directory, queried with method RsAreg800.MassMemory.currentDirectory command.
