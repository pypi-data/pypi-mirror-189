...
class NetworkCls:
	...
	def get_mac_address(self) -> str:
		...
		Snippet: value: str = driver.system.communicate.syst.network.get_mac_address() \n
		Queries the MAC address of the instrument connected to the R&S AREG800A via the system control interface. \n
			:return: zynq_mac_address: string
		"""
		response = self._core.io.query_str('SYSTem:COMMunicate:SYST:NETWork:MACaddress?')
		return trim_str_response(response)

	def set_mac_address(self, zynq_mac_address: str) -> None:
		"""SCPI: SYSTem:COMMunicate:SYST:NETWork:MACaddress \n
		Snippet: driver.system.communicate.syst.network.set_mac_address(zynq_mac_address = '1') \n
		Queries the MAC address of the instrument connected to the R&S AREG800A via the system control interface. \n
			:param zynq_mac_address: string
		"""
		param = Conversions.value_to_quoted_str(zynq_mac_address)
		self._core.io.write(f'SYSTem:COMMunicate:SYST:NETWork:MACaddress {param}')

	def get_status(self) -> bool:
		"""SCPI: SYSTem:COMMunicate:SYST:NETWork:STATus \n
		Snippet: value: bool = driver.system.communicate.syst.network.get_status() \n
		Queries the network configuration state. \n
			:return: zynq_net_status: 1| ON| 0| OFF
		"""
		response = self._core.io.query_str('SYSTem:COMMunicate:SYST:NETWork:STATus?')
		return Conversions.str_to_bool(response)

	def set_status(self, zynq_net_status: bool) -> None:
		"""SCPI: SYSTem:COMMunicate:SYST:NETWork:STATus \n
		Snippet: driver.system.communicate.syst.network.set_status(zynq_net_status = False) \n
		Queries the network configuration state. \n
			:param zynq_net_status: 1| ON| 0| OFF
		"""
		param = Conversions.bool_to_str(zynq_net_status)
		self._core.io.write(f'SYSTem:COMMunicate:SYST:NETWork:STATus {param}')
