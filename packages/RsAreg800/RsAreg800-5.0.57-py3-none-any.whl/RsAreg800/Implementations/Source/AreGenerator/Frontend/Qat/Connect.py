...
class ConnectCls:
	...
	def set(self, qatFrontent=repcap.QatFrontent.Default) -> None:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:QAT<CH>:CONNect \n
		Snippet: driver.source.areGenerator.frontend.qat.connect.set(qatFrontent = repcap.QatFrontent.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:QAT{qatFrontent_cmd_val}:CONNect')

	def set_with_opc(self, qatFrontent=repcap.QatFrontent.Default, opc_timeout_ms: int = -1) -> None:
		qatFrontent_cmd_val = self._cmd_group.get_repcap_cmd_value(qatFrontent, repcap.QatFrontent)
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:QAT<CH>:CONNect \n
		Snippet: driver.source.areGenerator.frontend.qat.connect.set_with_opc(qatFrontent = repcap.QatFrontent.Default) \n
		No command help available \n
		...
		self._core.io.write_with_opc(f'SOURce<HwInstance>:AREGenerator:FRONtend:QAT{qatFrontent_cmd_val}:CONNect', opc_timeout_ms)
