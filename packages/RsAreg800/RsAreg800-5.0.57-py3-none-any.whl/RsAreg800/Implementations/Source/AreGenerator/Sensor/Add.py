...
class AddCls:
	...
	def set(self, sensor=repcap.Sensor.Default) -> None:
		...
		Snippet: driver.source.areGenerator.sensor.add.set(sensor = repcap.Sensor.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:SENSor{sensor_cmd_val}:ADD')

	def set_with_opc(self, sensor=repcap.Sensor.Default, opc_timeout_ms: int = -1) -> None:
		sensor_cmd_val = self._cmd_group.get_repcap_cmd_value(sensor, repcap.Sensor)
		"""SCPI: [SOURce<HW>]:AREGenerator:SENSor<CH>:ADD \n
		Snippet: driver.source.areGenerator.sensor.add.set_with_opc(sensor = repcap.Sensor.Default) \n
		No command help available \n
		...
		self._core.io.write_with_opc(f'SOURce<HwInstance>:AREGenerator:SENSor{sensor_cmd_val}:ADD', opc_timeout_ms)
