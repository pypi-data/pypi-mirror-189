...
class IdCls:
	...
	def set(self, areg_sens_id: int, sensor=repcap.Sensor.Default) -> None:
		"""SCPI: [SOURce<HW>]:AREGenerator:SENSor<CH>:ID \n
		Snippet: driver.source.areGenerator.sensor.id.set(areg_sens_id = 1, sensor = repcap.Sensor.Default) \n
		No command help available \n
			:param areg_sens_id: integer Range: 0 to 1000
			:param sensor: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sensor')
		"""
		param = Conversions.decimal_value_to_str(areg_sens_id)
		sensor_cmd_val = self._cmd_group.get_repcap_cmd_value(sensor, repcap.Sensor)
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:SENSor{sensor_cmd_val}:ID {param}')
	...
	def get(self, sensor=repcap.Sensor.Default) -> int:
		...
		No command help available \n
			:param sensor: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Sensor')
			:return: areg_sens_id: integer Range: 0 to 1000"""
		sensor_cmd_val = self._cmd_group.get_repcap_cmd_value(sensor, repcap.Sensor)
