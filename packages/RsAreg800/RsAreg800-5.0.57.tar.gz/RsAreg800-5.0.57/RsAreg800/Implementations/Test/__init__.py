...
class TestCls:
	"""Test commands group definition. 15 total commands, 7 Subgroups, 4 group commands"""
	...
	def get_hs(self) -> str:
		...
		No command help available \n
			:return: areg_test_int_face: No help available
		"""
		response = self._core.io.query_str('TEST:HS?')
		return trim_str_response(response)

	def set_hs(self, areg_test_int_face: str) -> None:
		"""SCPI: TEST:HS \n
		Snippet: driver.test.set_hs(areg_test_int_face = '1') \n
		No command help available \n
			:param areg_test_int_face: No help available
		"""
		param = Conversions.value_to_quoted_str(areg_test_int_face)
		self._core.io.write(f'TEST:HS {param}')
