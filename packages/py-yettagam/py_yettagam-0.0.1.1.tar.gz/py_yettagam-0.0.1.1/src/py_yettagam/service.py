...
class Service(KuriSyncBase):
    ...
    async def __ipfs_pubsub_subscribe(self, topic:str=None):
        ...
        with open(f"{self.data_directory}/mappings.json", "r") as f:
            ...
            if kuri not in mappings:
                ...
                if kuri not in kuris_in_rff:
                    ...
                    # await self.__ipfs_pubsub_subscribe(topic=kuri)

    async def sync_scribe_kuris(self,
        ...
        await self.sync(direction, start_block_number, block_count, finalized_only, filters=filters)
