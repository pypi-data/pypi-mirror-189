...
class KuriSyncBase(object):
    ...
    async def save_kuri(self, kuri, filters:dict={}):
        raise NotImplementedError
    
    async def delete_kuri(self, kuri, filters:dict={}):
        raise NotImplementedError
    ...
    async def _sync(self, 
                ...
                listener:KuriListener, tip_block_number:int, filters:dict={},
            ):
        ...
        for block in listener.listen(direction, start_block_hash, block_count, query=query):
            print(f"block:\n{block}")
            if block["block_number"] >= tip_block_number:
                ...
                if self.__synced_kuri_block_number(kuri=kuri, filters=filters) <= self.__cached(self.CACHE_KEY_CURRENT_BLOCK_NUMBER, default=0):
                    ...
                    try:
                        if kuri_info["call_function"] in (
                                KURI_OPERATION.create, KURI_OPERATION.accept
                            ):
                            # save kuri by pinning it's CID
                            await self.save_kuri(kuri=kuri, filters=filters)
                        elif kuri_info["call_function"] in (
                                KURI_OPERATION.transfer, KURI_OPERATION.delete
                            ):
                            # delete kuri by un-pinning it's CID
                            await self.delete_kuri(kuri=kuri, filters=filters)
    ...
    async def sync(self,
        ...
        self.__reset_cache()
        # set up listener
        listener = KuriListener(self.metarium_node_url)
