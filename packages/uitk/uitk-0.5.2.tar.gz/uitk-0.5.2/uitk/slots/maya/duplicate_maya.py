...
class Duplicate_maya(Duplicate, Slots_maya):
	...
	def chk015(self, create=False):
		...
		if self.sb.duplicate_radial.chk015.isChecked():
			self.sb.toggleWidgets(self.sb.duplicate_radial, setEnabled='b003')
			...
			if selection:
				...
				for obj in selection:
					...
					x=y=z = 0
					if self.sb.duplicate_radial.chk012.isChecked(): x = angle
					if self.sb.duplicate_radial.chk013.isChecked(): y = angle
					if self.sb.duplicate_radial.chk014.isChecked(): z = angle
			...
			self.sb.toggleWidgets(self.sb.duplicate_radial, setDisabled='b003')
	...
	def chk016(self, create=False):
		...
		if self.sb.duplicate_linear.chk016.isChecked():
			self.sb.toggleWidgets(self.sb.duplicate_linear, setEnabled='b002')
			...
			transXYZ = [float(self.sb.duplicate_linear.s002.value()),float(self.sb.duplicate_linear.s003.value()),float(self.sb.duplicate_linear.s004.value())]
			rotXYZ = [float(self.sb.duplicate_linear.s007.value()),float(self.sb.duplicate_linear.s008.value()),float(self.sb.duplicate_linear.s009.value())]
			scaleXYZ = [float(self.sb.duplicate_linear.s010.value()),float(self.sb.duplicate_linear.s011.value()),float(self.sb.duplicate_linear.s012.value())]
			translateToComponent = self.sb.duplicate_linear.chk007.isChecked()
			alignToNormal = self.sb.duplicate_linear.chk008.isChecked()
			componentList = [self.sb.duplicate_linear.cmb001.itemText(i) for i in range(self.sb.duplicate_linear.cmb001.count())]
			...
			del duplicateObjList[:] #clear the list
			self.sb.toggleWidgets(self.sb.duplicate_linear, setDisabled='b002')
