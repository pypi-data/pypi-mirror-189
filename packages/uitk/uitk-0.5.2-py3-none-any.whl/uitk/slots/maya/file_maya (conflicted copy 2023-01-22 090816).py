...
class File_maya(File, Slots_maya):
	...
	def cmb006(self, index=-1):
		...
		project = ptk.File.formatPath(path, 'dir') #add current project path string to label. strip path and trailing '/'
	...
	def getRecentAutosave(self, timestamp=False):
		...
		files = ptk.File.getDirContents(dir1, 'filepaths', incFiles=('*.mb', '*.ma')) + ptk.File.getDirContents(dir2, 'filepaths', incFiles=('*.mb', '*.ma'))
		result = [ptk.File.formatPath(f) for f in list(reversed(files))] #Replace any backslashes with forward slashes and reverse the list.
	...
	def getWorkspaceScenes(self, fullPath=True):
		...
		files = ptk.File.getDirContents(workspace_dir, 'filepaths', incFiles=('*.mb', '*.ma'))
		result = [ptk.File.formatPath(f) for f in files] #Replace any backslashes with forward slashes.
