...
def phot_mod (gal, tab_gal, conn, size_image_phy=50, snr_g = 2.0, areagal=2,
              ...
              segmap = None, mask_stars=None, petro_extent_cas=1.5, eta=0.2,
              outputfile='properties.dat',  show_plot='yes', run_auto='no'):
...
def ex_phot_mod (gal, tab_gal, conn, size_image_phy=50, snr_g = 2.0, areagal=10,
                 areagal_deblend=20,
    ...
    print (Fore.BLUE +'image size in pix: {:.1f}'.format(size_image))
    #area_min=(((np.square(areagal)*np.pi)/arc_kpc**2)/0.55**2)
    area_min=areagal/arc_kpc**2/0.55**2
    area_min_deblend=areagal_deblend/arc_kpc**2/0.55**2
    print (Fore.BLUE +'Minimum detection area {:.1f} kcp^2 = {:.0f} pix'.format(areagal,
           area_min))
    print (Fore.BLUE +'Minimum deblending detection area {:.1f} kcp^2 = {:.0f} pix'.format(areagal_deblend,
                   int(area_min_deblend)))
    ...
    if mask_stars == 'yes':
        ...
        apertures = CircularAperture(positions, r=aper_stars)
        ...
        for i, id in enumerate(sources['id']):
            xy_circ = pfunc.circ(sources['xcentroid'][i],
                                 sources['ycentroid'][i], aper_stars, 50,
                                 plotr='no')
            aber_mask = pfunc.maskpoly(xy_circ - 0.5, imaxy, imag* np.nan)
        ...
        pathstars = tab_gal['gal'][idg]+'_{}_{}kpc_mask_stars.fits'.format(band,
                  size_image_phy)
        fits.writeto('fits_images/' + pathstars,  stars_mask.astype(int), overwrite=True)
    ...
    npixels_g = int(area_min) # minimum number of connected pixels
    threshold = photutils.detect_threshold(imag, nsigma=snr_g)
    segm = photutils.detect_sources(imag, threshold, npixels_g, mask=stars_mask)
    ...
    if deblend=='on':
        ...
        if show_plot=='yes':
            ...
            plt.close()
        segm = deblend_sources(imag, segm, npixels=int(area_min_deblend),
                               #kernel=kernel,
                                nlevels=32, contrast=0.001)
    ...
    if run_auto!='yes':
        ...
        part2 = f"""user_order=[1,2], user_list=["A","B"], band='{band}', segmap='{segmap}', mask_stars='{mask_stars}', """
        part3 = f"""petro_extent_cas='{petro_extent_cas}', eta='{eta}', outputfile='{outputfile}')"""
        ...
        part0 = f"\n\nSL.phot_mod('{gal}', tab_gal, conn, size_image_phy={size_image_phy}, "
        part1 = f""" snr_g = {snr_g}, areagal={areagal},sky_box = {sky_box},\n deblend ="{deblend}", """
        part2 = f"""user_order=[{maxarea}], user_list=[{band}], band='{band}', segmap='{segmap}', mask_stars='{mask_stars}', """
        part3 = f"""petro_extent_cas='{petro_extent_cas}', eta='{eta}', outputfile='{outputfile}', show_plot='{show_plot}', run_auto='yes')"""
        print(part0 + part1 + part2 + part3)
        ...
        phot_mod (gal, tab_gal, conn, size_image_phy=size_image_phy,
                  ...
                  segmap=segmap, mask_stars=mask_stars, run_auto='yes')
...
def ex_phot_mod_decals (gal, tab_gal, size_image_phy=50, snr_g = 2.0,
                        areagal=10, areagal_deblend=20,
                        band='r', sky_box=[100, 100], deblend='off',
                        psf=1, run_auto='no', show_plot='yes',
                        nsigma=2, npixels=5, dilate_size=11,
                        outputfile='properties.dat', dr=8, petro_extent_cas=1.5,
                        eta=0.2, segmap=None, mask_stars='no',
                        fwhm=3.0, threshold=10, roundlo=-0.2, roundhi=0.2,
                        sharplo=0.2, aper_stars=5, aper_center=10):
    ...
    area_min=areagal/arc_kpc**2/0.55**2
    area_min_deblend=areagal_deblend/arc_kpc**2/0.55**2
    print (Fore.BLUE +'Minimum detection area in the Field {:.1f} kcp^2 = {:.0f} pix'.format(areagal,
           int(area_min)))
    print (Fore.BLUE +'Minimum deblending detection area {:.1f} kcp^2 = {:.0f} pix'.format(areagal_deblend,
               int(area_min_deblend)))
    ...
    if run_auto == 'no':
        ...
        part3 = f"""user_order=[1,2], user_list=["A","B"], outputfile='{outputfile}', """
        part4 = f"""segmap='{segmap}', petro_extent_cas={petro_extent_cas}, eta={eta}, mask_stars='{mask_stars}')"""
        ...
        part4 = f"""segmap='{segmap}', petro_extent_cas={petro_extent_cas}, \n eta={eta}, mask_stars='{mask_stars}', run_auto='yes')"""
        print(part0 + part1 + part2 + part3 + part4)
        ...
        phot_mod_decals(gal, tab_gal, size_image_phy=size_image_phy, snr_g = snr_g,
                           ...
                           segmap=segmap, mask_stars = mask_stars,
                           run_auto='yes')
