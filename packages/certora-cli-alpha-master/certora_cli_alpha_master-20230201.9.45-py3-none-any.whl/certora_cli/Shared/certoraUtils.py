...
DEFAULT_SOLC = "solc"
PRODUCTION_DOMAIN = 'https://prover.certora.com'
STAGING_DOMAIN = 'https://vaas-stg.certora.com'
    ...
    if build_dir_str is None:
        ...
        try:
            if last_build.exists():
                last_build.unlink()
            # last_build.unlink(missing_ok=True) # missing_ok is 3.8 we are still on 3.7
            last_build.symlink_to(build_uuid, target_is_directory=True)
...
def get_package_and_version() -> Tuple[str, str]:
    """
    @return: A tuple (package name, version)
    package name - either certora-cli or certora-cli-alpha-master
    version - the python package version in format X.Y.Z if found
    If neither certora-cli not certora-cli-alpha-master distrbutions were found, then the package name is an error
    message, and the version is empty
    """
    # Note: the most common reason not to have an installed package is in circleci

    distribution = pkg_resources.working_set.by_key.get(PRODUCTION_PACKAGE_NAME, None)  # type: ignore
    if distribution is None:
        for python_installed_package in pkg_resources.working_set.by_key.keys():  # type: ignore
            if python_installed_package.startswith(DEV_PACKAGE_NAME_PREFIX):
                distribution = pkg_resources.working_set.by_key[python_installed_package]  # type: ignore
                break
    if distribution is not None:
        return distribution.project_name, distribution.version
    return f"couldn't find {PRODUCTION_PACKAGE_NAME} distributed package. Try\n" \
           f"pip install {PRODUCTION_PACKAGE_NAME}", ""
...
def run_local_spec_check(with_typechecking: bool) -> int:
    ...
    try:
        ...
        major_java_version = re.search(r'version \"(\d+).*', java_version_str).groups()[0]  # type: ignore
    ...
    certora_root_dir = get_certora_root_directory()
    local_certora_path = certora_root_dir / "certora_jars" / "Typechecker.jar"
    installed_certora_path = Path(__file__).parent.parent.parent / "certora_jars" / "Typechecker.jar"

    path_to_typechecker = local_certora_path if local_certora_path.is_file() else installed_certora_path
    ...
    if with_typechecking:
        typecheck_cmd = f"java -jar {path_to_typechecker} -b {get_certora_build_file()} " \
                        f"-v {get_certora_verify_file()} -m {get_certora_metadata_file()} " \
                        f"-d {get_certora_internal_dir()}"
    else:
        typecheck_cmd = f"java -jar {path_to_typechecker} -v {get_certora_verify_file()} " \
...
def run_jar_cmd(jar_cmd: str, override_exit_code: bool, custom_error_message: Optional[str] = None,
                logger_topic: Optional[str] = "run", print_output: bool = False) -> int:
    """
    ...
    @param print_output If True, the process' output will be printed on the screen
    ...
    try:
        ...
        if return_code:
            ...
            # We log all lines in stderr, as they contain useful information we do not want the Python loggers to miss
            log_level = logging.INFO if override_exit_code else logging.CRITICAL
...
def run_solc_cmd(solc_cmd: str, output_file_name: str, config_path: Path, solc_input: bytes = None) \
        -> None:
    """
