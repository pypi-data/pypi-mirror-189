...
class Model:
    ...
    def __init_default_puuid(self,
        ...
        def _walrus_wrapper_default_project_name_68d880fdf9a645acabbab28dbfe603e0(expr):
            """Wrapper function for assignment expression."""
            nonlocal default_project_name
            default_project_name = expr
            return default_project_name

        def _walrus_wrapper_default_puuid_9eacd4afeafd4bb39492d3db62b2c942(expr):
            """Wrapper function for assignment expression."""
        ...
        elif (_walrus_wrapper_default_puuid_9eacd4afeafd4bb39492d3db62b2c942(config.get_value(config.OPTION_DEFAULT_PUUID))):
            self.default_puuid = default_puuid
        elif (_walrus_wrapper_default_project_name_68d880fdf9a645acabbab28dbfe603e0(config.get_value(config.OPTION_DEFAULT_PROJECT_NAME))):
            self.default_puuid = self.to_puuid(default_project_name)
        else:
    ...
    def __init_default_muuid(self,
        ...
        def _walrus_wrapper_default_model_name_bb010bbea9d448f4b3bc3440e5806e2f(expr):
            """Wrapper function for assignment expression."""
            nonlocal default_model_name
            default_model_name = expr
            return default_model_name

        def _walrus_wrapper_default_muuid_238ebfbd9ca7492083c106d1a1822fcf(expr):
            """Wrapper function for assignment expression."""
        ...
        elif (_walrus_wrapper_default_muuid_238ebfbd9ca7492083c106d1a1822fcf(config.get_value(config.OPTION_DEFAULT_MUUID))):
            self.default_muuid = default_muuid
        elif (_walrus_wrapper_default_model_name_bb010bbea9d448f4b3bc3440e5806e2f(config.get_value(config.OPTION_DEFAULT_MODEL_NAME))):
            self.default_muuid = self.to_muuid(default_model_name, puuid=self.default_puuid)
        else:
    ...
    def _update_sys_paths_modules(self, abs_syspath: Path, abs_pkgbase: Path):
        ...
        def _walrus_wrapper_abs_syspath_str_864215ea9d2642e49ecc03d377aaa602(expr):
            """Wrapper function for assignment expression."""
            nonlocal abs_syspath_str
            abs_syspath_str = expr
            return abs_syspath_str

        if (_walrus_wrapper_abs_syspath_str_864215ea9d2642e49ecc03d377aaa602(str(abs_syspath))) not in sys.path:
