...
from tinybird.datafile import get_name_tag_version, wait_job, format_datasource, format_pipe, get_project_filenames
...
async def get_current_main_workspace(client, config):
    ...
    if current_workspace.get('is_branch'):
        current_workspace = next((workspace for workspace in workspaces if workspace['id'] == current_workspace['main']),
                                 None)
...
async def get_current_workspace_branches(client, config):
    ...
    branches: List[Dict[str, Any]] = (await client.user_workspace_branches()).get('workspaces', [])
...
def _get_tb_client(token: str, host: str) -> TinyB:
    ...
    return TinyB(token, host, version=VERSION, disable_ssl_checks=disable_ssl)
...
async def folder_pull(client, folder, auto, match, tag, force, verbose=True):  # noqa: C901
...
async def create_workspace_branch(ctx: Context, branch_name: Optional[str], last_partition: Optional[bool], all: Optional[bool], wait: Optional[bool]):
    ...
    try:
        ...
        if not branch_name:
            ...
            branch_name = click.prompt("\nBranch name", default=default_name, err=True, type=str)  # type: ignore

        response = await client.create_workspace_branch(branch_name, last_partition, all)  # type: ignore
        click.echo(FeedbackManager.success_workspace_branch_created(workspace_name=workspace['name'],
                                                                    branch_name=branch_name))
        await switch_workspace(ctx, branch_name, only_branches=True)
...
async def print_data_branch_summary(client, job_id, response=None):
    ...
    columns = ['Data Source', 'Data Branching Result']
    table = []
    for partition in response['partitions']:
        table.append([partition, '\n'.join(response['partitions'][partition])])
...
async def switch_workspace(ctx, workspace_name_or_id, only_branches=False):
    client: TinyB = ctx.ensure_object(dict)['client']
    config = ctx.ensure_object(dict)['config']
...
async def switch_to_workspace_by_user_workspace_data(ctx, user_workspace_data: Dict):
    ...
    try:
        ...
        client = TinyB(user_workspace_data['token'], config['host'], version=VERSION)
...
async def diff_command(filename, fmt, client, no_color=False):
    if filename and len(filename) != 1:
        click.echo("Usage: tb diff filename. Try 'tb diff -h' for help.")
        return
    ...
    if filename:
        filenames = [filename[0]] + get_project_filenames(filename[0])
    else:
    ...
    for resource in remote_datasources + remote_pipes:
        ...
        if name:
            (rfilename, file) = next(((rfilename, file) for (rfilename, file) in local_resources.items() if name == rfilename), ('', None))
            if not file:
                if not filename:
                    click.echo(f"{resource['name']} only exists in remote")
                continue
            ...
            diff_lines = diff_files(target, file, with_format=fmt, with_color=(not no_color))
...
def diff_files(from_file, to_file, from_file_suffix='[remote]', to_file_suffix='[local]', with_format=True, with_color=False):
    ...
    def parse(filename, with_format=True, unroll_includes=False):
        ...
        lines = None
        if extension == '.datasource':
            lines = format_datasource(filename, unroll_includes=unroll_includes) if with_format else file_lines(filename)
        elif extension in ['.pipe', '.incl']:
            lines = format_pipe(filename, 100, unroll_includes=unroll_includes) if with_format else file_lines(filename)
        else:
    ...
    diff = cydifflib.unified_diff(lines1, lines2, fromfile=f'{from_file} {from_file_suffix}', tofile=f'{to_file} {to_file_suffix}')
