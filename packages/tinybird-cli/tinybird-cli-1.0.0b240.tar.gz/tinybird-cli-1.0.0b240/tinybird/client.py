...
class TinyB(object):
    ...
    def __init__(self, token: str, host: str = HOST, version: Optional[str] = None, disable_ssl_checks: bool = False):
    ...
    async def _req(self, endpoint: str, data=None, method: str = 'GET', retries: int = 1, **kwargs):  # noqa: C901
        ...
        try:
            ...
            if method == 'POST':
                response = await requests_post(url, data=data, verify=verify_ssl, **kwargs)
            elif method == 'PUT':
                response = await requests_put(url, data=data, verify=verify_ssl, **kwargs)
            elif method == 'DELETE':
                response = await requests_delete(url, verify=verify_ssl, **kwargs)
            else:
                response = await requests_get(url, verify=verify_ssl, **kwargs)

        # When we hit the rate limit and we get a 409 error, the server is cutting the connection.
        ...
        status_codes_to_retry = [504, 502, 598, 599]
        if response.status_code == 429 or (response.status_code in status_codes_to_retry and method == 'GET'):
            if retries > LIMIT_RETRIES:
                error = parse_error_response(response)
                raise ReachRetryLimit(error)
            retry_after = int(response.headers.get('Retry-After', 0)) + retries
    ...
    async def create_token(self, name: str, scope: str, origin_code: Optional[str], origin_resource_name_or_id: Optional[str]):
        origin = origin_code or "C"  # == Origins.CUSTOM  if none specified
        return await self._req(f"/v0/tokens?name={name}&scope={scope}&origin={origin}&resource_id={origin_resource_name_or_id}", method='POST', data='')
    ...
    async def alter_datasource(self, ds_name: str, new_schema: Optional[str] = None, description: Optional[str] = None, ttl: Optional[str] = None, dry_run: bool = False):
    ...
    async def datasource_append_data(self, datasource_name: str, f, mode: str = 'append', status_callback=None, sql_condition: Optional[str] = None, format: str = 'csv', replace_options: Optional[Set[str]] = None):
        ...
        res = await self._req(f"v0/datasources?{urlencode(params, safe='')}",
                              data=m,
                              method='POST',
                              headers={
    ...
    async def analyze_pipe_node(self, pipe_name: str, node: Dict[str, Any], dry_run: str = 'false', datasource_name: str = None):
        params = {**{
    ...
    async def populate_node(self, pipe_name: str, node_name: str, populate_subset: bool = False, populate_condition: str = None, truncate: bool = True, unlink_on_populate_error: bool = False):
        params: Dict[str, Any] = {
        ...
        response = await self._req(f"/v0/pipes/{pipe_name}/nodes/{node_name}/population?{urlencode(params)}", method='POST')
        return response
    ...
    async def pipe_data(self, pipe_name_or_uid: str, sql: Optional[str] = None, format: Optional[str] = 'json', params: Mapping[str, Any] = None):
        params = {**params} if params else {}
        if sql:
            params['q'] = sql

        return await self._req(f"/v0/pipes/{pipe_name_or_uid}.{format}?{urlencode(params)}")
    ...
    async def user_workspaces(self):
        return await self._req("/v0/user/workspaces/?with_branches=false")

    async def user_workspaces_and_branches(self):
        return await self._req("/v0/user/workspaces/?with_branches=true")

    async def user_workspace_branches(self):
        return await self._req("/v0/user/workspaces/?with_branches=true&only_branches=true")

    async def branches(self):
        return await self._req("/v0/branches")
    ...
    async def create_workspace_branch(self, branch_name: str, last_partition: Optional[bool], all: Optional[bool]):
        params = {
            'name': branch_name,
        }
        if last_partition:
        ...
        return await self._req(f"/v0/branches?{urlencode(params)}", method='POST',
                               data=b'')

    async def branch_workspace_data(self, workspace_id: str, last_partition: Optional[str], all: Optional[str]):
        params = {}
        if last_partition:
        ...
        url = f"/v0/branches/{workspace_id}/data?{urlencode(params)}"
        return await self._req(url, method='POST', data=b'')
    ...
    async def delete_branch(self, id: str):
        return await self._req(f"/v0/branches/{id}", method='DELETE')

    async def add_users_to_workspace(self, workspace: Dict[str, Any], users_emails: List[str]):
        users = ','.join(users_emails)
        return await self._req(f"/v0/workspaces/{workspace['id']}/users/", method='PUT', data={'operation': 'add', 'users': users})

    async def remove_users_from_workspace(self, workspace: Dict[str, Any], users_emails: List[str]):
        users = ','.join(users_emails)
        return await self._req(f"/v0/workspaces/{workspace['id']}/users/", method='PUT', data={'operation': 'remove', 'users': users})

    async def set_role_for_users_in_workspace(self, workspace: Dict[str, Any], users_emails: List[str], role: str):
        users = ','.join(users_emails)
        return await self._req(f"/v0/workspaces/{workspace['id']}/users/", method='PUT', data={'operation': 'change_role', 'users': users, 'new_role': role})
