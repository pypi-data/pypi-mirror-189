...
class TaggerCollection:
    ...
    def calibrate(self, corr=True, stats=True, perf=True, params=True, basis=True, quiet=False):
        ...
        """
        # Sanity checks
        raise_warning(len(set([t.name for t in self._taggers])) == len(self._taggers), "ERROR: Tagger names not unique")
        ...
        warnings = {}
        for tagger in self._taggers:
            warnings[tagger.name] = tagger.calibrate()
        ...
        report_tagger_warnings(warnings)
    ...
    def _prepare_combination(self, name, calibrated, next_selection=None, tagger_subset=None, append=False):
    ...
    def combine_taggers(self, name, calibrated, next_selection=None, tagger_subset=None, append=False):
        ...
        if calibrated and constants.propagate_errors:
            combination.stats._compute_combination_statistics(omega_combined, gradients_combined, covariances)
    ...
    def retry_on_error(self, use_link_alternative=None, increase_func_order=False):
