...
StartLearning = Button(root, text="Start Learning -->", command=Run, bg="lime", padx=1, pady=1, bd=10).place(x=130, y=240)
...
def Gamemode_window():
    ...
    CodingSkillsLabel = Label(top, text="Coding knowledge: " + CodingSkills + "/10", font="Cornerstone 11").place(x=10, y=105)
    LineSeparatorMsg = "-------------------------------------------------------"
    LineSeparator = Label(top, text=LineSeparatorMsg*5, font = "Cornerstone 11").place(x=-10, y=125)
    #Description
    #Planets
    planetsHeading = Label(top,text="Planets", font="Gothic 15 underline", width=8, bg="black",fg="aqua").place(x=40,y=250)
    planetsDescription = Label(top,text="- Learn about the motion of the celestial bodies in our Solar System", font="Helvetica 11").place(x=40, y=300)
    ...
    planetsLaunch = Button(top, text="- Play", width = 6, bg="lime", borderwidth=3, command = planet_simulation).place(x=60,y=300)

    #Gravity
    gravityHeading = Label(top,text="Gravity", font="Gothic 15 underline", width=8, bg="black",fg="aqua").place(x=40,y=300)
...
def check_inputs(): #Returns True if all conditions are satisfied. False if they need to be completed
    if Username == "No Name":
        return False

message = "Please correctly input your "


def popup():
    response = messagebox.askquestion("Introduction Question", message)
