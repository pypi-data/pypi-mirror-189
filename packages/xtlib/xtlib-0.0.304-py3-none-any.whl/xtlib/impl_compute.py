...
class ImplCompute(ImplBase):
    ...
    def monitor_job_node(self, workspace, job_id, sleep, node_index, log_name, escape):
        ...
        console.print("==> monitoring: {}, node{} [{}]".format(job_id, node_index, target_str), flush=True)
            ...
            with KeyPressChecker() as checker:
                ...
                while simple_status != "completed":
                    ...
                    if sleep_count == sleeps_per_call:
                        ...
                        result = backend.read_log_file(service_node_info, log_name=log_name, start_offset=start_offset, log_source=log_source)
                        ...
                        if first_call or new_log != log_name:
                            log_name = new_log                
                            if file_path:
                                console.print("==> node{} streaming log: {} ({})".format(node_index, log_name, file_path))
                            else:
                        ...
                        if new_text:
                            if "$status: " in new_text:
                                index = new_text.index("$status: ")
                                index2 = new_text.index("\n", index)

                                if index2:
                                    msg = new_text[index+9:index2]
                                else:
                                    msg = new_text[index+9:]
                                # change title of our console window to show the status msg
                                os.system("title " + msg)
        ...
        for job_id in job_names:
            ...
            if cancel:
                ...
                try:
                    ...
                    result_by_node = service_info_by_node
            ...
            if wrapup:
                ...
                runs = run_helper.wrapup_runs_for_nodes(self.store, workspace, job_id, node_list, max_workers)
                ...
                # cleanup the JOB
                self.store.database.mark_job_cancelled(workspace, job_id)
    ...
    def send_approval_email_to_requester(self, request_id, requester, approver, job_id):
        ...
        body += "<br><b>approver by:</b> {}".format(approver)
    ...
    def approve(self, names, workspace, force):
        ...
        for record in records:
            ...
            if force or status == "requested":
                ...
                job_id = output_parts[0].split(",", 1)[0]
    ...
    #---- CANCEL QUEUES command ----
    @argument("target", help="the name of the compute target")
    @option("queues", type="str_list", values=["queued", "running", "completed"], help="the queues to cancel")
    @option("username", default="$general.username", help="the username to filter entries with")
    @option("workspace", default="$general.workspace", help="the workspace that contains the jobs to be cancelled")
    @option("max-workers", type=int, default="$general.max-run-workers", help="the max number of background workers for this command")
    @flag("cancel", default=1, help="call the backend service to cancel the job")
    @flag("wrapup", default=1, help="wrapup the runs associated with the job")
    @example(task="cancel my all of my queued jobs on the target philly-rr1", text="cancel queues philly-rr1  --queues=queued ")
    @command(kwgroup="cancel", help="cancel the queue entries for the specified compute target")
    def cancel_queues(self, target, username, queues, workspace, max_workers, cancel, wrapup):
        backend = self.core.create_backend(target, username=username)
        target_def = self.config.get_target_def(target)

        results_by_queue, columns = backend.get_queue_jobs(target_def, username, queues, workspace)
        
        for queue, jobs in results_by_queue.items():
            cancel_results = []
            ...
            for job in jobs:
                ...
                if cancel:
                    job_id = job["job_id"]
                    console.print("cancelling {} (service: {}):".format(job_id, backend.get_name()))
                    result = backend.cancel_job_from_service_dict(job, workspace)
                ...
                if wrapup:
                    ...
                    try:
                        ...
                        console.print("exception during wrapup: {}".format(ex))
    ...
    @option("aml_compute", type=str, default=DEFAULT_FROM_COMPUTE, help="specifies the name of the AML compute resource for the job")
    ...
    @flag("log-reports", default="$after-files.log-file-uploads", help="if set, log each AFTER file/size before uploading")
    @hidden("log-file-uploads", default="$logging.log-reports", help="specifies if CPU, GPU, and MEMORY reports should be included in setup log")
    ...
    @hidden("mirror-dest", default="$logging.mirror-dest", help="the location where mirrored information is stored (none, storage, grok)")
    @hidden("mirror-files", default="$logging.mirror-files", help="the wildcard path that specifies files to be mirrored")
    @hidden("mirror-log-files", default="$logging.mirror-log-files", help="when true, the job's log file directory is mirrored")
    ...
    @hidden("mount-retry_count", type=int, default="$general.mount-retry-count", help="the number of times to retry each blobfuse mount command")
    @hidden("mount-retry_interval", type=str, default="$general.mount-retry-interval", help="the time to sleep between retries of blobfuse mount command")
    ...
    def run(self, args):
        ...
        if result:
            ...
            if monitor == "same":
                ...
                store_name = self.config.data["store"]
                cmd = "xt --echo --store={} monitor {} --workspace={}".format(store_name, job_id, workspace)
