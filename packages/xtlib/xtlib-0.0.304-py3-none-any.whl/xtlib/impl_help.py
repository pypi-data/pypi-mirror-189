...
from .qfe import command, argument, option, flag, root, example, command_help, kwgroup_help, keyword_arg
...
class ImplHelp(ImplBase):
    ...
    def set_basic_mode(self, value):
        self.basic_mode = value
        self.name = "xt"  
            ...
            if not "hidden" in fi or not fi["hidden"]:
                ...
                if grouping_enabled and "kwgroup" in fi:
                    ...
                    if kwgroup:
                        ...
                        help_text = fi["kwhelp"]
                        if not help_text:
                            errors.internal_error("kwhelp must be defined for this LAST kwgroup member: " + fi[name_field])
    ...
    def help_about(self):
        ...
        adv = "[advanced mode], " if not self.basic_mode else ""

        console.print("  - " + adv + constants.BUILD)
        console.print("  - Copyright (c) 2022, Microsoft Corporation")
        console.print("  - Developed by the MSR Redmond Tools team")
        console.print()

        console.print("XT Key Features")
        console.print("  - Scale ML experiments across multiple COMPUTE services:")
        console.print("      - local machine, VM's, Azure Batch, Azure AML, Singularity")
        ...
        console.print("  - SQL Server database (for fast, scalable access to stats and metrics)")
        console.print("  - XT controller app manages runs on all nodes")
        console.print()

        console.print("XT Behavior")
        console.print("  - controlled by hierarichal set of user options:")
        console.print("    - factory config file (readonly; default XT behavior)")
        console.print("    - global config file (project-level overrides)")
        console.print("    - local directory config file (task-specific overrides")
        console.print("    - command line options (command-specific overrides")

    def help_docs(self, browse=False):
        #url = "https://xtdocs.z22.web.core.windows.net/"
        url = "https://xtdocsfordilbert.z22.web.core.windows.net/"
    ...
    def help_version(self):
        console.print("XT {}".format(constants.BUILD))
    ...
    def command_summary(self):
        ...
        if self.basic_mode:
            ...
            console.print("  the local machine, VM's, Azure Batch, Singularity, and Azure ML.")
            console.print()
    ...
    def command_help(self, cmd_info, syntax_only=False, args_only=False):
        ...
        if show_all and not self.basic_mode:
            # print command help    
        ...
        if syntax_only:        
            # show keywords for a keyword_arg, if any
            if args:
                arg = args[-1]
                if "keywords" in arg:
                    keywords = arg["keywords"]
                    console.print("  {}: {}".format(arg["name"], or_join(keywords)))
