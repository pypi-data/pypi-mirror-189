...
def process_template(config, cmd):
    '''
    ...
    arg_dict = args_to_dict(template_actual_args)
    ...
    if len(template_formal_args) > len(template_actual_args):
        errors.user_error("templates requires {} args, but only {} were supplied".format(len(template_formal_args), 
    ...
    print("template expansion: {}".format(cmd))
    return cmd
...
def main(cmd, new_start_time=None, capture_output=False, basic_mode=False, raise_syntax_exception=True):
    ...
    if cmd.startswith("@"):
        cmd = process_template(config, cmd)
