...
class CanvasRobot(object):
    ...
    TOT_WEIGHT: int = 100
    year: int = AC_YEAR
    def __init__(self, reset_api_keys=False, years_back = 0):
    ...
    def course_metadata(self, course_id, examination_names=None):
        """
        ...
        assignments_summary = "Assignments (online_uploads have details)\n"
            ...
            for submission in submissions:
                if submission.submission_type == "online_upload":
                    if assignment.name in examination_names:
                        submissions_summary += "Examination:"
                    else:
                        submissions_summary+="Possible Examination:"
                    ...
                    submissions_summary+=(f"/ngraded "
                                          f"{submission.grade} at "
                                          f"{submission.graded_at}")
            assignments_summary+=assignment.name + ":"
            assignments_summary+=submissions_summary
        ...
        if examination_files == 0:
            examinations_summary += f"\nNo examination_files in {EXAMINATION_FOLDER}"
        else:
    ...
    def get_examinations_from_database(self, candidate=False):
        db = self.db
        records = db((db.examination.id>0)&
                     (db.examination.candidate==candidate)
                     ).select(db.examination.ALL)
        return records
    ...
    def update_database_from_canvas(self, single_course=None):
        """
        ...
        logging.info(f'open courselist for year {self.year} - {self.year+1}')
        num_rows = 0
        ...
        for course in courses:
            ...
            course_id = None
            canonical_examination_names = [row.name for row in examinations if row.course==course_id]
