...
from kaiju_auth.sessions import UserSession
...
class UserService(SQLService, Contextable, AbstractRPCCompatible):
    ...
    async def register(self, username: str, email: str, password: str, columns='*', **settings):
        """Addsa new user. Used by administrators or user managers."""
    ...
    def get_user_info(self, session: UserSession):
    ...
    async def get_profile(self, session: UserSession):
        user_id = session['user_id']
        data = await self.get(user_id, columns=['settings'])
        return {'id': user_id, 'settings': data['settings']}

    async def update_profile(self, session: UserSession, settings: dict):
        user_id = session['user_id']
        data = await self.get(user_id, columns=['settings'])
        meta = data['settings']
