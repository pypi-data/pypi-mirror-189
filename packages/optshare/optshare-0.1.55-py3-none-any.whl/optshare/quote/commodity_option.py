...
class CommodityOption:
    ...
    def get_expiry_date(self, option_underlying_contract, cal):
        if self.exchange_name == "dce":
            ...
            month = int(yymm[2:4])
            last_month = add_months(date(year, month, 1), -1) # todo: update package later

            return optshare.get_nth_trading_day(5, last_month.year(), last_month.month(), cal)

        elif self.exchange_name == "shfe" or "ine":
            yymm = option_underlying_contract[-4:]
            year = int(yymm[0:2]) + 2000
            month = int(yymm[2:4])
            previous_month = add_months(date(year, month, 1), 1)

            if self.exchange_name == "ine":
                # 原油
                return optshare.get_lastnth_trading_day(13, previous_month.year(), previous_month.month(), cal)

            return optshare.get_lastnth_trading_day(5, previous_month.year(), previous_month.month(), cal)
