      ...
      if os.path.exists(path.group(1)):
        ...
        if os.path.isdir(path.group(1)):
          ...
          new_filepath = "./"
            ...
            new_filepath = os.path.basename(path.group(1))
...
def deploy(user_email: str):
  from google.colab import _message # will only work inside colab nb 
  ...
  for cell in notebook_json_string["ipynb"]["cells"]:
    for line in cell["source"]:
      ...
      elif "google.colab" in line or "drive.mount('/content/drive')" in line or "deploy(" in line:
        continue
