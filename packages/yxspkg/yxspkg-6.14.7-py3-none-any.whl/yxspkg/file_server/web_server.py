...
def check_cookie():
    if global_setting['check_user']:
        ...
        if user_token in global_cookies:
            global_setting['succeed'] = time.time()
            return True 
        else:
            if 'succeed' in global_setting:
                pt = global_setting['succeed']
                nt = time.time()
                if nt-pt < 3600:
                    return True 
                else:
...
class FileSystem:
    def GET(self,*d):
        ...
        for i in x:
            filename=p+i
            if yxsfile.yxsFile(i).decode_filename().name.startswith('__'):
                continue
...
class player:
    ...
    <source src="/file_downloader/x{mp4}"  type="video/mp4" />
    ...
    def GET(self):
        ...
        html_string1 = self.player_html.format(mp4=encode(file_name),vtt=encode(str(Path(file_name).with_suffix('.vtt'))),title=namet,vfile=vfile_name)
        html_bytes = BytesIO()
...
class download:
    def GET(self):
        ...
        for i in download_file(fy,length,path.basename(file_name),hrange=hrange):
            yield i
def getip(ipv6):
    if ipv6:
        afi = socket.AF_INET6
...
def main(port,ssl,ipv6=False,check_user=False,generate_poster=False):
...
if __name__ == '__main__':
    main(8080,False,check_user=True,generate_poster=False)
