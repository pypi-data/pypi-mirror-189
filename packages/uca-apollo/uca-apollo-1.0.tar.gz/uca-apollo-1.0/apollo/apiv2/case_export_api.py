...
class CaseExportApi(object):
    ...
    def get_case_export_callbacks(
        ...
        **kwargs
        ...
        Keyword Args:
            date (datetime): Date for which fetch the Case Export Callback list. By default, the date will be set to current date.. [optional]
            status (str): Status of Case Export callbacks. Default status will fetch all callbacks.. [optional]
            page_number (int): Page to fetch the Case Export Callback list.<br>   - Minimum: 1 . [optional] if omitted the server will use the default value of 1
            _return_http_data_only (bool): response data without head status
                code and headers. Default is True.
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
