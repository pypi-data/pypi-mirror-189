...
def push(session: Session, metadata, workbook_context: WorkbookContext, datasource_output, sync_token,
         ...
         state_file: Optional[str] = None, planning_to_archive: bool = False):
...
TYPE_SIMPLIFIER_REGEX = re.compile(r'(Stored|Calculated|Threshold)')
    ...
    if PUSH_DIRECTIVE_CREATE_ONLY in push_directives or PUSH_DIRECTIVE_UPDATE_ONLY in push_directives:
        ...
        if previous_push_item is not None:
            ...
            search_results = items_api.search_items(
                filters=[f'Datasource Class=={push_context.datasource_output.datasource_class}&&'
                         f'Datasource ID=={push_context.datasource_output.datasource_id}&&'
                         f'Data ID=={scoped_data_id}'])
    ...
    for path_item in path_items:
        ...
        if asset_input.data_id not in push_context.reified_assets:
            ...
            if existing_asset_row is not None:
                ...
                # Need to re-grab it from push_results so we handle it as a PushItem
                asset_dict = push_context.push_results.loc[asset_index]
