...
def test_my_csv_tree_with_incremental_push():
    ...
    csv_file = (pathlib.Path(__file__).absolute().parent
                / ".." / ".." / ".." / "docs" / "Documentation" / "spy_tree_example.csv")
    my_csv_tree = spy.assets.Tree(str(csv_file), workbook=workbook)
...
def test_insert_move_and_remove_displays():
    ...
    workbook, _, _, template = test_common.create_workbook_workstep_asset_template()
    tree = Tree('My Tree', workbook=workbook.id)
...
def test_replace_non_sdl_displays():
    ...
    workbook, _, _, template = test_common.create_workbook_workstep_asset_template(datasource='Non-sdl')
    tree = Tree('Example', workbook=workbook.id)
    push_results = tree.push()
    ...
    tree = Tree(push_results.at[0, 'ID'], workbook=workbook.id)
    tree.push()
    ...
    assert (new_displays['Datasource Name'] == 'Seeq Data Lab').all()
    ...
    assert sample_display_output.datasource_id == 'Seeq Data Lab'
...
def test_insert_rollup():
    workbook = f'Workbook {_common.new_placeholder_guid()}'
    tree = Tree('Rollup Tree', workbook=workbook)
    tree.insert(['Cooling Tower 1', 'Cooling Tower 2'])
...
def test_formula_parameter_absolute_path():
    workbook = f'Workbook {_common.new_placeholder_guid()}'
    tree = Tree('Absolute Path Tree', workbook=workbook)
    tree.insert(['A', 'B'])
...
def test_formula_parameter_relative_path_above():
    workbook = f'Workbook {_common.new_placeholder_guid()}'
    tree = Tree('Relative Path Tree', workbook=workbook)
    tree.insert(['A', 'B'])
    tree.insert('Parameter', formula='sinusoid()', parent='A')
...
def test_select():
    ...
    tree = spy.assets.Tree('My Tree', workbook=workbook)
...
def test_change_tree_scope():
    ...
    tree = Tree('Test Tree Scope Change', workbook=workbook_1)
    ...
    # Clean up
    items_api.archive_item(id=tree._dataframe.loc[0, 'ID'])


@pytest.mark.system
def test_create_globally_scoped_tree():
    items_api = ItemsApi(spy.session.client)
    tree = Tree('Test Create Global Scope Tree', workbook=None)
...
def test_tree_redaction():
    ...
    admin_tree = Tree('Admin Tree', workbook=workbook, session=admin_session)
    ...
    for subset in itertools.chain.from_iterable(itertools.combinations(range(4), r) for r in range(5)):
        ...
        # Test insert
        tree = Tree('My Tree', workbook=workbook_id)
        status = spy.Status()
        tree.insert(admin_tree, errors='catalog', status=status)
        ...
        tree = Tree(admin_tree.items(), workbook=workbook_id, status=status)
...
def test_update_global_spy_tree():
    original_tree = spy.assets.Tree(f'My Tree {_common.new_placeholder_guid()}', workbook=None)
    original_tree.insert(name='My Signal', formula='sinusoid()')
    original_tree.push()

    pulled_tree = spy.assets.Tree(original_tree.items().loc[0, 'ID'], workbook=None)
