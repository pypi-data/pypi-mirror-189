...
from tala.model.plan_item import JumpToPlanItem, IfThenElse, ForgetAllPlanItem, ForgetPlanItem, ForgetSharedPlanItem,\
    InvokeServiceQueryPlanItem, InvokeDomainQueryPlanItem, InvokeServiceActionPlanItem, AssumeSharedPlanItem, \
    AssumeIssuePlanItem, LogPlanItem, AssumePlanItem, GetDonePlanItem, GoalPerformedPlanItem, GoalAbortedPlanItem, \
    EndTurnPlanItem, ResetDomainQueryPlanItem, IteratePlanItem, ChangeDDDPlanItem
...
class DomainCompiler(XmlCompiler):
    ...
    def _compile_query_content(self, query, element):
    ...
    def _compile_randomized_query(self, query, element):
        try:
            selection = self._get_single_child_element(element, ["select_at_random"])
            predicate_name = self._get_mandatory_attribute(element, "predicate")
            individual_elements = self._find_child_nodes(selection, "individual")
    ...
    def _compile_inform_element(self, element):
        def create_non_forgetting_plan():
            ...
            return [IfThenElse(cond, [], alternative)]

        def create_forgetting_plan():
            return [ForgetPlanItem(predicate)] + assume_prop_and_issue + generate_end_turn_plan()

        def generate_end_turn_plan():
            if generate_end_turn:
                return [EndTurnPlanItem(float(expected_passivity))]
            else:
                return []
        ...
        generate_end_turn = self._get_optional_attribute(element, "generate_end_turn")
        expected_passivity = self._get_optional_attribute(element, "expected_passivity", 0.0)
        assume_prop_and_issue = [AssumePlanItem(predicate_proposition), AssumeIssuePlanItem(question)]
    ...
    def _compile_assume_issue_element(self, element):
        ...
        question = self._compile_question(element)
        return [AssumeIssuePlanItem(question, insist=insist)]
    ...
    def _compile_signal_action_completion_element(self, element):
        ...
        return [GoalPerformedPlanItem(self._parse_boolean(postconfirm))]
    ...
    def _compile_end_turn_element(self, element):
        ...
        return [EndTurnPlanItem(float(timeout))]

    def _compile_reset_domain_query_element(self, element):
        question = self._compile_question(element)
        return [ResetDomainQueryPlanItem(question)]
    ...
    def _compile_query_element(self, element):
        ...
        self._compile_query_content(query, element)
        return query
