...
from . import xproblem, xpd, xsettings, xcalc, xstats
...
def post_plot(title='', legend=True, legend_loc='best', xlim=None, ylim=None, xlabel=None, ylabel=None, x_rotation=None, tight_layout=True, show=True, y_auc=False, add_date=False):
    """
...
def plot_multi(df, kind=None, plot_func=None, x=None, y=None, plot_on=None, group_on=None, color_on=None, cmap=None, label_on=None, style_on=None, size_on=None, figsize=(10,6), alpha=1.0, hdi_probs=(0.1, 0.25, 0.5, 0.8, 0.999), color_dict=None, title='', x_axis_type=None, y_axis_type=None, x_axis_fmt=None, y_axis_fmt=None, legend_loc='best', xlim=None, ylim=None, save_to=None, clear_folder=False, add_date=True, drop_na=True, show=True, **kwargs):
    """
    ...
    if kind in ['%', 'percentile', 'percent', 'perc']:
        kind = 'percentiles'
    ...
    for df_p, _, plot_title in xpd.x_iter_groups(df, plot_on):
        ...
        for df_g, _, group_title in xpd.x_iter_groups(df_p, group_on):
            ...
            if kind in ['scatter', 'reg']:
                ...
                if kind == 'reg':
                    ...
                    st = xstats.x_model_pred_stats(_y, _y_pred)
                    text = f"R2={st.r2:.3f} MAE={st.mae:.3f}"
                    x_min = _x[_x.argmin()]
                    x_max = _x[_x.argmax()]
                    ...
                    plt.plot([x_min, x_max], [y_min, y_max], ls=':', color='black')
                ...
                df_means = g_hdi[y].median().reset_index(name='_mean_val')
                plt.plot(df_means[x], df_means['_mean_val'], ls='--', color=color)
        ...
        if y_axis_fmt:
            ax.xaxis.set_major_formatter(y_axis_fmt)
