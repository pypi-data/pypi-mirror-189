...
def local_source_to_image(
    ...
    client = ServiceFoundryServiceClient()
    response = client.get_docker_registry_creds(
        docker_registry_fqn=docker_registry_fqn,
    )

    registry_url = response["registryUrl"].rstrip("/")
    registry_username = response["username"]
    registry_password = response["password"]

    response = client.create_repo_in_registry(
        docker_registry_fqn=docker_registry_fqn,
        workspace_fqn=workspace_fqn,
        application_name=component_name,
    )
    repo_name = response["repoName"]
