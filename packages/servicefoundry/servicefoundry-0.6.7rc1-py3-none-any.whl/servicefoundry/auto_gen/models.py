...
#   timestamp: 2023-02-02T08:52:50+00:00
...
class FileMount(BaseModel):
    ...
    is_base64_encoded: bool = Field(
        False, description="+usage=Flag to add base64 encoded data"
    )
    data: Dict[str, str] = Field(
        ...,
        description="+usage=Data to be mounted, the key will be the filename, and the value will be the file content. Files will be mounted under mount_path",
    )
...
class GPU(BaseModel):
    type: str = Field(
        ...,
        description="+label GPU Name\n+usage=Name of the Nvidia GPU Type. One of [T4, K80, V100]",
    )
...
class HuggingfaceModelHub(BaseModel):
    ...
    type: constr(regex=r"hf-model-hub")
    model_id: constr(regex=r"[A-Za-z0-9_.-/]+") = Field(
        ...,
    ...
    pipeline: Optional[constr(regex=r"[a-z-]+")] = Field(
        None,
...
class Restart(str, Enum):
    """
    +usage=When to restart the job (Deprecated)
    """
...
class LocalSource(BaseModel):
    ...
    local_build: bool = Field(False, description="run docker build locally")
...
class AppProtocol(str, Enum):
    """
    +usage=App Protocol. Optional. Only effective when `expose` is true
    This can be used to enforce a specific app level protocol when traffic goes via Truefoundry Platform gateway
    """
...
class Port(BaseModel):
    ...
    app_protocol: Optional[AppProtocol] = Field(
        None,
        description="+usage=App Protocol. Optional. Only effective when `expose` is true\nThis can be used to enforce a specific app level protocol when traffic goes via Truefoundry Platform gateway",
    )
    host: Optional[
        constr(
...
class Resources(BaseModel):
    ...
    memory_request: confloat(ge=1.0, le=32000.0) = Field(
        200,
    ...
    memory_limit: confloat(ge=1.0, le=32000.0) = Field(
        500,
    ...
    ephemeral_storage_request: float = Field(
        1000,
    ...
    ephemeral_storage_limit: float = Field(
        2000,
...
class TruefoundryModelRegistry(BaseModel):
    ...
    type: constr(regex=r"tfy-model-registry")
...
class Job(BaseModel):
    ...
    restart: Restart = Field(
        "Never", description="+usage=When to restart the job (Deprecated)"
    )
    ...
    file_mounts: Optional[List[FileMount]] = Field(
        None, description="+usage=Files to be mounted\n+ignore\n+sort=10010"
    )
