...
def Read_Alaw(filename,sr=8000,ffmpeg_path='c:\\ffmpeg.exe'):
           ffmpeg_command =  '%s -ar 8000 -f alaw -i "%s" -ac 1 -ar %s -f wav pipe:1' %(ffmpeg_path ,filename,sr)
...
def ReadMp3(filename,sr=16000,mono=True,ffmpeg_path='c:\\ffmpeg.exe'):
     ...
     if (_system == 'windows'):      
           ffmpeg_command =  '%s -i "%s" -ac %s -ar %s -f wav pipe:1' %(ffmpeg_path ,filename, chnum,sr)
        ...
        chnum=1
        ffmpeg_command =  'ffmpeg -i "%s" -ac %s -ar %s -f wav "%s"' %(filename, chnum,sr , PN(filename) + '_temp.wav')
        os.system(ffmpeg_command) 
        import librosa
        x,_ = librosa.load(PN(filename) + '_temp.wav' , sr)
        os.system('rm "%s"' %(PN(filename) + '_temp.wav'))
        return( x )       
...
def change_format(fname , sr=16000 , ext='mp3' , mono=True ,ffmpeg_path='c:\\ffmpeg.exe' ,oname=None ):
    ...
    try:
      ffmpeg_command =  '%s %s -i "%s" -ac %s -ar %s "%s"' %(ffmpeg_path , forext,fname, chnum,sr ,outfile) 
