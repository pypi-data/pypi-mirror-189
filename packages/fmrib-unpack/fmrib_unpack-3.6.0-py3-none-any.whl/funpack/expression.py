...
def calculateExpressionEvaluationOrder(vids, exprs):
    ...
    :returns:   A list of tuples, each containing:
                ...
                The list is in ascending order, by the hierarchy level, from
    ...
    # get a list of parents for each var
    parents = []
    for expr in exprs:
    ...
    # Record all variable IDs, and create
    # a { vid : parents } mapping, then
    # invert that mapping to create a
    # { vid : children } mapping
    allvids  = sorted(set(it.chain(vids, *parents)))
    parents  = {v : set(p)                for v, p in zip(vids, parents)}
    parents  = {v : parents.get(v, set()) for v in allvids}
    children = {v : set() for v in allvids}

    for vid in allvids:
        for parent in parents[vid]:
            children[parent].add(vid)

    # Use Kahn's topological sort to identify
    # the hierarchy level of each variable in
    # the dependency graph. This involves a
    # breadth-first traversal through the
    # graph, recording the hierarchy level for
    # each node as a { vid : level } mapping.
    # Start from the hierarchy roots (level
    # == 0), and work down to the leaves.
    levels   = {}
    level    = 0
    lcurrent = {vid for vid in allvids if len(parents[vid]) == 0}

    while len(lcurrent) > 0:
        lnext = set()
        for vid in lcurrent:
            levels[vid] = level

            # Remove each edge as we traverse the graph
            for cvid in list(children[vid]):
                children[vid].remove(cvid)
                parents[cvid].remove(vid)

                # nodes with no parents form the
                # next level in the hierarchy
                if len(parents[cvid]) == 0:
                    lnext.add(cvid)
    ...
    remaining = [v for v, p in parents.items() if len(p) > 0]
    if len(remaining) > 0:
        raise ValueError(f'Circular dependencies identified: {remaining}')

    # Now we can order the variables by
    # hierarchy, from leaves to roots, to
    # get the expression evaluation order
    bylevel = {}
    ...
    return list(sorted(bylevel.items(), key=lambda l: l[0], reverse=True))
