...
from .config import Config, ScanConfig, MetadataProviderApi, OperationOptions, MediaType, Action
...
class MediaSorter:
    ...
    async def scan(
        ...
        if os.path.isdir(src_path):
            ...
            if op := await self.suggest(src_path, media_type=media_type, action=action):
                if op.type == MediaType.TV_SHOW and tv_shows_output:
    ...
    async def suggest_movie(self, src_path: str) -> Tuple[Optional[str], str]:
        ...
        metainfo = list()
        if self.config.parameters.movie.allow_metadata_tagging:
    ...
    async def suggest(
        ...
        if media_type in [MediaType.AUTO, MediaType.TV_SHOW]:
            try:
                ...
                # Build the final path+filename
                suggested_dest = os.path.join(season_dir, filename)
