...
from mediasorter.config import Config, read_config, DEFAULT_CONFIG_PATHS, ScanConfig, \
    OperationOptions
from mediasorter.sorter import MediaSorter, Action, MediaType, Operation
...
def cli_root(
    ...
    if metainfo_tag is not None:
        config.parameters.movie.allow_metadata_tagging = metainfo_tag
    ...
    ops = sorted(ops, key=lambda o: o.is_error, reverse=True)
    ...
    with Progress(transient=True) as progress:
        ...
        for operation in ops:
            ...
            time.sleep(0.1)
            progress.update(task, description=os.path.basename(operation.output_path), advance=1)
    ...
    if any([op.is_error for op in ops]):
        sys.exit(1)
