...
class CommitValidator(Commit):
    ...
    def _validate_commit_message(self, commit_msg: str):
        ...
        type, scope = self._get_conventional_commit_type_and_scope(commit_msg=commit_msg)
        match type:
            case None:
                return False
            case "feat":
            ...
            case _:
                return self._validate_commit_message_for_generic_type(type=type, scope=scope)
    ...
    def _validate_commit_message_for_feat_type(self, scope: str) -> bool:
        ...
        if scope is None or scope not in ["(add)", "(change)", "(remove)"]:
            self.error_counter.increment_feat_scope_error()
            return False
        else:
            return True
    ...
    def _validate_commit_message_for_generic_type(self, type, scope: str) -> bool:
        ...
        if scope is None:
            ...
            self.error_counter.add_generic_type_scope_error_message(
                message=f"Commits of type '{type}' must not specify a scope but simply use the form '{type}'"
            )
            return False
