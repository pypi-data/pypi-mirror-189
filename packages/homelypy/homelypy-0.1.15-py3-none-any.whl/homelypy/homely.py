...
class Homely:
    ...
    def _register_callbacks(self):
        @self.sio.event
        def connect():
            logger.info("Homely websocket: connected to server")

        @self.sio.event
        def disconnect():
            logger.info("Homely websocket: disconnected from server")
    ...
    def run_socket_io(
        ...
        while True:
            try:
                ...
                logger.error("Error while connecting for authentication. Sleeping for 1 minute.")
                time.sleep(60)
                continue
            logger.info(f"Connecting to web socket {url}")
            self.sio = socketio.Client(logger=logger, engineio_logger=logger, reconnection=False)
            ...
            try:
                logger.info("Connecting to Homely websocket")
