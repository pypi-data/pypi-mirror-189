...
from majormode.perseus.utils import module_util
...
class AccountService(BaseRdbmsService):
    ...
    # - Contain at least one symbol character (i.e., other than alphanumeric
    #   character)
    ...
    def __add_picture(
        ...
        @param image_file_checksum: Message digest of the the binary data of
    ...
    def __generate_password(cls, length=8, are_symbols_allowed=False):
        ...
        if are_symbols_allowed:
            alphabet += cls.PASSWORD_ALLOWED_SYMBOL_CHARACTERS
    ...
    def __get_picture(
        ...
        @return: An object containing the following attributes:
            ...
            * `creation_time` (required): The time when this photo has been
              registered to the platform

            * `image_file_checksum` (required): The message digest of the the binary
              data of the user's original photo image file.

            * `image_file_size` (required): The size in bytes of the user's original
                photo image file.

            * `image_height` (required): The number of pixel rows of the user's
              original photo image.

            * `image_width` (required): The number of pixel columns of the user's
              original photo image.

            * `is_review_required` (required): Indicate whether the picture needs to
              be reviewed by someone who has authority on the online service.

            * `object_status` (required): The current status of the picture.
    ...
    def __set_picture_status(
        ...
        """
        with self.acquire_rdbms_connection(auto_commit=True, connection=connection) as connection:
            picture = self.__get_picture(picture_id, check_status=False, connection=connection)
            ...
            if status == ObjectStatus.enabled:
                connection.execute(
                    ...
                    """,
                    {
                        ...
                        'picture_id': picture_id,
                        'picture_time': picture.creation_time,
                    }
                )
    ...
    def find_picture(
        ...
        @param image_file_checksum: The message digest of the the binary data
            of the user's original photo image file.
        ...
        with self.acquire_rdbms_connection(connection=connection) as connection:
            ...
            row = cursor.fetch_one()
            picture = row and row.get_object({
                'account_id': cast.string_to_uuid,
                'object_status': ObjectStatus,
                'picture_id': cast.string_to_uuid,
                'submitter_account_id': cast.string_to_uuid,
    ...
    def get_account(
        ...
        with self.acquire_rdbms_connection(auto_commit=False, connection=connection) as connection:
            ...
            account = row.get_object({
                ...
                'object_status': ObjectStatus,
                'picture_time': cast.string_to_timestamp,
                'update_time': cast.string_to_timestamp
            })
    ...
    def get_picture(
            ...
            connection: RdbmsConnection = None,
            include_image_info: bool = False) -> any:
        """
        ...
        @param check_status: Indicate whether the check the status of the
            picture.
        ...
        @return: The information about the picture:

            - `account_id: uuid`.UUID` (optional): The identification of the user
              account that the picture is associated with.  This attribute may be
              null if this account was hard-deleted.

            - `app_id: uuid` (required): The identification of the client
              application that uploaded the picture.

            - `creation_time: ISO8601DateTime` (required): The time when this photo
              has been registered.

            - `image_file_checksum: string` (optional): The message digest of the
              binary data of the user's original photo image file.  This attribute
              is only returned if the argument `include_image_info` is `True`.

            - `image_file_size: int` (optional): The size in bytes of the user's
              original photo image file.  This attribute is only returned if the
              argument `include_image_info` is `True`.

            - `image_height: int`: (optional): The number of pixel rows of the
              user's original photo image.  This attribute is only returned if the
              argument `include_image_info` is `True`.

            - `image_width: int` (optional): The number of pixel columns of the
              user's original photo image.  This attribute is only returned if the
              argument `include_image_info` is `True`.
            ...
            - `picture_url: str` (optional): The Uniform Resource Locator (URL) that
              specifies the location of the picture.  A client application can
              append the query parameter `size` to this URL to specify a given pixel
              resolution of the picture, such as `thumbnail`, `small`, `medium`, or
              `large`.  This attribute is only returned if the picture is
              `ObjectStatus.disabled` or `ObjectStatus.enabled`.

            - `submitter_account_id: uuid.UUID` (optional): The identification of
              the account of the user who submitted the picture.  This attribute
              may be null if this account was hard-deleted.

            - `team_id: uuid.UUID` (optional): The identification of the
              organization of the user who submitted the picture.
    ...
    def sign_in_with_contact(
        ...
        @return: An object containing the following members:
            ...
            - `account_id: uuid.UUID` (required): The identification of the accoun
              of the user.
    ...
    def upload_picture(
        ...
        @param connection: An object `RdmbsConnection` with auto-commit.
        ...
        if picture:
            if picture.account_id != account_id:
                raise self.IllegalAccessException('This picture is already associated to another user')
...
ContactService = module_util.load_class('majormode.perseus.service.account.contact_service.ContactService')
