...
def toCalendarCls(arg) -> mx.Calendar:
    ...
    if arg_s in [str(mx.SouthKorea()), 'KR']: return mx.SouthKorea()
    elif arg_s in [str(mx.NullCalendar()), 'NULL']: return mx.NullCalendar()
    elif arg_s in [str(mx.Japan()), 'JP']: return mx.NullCalendar()
...
def toCurrencyCls(arg) -> mx.Currency:
    ...
    if arg_s in [str(mx.KRWCurrency()), 'KRW']: return mx.KRWCurrency()
    elif arg_s in [str(mx.USDCurrency()), 'USD']: return mx.USDCurrency()
    elif arg_s in [str(mx.JPYCurrency()), 'JPY']: return mx.JPYCurrency()
...
def currencyToString(arg) -> str:
    ...
    if arg_s in [str(mx.KRWCurrency()), 'KRW']: return str(mx.KRWCurrency())
    elif arg_s in [str(mx.USDCurrency()), 'USD']: return str(mx.USDCurrency())
    elif arg_s in [str(mx.JPYCurrency()), 'JPY']: return str(mx.JPYCurrency())
...
def toDayCounterCls(arg) -> mx.DayCounter:
    ...
    if arg_s in [str(mx.Actual360()), 'Act360']: return mx.Actual360()
    elif arg_s in [str(mx.Actual365Fixed()), 'Act365Fixed']: return mx.Actual365Fixed()
    # elif arg_s in [str(mx.Actual365NoLeap()), 'Act365NoLeap']: return mx.Actual365NoLeap()
    elif arg_s in [str(mx.ActualActual()), 'ActAct']: return mx.ActualActual()
...
def toBusinessDayConvention(arg):
    ...
    if arg_s in [str(mx.Following),  'Following'] : return mx.Following
    elif arg_s in [str(mx.ModifiedFollowing),  'ModifiedFollowing'] : return mx.ModifiedFollowing
    elif arg_s in [str(mx.Preceding),  'Preceding'] : return mx.Preceding
    elif arg_s in [str(mx.ModifiedPreceding),  'ModifiedPreceding'] : return mx.ModifiedPreceding
    elif arg_s in [str(mx.Unadjusted),  'Unadjusted'] : return mx.Unadjusted
    elif arg_s in [str(mx.HalfMonthModifiedFollowing),  'HalfMonthModifiedFollowing'] : return mx.HalfMonthModifiedFollowing
    elif arg_s in [str(mx.JoinHolidays),  'JoinHolidays'] : return mx.JoinHolidays
    elif arg_s in [str(mx.JoinBusinessDays),  'JoinBusinessDays'] : return mx.JoinBusinessDays
...
def businessDayConventionToString(arg) -> str:
    ...
    if arg_s in [str(mx.Following),  'Following'] : return 'Following'
    elif arg_s in [str(mx.ModifiedFollowing),  'ModifiedFollowing'] : return 'ModifiedFollowing'
    elif arg_s in [str(mx.Preceding),  'Preceding'] : return 'Preceding'
    elif arg_s in [str(mx.ModifiedPreceding),  'ModifiedPreceding'] : return 'ModifiedPreceding'
    elif arg_s in [str(mx.Unadjusted),  'Unadjusted'] : return 'Unadjusted'
    elif arg_s in [str(mx.HalfMonthModifiedFollowing),  'HalfMonthModifiedFollowing'] : return 'HalfMonthModifiedFollowing'
    elif arg_s in [str(mx.JoinHolidays),  'JoinHolidays'] : return 'JoinHolidays'
    elif arg_s in [str(mx.JoinBusinessDays),  'JoinBusinessDays'] : return 'JoinBusinessDays'
...
class NameHelper:
    ...
    def getTypeFrom(name: str):
        ...
        if NameHelper.isCompounding(name):
            ...
            res = 'dayCounter'
        elif NameHelper.isCurrency(name):
            res = 'currency'
    ...
    def toDictArg(name: str, value):
        ...
        if NameHelper.isCompounding(name):
            ...
            res = businessDayConventionToString(value)
        elif NameHelper.isInterpolationType(name):
            res = interpolator1DToString(value)
        elif NameHelper.isExtrapolationType(name):
            res = extrapolator1DToString(value)
        elif NameHelper.isTenor(name):
            res = str(value) if not isinstance(value, numbers.Number) else value
        elif NameHelper.isDate(name):
            res = str(value)
        elif NameHelper.isDates(name):
    ...
    def toClassArg(name: str, value):
        ...
        if NameHelper.isDate(name):
            ...
            arg = toDateClsList(value)
        elif NameHelper.isCompounding(name):
            arg = toCompounding(value)
        elif NameHelper.isCalendar(name):
            arg = toCalendarCls(value)
        elif NameHelper.isCurrency(name):
            arg = toCurrencyCls(value)
        elif NameHelper.isDayCounter(name):
            arg = toDayCounterCls(value)
        elif NameHelper.isBusinessDayConvention(name):
            arg = toBusinessDayConvention(value)
        elif NameHelper.isInterpolationType(name):
            arg = toInterpolator1DType(value) if isinstance(value, str) else value
        elif NameHelper.isExtrapolationType(name):
            arg = toExtrapolator1DType(value) if isinstance(value, str) else value
        elif NameHelper.isTenor(name):
            arg = toPeriodCls(value) if not isinstance(value, numbers.Number) else value
        elif isinstance(value, (numbers.Number, str, list)):
            arg = value
