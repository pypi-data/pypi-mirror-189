...
def apexLog_record_to_string(logRecord):
    ...
    logLine = f"""{utils.CFAINT}LOGDATA:    Id: {log['Id']}   LogUserId: {log['LogUserId']} {utils.CGREEN}({username}){utils.CEND}{utils.CFAINT}    Request: {log['Request']}  Operation: {utils.CGREEN}{log['Operation']}{utils.CEND}{utils.CFAINT}    lenght: {log['LogLength']}    duration:  {utils.CGREEN}{log['DurationMilliseconds']} {utils.CFAINT} {utils.CFAINT}
...
def parse_apexlog_body(pc):
    ...
    #    context['lines'] = logData.splitlines()
      ...
      #  parsed_lines =  context['parsedLines']
      ...
      #  context['openItemsList'] = []
        ...
        for num,line in enumerate(context['lines']):
            ...
            #file://Users/uormaechea/Documents/Dev/python/InCliLib/.incli/logs/07L3O00000DhuITUAZ.log
...
def print_parsed_line(pc,d):
    ...
    if 'printing_started' not in pc:
        pc['printing_started'] = True

        print( f"Log time: {pc['context']['parsedLines'][3]['lines'][0].split(' ')[0]}")
    ...
    time_stamp_entry = utils.CFAINT + f"{time_stamp_entry:12}" + utils.CEND
    time_stamp_exit = utils.CFAINT + f"{time_stamp_exit:12}" + utils.CEND

    s = utils.CFAINT + "|" + utils.CEND
    ...
    output_string = utils.CYELLOW+utils.CFAINT+ f"{wait_time:>5}{identation}"+et_color+f"{element_time:>5}"+utils.CDARK_GRAY+f"{soql_total_queries_delta:>4}" +utils.CEND + Cinit +f"    {output_string}"
