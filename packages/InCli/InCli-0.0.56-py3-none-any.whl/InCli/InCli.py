...
def option_readme(args):
    ...
    {utils.CYELLOW}Significant updates:{utils.CEND}
     ...
     - Changes in version 0.49.0
       Added {utils.CYELLOW}-auto{utils.CEND} to the log parsing. This options activates the debug logs automatically for the -loguser especified. If no -loguser is specified it will do it for the user used to authenticate with the org.  
            {utils.CGREEN}InCli -logs -u DTI -tail -loguser username:onboarding@nosdti-parceiros.cs109.force.com -deletelogs -auto.{utils.CEND}
            ...
            {utils.CGREEN}InCli -logs -u DTI -tail -deletelogs -auto.{utils.CEND}
    ...
    {utils.CYELLOW}Parsed output fields:{utils.CEND}
     ...
     - {utils.CGREEN}Q{utils.CEND}: Total queries performed by the user APEX program. This field is almost accurate.
     - {utils.CGREEN}Qcm{utils.CEND}: Total queries performed by the managed package. We do not have control when they appear. When it changes is accurate.
     - {utils.CGREEN}Qe{utils.CEND}: Estimate queries performed by the managed package. This is an estimate for the manage package queries. Almost accurate. 
...
def option_logs(args):
    help = f"""
        ...
        {utils.CYELLOW}-logs{utils.CEND} --> lists log records in the org. Default 50 lines. 
    ...
    if logId == None and last== None and inputfile==None and tail == False and store==False:
        connectionInit(args)
