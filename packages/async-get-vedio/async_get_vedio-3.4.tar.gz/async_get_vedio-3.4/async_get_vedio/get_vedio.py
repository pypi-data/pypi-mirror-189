...
class File_create:
    ...
    def create(self):
        os.system(rf'md {self.file_path}\{self.video_name}\ts')
        os.system(rf'md {self.file_path}\{self.video_name}\decode_ts')
...
class Async:
    ...
    def __init__(self, m3u8_url, file_path, video_name):
        self.m3u8_url = m3u8_url
        self.file_path = file_path
        self.video_name = video_name

    async def decode_one_ts(self, aes, name):
        async with aiofiles.open(rf'{self.file_path}\{self.video_name}\ts\{name}', 'rb') as fp1:
            data = aes.decrypt(await fp1.read())
        async with aiofiles.open(rf'{self.file_path}\{self.video_name}\decode_ts\{name}', 'wb') as fp2:
            await fp2.write(data)
        print(name + '解密完毕!')

    async def decode_all_ts(self, key):
        aes = AES.new(key=key, IV=b'0000000000000000', mode=AES.MODE_CBC)
        async with aiofiles.open(rf'{self.file_path}\{self.video_name}\ts文件的名称.text', 'r') as fp:
            tasks = []
    ...
    async def one_ts_download(self, session, ts_url, name):
        while True:
            ...
            if judge == 'yes':
                break
        async with aiofiles.open(rf'{self.file_path}\{self.video_name}\ts\{name}', 'wb') as fp:
            await fp.write(data)
        print(name + '下载完毕!')

    async def download_all_ts(self, session, ts_urls):
        async with aiofiles.open(rf'{self.file_path}\{self.video_name}\ts文件的名称.text', 'w') as fp:
            tasks = []
            num = 0
    ...
    async def all_urls_get(self, session):
        ...
        async with aiofiles.open(rf'{self.file_path}\{self.video_name}\{self.video_name}.m3u8', 'wb') as fp:
            await fp.write(m3u8_content)

        url_header = self.m3u8_url.strip('index.m3u8')
        key_url = ''
        ts_urls = []
        async with aiofiles.open(rf'{self.file_path}\{self.video_name}\{self.video_name}.m3u8', 'r') as fp:
    ...
    async def async_main(self):
        ...
        if key_url == '':
            os.system(
                rf'copy /b {self.file_path}\{self.video_name}\ts\*.ts {self.file_path}\{self.video_name}.mp4')
        else:
            try:
            ...
            os.system(
                rf'copy /b {self.file_path}\{self.video_name}\decode_ts\*.ts {self.file_path}\{self.video_name}.mp4')
