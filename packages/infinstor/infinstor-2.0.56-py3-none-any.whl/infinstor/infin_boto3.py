...
class InfinBotoClient():
    ...
    def get_version_id(self, bucket, prefix, caller_name):
        attempt = 0
        while attempt < 2:
            if attempt == 0:
                force = False
...
def get_customerinfo() -> Tuple[List[str], bool, bool]:
    ...
    if 'customer_info' in user_info_cache:
        return user_info_cache['customer_info']
    ...
    while num_attempts > 0:
        ...
        try:
            ...
            user_info_cache['customer_info'] = rv, use_presigned_url_for_infinsnap, use_presigned_url_for_mlflow
...
def call_requests_get(url, headers=None, retry=False):
    ...
    try:
        ...
        print('Exception {ex} in request ' + str(ex) + ", for url: " + url)
        raise ex
