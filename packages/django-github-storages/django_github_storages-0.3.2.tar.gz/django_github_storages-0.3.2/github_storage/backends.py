...
class GitHubStorage(Storage):
    ...
    def get_url(self, name, path):
        """
        Helper function to construct the url of the file.
        """
        url = (
            f"https://api.github.com/repos/{self.username}/{self.target_repo}/contents/"
        )
        url += str(name) if self.media_root is None else f"{self.media_root}/"
        if len(path) != 0:
    ...
    def exists(self, name, path):
        ...
        """
        fetch_url = self.get_url(name, path)
    ...
    def upload_file_to_git(self, name, path, content):
        ...
        """
        upload_url = self.get_url(name, path)
    ...
    def delete(self, name):
        ...
        delete_url = self.get_url(name, path)
        headers = {"Authorization": f"token {self.token}"}
        payload = {
    ...
    def size(self, name):
        ...
        path = path_with_file_name
        get_size_url = self.get_url(name, path)
        headers = {"Authorization": f"token {self.token}"}
