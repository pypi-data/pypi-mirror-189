...
class AmbienteDeAprendizagem:
    def __init__(self, nome_sala, nome_sala_abrev, capacidade_sala, status, disciplina, tipo, escola):
...
def create_objects(file_name, dict_retorna_status_type, list):
...
def insert_excel(nome_arquivo, dict_retorna_sugestao, dict_retorna_sugestao_list, dict_retorna_sugestao_subutilizada):
    ...
    worksheet.add_table(0, 0, row - 1, 16, {'style': 'Table Style Light 1', 'columns': [
        {'header': 'BLOCO'},
        {'header': 'ADJETIVO DO BLOCO'},
        {'header': 'ANDAR'},
        ...
        {'header': 'OBSERVAÇÃO'},
        {'header': 'OBSERVAÇÃO 2'},
        {'header': 'HÁ POSSIBILIDADE DE ENSALAR?'},
        {'header': 'TIPO DE INSTALAÇÃO'},
        {'header': 'METODOGIA ATIVA?'},
        {'header': 'AMBIENTE UNIFICADO?'},
        {'header': 'SALA INTERNA?'},
        {'header': 'CAPACIDADE'},
        {'header': 'ÁREA TOTAL (m²)'},
        {'header': 'Relação m² / Aluno'},
    ]})
...
def create_dict_status_type(arquivo_capacidade):
    dict_retona_status = {}
    ...
    for data in df.iterrows():
        ...
        dict_retona_status[nome_sala] = status, tipo

    return dict_retona_status
...
def analyze_matriz(matriz, df_capacidade_excedida, dict_retorna_status_type, dict_retorna_prioridade_escola):
    ...
    for data in df_capacidade_excedida.iterrows():
        ...
        bloco = classroom_name = classroom_name.split("-")[0]
        ...
        for numero_bloco in list_blocos:
            ...
            # todo 2 verificação
            # verificando bloco prioridade e tipo
            for classroom in list_empty_classroom:
        ...
        # todo 4 verificação
        # sem verificar o bloco e o tipo
        for classroom in list_empty_classroom:
...
def analyze_matriz_list(matriz, df_capacidade_excedida, dict_retorna_status_type, dict_retorna_prioridade_escola):
    ...
    for data in df_capacidade_excedida.iterrows():
        ...
        # todo 1 verificação
        # utilizando o bloco para validar e o tipo
        for classroom in list_empty_classroom:
        ...
        # todo 2 verificação
        # utilizando o bloco prioridade da escola para validar e o tipo
        for numero_bloco in list_blocos:
            ...
            # todo 2 verificação
            # utilizando o bloco prioridade para validar e o tipo
            for classroom in list_empty_classroom:
        ...
        # todo 4 verificação
        # sem utilizar o bloco e sem utilizar o tipo
        for classroom in list_empty_classroom:
...
def analyze_matriz_subutilizada(matriz, df_capacidade_excedida, dict_retorna_status_type,
    ...
    for data in df_capacidade_excedida.iterrows():
        ...
        #todo 2 verificação
        #pega prioridade de blocos por escola
        for numero_bloco in list_blocos:
...
def create_dict_prioridade_escola():
    df_prioridades = pandas.read_excel('./Arquivos_Auxiliares/Prioridade_escolas.xlsx')
    dict_retorna_prioridade_escola = {}
...
def main(nome_arquivo, arquivo_capacidade):
    ...
    dict_retorna_status_type = create_dict_status_type(arquivo_capacidade)

    # dict para retornar os blocos prioritarios de cada escola
    dict_retorna_prioridade_escola = create_dict_prioridade_escola()
    ...
    print('criando sugestões de subutilizadas...')
