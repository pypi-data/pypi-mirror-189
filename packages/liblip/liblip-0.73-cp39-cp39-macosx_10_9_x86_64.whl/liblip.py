...
def convert_py_float_to_cffi( x):
    if isinstance( x, np.ndarray) or x != None:
        if not isinstance( x, np.ndarray) or ( x.flags.c_contiguous == False) or ( x.dtype != np.float64):
...
def convert_py_int_to_cffi( x):
    if isinstance( x, np.ndarray) or x != None:
        if not isinstance( x, np.ndarray) or ( x.flags.c_contiguous == False) or ( x.dtype != np.int32):
