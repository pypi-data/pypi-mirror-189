...
import threewfund.datatool as tfdt


def find_income(company_id, n, google_driver_path, com_revenue_monthly_path):
    ...
    if tr_tag == []:
        ...
        ws = wb.active
        ws._current_row = n
        ws.append({"I": new_income[0]})
        ws._current_row = n
        ws.append({"H": new_date[0]})
...
def twweb_to_excel(num,com_revenue_monthly_path):
    ...
    for i in range(num,179):
        tfdt.find_income(v[i][:4],i,google_driver_path,com_revenue_monthly_path)
        print(i)