...
import json, traceback,sys, time,  fileinput, os,en, sqlite3,re
from collections import Counter
from pathlib import Path

doctoks = lambda doc: ['<s>'] + [ f"_{t.pos_}" if t.pos_ in ('PROPN','NUM','CD','X','SPACE') else f"_{t.ent_type_}" if t.ent_type_ in ('NP','CL') else t.text.lower()  for t in doc ]
...
def run(infile, outfile:str=None,  n:int=7, batch:int=3000, remove:bool=False):
	''' c4-train.00604-of-01024.docjsonlg.3.4.1.gz -> c4-train.sqlsi | 2023.1.22 '''
	if outfile is None: outfile = infile.strip('.').split('.docjson')[0].strip('/') + ".gramsi" 
	...
	for sid, line in enumerate(fileinput.input(infile,openhook=fileinput.hook_compressed)): #356317 docs of each doc file
		try:
			...
			for sp in tdoc.sents:
				...
				doc = merge_leave_cl(doc)
				en.merge_np(doc) 
				count( doctoks(doc), ts, n) 
				count( lemtoks(doc), ts, n) 
				count( postoks(doc), ts, n) 
				...
				for s in ts:  
					...
					#s1 = re.sub(rf"\b(are|is|were|was|am|'m|'re)\b", r'_be', s, flags=re.IGNORECASE) # added 2023.1.24
					#if s1 != s : conn.execute(f"INSERT INTO si(s,i) VALUES(?,?) ON CONFLICT(s) DO UPDATE SET i = i + 1", (s1,1))
			if (sid+1) % batch == 0 : 
	...
	#conn.execute(f"INSERT or ignore INTO si(s,i) VALUES(?,?)", (infile,time.time() - start))
	conn.commit()
