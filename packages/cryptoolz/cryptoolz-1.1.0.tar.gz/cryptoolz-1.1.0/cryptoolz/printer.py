...
def _write(data: Any, *args, **kwargs) -> None:
...
class QRCodeWriter(Base64Writer):
    def write(self, data: QRCode, **kwargs) -> None:
        _write_qr(data, **kwargs)

    def encode(self, data: bytes, **kwargs) -> QRCode:
        return make_qr(super().encode(data), error=kwargs.get("error") or "H", **kwargs)
