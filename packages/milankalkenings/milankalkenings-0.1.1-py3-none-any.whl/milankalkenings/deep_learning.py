...
class Trainer:
    ...
    def debug_lr(self, n_iters: int, batch_debug: list, lr_candidates: list = [5e-1, 1e-1, 5e-3, 1e-3, 5e-4, 1e-4, 5e-5, 1e-5, 5e-6, 1e-6], fig_name: str = "lr_debug"):
        """
        ...
        for i, cand in enumerate(lr_candidates):
            mimo = torch.load(self.checkpoint_initial)
