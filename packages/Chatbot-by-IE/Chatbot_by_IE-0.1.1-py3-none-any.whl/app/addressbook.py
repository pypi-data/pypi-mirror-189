...
class Phone(Filed):
    ...
    def value(self, new_value):
        ...
        if new_value.isdigit():
            ...
            print(f"{chr(9940)} Entered wrong phone: {new_value}, correct phone: 0674523698")
...
class Birthday(Filed):
    ...
    def value(self, birthday):
        try:
            ...
            print(f"{chr(9940)} ValueError! Enter correct date %dd.%mm.%yyyy")
...
class Email(Filed):
    ...
    def value(self, email):
        ...
        if new_email:
            ...
            print(f"{chr(9940)} You entered wrong email! >> {new_email}")
...
def add_phone(*args, **kwargs: AddressBook):
    ...
    ab.add_record(rec)
    return f"{chr(59259)} {name} was created. {chr(9742)} phone: {phone}"
...
def change(*args, **kwargs: AddressBook):
    ...
    if rec:
        ...
        ab.add_record(rec)
        return f"{chr(59259)} {name} was change -> old phone: {phone_old} new phone: {phone_new}"
    else:
        return f"{chr(10006)} Name {name} isn't in the AddressBook"
...
def phone(*args, **kwargs: AddressBook):
    ...
    if rec:
        for value in ab.values():
            return f'{chr(59259)} {name}: {chr(9742)} phone:{value.phones}'
...
def delete(*args, **kwargs: AddressBook):
    ...
    if rec:
        ab.pop(name.value)
        return f"{chr(59259)} {name} deleted {chr(9760)} "
    return f"{chr(10006)} Name {name} isn't in the AddressBook"
...
def add_birthday(*args, **kwargs: AddressBook):
    ...
    if rec:
        ...
        ab.add_record(rec)
        return f"{chr(59259)} {name} was add {chr(9787)} birthday: {bir_day}"
    return f"{chr(10006)} Name {name} isn't in the AddressBook"
...
def add_email(*args, **kwargs: AddressBook):
    ...
    if rec:
        ...
        ab.add_record(rec)
        return f"{chr(59259)} {name} was add {chr(9993)} email: {email}"
    return f"{chr(10006)} Name {name} isn't in the AddressBook"
...
def add_address(*args, **kwargs: AddressBook):
    ...
    if rec:
        ...
        ab.add_record(rec)
        return f"{chr(59259)} {name} was add {chr(60042)} address: {address}"
    return f"{chr(10006)} Name {chr(59259)} {name} isn't in the AddressBook"
...
def next_birthdays(*args, **kwargs: AddressBook):
    ...
    if not bd_list:
        return f"{chr(10006)} No birthdays for the next {days} days"
    for contact in bd_list:
        print(f"{(chr(10027) + ' ') * 60}\n{contact}")
    return f"{(chr(10027) + ' ') * 60}\nTo congratulate on his birthday!!!"
