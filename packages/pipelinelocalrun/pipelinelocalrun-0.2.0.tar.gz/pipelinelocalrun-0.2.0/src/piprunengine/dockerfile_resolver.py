...
from azure.ai.ml._local_endpoints.dockerfile_instructions import From, Run, Workdir, Copy, Expose, Cmd, Env
from .constants import CONDA_ENV_NAME


module_logger = logging.getLogger(__name__)
AZUREML_CONDA_ENVIRONMENT_PATH = "AZUREML_CONDA_ENVIRONMENT_PATH"
...
class DockerfileResolver(object):
    ...
    def __init__(
        ...
        docker_azureml_env_path: str = None,
    ):
        ...
        :type docker_conda_file_name: str
        :param docker_azureml_env_path: name of local deployment
        :type docker_azureml_env_path: (str, optional)
        ...
        self.docker_azureml_env_path = docker_azureml_env_path
        self._construct()
    ...
    def _construct(self, install_debugpy: bool = False) -> None:
        ...
        if self._docker_base_image:
            ...
            self._instructions.extend([Run(f"mkdir -p {SETUP_DIRECTORY_NAME}")])
        else:
            self._instructions = [self._dockerfile]
        
        if self._docker_conda_file_name:
            base_name = os.path.basename(self._docker_conda_file_name)
            conda_file_path = f"${SETUP_DIRECTORY_NAME}/{base_name}"
            self._instructions.extend(
                [
                    ...
                    Run(
                        f"conda env create -p {self.docker_azureml_env_path} -f {conda_file_path}"
                    ),
                ]
            )
            if install_debugpy:
                self._instructions.extend(
                    [Run(f"conda run -n {CONDA_ENV_NAME} pip install debugpy")]
                )
            self._instructions.extend(
                [
                    Env(AZUREML_CONDA_ENVIRONMENT_PATH, self.docker_azureml_env_path),
                    Env("PATH", f"${AZUREML_CONDA_ENVIRONMENT_PATH}/bin:$PATH"),
                    Env("LD_LIBRARY_PATH", f"${AZUREML_CONDA_ENVIRONMENT_PATH}/lib:$LD_LIBRARY_PATH"),
                ]
            )
        else:
            if install_debugpy:
