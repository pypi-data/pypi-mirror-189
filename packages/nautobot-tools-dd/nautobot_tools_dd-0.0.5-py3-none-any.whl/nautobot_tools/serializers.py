...
from rest_framework.serializers import raise_errors_on_nested_writes, Serializer, ALL_FIELDS
from yangson import datatype, DataModel
from yangson.datatype import LinkType, LeafrefType
from yangson.schemanode import SchemaNode, LeafListNode

from nautobot_tools import model_meta
from nautobot_tools.field_mapping import ClassLookupDict, get_field_kwargs, get_nested_relation_kwargs, \
    to_internal_value_nos
...
class YANGModelSerializer(Serializer):
    ...
    * Default `.create()` and `.update()` implementations are provided.
    ...
    def get_fields(self):
        ...
        if not isinstance(self.Meta.model, (SchemaNode,)):
            raise ValueError(
    ...
    def build_standard_field(self, field_name, model_field):
        ...
        if isinstance(model_field.type, LeafrefType) \
                and model_field.type.path.as_instance_route()[0].namespace == 'dd-nos':
            field_class = field_mapping[model_field.type.ref_type]

            class NOSField(field_class):
                def to_internal_value(self, data):
                    return to_internal_value_nos(model_field, data)
        ...
        if not issubclass(field_class, datatype.DataType):
