...
class XGBModel(Model, FeatureInt):
    ...
    def fit(
        ...
        **kwargs
        ...
        df_train, df_valid = dataset.prepare(
            ["train", "valid"],
            col_set=dataset.feature_columns + [dataset.label_column],
        )
        x_train, y_train = df_train[dataset.feature_columns], df_train[dataset.label_column]
        x_valid, y_valid = df_valid[dataset.feature_columns], df_valid[dataset.label_column]
    ...
    def predict(self, dataset: DatasetH, split: str = "test"):
        if self.model is None:
            raise ValueError("model is not fitted yet!")
        x_test = dataset.prepare(split, col_set=dataset.feature_columns)
