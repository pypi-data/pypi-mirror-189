...
class MLKitsLogger(metaclass=MetaLogger):
    """
    Customized logger for MLKits.
    """
...
class _MLKitsLoggerManager:
    ...
    def __call__(self, module_name, level: Optional[int] = None) -> MLKitsLogger:
        """
        ...
        if not module_name.startswith("mlkits."):
            # Add a prefix of mlkits. when the requested ``module_name`` doesn't start with ``mlkits.``.
            # If the module_name is already mlkits.xxx, we do not format here. Otherwise, it will become mlkits.mlkits.xxx.
            module_name = "mlkits.{}".format(module_name)

        # Get logger.
        module_logger = self._loggers.setdefault(module_name, MLKitsLogger(module_name))
        module_logger.setLevel(level)
        return module_logger


get_module_logger = _MLKitsLoggerManager()
...
def set_global_logger_level(level: int, return_orig_handler_level: bool = False):
    """set mlkits.xxx logger handlers level
    ...
    ---------
        ...
        .. code-block:: python
            ...
            import mlkits
            import logging
            from mlkits.log import get_module_logger, set_global_logger_level
            mlkits.init()
    ...
    """
    _handler_level_map = {}
    mlkits_logger = logging.root.manager.loggerDict.get("mlkits", None)  # pylint: disable=E1101
    if mlkits_logger is not None:
        for _handler in mlkits_logger.handlers:
...
def set_global_logger_level_cm(level: int):
    """set mlkits.xxx logger handlers level to use contextmanager
    ...
    ---------
        ...
        .. code-block:: python
            ...
            import mlkits
            import logging
            from mlkits.log import get_module_logger, set_global_logger_level_cm
            mlkits.init()
