...
__all__ = ["sort_excel_file", "generate_template", "update_jira_access_token"]
...
def get_args_for_sort_excel_file() -> Namespace:
    ...
    parser.add_argument(
        "--jira_configuration_file",
        metavar="",
        type=pathlib.Path,
        required=False,
        help="JIRA configuration file including status information and something else. File format: JSON.",
    )
    parser.add_argument(
        "--over_write",
        metavar="",
...
def sort_excel_file() -> None:
    try:
        ...
        jira_configuration_file_absolute_path = None

        if args.jira_configuration_file is not None:
            jira_configuration_file_absolute_path = pathlib.Path(
                pathlib.Path.cwd() / args.sprint_schedule_file.as_posix()
            ).resolve()

            if jira_configuration_file_absolute_path.suffix.lower() != ".json":
                print(
                    f"Please provide an JSON file for sprint schedule. File: {jira_configuration_file_absolute_path}."
                )
                quit(1)
        ...
        process_excel_file(
            ...
            jira_configuration_file_absolute_path,
            over_write,
        )
...
def get_args_for_generate_template() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        help="What kind of file template you want to generate. Choices: excel, excel-definition, sprint-schedule or jira-configuration.",
        choices=["excel", "excel-definition", "sprint-schedule", "jira-configuration"],
    )
...
def generate_template():
    try:
        ...
        if template_type == "excel":
            ...
            result = copyfile(
                SRC_ASSETS / "jira_configuration.json",
                _generate_timestamp_filename("jira-configuration-template", ".json"),
            )
...
def get_args_for_update_jira_access_token() -> Namespace:
    parser = ArgumentParser(
        description="Jira tool: Used to add/update jira access token",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
...
def update_jira_access_token():
    try:
        args = get_args_for_update_jira_access_token()

        access_token: str = str(args.access_token)

        if len(access_token) == 0 or access_token.isspace():
            print("Please provide invalid access token.")
            quit(1)
