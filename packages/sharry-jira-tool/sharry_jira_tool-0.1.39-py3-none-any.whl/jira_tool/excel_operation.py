...
def process_excel_file(
    ...
    excel_definition_config: "str | Path | None" = None,
    sprint_schedule_config: "str | Path | None" = None,
    over_write: bool = True,
):
    """
    ...
    if jira_acccess_token is None:
        print(
            "The jira access token is invalid. Please use the update-jira-access-token command to add/update token."
        )
        return

    jira_client = JiraClient(jira_acccess_token)

    if not jira_client.health_check():
        print(
            "The jira access token is revoked. Please use the update-jira-access-token command to add/update token."
        )
        return
    ...
    jira_query_result = jira_client.get_stories_status(
        [s["storyId"].strip() for s in stories]
    )
    ...
    for story in stories:
        if story["storyId"].lower() in jira_query_result:
            story["status"] = jira_query_result[story["storyId"].lower()]
    ...
    for sort_strategy in sort_strategies:
        ...
        if sort_strategy["name"].lower() in "InlineWeights".lower():
            ...
            sort_stories_by_property_and_order(stories_need_sort, excel_definition)
