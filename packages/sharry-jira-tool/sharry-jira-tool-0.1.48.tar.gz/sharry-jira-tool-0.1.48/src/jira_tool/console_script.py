...
def sort_excel_file() -> None:
    try:
        ...
        jira_status_mapping_file_absolute_path = None

        if args.jira_status_mapping is not None:
            jira_status_mapping_file_absolute_path = pathlib.Path(
                pathlib.Path.cwd() / args.sprint_schedule_file.as_posix()
            ).resolve()

            if jira_status_mapping_file_absolute_path.suffix.lower() != ".json":
                print(
                    f"Please provide an JSON file for sprint schedule. File: {jira_status_mapping_file_absolute_path}."
                )
                quit(1)
        ...
        process_excel_file(
            ...
            jira_status_mapping_file_absolute_path,
            over_write,
        )
...
def get_args_for_generate_template() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        help="What kind of file template you want to generate. Choices: excel, excel-definition, sprint-schedule or jira-status-mapping.",
        choices=["excel", "excel-definition", "sprint-schedule", "jira-status-mapping"],
    )
    ...
    try:
        ...
        if template_type == "excel":
            ...
            result = copyfile(
                ...
                _generate_timestamp_filename("sprint_schedule-template", ".json"),
            )
        else:
            result = copyfile(
                SRC_ASSETS / "jira_status_mapping.json",
                _generate_timestamp_filename("jira_status_mapping-template", ".json"),
            )
