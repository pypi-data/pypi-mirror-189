...
import xarray as xr
import pandas as pd
from nptyping import NDArray, Structure, Shape
...
class ROI(BaseModel):
    ...
    def vector_data(self) -> np.ndarray:
        ...
        """
        return self.get_vector_data(dims="xy")

    def get_vector_data(self, dims="xy") -> np.ndarray:
    ...
    def center(self) -> "InputVector":
        ...
        if self.type == RoiTypeInput.RECTANGLE:
            return InputVector.from_array(self.get_vector_data(dims="ctzxy").mean(axis=0))

        raise NotImplementedError(f"Center calculation not implemented for this ROI type {self.type}")

    def center_as_array(self) -> NDArray:
        """The center of the ROI
        ...
        assert hasattr(self, "type"), "Please query 'type' in your request on 'ROI'. Center is not accessible otherwise"
        if self.type == RoiTypeInput.RECTANGLE:
            return self.get_vector_data(dims="ctzxy").mean(axis=0)
