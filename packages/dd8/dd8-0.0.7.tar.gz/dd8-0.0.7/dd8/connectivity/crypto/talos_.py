...
class TalosRest(AsyncConnection):
    ...
    def get_orders(self, order_id: str = '', client_order_id: str = '', 
        ...
        initial_responses = [resp['data'] for resp in self.get(path, query)]
        paginations = [data['after'] for data in initial_responses if 'after' in data]
        while paginations:
            pages = []
            for page in paginations:
                ...
                pages.append(query)
            responses = [resp['data'] for resp in self.get(path, pages)]
            initial_responses += responses
            paginations = [data['after'] for data in responses if 'after' in data]
