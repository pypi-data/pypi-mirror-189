...
from blazetest.core.exceptions import S3BucketNotFound, LambdaNotCreated
...
class Lambda:
    """
    Lambda Function class, which is used to interact with AWS Lambda
    ...
    def __init__(self, region: str = None, stack_name: str = None):
        self.region = region
    ...
    def invoke(
        ...
        return bool(result_body["test_result"])
    ...
    def get_created_lambda_function_details(self) -> Dict[str, str]:
        ...
        function = next(
            (
                f
                for f in functions
                if f["FunctionName"].startswith(f"{self.stack_name}-ServiceTests")
            ),
            None,
        )
