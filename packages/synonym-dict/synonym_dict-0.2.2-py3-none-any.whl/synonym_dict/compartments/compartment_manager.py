...
class CompartmentManager(SynonymDict):
    ...
    def _merge(self, existing_entry, ent):
        ...
        print('$$$*** None-ing parent for %s' % ent)
        ent.parent = None
        for sub in list(ent.subcompartments):
            print('$$$*** Re-parenting %s to %s' % (sub, existing_entry))
    ...
    def _check_subcompartment_lineage(self, current, c):
        ...
        if c in self._d:
            ...
            if new.parent is None:
                print('$$$*** Installing %s as parent to %s' % (current.as_list(), new.as_list()))
                new.parent = current
                return new
        ...
        for c in comps:
            try:
                ...
                if conflict == 'match':
                    ...
                    new = self.get(c)
                    print('$$$*** Attaching %s to new parent %s' % (new.top(), current))
