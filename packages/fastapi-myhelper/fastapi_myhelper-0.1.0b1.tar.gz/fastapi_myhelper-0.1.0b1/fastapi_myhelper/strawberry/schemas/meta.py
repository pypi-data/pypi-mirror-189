    ...
    def make_optional(
        ...
        for field_name in annotations:
            ...
            if getattr(annotations, field_name, None) == strawberry.auto:
                continue
            if isinstance(fields_to_proccess.get(field_name, None), StrawberryField):
                fields_to_proccess[field_name].default = strawberry.UNSET
                if not isinstance(fields_to_proccess[field_name].type, StrawberryOptional):
                    fields_to_proccess[field_name].type = fields_to_proccess[field_name].type | None
                kls_fields[field_name] = fields_to_proccess[field_name]
            else:
