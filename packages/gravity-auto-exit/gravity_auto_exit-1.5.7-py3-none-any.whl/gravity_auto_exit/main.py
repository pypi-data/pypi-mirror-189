        ...
        if datetime.datetime.now() - self.last_take > datetime.timedelta(
            ...
            for i in range(5):
                ...
                if 'error' in response:
                    logger.error(f"Error {response['error']}, {response['info']}")
                else:
                    result, photo = response
                    if result:
                        self.last_take = datetime.datetime.now()
                        logger.info(f"Success recognise. Number {result}")
                        return result
                if self.failed_callback and i == 2:
                    logger.debug(f'Failed callbakc working photo...')
