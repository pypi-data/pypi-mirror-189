...
class COINNLearner:
    ...
    def step(self) -> dict:
        out = {}
        grads_mask = _tu.load_arrays_learner(self.state['baseDirectory'] + _sep + self.input['avg_grads_file'])
        #grad_index = 0
        first_model = list(self.trainer.nn.keys())[0]
        #self.trainer.register_pre_hook_mask(grads_mask)
        self.trainer.nn[first_model]= self.trainer.apply_mask_to_model(self.trainer.nn[first_model], grads_mask)
