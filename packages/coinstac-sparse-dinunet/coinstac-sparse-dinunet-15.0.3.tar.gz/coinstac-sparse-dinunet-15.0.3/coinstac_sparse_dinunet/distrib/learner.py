...
class COINNLearner:
    ...
    def step(self, applyMasksToModel=False) -> dict:
        out = {}
        grads = _tu.load_arrays_learner(self.state['baseDirectory'] + _sep + self.input['avg_grads_file'])
        grad_index = 0
        first_model = list(self.trainer.nn.keys())[0]
        #self.trainer.register_pre_hook_mask(grads_mask)
        if applyMasksToModel and self.cache["sparse_training"]:
            # with open(self.state['outputDirectory'] + _os.sep + self.cache['task_id'] + '/inspect_logs.json', 'a+') as file:
            #     file.write("\n Inside sparse training to register masks. The first mask is "+ str(grads[0]))
            #self.trainer.nn[first_model]= self.trainer.apply_mask_to_model(self.trainer.nn[first_model], grads)
            self.trainer.register_pre_hook_mask(grads)
            self.trainer.nn[first_model]= self.trainer.apply_mask_to_model(self.trainer.nn[first_model], grads)
        else:
            # with open(self.state['outputDirectory'] + _os.sep + self.cache['task_id'] + '/inspect_logs.json', 'a+') as file:
            #     file.write("\n Inside to aggregate gradients. The first grad is "+ str(grads[0]))
    ...
    def to_reduce(self, transferGradForMasks=False):
