...
class COINNLocal:
    ...
    def compute(self, mp_pool,
        ...
        #self.mylogger = open(self.cache['logger_directory']+"/log_masks.json", 'a+')
        
        #Remove all time logs
        # open(self.state['outputDirectory'] + _os.sep + self.cache['task_id'] + '/time_logs.json', 'w+').close()
        # open(self.state['outputDirectory'] + _os.sep + self.cache['task_id'] + '/epoch_logs.json', 'w+').close()
        # #open(self.cache['logger_directory'] + '/epoch_logs.json', 'w').close()
        # open(self.state['outputDirectory'] + _os.sep + self.cache['task_id'] + '/timelogsremote.json', 'w+').close()
        ...
        if self.out['phase'] == Phase.INIT_RUNS:
            self.cache["iteration_count"]=0
            #self.mylogger.write("\n ******************************************************************")
            #self.mylogger.write("\n The iteration count is " + str(self.cache["iteration_count"]))
        ...
        if self.out['phase'] == Phase.COMPUTATION:
            ...
            # first_optim = list(learner.trainer.optimizer.keys())[0]
            # num_epochs = self.cache['epochs']
            # scheduler = lr_scheduler.MultiStepLR(learner.trainer.optimizer[first_optim], [0.5*num_epochs, 0.75*num_epochs], gamma=0.1)

            #self.cache["iteration_count"]= self.cache["iteration_count"] + 1 
            

            """Initialize Learner and assign trainer"""

            if self.input.get('update'):
                #self.mylogger.write("\n The iteration count inside update is " + str(self.cache["iteration_count"]))
                if self.cache["iteration_count"]==1 and self.cache["sparse_training"]==True :
                   #self.mylogger.write("\n Registering masks ")
                   self.out.update(**learner.step(True)) 
                else:
                    #self.mylogger.write("\n Updating weights in STEP")
            ...
            if any(m == Mode.TRAIN for m in learner.global_modes.values()):
                ...
                """
                #self.mylogger.write("\n Inside TRAIN")
                if self.cache["iteration_count"]==0 and self.cache["sparse_training"]==True:
                    #self.mylogger.write("\n Learner - Creating sailency scores and transferring masks ")
                    it, out = learner.to_reduce(True)
                    iteration_count= int(self.cache["iteration_count"]) + 1 
                    self.cache.update({"iteration_count": iteration_count})
                    self.out.update(**out)
                else:
                    #self.mylogger.write("\n Learner - Recalculating gradients ")
            ...
            epochelapsed= epochelapsed + (end - start)
            epochlogsdict["epoch_time"] = epochelapsed
            _utils.save_epoch_logs(epochlogsdict,self.cache['log_dir'])
            with open(self.cache['logger_directory'] + '/epoch_logs.json', 'a+') as file:
                file.write("\n" + str(epochlogsdict))
            #self.epochslogger.write("\n" + str(epochlogsdict))

        elif self.out['phase'] == Phase.SUCCESS:
            """ This phase receives global scores from the aggregator."""
            #_shutil.copy(self.state['transferDirectory']+ _os.sep+"timelogsremotenew.json",self.state['outputDirectory'] + _os.sep + self.cache['task_id'] +_os.sep+ "timelogsremoteout.json")
            #_shutil.copy(self.state['baseDirectory'] + _os.sep  + 'timelogsremotebase.json',self.state['transferDirectory'] + _os.sep + self.cache['task_id'] +_os.sep+ "timelogsremoteout.json")
