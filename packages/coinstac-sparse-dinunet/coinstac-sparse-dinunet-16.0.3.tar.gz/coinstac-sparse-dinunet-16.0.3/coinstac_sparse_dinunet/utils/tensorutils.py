...
def save_arrays_reducer(file_path, arrays):
    _np.save(file_path, arrays)

def save_arrays(file_path, arrays):
    #_np.save(file_path, arrays)
    _torch.save( arrays, file_path)


def load_arrays(file_path):
    #return _np.load(file_path, allow_pickle=True)
    #return _np.array(_torch.load(file_path).to_dense())
    reconstructedDense = []
    for ind_grads in _torch.load(file_path):
        reconstructedDense.append(ind_grads.to_dense().numpy())
    return reconstructedDense


def load_masks(file_path):
    return _torch.load(file_path)

def load_arrays_learner(file_path):
    return _np.load(file_path, allow_pickle=True)
