...
class COINNLocal:
    ...
    def _next_run(self, trainer, dataset_cls=None):
        ...
        if self.cache["sparse_training"]:
            ...
            #_tu.save_arrays(self.cache['log_dir']+"/initial_masks.pt", masks_for_pruning)
    ...
    def compute(self, mp_pool,
        ...
        if self.out['phase'] == Phase.COMPUTATION:
            ...
            if self.input.get('masks_update'):
                if self.cache["sparse_training"]==True:
                    self.out.update(**learner.applyMasksInStep())
            ...
            if any(m == Mode.TRAIN for m in learner.global_modes.values()):
                ...
                if self.cache["iteration_count"]==0 and self.cache["sparse_training"]==True:
                    out = learner.generateGradsForMasks()
