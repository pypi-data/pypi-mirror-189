...
from coinstac_sparse_dinunet.utils import performance_improved_, stop_training_
...
class COINNRemote:
    ...
    def _accumulate_epoch_info(self, trainer):
        #my_logger = open(self.cache['log_dir'] + _os.sep + f"remotelogs.json", 'a')
        out = {}
        ...
        #my_logger.write("\n Train scores are " + str(train_scores))
        ...
        #my_logger.write("\n New metrics are " + str(trainer.new_metrics()))
        #my_logger.write("\n Train metrics are " + str(train_scores['metrics']))
        out['train_metrics_out'] = out['train_metrics'].reduce_sites(train_scores['metrics'])
        ...
        #my_logger.write("\n Validation scores are " + str(val_scores))
        ...
        out['val_metrics_out'] = out['val_metrics'].reduce_sites(val_scores['metrics'])
        return out
    ...
    def _on_epoch_end(self, reducer):
        ...
        #my_logger.write("\n Train averages are" + str(epoch_info['train_averages'].get()))
        #my_logger.write("\n Train metrics are" + str(*epoch_info['train_metrics'].get()))

        self.cache[Key.TRAIN_LOG].append([*epoch_info['train_averages'].get(), *epoch_info['train_metrics_out']])
        self._save_if_better(**epoch_info)
        if epoch_info.get('val_averages'):
            self.cache[Key.VALIDATION_LOG].append([*epoch_info['val_averages'].get(), *epoch_info['val_metrics_out']])
    ...
    def _on_run_end(self, trainer):
        ...
        test_metrics_out = test_metrics.reduce_sites(test_scores['metrics'])

        self.cache[Key.TEST_METRICS].append([*test_averages.get(), test_metrics_out])
        self.cache[Key.GLOBAL_TEST_SERIALIZABLE].append(
            {'averages': test_averages.serialize(), 'metrics': test_metrics_out}
        )
    ...
    def _send_global_scores(self, trainer):
        ...
        global_test_out = metrics.reduce_sites(global_test_scores['metrics'])

        self.cache[Key.GLOBAL_TEST_METRICS] = [[*averages.get(), global_test_out]]
        _utils.save_scores(
            self.cache,
    ...
    def compute(self, mp_pool,
        ...
        #custom_logger = open(self.cache['logger_directory'] + _os.sep + f"awesomeremotelogs.json", 'a+')
        ...
        if check(all, 'phase', Phase.INIT_RUNS, self.input):
            ...
            """
            self._init_runs()
            self.cache["remote_iteration_count"]=0
            #custom_logger.write("\n Inside init runs")
        ...
        if check(all, 'phase', Phase.COMPUTATION, self.input):
            ...
            if check(all, 'reduce_for_masks', True, self.input):
                #custom_logger.write("\n Inside computation reduce")
                #remoteIterationCount =int(self.cache["remote_iteration_count"]) + 1
                #self.cache.update({"remote_iteration_count":remoteIterationCount })
                self.out.update(**reducer.reduceForMasks())
