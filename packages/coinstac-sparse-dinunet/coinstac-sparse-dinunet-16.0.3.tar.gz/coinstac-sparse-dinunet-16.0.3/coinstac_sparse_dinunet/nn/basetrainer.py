...
class NNTrainer:
    ...
    def register_pre_hook_mask(self, masks=None):
        ...
        for model_key in self.nn:
            for name, module in self.nn[model_key].named_modules():
                ...
                if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                    #module.mask = nn.Parameter(masks[masks_count].to(self.device['gpu'])).requires_grad_(False)
                    module.mask = nn.Parameter(masks[masks_count].to(self.device['gpu'])).requires_grad_(False).to(
                        module.weight.to(self.device['gpu']))
                    masks_count += 1
    ...
    def unregister_mask(self, model):
        for module in model.modules():
                module._backward_hooks = _ODict()
                module._forward_pre_hooks = _ODict()
        return model
    ...
    def _set_gpus(self):
        ...
        if self.cache.get('gpus') is not None and len(self.cache['gpus']) > 0:
            if _conf.CUDA_AVAILABLE:
                ...
                pass
                #raise Exception(f'*** GPU not detected in {_torch}. ***')
