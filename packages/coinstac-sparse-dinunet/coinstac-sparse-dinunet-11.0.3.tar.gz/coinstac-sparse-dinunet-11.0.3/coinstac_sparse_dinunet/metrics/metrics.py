...
from coinstac_sparse_dinunet.config import metrics_num_precision as _nump, metrics_eps as _eps
...
class COINNAverages(COINNMetrics):
    def __init__(self, num_averages=1, **kw):
        ...
        """
        super().__init__(**kw)
        self.values = _np.array([0.0] * num_averages, dtype=float)
        self.counts = _np.array([0.0] * num_averages, dtype=float)
...
class ConfusionMatrix(COINNMetrics):
    ...
    def precision(self, average=True):
        ...
        finalprecision =  sum(precision) / self.num_classes if average else precision
        if(torch.is_tensor(finalprecision)):
            finalprecision= finalprecision.numpy().item()
    ...
    def recall(self, average=True):
        ...
        finalrecall =sum(recall) / self.num_classes if average else recall
        if(torch.is_tensor(finalrecall)):
            finalrecall = finalrecall.numpy().item()
    ...
    def get(self):
        # return [round(_np.mean(self.accuracy()), self.num_precision), round(_np.mean(self.f1()), self.num_precision),
        #         round(_np.mean(self.precision()), self.num_precision), round(_np.mean(self.recall()), self.num_precision)]
        return [round(_np.mean(self.accuracy()), self.num_precision), round(_np.mean(self.f1()), self.num_precision),
                round(_np.mean(self.precision()), self.num_precision), round(_np.mean(self.recall()), self.num_precision)]
    ...
    def reduce_sites(self, scores: list):
        self._accuracy, self._precision, self._recall = _np.array(scores).mean(0)
        return [self._accuracy, self._precision, self._recall]
