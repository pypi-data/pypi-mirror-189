...
int hasEntries(bigWigFile_t *bw) {
    if (bw->hdr->indexOffset != 0) return 1; // no index, no entries
    return 0;
}
...
PyObject* pyBwEnter(pyBigWigFile_t *self, PyObject *args) {
    ...
    if (!bw) {
        PyErr_SetString(PyExc_RuntimeError, "file handle not opened");
        return NULL;
    }
    Py_INCREF(self);
...
static PyObject *pyBwGetHeader(pyBigWigFile_t *self, PyObject *args) {
    ...
    if (bw->isWrite == 1) {
        PyErr_SetString(PyExc_RuntimeError, "header can't be accessed if opened for writing");
        return NULL;
    }
    ret = PyDict_New();
...
error :
    Py_XDECREF(val);
    Py_XDECREF(ret);
    PyErr_SetString(PyExc_RuntimeError, "error while getting the header");
    return NULL;
}
...
static PyObject *pyBwGetChroms(pyBigWigFile_t *self, PyObject *args) {
    ...
    if (bw->isWrite == 1) {
        PyErr_SetString(PyExc_RuntimeError, "chromosomes can't be accessed if opened for writing");
        return NULL;
    }
...
error :
    Py_XDECREF(val);
    Py_XDECREF(ret);
    PyErr_SetString(PyExc_RuntimeError, "error while adding an item to the output dictionary");
    return NULL;
}
...
static PyObject *pyBwGetStats(pyBigWigFile_t *self, PyObject *args, PyObject *kwds) {
    ...
    if (bw->isWrite == 1) {
        PyErr_SetString(PyExc_RuntimeError, "statistics can't be accessed if opened for writing");
        return NULL;
    }
    if (bw->type == 1) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|OOsiOO", kwd_list, &chrom, &starto, &endo, &type, &nBins, &exact)) {
        PyErr_SetString(PyExc_RuntimeError, "a chromosome must at least be supplied");
        return NULL;
    }
    ...
    if (starto) {
        if (PyLong_Check(starto)) {
            ...
            PyErr_SetString(PyExc_RuntimeError, "start coordinate must be a number");
            return NULL;
        }
    }
    if (endo) {
        if (PyLong_Check(endo)) {
            ...
            PyErr_SetString(PyExc_RuntimeError, "end coordinate must be a number");
            return NULL;
        }
    }
    ...
    if (!val) {
        PyErr_SetString(PyExc_RuntimeError, "error while fetching statistics");
        return NULL;
    }
    ret = PyList_New(nBins);
    for (i = 0; i < nBins; i++) {
...
static PyObject *pyBwGetValues(pyBigWigFile_t *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "sOO", &chrom, &starto, &endo)) {
        PyErr_SetString(PyExc_RuntimeError, "a chromosome, start and end must be supplied\n");
        return NULL;
    }
    tid = bwGetTid(bw, chrom);
    if(PyLong_Check(starto)) {
    ...
    if (!hasEntries(self->bw)) return PyList_New(0);
    o = bwGetValues(self->bw, chrom, start, end, 1);
    if (!o) {
        PyErr_SetString(PyExc_RuntimeError, "error while fetching values");
        return NULL;
    }
