...
from ..schemas.raws import TypeTag
from ..shared.shared_definitions import branch_digest

TypeTags = Union[None, List[TypeTag]]
...
class Search:
    ...
    __config__ = {
        IndexTag: TypeProxyIndex(method='raw_tags', serializer=TypeTag),
    }
    ...
    async def tags(
        ...
        try:
            ...
            except Exception as _any_serialize:
                print(_any_serialize)
                return TagResults(
                    items=[],
                    details=f'Error serialize model data: {_any_serialize}',
                    exists=False,
                    count=0,
                    total=total,
                )
