...
import os, argparse, json, dotenv, sys, pprint
...
def init_app(args, app_name=None):
  ...
  os.system(f"bash build_volumes.sh && bash build.sh && bash run.sh && bash stop.sh && bash init.sh")
...
def list_services(args):
  ...
  for k in services:
    item = services[k]
    print(f"{'{0: <25}'.format(item.get('name'))}\t{os.path.join(base_dir, item.get('base_dir'))}")
