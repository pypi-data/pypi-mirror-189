...
EMBEDDING_MODEL = lambda prompt: openai.Embedding.create(input=[prompt], model='text-embedding-ada-002')['data'][0]['embedding']
SEPARATOR = "\n"
...
def get_example_length(example):
    ...
    example_prompt = add_output_to_prompt(example_prompt, type(example['output']), example['output'])
...
class SimilaritySelector(BaseSelector):
    def __init__(self, max_input_tokens=MAX_INPUT_TOKENS, embedding_model=EMBEDDING_MODEL):
    ...
    def sort(self, examples, input):
        _, sorted_example_indices = self.index.search(np.array([self._get_embedding(input)], dtype=np.float32), len(examples))
        return [examples[sorted_example_index] for sorted_example_index in sorted_example_indices[0]]
...
class MaxMarginalRelevanceSelector(SimilaritySelector):
    ...
    def __sort_by_max_marginal_relevance(
        ...
        while len(sorted_indices) < len(embeddings):
            ...
            for i, embedding in enumerate(embeddings):
                ...
                curr_score = lambda_multiplier * similarity_score - (1 - lambda_multiplier) * diversity_score
    ...
    def sort(self, examples, input):
        ...
        _, sorted_example_indices = self.index.search(np.array([input_embedding], dtype=np.float32), len(examples))
        example_embeddings = [self.index.reconstruct(int(sorted_example_index)) for sorted_example_index in sorted_example_indices[0]]
        sorted_example_indices = self.__sort_by_max_marginal_relevance(input_embedding, example_embeddings)
        return [examples[sorted_example_index] for sorted_example_index in sorted_example_indices]
...
class Llama:
    ...
    def fit(self, examples: list):
        ...
        if issubclass(type(self.example_selector), SimilaritySelector):
            embeddings = [self.example_selector._get_embedding(example['input']) for example in examples]
            index = faiss.IndexFlatL2(len(embeddings[0]))
            index.add(np.array(embeddings, dtype=np.float32))
            self.example_selector.index = index

    def run(self, input: Spec, output_spec: Spec, temperature=FIXED_TEMPERATURE, random: bool = False):
...
def parse_properties(properties, definitions):
    if "$ref" in properties:
        ...
        return zip(*ref_class['properties'].items()) # ref_param_attr (e.g. 'example'), ref_properties (e.g. {'type': 'string'})
    else:
        return [None], [properties]

def add_instruction_to_prompt(prompt: str, input: Spec, output_spec: Spec):
    parsed_input = input.schema()
...
def add_input_to_prompt(prompt: str, input: Spec):
    parsed_input = input.schema()
    ...
    for param, properties in input_properties.items():
        ...
        if properties["type"] == 'array' and len(user_attr) > 1:
...
def add_output_to_prompt(prompt: str, output_spec: Spec, output=None):
    parsed_output = output_spec.schema()
...
def apply_cue(output_spec: Spec):
...
def parse_cue(output, output_spec: Spec):
    output_values = {}
    remainder = output
