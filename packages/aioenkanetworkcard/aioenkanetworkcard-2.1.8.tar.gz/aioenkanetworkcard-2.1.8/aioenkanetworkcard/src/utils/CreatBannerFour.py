...
class Mini:
    ...
    async def weapon(self,info, nameCharter,element,lvl):
        ...
        bgTwo = bg.copy()
        nameCharter = nameCharter.replace("FischlCostumeHighness", "")
    ...
    async def creat_bg(self,rarity,element,link,characters,lvl):
        ...
        centrName,fonts = await centrText(lvlText, witshRam = 151, razmer = 17,start = 55)
...
async def generationFour(items,assets,lang, mini = True, userName = "User", uid = "UID: Hide"):
    if mini:
        ...
        for characters in items:
            ...
            task.append(Mini().weapon(characters.equipments[-1],characters.image.icon.filename.replace("CostumeFloral","").split("AvatarIcon_")[1],person.element.value,lang["lvl"]))
