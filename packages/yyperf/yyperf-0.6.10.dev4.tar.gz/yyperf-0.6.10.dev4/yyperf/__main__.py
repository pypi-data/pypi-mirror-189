...
from .web.handlers.perf_page import (DeviceListHandler,PackageListHandler,
    DeviceAPPHandler,PerfCaptrueHandler,ExportDataHandler)
...
PID_FILEPATH = os.path.expandvars("$HOME/.yyperf/yyperf.pid")
...
def make_app(settings={}):
    application = tornado.web.Application(
        [
            (r"/", MainHandler),
            (r"/api/v1/devices/list", DeviceListHandler),
            (r"/api/v1/devices/app", DeviceAPPHandler),
            (r"/api/v1/devices/([^/]+)/packagelist", PackageListHandler),
            (r"/ws/v1/captrue", PerfCaptrueHandler),
            (r"/api/v1/export/(.*)/(.*)", ExportDataHandler),
...
def cmd_quit(port=17310):
    try:
        ...
        logger.info("yyperf quit successfully")
    except requests.ConnectionError:
        logger.info("yyperf already stopped")
...
def run_web(debug=False, port=17310, open_browser=False, force_quit=False):
    ...
    if version:
        if force_quit:
            logger.info(f"quit previous yyperf server (version: {version})")
            requests.get(base_url + "/quit")
            time.sleep(.5)
        else:
            sys.exit(f"Another yyperf({version}) is already running")
...
def create_shortcut():
    ...
    shortcut.SetArguments("-m yyperf")
...
def remove_data_file():
    ...
    for dirs in dir_list:
        try:
            dirs_path = folder + os.path.sep + dirs
            dir_day = datetime.datetime.strptime(dirs,"%Y-%m-%d")
            print((today - dir_day).days)
            if (today - dir_day).days > 7:
                shutil.rmtree(dirs_path,ignore_errors=True)
        except:
            pass
...
def main():
    ...
    ap.add_argument('-p', '--port', type=int, default=17310, help='local listen port for yyperf')
    ...
    ap.add_argument("--quit", action="store_true", help="stop yyperf")
    args = ap.parse_args()
    # yapf: enable
