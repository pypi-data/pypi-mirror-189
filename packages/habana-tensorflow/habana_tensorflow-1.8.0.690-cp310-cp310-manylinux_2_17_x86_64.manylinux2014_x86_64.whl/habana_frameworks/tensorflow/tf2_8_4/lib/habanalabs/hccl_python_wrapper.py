...
def hcclCommInitRank(comm, nranks, commId, rank):
    return _hccl_python_wrapper.hcclCommInitRank(comm, nranks, commId, rank)

def hcclCommInitAll(comm, ndev, devlist):
    return _hccl_python_wrapper.hcclCommInitAll(comm, ndev, devlist)
...
def hcclReduce(sendbuff, recvbuff, count, datatype, op, root, comm, stream_handle):
    return _hccl_python_wrapper.hcclReduce(sendbuff, recvbuff, count, datatype, op, root, comm, stream_handle)
...
def hcclAllReduce(sendbuff, recvbuff, count, datatype, op, comm, stream_handle):
    return _hccl_python_wrapper.hcclAllReduce(sendbuff, recvbuff, count, datatype, op, comm, stream_handle)

def hcclReduceScatter(sendbuff, recvbuff, recvcount, datatype, op, comm, stream_handle):
    return _hccl_python_wrapper.hcclReduceScatter(sendbuff, recvbuff, recvcount, datatype, op, comm, stream_handle)
...
def hcclBarrier(comm, stream_handle):
    return _hccl_python_wrapper.hcclBarrier(comm, stream_handle)
...
def hcclxLockDeviceAddress(address, device_address):
    return _hccl_python_wrapper.hcclxLockDeviceAddress(address, device_address)

def hcclxUnlockDeviceAddress(device_address):
    return _hccl_python_wrapper.hcclxUnlockDeviceAddress(device_address)

def hcclxAcquireCollectiveStream(device_id, stream_handle):
    return _hccl_python_wrapper.hcclxAcquireCollectiveStream(device_id, stream_handle)

def hcclxReleaseCollectiveStream(stream_handle):
    return _hccl_python_wrapper.hcclxReleaseCollectiveStream(stream_handle)

def hcclxAcquireCopyStream(device_id, stream_handle_ptr, kind):
    return _hccl_python_wrapper.hcclxAcquireCopyStream(device_id, stream_handle_ptr, kind)

def hcclxReleaseCopyStream(stream_handle):
    return _hccl_python_wrapper.hcclxReleaseCopyStream(stream_handle)

def hcclxMalloc(address_ptr, size):
    return _hccl_python_wrapper.hcclxMalloc(address_ptr, size)
...
def hcclxPrepareStream(stream_handle, input_addresses_array_ptr, array_length):
    return _hccl_python_wrapper.hcclxPrepareStream(stream_handle, input_addresses_array_ptr, array_length)

def hcclxSubmitEvents(stream_handle, input_addresses_array_ptr, array_length, cleanup_callback, callback_cookie):
    return _hccl_python_wrapper.hcclxSubmitEvents(stream_handle, input_addresses_array_ptr, array_length, cleanup_callback, callback_cookie)
