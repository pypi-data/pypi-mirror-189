...
def get_replication_method_from_stream(stream: Dict, default_replication_method: str) -> Optional[str]:
    """
    Search for the stream replication method
    Args:
        stream: stream dictionary
        default_replication_method: default replication

    Returns: replication method if defined, None otherwise

    """
    md_map = metadata.to_map(stream['metadata'])
    return md_map.get('replication-method', default_replication_method)


def is_log_based_stream(stream: Dict, default_replication_method: str) -> bool:
    """
    checks if stream uses log based replication method
    Returns: True if LOG_BASED, False otherwise
    """
    return get_replication_method_from_stream(stream, default_replication_method) == 'LOG_BASED'
...
def filter_streams_by_replication_method(streams_to_sync: List[Dict], default_replication_method: str) -> Tuple[List[Dict], List[Dict]]:
    """
    ...
    for stream in streams_to_sync:
        if is_log_based_stream(stream, default_replication_method):
