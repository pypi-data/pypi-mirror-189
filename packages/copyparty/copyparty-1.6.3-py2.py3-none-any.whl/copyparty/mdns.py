...
class MDNS(MCast):
    def __init__(self, hub , ngen )  :
        ...
        self.probing = 0.0
        self.unsolicited  = []  # scheduled announces on all nics
        self.defend   = {}  # server -> deadline
    ...
    def eat(self, buf , addr  , sck )  :
        ...
        srv  = self.srv[sck] if v6 else self.map_client(cip)  # type: ignore
        if not srv:
            return
