...
def __configure_fastapi(tracer_provider, fastapi_app=None):
    ...
    def create_context_from_headers(headers):
        trace_id_req = headers.get(__TRACE_ID_HEADER_NAME, None)
        span_id_req = headers.get(__SPAN_ID_HEADER_NAME, None)
        ctx = None
    ...
    async def add_trace_id_header(request, call_next):
        #request:Request
        ctx = create_context_from_headers(request.headers)
        span = trace.get_tracer(__name__).start_as_current_span(str(request.url), context=ctx)
        with span:
