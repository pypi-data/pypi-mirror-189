...
try:
    ...
    from conannio import Simulate, parse_args, Version
...
class Recipe:
    ...
    def standalone_create(self, profile_build="default", profile_host="sel4", build_type="Release", fake=False):
        ...
        install_command = f'{base_args} -r=nio --build=missing {relpath_conanfile} {self.partial_reference}'
        ...
        if fake:
            ...
            print("conan export_pkg", export_command)
        else:
    ...
    def create(self, profile_build="default", profile_host="sel4", build_type="Release", fake=False):
        ...
        for e in self.env_host:
            env += ' -e ' + e
        env += " -e source=1" if self.reference.name in only_host_build and self.force_build else ""
        ks = ' -ks' if self.keep_source else ""
        kb = ' -kb' if self.keep_build else ""
        prh = ' -pr:h=' + profile_host if profile_host else ""
        reference_str = str(self.reference)
        if self.reference.user is None and self.reference.channel is None:
            reference_str += '@'
        args = f'-pr:b={profile_build} -s build_type={build_type} -tf=None --build=missing -r=nio{env}{prh}{ks}{kb} {self.full_conanfile} {reference_str}'
        if fake:
            print("Fake running: conan create", args)
        else:
    ...
    def update_depends_version(self):
        ...
        with open(conandata_yaml_file, encoding='utf-8') as f:
            ...
            need_save = False
            if 'requires' in conandata_yaml:
                need_save = True
                requires = conandata_yaml['requires']
                if type(requires) == dict:
                    new_requires = {}
                    for v, d in requires.items():
                        new_requires[v] = update_version(d)
                    conandata_yaml['requires'] = new_requires
                elif type(requires) == list:
                    conandata_yaml['requires'] = update_version(requires)

                # yaml.dump(conandata_yaml, sys.stdout, allow_unicode=True, default_flow_style=False, sort_keys=False)
            if need_save:
...
class SeL4Center:
    ...
    def build(self, recipes_names=None, keep_source=False, keep_build=False, debug_bulld=True, release_build=True, deps=True, force=False, fake=False, env_host=[]):
        ...
        for name in recipes_names:
            if name not in only_host_build:
                sel4_osf = self.recipes['sel4-osf'] if 'sel4-osf' in self.recipes else None
    ...
    def run_disk(self, args):
        user_presets_path = os.path.join(self.cwd, "CMakeUserPresets.json")
        ...
        if "conan" not in data.get("vendor", {}):
            ...
            return
        found_disk = True
        if len(args.disk_image) > 0 and args.disk_image[0] != '/':
            found_disk = False
            for include in data["include"]:
                cmake_presets = json.loads(load(include))
                configure_preset = cmake_presets["configurePresets"][0]
                if configure_preset['name'] == args.build_type:
                    args.disk_image = os.path.join(configure_preset.get("binaryDir"), args.disk_image)
                    found_disk = os.path.exists(args.disk_image)
                    break
        if not found_disk:
            print(f"Disk image file not found, please run 'niobuild build' to build.")
            return True
        ...
        sel4_sysroot = self.conan_api.get_profile_key("default", "conf.tools.build:sel4_sysroot")
        if len(args.qemu_sim_initrd_file) > 0 and args.qemu_sim_initrd_file[0] != '/':
            args.qemu_sim_initrd_file = os.path.join(sel4_sysroot, args.qemu_sim_initrd_file)
        Simulate(args).run()
        return True


class DockerRunner(object):
    def __init__(self, images=None, interactive=False):
        self.DOCKER_IMAGE_NAME = images or "artifactory.nevint.com:5022/sel4/ubuntu-" + platform.machine() + ":" + Version
        USER = os.getlogin()
        UID = os.getuid()
        GID = os.getgid()
        ...
        self.docker_cmd = f"docker run -{interactive_str}t --rm {COMMON_VOLUMES} {CURRENT_USER} --workdir={CWD} {self.DOCKER_IMAGE_NAME} "
...
class BuildCommand:
    ...
    def run(self):
        ...
        if os.path.exists("/.dockerenv") or not opt.docker:
            if conan_installed:
                ...
                print(
                    "No conan is found, install conan or run it in a docker with the --docker")
                print(f'    {sys.argv[0]} --docker {" ".join(sys.argv[1:])}')
        else:
            DockerRunner(opt.docker_image, opt.func.__name__ == "conan_shell").run(sys.argv)
    ...
    def cmd_run(self):
        def sel4_run(args):
            return SeL4Center().run_disk(args)
        ...
        parser.set_defaults(func=sel4_run)
        parse_args(parser)
