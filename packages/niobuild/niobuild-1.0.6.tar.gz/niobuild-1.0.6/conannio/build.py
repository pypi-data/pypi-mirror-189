...
import re
import json
from conannio import Simulate, parse_args
...
class Recipe:
    ...
    def create(self, profile_build="default", profile_host="sel4", build_type="Release", fake=False):
        ...
        args = f'-pr:b={profile_build} -s build_type={build_type} -tf=None --build=missing --build={self.reference.name} -r=nio{env}{prh}{ks}{kb} {conanfile} {reference_str}'
        if fake:
...
class DockerRunner(object):
    def __init__(self, images=None, interactive=False):
        ...
        self.docker_cmd = f"docker run -{interactive_str}t --rm {COMMON_VOLUMES} {CURRENT_USER} -p 12345:1234 --workdir={CWD} {self.DOCKER_IMAGE_NAME} "
