...
from kolibri.utils.distribution import (
...
def _roll_out_time_series(
    ...
    def _f(x):
        ...
        return df_temp
    if not hasattr(grouped_data, column_sort):
        raise Exception("the sorting column: "+str(column_sort)+ " is not in the provided dataframe")

    if not hasattr(grouped_data, column_id):
        raise Exception("the id column: "+str(column_id)+ " is not in the provided dataframe")
