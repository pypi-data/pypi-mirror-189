...
class HitrustaiTrainTemple:
    ...
    def read_train_conf_apollo(self):
        ...
        bootstrap_servers = []
        for ip in range(int(kafka_node)):
            s_host, s_port = ip.split(":")
            host = config('KAFKA_HOST_' + s_host, default='')
            port = config('KAFKA_PORT_' + s_port, default='')
            if host == "" or port == "":
                print('Error: Invalid variable in env. Expected keys [KAFKA_HOST, KAFKA_PORT]')
                os.kill(0, 4)
            bootstrap_servers.append(host + ":" + port)
    ...
    def train(self, module_quantity=1):
        ...
        if config('ENV_METHOD') == "env":
            mq_info, train_info, SQLALCHEMY_DATABASE_URI, kg_path, lib_path, dataset_path, mq = self.read_train_conf_env()
        else:
            mq_info, train_info, SQLALCHEMY_DATABASE_URI, kg_path, lib_path, dataset_path, mq = self.read_train_conf_apollo()
