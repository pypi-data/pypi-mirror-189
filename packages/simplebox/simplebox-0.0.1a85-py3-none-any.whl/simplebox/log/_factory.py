...
_level = {LogLevel.CRITICAL: "critical", LogLevel.FATAL: 'critical',
          LogLevel.WARN: "warning", LogLevel.WARNING: 'warning', LogLevel.ERROR: 'error', LogLevel.INFO: 'info',
          LogLevel.DEBUG: 'debug', LogLevel.NOSET: 'debug'}
...
class _LoggerAdapterWrapper(LoggerAdapter):
    ...
    def simple(self, level: LogLevel = LogLevel.INFO, template: str = None):
        """
        quick save function meta info
        :param level: record log level
        :param template: log templates, only four parameters are supported(function name, args, kwargs, return)
        """
        def __inner(func):
            @wraps(func)
            def __wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    action = getattr(self, _level.get(level))
                    if not template:
                        content = f"Quick logs => function name: {func.__name__}, args: {args}, kwargs: {kwargs}, " \
                                  f"result: {result}"
                    else:
                        try:
                            content = template.format(func.__name__, args, kwargs, result)
                        except BaseException:
                            raise IndexError('There are too many positional parameters, '
                                             'and only a maximum of 4 positional parameters are allowed')
                    action(content)
            return __wrapper
        return __inner
