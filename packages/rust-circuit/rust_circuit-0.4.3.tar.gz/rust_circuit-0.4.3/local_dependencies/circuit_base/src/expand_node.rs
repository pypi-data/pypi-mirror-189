...
pub fn expand_node(
    ...
    let out = match &**circuit {
        Circuit::SetSymbolicShape(s) => {
            ...
            Ok(if s2 != *child_shape {
                SetSymbolicShape::nrc(c, s2, s.info().name)
            } else {
                c
            })
        }
        Circuit::Array(_) | Circuit::Symbol(_) | Circuit::Scalar(_) => Ok(circuit.clone()),
        ...
        Circuit::Rearrange(node) => {
            ...
            Rearrange::try_new(new_input, new_spec, circuit.info().name).map(|x| x.rc())
        }
        ...
        Circuit::Concat(node) => {
            ...
            Concat::try_new(inputs, new_axis, node.info().name).map(|x| x.rc())
        }
        Circuit::Add(node) => {
            ...
            Add::try_new(inputs, node.info().name).map(|x| x.rc())
        }
        ...
        Circuit::Einsum(node) => {
            ...
            Einsum::try_new(new_args, out_axes, node.info().name).map(|x| x.rc())
        }
        Circuit::Tag(node) => Ok(Tag::nrc(inputs[0].clone(), node.uuid, node.info().name)),
        Circuit::Cumulant(node) => {
            ...
            Ok(Cumulant::nrc(inputs.clone(), node.info().name))
        }
        Circuit::Module(node) => {
