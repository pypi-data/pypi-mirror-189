...
class Scenario:
  ...
  def _init_scenario(self, j):
    ...
    j2 = dict({"sready": True, "saddr": self.srv.host, "sport": self.srv.port}, **j)
  ...
  def _continue_scenario(self, j):
    j2 = dict({"stoken": self.token, "scontinue": True}, **j)
    self.cli._send(Scenario._encode_json(j2))
    
  def _terminate_scenario(self, j):
    j2 = dict({"stoken": self.token, "sfinish": True}, **j)
    self.token = None
    self.init = False
    self.cli._send(Scenario._encode_json(j2))
