...
class Parent(Component):
    ...
    def __init__(self, props):
        ...
        self.state = {"some_value": randint(1, 50)}

    def update_value(self, event):
        self.set_state({"some_value": self.state.get("some_value") + 1})

    def render(self):
        return wsx(
            [
                c.Frame,
                {"show": True, "size": (200, 100)},
                [
                    c.Block,
                    {},
                    [LabeledGauge,
                        {
                            "value": self.state["some_value"],
                            "range": 100,
                            "min_size": (100, 20),
                            "size": (100, 20),
                        },
                    ...
                    [Child, {"click_handler": self.update_value, "label": "Click me!"}],
                ],
            ]
        )
...
class Child(Component):
    ...
    def render(self):
        return wsx(
            [
                c.Button,
                {"on_click": self.props["click_handler"], "label": self.props["label"]},
            ]
        )


class LabeledGauge(Component):
    def __init__(self, props):
        super().__init__(props)
    ...
    def render(self):
        return wsx(
            [
                ...
                [c.StaticText, {"label": str(self.props.get("value", "Download"))}],
                [
                    c.Gauge,
                    {
