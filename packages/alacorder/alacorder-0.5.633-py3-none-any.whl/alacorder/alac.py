...
def getCharges(text: str):
	...
	c = pd.Series([re.findall(r'(\d{3}\s{1}.{1,100}?.{3}-.{3}-.{3}.{0,100}+)', str(text), re.MULTILINE)])
	c = c.str.replace("Sentences","").replace("Sentence","").replace("SentencesSentence","").replace("Sentence 1","").replace("Financial","").strip()
	cind = range(0, len(c))
