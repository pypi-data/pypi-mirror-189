# alacorder beta 0.4.4
# core functions c library
import cython
...
def getPgOneText(path: str) -> str:
	...
	#	return pdf.pages[0].dedupe_chars().extract_text(layout=False)
...
def getPDFText(path: str) -> str:
	...
	return str(text)
	#with pdfplumber.open(path) as pdf:
	#	alltext = pd.Series(pdf.pages).map(lambda x: x.dedupe_chars().extract_text(layout=False))
	#return "\n".join(alltext)

def getPDFTextExperimental(path: str) -> str:
	text = ""
	pdf = pdfs.PdfReader(path)
	for pg in pdf.pages:
		text += pg.extract_text() + "\n"
