...
def getCaseInfo(text: str):
	...
	try:
		...
		if len(name.strip())<5:
			name = re.search(r'(?:DOB)(.+)(?:Name)', str(text), re.MULTILINE, re.DOTALL).group()
		alias = re.search(r'(?:Alias 1\:){1}(.*)', str(text), re.MULTILINE).group().replace("Alias 1:","").strip()
	except (IndexError, AttributeError):
		try:
			name = re.search(r'(?:DOB)(.{0,100}?)(?:Name)', str(text), re.MULTILINE).group()
			almatch = re.search(r'(?:Name){1}(.{0,50}?)(?:Alias 1\:){1}(.*)', str(text), re.MULTILINE).group()
			alias: str = almatch.split(" Alias 1: ")[1].replace("Alias 2:","").strip()
		except (IndexError, AttributeError):
			name = ""
			alias = ""
...
def getCharges(text: str):
	...
	charges['Charges'] = charges['Charges'].map(lambda x: x.replace("SentencesSentence 1",""))
	# find table fields
