...
A.	EXPORTING DETAILED CASE INFORMATION AS A TABLE
	...
	Inputs:		PDF Directory (./path/to/pdfs) or 
				Full Text Archive (.pkl, .csv, .xls, .json)

	Outputs:	Detailed Cases Table (.pkl, .csv, .xls, .dta, .json) 
				or text file (.txt) if re-import is not needed
...
B.	CREATE A FULL TEXT ARCHIVE FROM PDF CASES
	...
	Outputs:	Full Text Archive (.pkl, .csv, .xls, .json) or
				text file (.txt) if re-import is not needed
...
if mode == "archive-from-directory":
	print(f'''
>>		Would you like to create a detailed cases information table from the full text archive data?
...
Output Path: ''')
		...
		run.start(in_dir,xpath,"tables-from-archive",batch_size=2000)
	if info == "N":
		print(f'''
