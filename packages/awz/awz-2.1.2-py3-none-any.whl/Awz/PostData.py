import aiohttp
import asyncio
from Awz.Encoder import encoderjson
from Awz.GtM import default_api
...
async def rubino(js,auth,clien,method):
	s = f'https://rubino{randint(1,30)}.iranlms.ir/'
	enc = encoderjson(auth)
	async with aiohttp.ClientSession() as session:
		...
		while 1:
			try:
				...
				if self.Type == "json":
					...
					return loop.run_until_complete(rubino(indata,self.Auth,wn,methode))
					break
			except:
				continue
