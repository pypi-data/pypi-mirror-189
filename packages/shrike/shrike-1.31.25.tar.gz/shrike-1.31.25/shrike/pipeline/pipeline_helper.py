...
from shrike._core import is_eyesoff_helper, POLYMER_FEED
...
class AMLPipelineHelper:
    ...
    def apply_recommended_runsettings(
        ...
        **custom_runtime_arguments,
        ...
        if synapse == "auto":
            synapse = str(module_instance.type) == "SparkComponent"
            if synapse:
    ...
    def _override_single_spec_yaml(
        ...
        if env_yaml_override_is_needed:
            ...
            log.info(f"Will remove {POLYMER_FEED} from environment.conda.")
            try:
                if spec["type"] == "spark":
                ...
                (
                    ...
                    spec_dirname, conda_dependencies_file, POLYMER_FEED
                )
                if found_index_url:
                    if spec["type"] == "spark":
            ...
            try:
                ...
                (
                    ...
                    spec_dirname, pip_requirements_file, POLYMER_FEED
                )
                if found_index_url:
    ...
    def _update_dc_configs(self, pipeline):
        hdi_compute = self.config.compute.hdi_dc_target
        synapse_compute = self.config.compute.synapse_dc_target
        if not hdi_compute and not synapse_compute:
            log.info("This pipeline doesn't involve HDI DC or Synapse DC.")
            return pipeline
        nodes = pipeline._expand_pipeline_nodes()
        graph = defaultdict(list)
        node_mapping = {}
        for n in nodes:
            node_mapping[n._instance_id] = n
            for input in n.inputs.values():
                try:
                    owner = (
                        input._get_data_owner()
                    )  # points to subgraph node if upstream component is subgraph
                    if owner:
                        graph[n._instance_id].append(owner._instance_id)

                except AttributeError:
                    continue
        subpipelines = pipeline._expand_pipeline_to_pipelines()
        for n in subpipelines:
            # https://msdata.visualstudio.com/DefaultCollection/Vienna/_workitems/edit/2026740/
            # if the last node of a subgraph needs to be updated
            # then we need to update subgraph.outputs datastore too
            node_mapping[n._instance_id] = n
            graph[n._instance_id].append(n.nodes[-1]._instance_id)
        sorted_node_ids = toposort_flatten(graph)
        use_dc = defaultdict(bool)
        for id in sorted_node_ids:
            if id not in node_mapping:
                continue
            node = node_mapping[id]
            if not isinstance(node.runsettings.target, str):
                log.warn(
                    f"The compute target of node {node.name} is a PipelineParameter. Shrike will not update even if this or the upstream steps use HDI DC or Synapse DC. Please update manually."
                )
            elif node.type != "ScopeComponent" and (
                (hdi_compute and node.runsettings.target == hdi_compute)
                or (synapse_compute and node.runsettings.target == synapse_compute)
            ):
                use_dc[id] = True
                continue
        ...
        for id, node in node_mapping.items():
            if use_dc[id]:
                if node.type == "ScopeComponent":
                ...
                log.info(
                    f"Writing outputs of {node.name} to dc_datastore {self.config.compute.dc_datastore}"
                )
                if not isinstance(node.runsettings.target, str):
                    log.info(
                        f"Node {node.name}'s target is a PipelineParameter, skipping."
                    )
                    continue
                elif node.runsettings.target == self.config.compute.hdi_prod_target:
                    node.runsettings.configure(target=self.config.compute.hdi_dc_target)
                elif (
                    node.runsettings.target == self.config.compute.linux_cpu_prod_target
                ):
                    node.runsettings.configure(
                        target=self.config.compute.linux_cpu_dc_target
                    )
                elif (
                    node.runsettings.target == self.config.compute.linux_gpu_prod_target
                ):
                    node.runsettings.configure(
                        target=self.config.compute.linux_gpu_dc_target
                    )
                elif (
                    node.runsettings.target
                    == self.config.compute.windows_cpu_prod_target
                ):
                    node.runsettings.configure(
                        target=self.config.compute.windows_cpu_dc_target
                    )
                elif node.runsettings.target == self.config.compute.synapse_prod_target:
                    node.runsettings.configure(
                        target=self.config.compute.synapse_dc_target
                    )
                else:
                    continue
                log.info(
                    f"Updating compute target for {node.name} {id} to {node.runsettings.target}"
                )
        return pipeline
    ...
    def _build_pipeline(self) -> Pipeline:
        ...
        if self.is_eyesoff():
            log.info(
                "Overriding compute target if upstream steps use HDI DC or Synapse DC..."
            )
