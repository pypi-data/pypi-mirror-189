...
def set_evaluation(
    ...
    a particular use for a certain phase of a specific project (in a
    course/semester).
    ...
    # Convert to a number if we can
    score_basis = _CONFIG.get("SCORE_BASIS", 100)
    if override != "":
        try:
    ...
    # Convert to a number if we can
    timeliness_basis = _CONFIG.get("TIMELINESS_POINTS", 10)
    if timeliness != "":
        try:
...
def fetch_old_outcomes(course, semester, username, exercise):
    """
    Fetches old outcomes for the given course/semester/uesrname/exercise.
    """
    # Redis key to use
...
def save_outcomes(
    ...
    value should be a number in [0, 1] where a higher number indicates a
    better submission.
