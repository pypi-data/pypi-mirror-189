...
from dnastack.alpha.client.ewes.models import WesServiceInfo, ExtendedRunStatus, ExtendedRunListOptions, \
    MinimalExtendedRun, MinimalBatch, \
    ExtendedRunListResponse, ExtendedRun, RunId, WorkbenchApiError, BatchActionResult, LogType, ExtendedRunRequest, \
    BatchRunRequest
...
class EWesClient(BaseServiceClient):

    def __init__(self, endpoint: ServiceEndpoint, namespace: str):
    ...
    def submit_run(self, data: ExtendedRunRequest = None) -> MinimalExtendedRun:
        if data is None:
            data = {}
        with self.create_http_session() as session:
            response = session.post(urljoin(self.endpoint.url, f'{self.namespace}/ga4gh/wes/v1/runs', json=data))
        return MinimalExtendedRun(**response.json())

    def submit_batch(self, data: BatchRunRequest = None) -> MinimalBatch:
        if data is None:
            data = {}
        with self.create_http_session() as session:
            response = session.post(urljoin(self.endpoint.url, f'{self.namespace}/ga4gh/wes/v1/batch', json=data))
