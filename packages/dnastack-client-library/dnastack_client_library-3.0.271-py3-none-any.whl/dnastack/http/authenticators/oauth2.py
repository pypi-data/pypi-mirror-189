...
class OAuth2Authenticator(Authenticator):
    def __init__(self,
        ...
        self._logger = get_logger(f'{type(self).__name__}: E/{endpoint.type}/{endpoint.id}'
                                  if endpoint
                                  else f'{type(self).__name__}: A/{self.session_id}')
    ...
    def authenticate(self) -> SessionInfo:
        logger = get_logger(f'{self._logger.name}/{self.session_id}')
        ...
        adapter.events.on('blocking-response-required',
                          lambda e: self.events.dispatch('blocking-response-required', e))
