...
class Push:
    ...
    def _create_import_new_model_params(
        ...
        return ImportNewModelParams(
            projectId=proj_id, fileName=filename, modelName=model_name, versionName=version, model_type=import_model_type, **additional_arguments)
    ...
    def _save_dataset(self, dataset_py_as_string: str, secret_name: Optional[str] = None) -> Dict:
        dataset_id = self.project.dataset_id(self._api)
    ...
    def _push_dataset(self, secret_name: Optional[str]) -> Dict:
    ...
    def _push_mapping(self, raw_dataset_version: Dict, branch_name: Optional[str], model_name: Optional[str]):
    ...
    def find_secret_id(self, secret_name: str, throw_on_not_found=True) -> Optional[str]:
        matches = [secret._id for secret in self._api.get_secret_manager_list().body.results if secret.name == secret_name]
        if len(matches) == 1:
            return matches[0]
        if len(matches) == 0:
