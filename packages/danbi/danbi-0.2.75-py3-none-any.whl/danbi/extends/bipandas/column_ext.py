...
class DanbiExtendSeries:
    ...
    def getStateUpDn(self, window: int, rate: float = 1, is_future: bool = False) -> List:
        if is_future:
        ...
        for up, dn, val in zip(ups, dns, self._col):
            ...
            if not is_up and up: # dn -> up 으로 변환
                ...
                if s_val != 0 and s_val * rate <= val: # dn상태가 rate_dn 조건을 만족하지 않음.
                    term = list(np.logical_not(term))
            elif is_up and dn: # up -> dn 으로 변환
                is_up, is_change = False, True
                if s_val != 0 and s_val * rate > val: # up상태가 rate_up 조건을 만족하지 않음.
                    term = list(np.logical_not(term))

        if (is_up and s_val * rate > e_val) or (not is_up and s_val * rate <= e_val):
            term = list(np.logical_not(term))
        updn += term
