...
def delete_bento_crd(ctx, bento, pod, download_url=None, selector=None, svcname=None, tolerations=None, labels=None, uid=None):
    uid = pod.split('-')[-1]
    manifest, _ = ctx.invoke(get_bento_manifest, bento=bento, download_url=download_url, selector=selector, svcname=svcname, tolerations=tolerations, labels=labels, uid=uid, silent=True)
    delete_from_yaml(manifest)
