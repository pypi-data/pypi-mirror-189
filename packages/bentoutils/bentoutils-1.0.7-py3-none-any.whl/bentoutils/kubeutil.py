...
def find_api_instance(obj, client=None):
    ...
    '''
    grp, ver = split_apiversion(obj['apiVersion'])
    ...
    if api_instance in ['ResourcesYataiAiV1alpha1Api','ServingYataiAiV1alpha1Api','ServingKnativeDevV1Api']:  #== 'ServingKnativeDevV1Api':
...
def invoke_api(api_instance, action, obj, **args):
    ...
    # get custom object attributes to pass to CustomObjectsApi functions
    group, ver = split_apiversion(obj['apiVersion'])
    plural = get_plural(obj['kind'])
    ...
    if hasattr(api_instance, fn_name):
        ...
        if is_custom_objects_api:
            if fn_name.startswith('create_namespaced_'):  # == 'create_namespaced_service':
                fn_name = 'create_namespaced_custom_object'
            elif fn_name.startswith('patch_namespaced'):  # == 'patch_namespaced_service':
                fn_name = 'patch_namespaced_custom_object'
            elif fn_name.startswith('delete_namespaced'):
                fn_name = 'delete_namespaced_custom_object'
    ...
    if is_custom_objects_api:
        if fn_name == 'create_namespaced_custom_object':
            return fn(group, ver, args['namespace'], get_plural(obj['kind']), obj)
        elif fn_name == 'patch_namespaced_custom_object':
            return fn(group, ver, args['namespace'], get_plural(obj['kind']), args['name'], obj)
        elif fn_name == 'delete_namespaced_custom_object':
            return fn(group=group, version=ver, namespace=args['namespace'], plural=plural, name=args['name'], body=obj)
...
def split_apiversion(apiVersion):
    grp, _, ver = apiVersion.partition('/')
    if ver == '':
        ver = grp
        grp = 'core'
