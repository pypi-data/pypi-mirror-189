...
def test_get_huggingface_dataset(gantry_dataset_obj):
    """
    Test get local edit diff
    """
    # overwrite the dataset name
    gantry_dataset_obj.dataset_name = "hf_test"

    hf_dataset = gantry_dataset_obj.get_huggingface_dataset()

    assert len(hf_dataset["train"]) == 40
    assert hf_dataset["train"].features == {
    ...
    assert hf_dataset["train"].builder_name == "hf_test"
...
def setup_resp_for_add_file(resp, commit_message, filename, file):
    resp.add(
        resp.POST,
...
@patch("uuid.uuid4", lambda: "test-uuid-1")
def test_create_commit_failure(datadir, gantry_dataset_obj, commit_history):
    """
...
def test_add_file(
    ...
    with responses.RequestsMock() as resp:
        ...
        setup_resp_for_add_file(
            ...
            expected_filename,
            open(datadir / "test_file.txt", read_method),
        )
...
@pytest.mark.parametrize(("function"), [("push_file"), ("push_tabular_file"), ("push_dataframe")])
def test_add_item_good_parent(gantry_dataset_obj, function, commit_history, add_file_commit):
    ...
    with responses.RequestsMock() as resp:
        ...
        resp.add(
            ...
            json={
                "response": "ok",
                "data": add_file_commit,
            },
            headers={"Content-Type": "application/json"},
        )

        if function == "push_file":
            new_commit = gantry_dataset_obj.push_file(input, parent_version_id=parent_commit_id)
        elif function == "push_tabular_file":
            new_commit = gantry_dataset_obj.push_tabular_file(
                input, parent_version_id=parent_commit_id
            )
        elif function == "push_dataframe":
        ...
        new_commit["version_id"] == add_file_commit["id"]
        new_commit["dataset"] == gantry_dataset_obj.dataset_name
        new_commit["message"] == "add file test commit"
