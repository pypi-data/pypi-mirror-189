...
from grai_source_snowflake.models import ID, Column, Constraint, Edge, Table, ColumnID, TableID
...
def build_grai_metadata_from_edge(current: Edge, version: Literal["v1"] = "v1") -> GenericEdgeMetadataV1:
    ...
    if isinstance(current.source, TableID) and isinstance(current.destination, ColumnID):
        data["edge_type"] = EdgeTypeLabels.table_to_column.value
        return TableToColumnMetadata(**data)
    elif isinstance(current.source, ColumnID) and isinstance(current.destination, ColumnID):
