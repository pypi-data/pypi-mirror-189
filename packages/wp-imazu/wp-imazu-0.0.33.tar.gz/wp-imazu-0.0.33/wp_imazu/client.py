...
class ImazuClient(WpSocketClient):
    ...
    def __init__(self, host: str, port: int, **kwds):
        ...
        self.async_receive_handler = self._async_packets_handler
    ...
    async def async_send_wait(self, packet: bytes):
        ...
        while packet_data.inc_try() <= self._send_retry_cnt:
            ...
            if not result:
                continue
            try:
                await asyncio.wait_for(packet_data.wait(),
    ...
    async def _async_packets_handler(self, packets: bytes):
    ...
    def _parse_packets(self, packets: bytes):
        ...
        while packets:
            ...
            if (start_idx := packets.rfind(_PACKET_HEADER, 0, end_idx)) == -1:
                _LOGGER.debug('parse unknown start packet, %s', packets[:end_idx + 1].hex())
                packets = packets[end_idx + 1:]
                continue
            if start_idx != 0:
                _LOGGER.debug('parse invalid start index, %s', packets[:start_idx].hex())
