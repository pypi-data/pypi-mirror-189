...
class host:
    def http(id_key):
        ...
        f.write("{}")
        f.close()
        db = pickledb.load('check-host.json', False)
    ...
    def http_list(id_key, result_key):
        ...
        for server_list in json.loads(result_key):
            try:
                ...
                data = f"{id_key['nodes'][server_list][1]}, {id_key['nodes'][server_list][2]}", "Connect Timeout", "0.00s", "408", "unknown IP"
                r.append(server_list)
                db.set(server_list, data)
        db.set("nodes", r)
        db.dump()
