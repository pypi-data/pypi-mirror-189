...
def write_qi_path_conf(directory, sdk_dirs, sdk_layout=True):
    """
    Write the <build>/sdk/share/qi/path.conf file.
    This file can be used for instance by qi::path::find() functions,
    to find files from the dependencies' build directories.
    """
    to_write = ""
...
def convert_qitest_cmake_to_json(qitest_cmake_path, qitest_json_path):
    """
    Convert the qitest.cmake file generated by/from CMake to a qitest.json file readable by qitest tool.
    This method can be called from outside with:
    python -c 'import qibuild.project;
               qibuild.project.convert_qitest_cmake_to_json("/build_dir/qitest.cmake", "/build_dir/sdk/qitest.json")'
    :param qitest_cmake_path: Path to the qitest.cmake file generated by CMake
    :param qitest_json_path: Path to the qitest.json file to be written
    :return:
     """
    tests = list()
    ...
    for line in lines:
        ...
        tests.append(test)
    with open(qitest_json_path, "w") as fp:
...
class BuildProject(object):
    ...
    def build_directory(self):
        """
        Return a suitable build directory, depending on the build setting
        of the worktree: the build config, and whether or not a build prefix is set.
        """
    ...
    def cmake_args(self):
        """
        The list of CMake arguments to use when configuring the project.
...
""" % qibuild_python
    ...
    def generate_qitest_json(self):
        """ Generate QiTest JSON """
        convert_qitest_cmake_to_json(os.path.join(self.build_directory, "qitest.cmake"), self.qitest_json)

    def build(self, rebuild=False, target=None, coverity=False, env=None):
        """ Build the project. """
    ...
    def to_test_project(self):
        ...
        if not res.tests:
            ui.error("No tests were found in qitest.json.\nAdd some in the CMakeLists using qi_add_test() or similar")
        res.name = self.name
        return res

    def fix_shared_libs(self, paths):
        """
        Do some magic so that shared libraries from other
        projects and packages from toolchains are found.
...
"""
    ...
    def get_build_dirs(self, all_configs=False):
        ...
        for k in ret:
            if build_prefix:
                ret[k] = [os.path.join(build_prefix, x, self.name) for x in ret[k]] # pylint: disable=modified-iterating-dict
            else:
                ret[k] = [os.path.join(self.path, x) for x in ret[k]] # pylint: disable=modified-iterating-dict
        return ret
