...
class NioConan(ConanFile):
    ...
    def apply_conandata_requires(self, name, filter=None):
        ...
        if isinstance(filter, dict):
            filter_func = lambda name : filter[name] if name in filter else False
    ...
    def source(self):
        try:
            ...
            url = info['url']
            if url[:4] == 'git@':
                branch = info['branch'] if 'branch' in info else self.version
    ...
    def disk_image(self, images_output="disk.img"):
        ...
        def part_args(part, value):
            ...
            if 'packages' in value:
                prefix = value['prefix'] if 'prefix' in value else "/usr"
        ...
        ld_paths = [os.path.join(sel4_sysroot, 'lib')]
