...
class PGPHelper(metaclass=SingletonMeta):
    ...
    def encrypt_xml(self, payload: str) -> str:
        # validate the structure of the xml
        try:
            data = ET.fromstring(payload)

            if data is None:
                # FIXME: show a more detailed error
            ...
            logger.error(f"XML is not valid: {e}")
            raise EncryptionError(f"Invalid XML: {e}")

    def encrypt_zip(self, file_path: str | os.PathLike, output: str | os.PathLike):
        try:
            zip_file = zipfile.ZipFile(file_path)
            errors = zip_file.testzip()

            if errors:
                raise Exception("Zip file contains error")
            ...
            raise EncryptionError("Cannot encrypt zip file")
