...
(import chiakilisp.proxies.keyword)  ;; import this for a `keyword?`
...
(defn nil? (x)               ;; Returns true if 'x' is a NoneType
 (= x nil))
(defn int? (x)               ;; Returns true if 'x' is integer number
 (isinstance x int))
...
(defn count (x)              ;; Returns number of a collection items.
 (.__len__ x))

(defn collection? (coll)    ;; Returns true if x is collection type.
  (or
...
(defn get (& args)           ;; Safely get item from collection type.
...
(defn get-in (& args)        ;; Same as get, but goes through a path.
 (when args
...
(defn assoc                ;; Behaves the same as assoc from Clojure.
