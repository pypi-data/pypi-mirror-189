...
@_server.route('/model', methods=['GET'])
@exception_handler
def _model():
    data = request.json
    project = data.get('project')
    ...
    try_fetch = data.get('try_fetch', True)
    params = data.get('params') or {}
    predictions, model = _manager.model( project, label_config, force_reload, try_fetch, **params)
#     response = {
...
def _toolbar_predict():
    ...
    try_fetch = data.get('try_fetch', True)
    params = data.get('params') or {}
    predictions, model = _manager.toolbar_predict(image, clickpoint, polygons, vertices,project, label_config,force_reload, try_fetch, **params)
    response = {
        'results': predictions,
        'model_version': model.model_version
    }
