...
class INA226:
    ...
    def reset(self):
        ...
        log.info("config register: 0x%02x, value: 0x%04x" % (self.__REG_CONFIG, self._read_configuration()))
        log.info("Calibration: 0x%02x, value: 0x%04x" % (self.__REG_CALI, self.__read_register(self.__REG_CALI)))
        log.info("mask register: 0x%02x, value: 0x%04x" % (self.__REG_MASK, self._read_mask_register()))
        log.info("limit register: 0x%02x, value: 0x%04x" % (self.__REG_LIMIT, self._read_limit_register()))
        log.info("manufacturer id: 0x%02x, value: 0x%04x" % (self.__REG_MANUFACTURER_ID, self._manufacture_id()))
        log.info("die id: 0x%02x, value: 0x%04x" % (self.__REG_DIE_ID, self._die_id()))
    ...
    def _calibrate(self, bus_volts_max, shunt_volts_max,
        ...
        log.info("max possible current: %.2fA" %
                         max_possible_amps)

        self._current_lsb = \
            self._determine_current_lsb(max_expected_amps, max_possible_amps)
        log.info("current LSB: %.3e A/bit" % self._current_lsb)

        self._power_lsb = self._current_lsb * 25.2
        log.info("power LSB: %.3e W/bit" % self._power_lsb)

        max_current = self._current_lsb * self.__MAX_CURRENT_VALUE
        log.info("max current before overflow: %.4fA" % max_current)

        max_shunt_voltage = max_current * self._shunt_ohms
        log.info("max shunt voltage before overflow: %.4fmV" %
    ...
    def _determine_current_lsb(self, max_expected_amps, max_possible_amps):
        if max_expected_amps is not None:
            ...
            log.info("max expected current: %.3fA" %
                             max_expected_amps)
            ...
            current_lsb = max_possible_amps / self.__CURRENT_LSB_FACTOR
        log.info("expected current LSB base on max_expected_amps: %.3e A/bit" % current_lsb)
        if current_lsb < self._min_device_current_lsb:
            current_lsb = self._min_device_current_lsb
            log.info("current_lsb is less equal than min_device_current_lsb, use the latter")
        return current_lsb
    ...
    def _configuration_register(self, register_value):
        log.debug("configuration: 0x%04x" % register_value)
    ...
    def _calibration_register(self, register_value):
        log.debug("calibration: 0x%04x" % register_value)
    ...
    def _mask_register(self, register_value):
        log.debug("mask/enable: 0x%04x" % register_value)
    ...
    def _limit_register(self, register_value):
        log.debug("limit value: 0x%04x" % register_value)
