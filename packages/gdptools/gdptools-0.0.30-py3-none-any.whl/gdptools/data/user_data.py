...
class ODAPCatData(UserData):
    ...
    def __init__(
        ...
        *,
        ...
        self.rotate_ds = bool((not is_intersect) & is_degrees & (not is_0_360))
...
class UserCatData(UserData):
    ...
    def __init__(
        ...
        self.var = var
        tcrs = _get_crs(self.f_feature.crs)
        self.proj_feature = proj_feature
        spcrs = _get_crs(self.proj_feature)
        if tcrs != spcrs:
            raise ValueError(f"f_feature.crs {tcrs} not equal to proj.feature {spcrs}")
        self.id_feature = id_feature
