...
def pad_sequence_paddle_old(sequences, padding_value=0, fix_len=None):
...
def pad_sequence_paddle(sequences, padding_value=0, fix_len=None):
    """Fill sequences(variable) into a fixed-length matrix"""
    max_size = sequences[0].shape
    trailing_dims = max_size[1:]
    max_len = max([s.shape[0] for s in sequences])
...
def stripe_old(x, n, w, offset=(0, 0), dim=1):
...
def stripe(x, n, w, offset=(0, 0), dim=1):
    ...
    if dim == 1:
        for i in range(n):
            output.append(x[row + i:row + i + 1, col + i:col + i + w])
        output = paddle.concat(output, axis=0)
    elif dim == 0:
        for i in range(n):
            output.append(x[row + i:row + i + w, col + i])
        output = paddle.stack(output, axis=0)
    else:
        raise ValueError("error dim in stripe")
    return output


def diagonal(x, offset=0):
    """return the diagonal of x
    ...
    [[ 9, 21],
     ...
     [11, 23]]
    """
    row_len, col_len = x.shape[:2]
    if offset >= 0:
        diagonal_len = min(row_len, col_len - offset)
        output = x[list(range(diagonal_len)), list(range(offset, offset + diagonal_len))]

    else:
        diagonal_len = min(row_len + offset, col_len)
        output = x[list(range(-offset, diagonal_len - offset)), list(range(diagonal_len))]
    output = output.transpose((*list(range(1, len(output.shape))), 0))
    return output
