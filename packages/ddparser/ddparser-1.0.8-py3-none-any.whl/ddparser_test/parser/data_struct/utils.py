...
def eisner(scores, lens):
    ...
    Args：
        ...
        lens: lens matrix，shape=(batch, 1)
    ...
    s_i = paddle.full_like(scores, 0)
    s_i += float('-inf')
    # score for complete span
    s_c = paddle.full_like(scores, 0)
    s_c += float('-inf')
    # incompelte span position for backtrack
    p_i = paddle.zeros((seq_len, seq_len, batch_size), dtype='int64')
    # compelte span position for backtrack
    p_c = paddle.zeros((seq_len, seq_len, batch_size), dtype='int64')
    # set 0 to s_c.diagonal
    s_c.fill_diagonal_tensor_(paddle.zeros((s_c.shape[2], max(s_c.shape[:2]))))
    ...
    for w in range(1, seq_len):
        ...
        n = seq_len - w
        starts = paddle.arange(n, dtype='int64').unsqueeze(0)
        # ilr = C(i->r) + C(j->r+1)
        ...
        il = ilr + nn.diagonal(scores, -w).unsqueeze(-1)
        # I(j->i) = max(C(i->r) + C(j->r+1) + s(j->i)), i <= r < j
        il_span, il_path = il.max(-1), il.argmax(-1)
        start = time.time()
        s_i = s_i.fill_diagonal_tensor_(il_span, offset=-w)
        p_i = p_i.fill_diagonal_tensor_(il_path + starts, offset=-w)
        end = time.time()
        a.append(end - start)
        print("fill_diagonal paddle:", sum(a))
        ir = ilr + nn.diagonal(scores, w).unsqueeze(-1)
        # I(i->j) = max(C(i->r) + C(j->r+1) + s(i->j)), i <= r < j
        ir_span, ir_path = ir.max(-1), ir.argmax(-1)

        s_i = s_i.fill_diagonal_tensor_(ir_span, offset=w)
        p_i = p_i.fill_diagonal_tensor_(ir_path + starts, offset=w)

        # C(j->i) = max(C(r->i) + I(j->r)), i <= r < j
        cl = nn.stripe(s_c, n, w, (0, 0), 0) + nn.stripe(s_i, n, w, (w, 0))
        cl = cl.transpose((2, 0, 1))
        cl_span, cl_path = cl.max(-1), cl.argmax(-1)
        s_c = s_c.fill_diagonal_tensor_(cl_span, offset=-w)
        p_c = p_c.fill_diagonal_tensor_(cl_path + starts, offset=-w)

        # C(i->j) = max(I(i->r) + C(r->j)), i < r <= j
        cr = nn.stripe(s_i, n, w, (0, 1)) + nn.stripe(s_c, n, w, (1, w), 0)
        cr = cr.transpose((2, 0, 1))
        cr_span, cr_path = cr.max(-1), cr.argmax(-1)
        s_c = s_c.fill_diagonal_tensor_(cr_span, offset=w)
        for i in range(len(lens)):
            s_c[0, w, paddle.fluid.layers.where(lens != w)] = float('-inf')
        p_c = p_c.fill_diagonal_tensor_(cr_path + starts + 1, offset=w)
    ...
    for i, length in enumerate(lens.tolist()):
        heads = paddle.ones([length + 1], dtype='int64')
        nn.backtrack(p_i[i], p_c[i], heads, 0, length, True)
        predicts.append(heads)

    return nn.pad_sequence_paddle(predicts, fix_len=seq_len)


b = []


def eisner_old(scores, lens):
    """Eisner algorithm is a general dynamic programming decoding algorithm for bilexical grammar.
    ...
    for w in range(1, seq_len):
        ...
        # ilr = C(i->r) + C(j->r+1)
        ilr = nn.stripe_old(s_c, n, w) + nn.stripe_old(s_c, n, w, (w, 1))
        ...
        b.append(end - start)
        print("fill_diagonal numpy:", sum(b))
        ...
        # C(j->i) = max(C(r->i) + I(j->r)), i <= r < j
        cl = nn.stripe_old(s_c, n, w, (0, 0), 0) + nn.stripe_old(s_i, n, w, (w, 0))
        cl = cl.transpose(2, 0, 1)
        cl_span, cl_path = cl.max(-1), cl.argmax(-1)
        ...
        # C(i->j) = max(I(i->r) + C(r->j)), i < r <= j
        cr = nn.stripe_old(s_i, n, w, (0, 1)) + nn.stripe_old(s_c, n, w, (1, w), 0)
        cr = cr.transpose(2, 0, 1)
        cr_span, cr_path = cr.max(-1), cr.argmax(-1)
