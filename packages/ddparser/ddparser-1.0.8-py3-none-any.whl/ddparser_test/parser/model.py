...
def epoch_train(args, model, optimizer, loader, epoch):
    ...
    for batch, inputs in enumerate(loader(), start=1):
        ...
        logging.info("epoch: {}, batch: {}/{}, batch_size: {}, loss: {:.4f}".format(epoch, batch,
                                                                                    math.ceil(len(loader)), len(words),
                                                                                    loss.numpy().item()))
...
def decode(args, s_arc, s_rel, mask):
    ...
    # mask = mask.numpy()
    lens = paddle.sum(mask, -1)
    # prevent self-loops
    arc_preds = layers.argmax(s_arc, -1)
    start = time.time()
    bad = [not utils.istree(seq[:i + 1]) for i, seq in zip(lens.numpy(), arc_preds.numpy())]
    end = time.time()
    hhh.append(end - start)
    start = time.time()
    bad = [not utils.istree(seq[:i + 1]) for i, seq in zip(lens, arc_preds)]
    end = time.time()
    jjj.append(end - start)
    print("遍历numpy:", sum(hhh))
    print("遍历paddle:", sum(jjj))

    # bad = [True]
    if args.tree and any(bad):

        bad_index = paddle.to_tensor([i for i in range(len(bad)) if bad[i]])
        start = time.time()
        utils.eisner_old(s_arc.numpy()[bad], lens.numpy()[bad])
        end = time.time()
        xxx.append(end - start)
        start = time.time()

        eisner_result = utils.eisner(s_arc.index_select(bad_index), lens.index_select(bad_index))
        end = time.time()
        yyy.append(end - start)
        print("eisner numpy:", sum(xxx))
        print("eisner paddle:", sum(yyy))
        if len(bad_index) == 1:
            eisner_result = eisner_result.squeeze(0)
        arc_preds[bad_index] = eisner_result
