...
from ddparser.ernie.file_utils import _fetch_from_remote, add_docstring
...
def _build_linear(n_in, n_out, name, init, act=None):
    return D.Linear(n_in,
                    ...
                    param_attr=F.ParamAttr(
                        name='%s.w_0' % name if name is not None else None, initializer=init),
...
class AttentionLayer(D.Layer):
    def __init__(self, cfg, name=None):
        ...
        self.k = _build_linear(
            d_model, d_model_q, append_name(name, 'key_fc'), initializer)
...
class PositionwiseFeedForwardLayer(D.Layer):
    def __init__(self, cfg, name=None):
        ...
        self.i = _build_linear(d_model, d_ffn, append_name(
            name, 'fc_0'), initializer, act=cfg['hidden_act'])
        self.o = _build_linear(
            d_ffn, d_model, append_name(name, 'fc_1'), initializer)
...
class ErnieBlock(D.Layer):
    ...
    def forward(self, inputs, attn_bias=None, past_cache=None):
        attn_out, cache = self.attn(
            inputs, inputs, inputs, attn_bias, past_cache=past_cache)  # self attn
...
class ErnieEncoderStack(D.Layer):
    def __init__(self, cfg, name=None):
        ...
        self.block = D.LayerList(
            [ErnieBlock(cfg, append_name(name, 'layer_%d' % i)) for i in range(n_layers)])
...
class ErnieModel(D.Layer, PretrainedModel):
    def __init__(self, cfg, name=None):
        ...
        self.pos_emb = D.Embedding([d_pos, d_emb],
                                   param_attr=F.ParamAttr(name=append_name(name, 'pos_embedding'),
                                                          initializer=initializer))
        self.sent_emb = D.Embedding([d_sent, d_emb],
                                    param_attr=F.ParamAttr(name=append_name(name, 'sent_embedding'),
                                                           initializer=initializer))
        ...
        self.encoder_stack = ErnieEncoderStack(
            cfg, append_name(name, 'encoder'))
        if cfg.get('has_pooler', True):
            self.pooler = _build_linear(cfg['hidden_size'],
                                        cfg['hidden_size'],
    ...
    def forward(self,
        ...
        assert len(src_ids.shape) == 2, 'expect src_ids.shape = [batch, sequecen], got %s' % (
            repr(src_ids.shape))
        ...
        if pos_ids is None:
            pos_ids = L.reshape(
                L.range(0, d_seqlen, 1, dtype='int32'), [1, -1])
        ...
        if attn_bias is None:
            ...
            if use_causal_mask:
                ...
                causal_mask = L.cast(
                    (L.matmul(sequence, 1. / sequence, transpose_y=True) >= 1.), 'float32')
                attn_bias *= causal_mask
        else:
            assert len(
                attn_bias.shape) == 3, 'expect attn_bias tobe rank 3, got %r' % attn_bias.shape
        attn_bias = (1. - attn_bias) * -10000.0
        attn_bias = L.unsqueeze(attn_bias, [1])
        # avoid broadcast =_=
        attn_bias = L.expand(attn_bias, [1, self.n_head, 1, 1])
...
class ErnieModelForSequenceClassification(ErnieModel):
    ...
    def __init__(self, cfg, name=None):
        super(ErnieModelForSequenceClassification,
              self).__init__(cfg, name=name)

        initializer = F.initializer.TruncatedNormal(
            scale=cfg['initializer_range'])
        self.classifier = _build_linear(
            cfg['hidden_size'], cfg['num_labels'], append_name(name, 'cls'), initializer)
...
class ErnieModelForTokenClassification(ErnieModel):
    ...
    def forward(self, *args, **kwargs):
        ...
        pooled, encoded = super(
            ErnieModelForTokenClassification, self).forward(*args, **kwargs)
...
class ErnieModelForQuestionAnswering(ErnieModel):
    ...
    def __init__(self, cfg, name=None):
        ...
        initializer = F.initializer.TruncatedNormal(
            scale=cfg['initializer_range'])
        self.classifier = _build_linear(
            cfg['hidden_size'], 2, append_name(name, 'cls_mrc'), initializer)
    ...
    def forward(self, *args, **kwargs):
        ...
        pooled, encoded = super(
            ErnieModelForQuestionAnswering, self).forward(*args, **kwargs)
...
class NSPHead(D.Layer):
    def __init__(self, cfg, name=None):
        ...
        initializer = F.initializer.TruncatedNormal(
            scale=cfg['initializer_range'])
        self.nsp = _build_linear(
            cfg['hidden_size'], 2, append_name(name, 'nsp_fc'), initializer)
...
class ErnieModelForPretraining(ErnieModel):
    ...
    def forward(self, *args, **kwargs):
        ...
        logits_2d = L.matmul(encoded_2d, self.word_emb.weight,
