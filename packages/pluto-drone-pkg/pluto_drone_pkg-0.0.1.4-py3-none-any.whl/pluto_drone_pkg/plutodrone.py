...
class plutodrone:
    ...
    def __init__(self,TCP_IP = "192.168.4.1",TCP_PORT = 23):
        ...
        check=self.mySocket.connect((self.TCP_IP, self.TCP_PORT))
        ...
        self.yaw=1500
        print(check)
        # update block to check connection is successful or not
    ...
    # def box_takeoff(self):
    #     # self.disarm()
    #     # self.rcvalues[9]=220
    #     # self.rcvalues[10]=5
    #     # self.arm()
    #     self.cmdvalues[5] = 1
    #     self.cmdvalues[7] = 217^1^2
    #     self.send(self.cmdvalues)

    def land_msp(self):
        self.cmdvalues[5] = 2
        self.cmdvalues[7] = 217^2^2
        self.send(self.cmdvalues)
    ...
    def converttobytes(self,command):
        temp=bytearray([])
        temp.append(command%256)
        temp.append(math.floor(command/256))
        return temp
