...
def get_feature_view_view_schema(
    ...
    # TEC-11798 materialization_context must be created AFTER the transformation spec since we deserialize the transformation in the main scope.
    materialization_context_ = materialization_context.BoundMaterializationContext._create_internal(
    ...
    sql_str = pipeline_helper.pipeline_to_sql_string(
        ...
        materialization_context=materialization_context_,
    )
    cur.execute(sql_str)
    # Get the schema from the previously ran query
    query_id = cur.sfqid
    ...
    for row in schema_list:
        ...
        column_proto = core_schema_derivation_utils.TECTON_TYPE_TO_COLUMN_TYPE[str(tecton_type)]
        column_proto.raw_snowflake_type = raw_snowflake_type
