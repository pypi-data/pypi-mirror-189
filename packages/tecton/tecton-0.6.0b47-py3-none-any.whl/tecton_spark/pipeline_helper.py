...
def _populate_empty_passed_in_inputs_helper(
    node: PipelineNode,
    empty_passed_in_inputs: Dict[str, specs.DataSourceSpec],
    ds_map: Dict[str, specs.DataSourceSpec],
    spark: SparkSession,
):
    if node.HasField("data_source_node"):
        ...
        assert (
            ds_spec.type != DataSourceType.PUSH_WITH_BATCH
        ), "This utility does not support FeatureView with PushSources."
