...
def TECTON_OBJECT_REQUIRES_VALIDATION(function_name: str, example_instance_name: str):
    return TectonValidationError(
        f"This object must be validated before `{function_name}` can be called. Call `{example_instance_name}.validate()` on this instance first, or enable automatic validation."
    )


def AUTOMATIC_VALIDATION_FAILED(function_name: str, example_instance_name: str):
    return TectonValidationError(
        f"This object must be validated before `{function_name}` can be called. Validation was attempted and failed. Please fix the above errors and trying calling this method again or `{example_instance_name}.validate()`."
    )


def INVALID_USAGE_FOR_LOCAL_TECTON_OBJECT(function_name: str):
    return TectonValidationError(
        f"`{function_name}` can only be called on Tecton objects that have been applied to the Tecton backend. This object was defined locally."
    )


def INVALID_USAGE_FOR_REMOTE_TECTON_OBJECT(function_name: str):
    return TectonValidationError(
        f"`{function_name}` can only be called on Tecton objects that have been defined locally. This object was fetched from the Tecton backend."
    )


def INVALID_USAGE_FOR_LOCAL_FEATURE_TABLE_OBJECT(function_name: str):
    return TectonValidationError(
        f"`{function_name}` can only be called on Tecton Feature Tables that have been applied to the Tecton backend. This object was defined locally, which means this Feature Table cannot be materialized. Feature tables require materialization in order to ingest features and perform feature retrieval."
    )
