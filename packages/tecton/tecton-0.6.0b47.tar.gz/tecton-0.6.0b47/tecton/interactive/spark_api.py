...
def get_historical_features_for_feature_service(
    ...
    tree = builder.build_feature_set_config_querytree(
        feature_set_config,
        nodes.UserSpecifiedDataNode(TectonDataFrame._create(spine)).as_ref(),
        timestamp_key,
        from_source,
    )
...
def derive_materialization_schema_for_feature_view(
    ...
    spark = TectonContext.get_instance()._spark
    is_aggregate = len(fv_args.materialized_feature_view_args.aggregations) > 0
    if not is_aggregate:
        return schema_derivation_utils.get_feature_view_view_schema(spark, fv_args, transformations, data_sources)

    join_keys = []
    for entity in fv_args.entities:
        join_keys.extend(entity.join_keys)

    # TODO: call schema_derivation_utils and derive materialization schema for aggregates
    return schema_derivation_utils.get_feature_view_view_schema(spark, fv_args, transformations, data_sources)
