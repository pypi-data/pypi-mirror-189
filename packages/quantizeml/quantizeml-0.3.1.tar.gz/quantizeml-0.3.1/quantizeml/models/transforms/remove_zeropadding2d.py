...
from ..utils import (deep_clone_model, _get_layers, _get_layer_index, _inbound_node_generator,
                     _replace_layer_name_for_connection_info, get_layers_by_type)
...
def _get_zeropadding_less_model(model, map_zeropadding_next):
    ...
    for zeropadding, next_layer in map_zeropadding_next.items():
        ...
        next_index = _get_layer_index(layers, next_layer.name)
        ...
        zeropadding_index = _get_layer_index(layers, zeropadding.name)
        ...
        # inbound > outbounds.
        for inbound_node in _inbound_node_generator(layers[next_index]):
            if isinstance(inbound_node, dict):
                inbound_node = inbound_node.values()
            for connection_info in inbound_node:
                _replace_layer_name_for_connection_info(connection_info, zeropadding.name,
                                                        updated_inbound)

    # Remove ZeroPadding2D layers
    layers_to_remove = _get_layers(config, [zp.name for zp in map_zeropadding_next.keys()])
...
def remove_zeropadding2d(model):
    ...
    updated_model = _get_zeropadding_less_model(clone_model, map_zeropadding_next)
