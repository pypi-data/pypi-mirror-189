...
class QuantizedShiftmax(keras.layers.Layer):
    ...
    def call(self, x):
        ...
        if not isinstance(x, (FixedPoint, tf.Tensor)):
            raise TypeError("QuantizedShiftmax only accepts FixedPoint or" +
                            f"tf.Tensor inputs. Receives {type(x)} inputs.")

        if isinstance(x, tf.Tensor):
            # Assume the inputs are integer stored as float, which is the only
            # tf.Tensor inputs that are allowed
            x = FixedPoint(x, 8, 0)
        ...
        outputs = FixedPoint(outputs.values, self.exp_bitwidth + 1, self.exp_bitwidth)
