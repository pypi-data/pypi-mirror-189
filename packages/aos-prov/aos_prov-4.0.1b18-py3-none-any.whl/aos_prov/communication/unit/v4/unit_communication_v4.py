...
class UnitCommunicationV4:
    ...
    def clear(self, certificate_type: str, node_id: str) -> None:
        with self.unit_provisioning_stub() as stub:
            print('Clear certificate: ' + certificate_type + 'on Node ID: ' + node_id)
    ...
    def set_cert_owner(self, certificate_type: str, password: str, node_id: str) -> None:
        with self.unit_provisioning_stub() as stub:
            print('Set owner: ' + certificate_type + 'on Node ID: ' + node_id)
    ...
    def get_cert_types(self, node_id: str) -> [str]:
        with self.unit_provisioning_stub() as stub:
            print(f'Getting certificate types to renew on node {node_id}...')
    ...
    def encrypt_disk(self, password: str, node_id: str):
        print(f'Starting disk encryption on node {node_id}...')
        try:
            with self.unit_provisioning_stub(wait_for_close=True) as stub:
                ...
                print('Encryption process has been finished.')
    ...
    def wait_for_connection(self):
        try:
            ...
            grpc.channel_ready_future(grpc.insecure_channel(self.__unit_address)).result(timeout=800)
