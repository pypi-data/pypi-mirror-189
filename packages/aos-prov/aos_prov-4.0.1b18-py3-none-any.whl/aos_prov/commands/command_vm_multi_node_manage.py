...
def _forward_provisioning_ports(network_name: str, forward_to_port=None) -> int:
    ...
    command = [
        ...
        network_name,
        '--port-forward-4',
        f'provisioningPortForward:tcp:[0.0.0.0]:{forward_to_port}:[10.0.0.100]:8089',
    ]
...
def new_vm(vm_name: str, disk_location: str):
    ...
    if not ((disk_location_path / NODE0_IMAGE_FILENAME).exists()) or not ((disk_location_path / NODE1_IMAGE_FILENAME).exists()):
    ...
    provisioning_port = _forward_provisioning_ports(units_network_name)
