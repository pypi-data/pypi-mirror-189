...
class legacy:
    ...
    def run_hpu(self, pipeManager, input_list, input_type, rand_crp, host_buffer, bypass_host_buffers, out_dev_buffer_list):
        """
        ...
        hb_pp[0] = c_void_p(0)
        for i in range(num_host_buffer - decoder_node_offset):
            pointer = host_buffer[i].__array_interface__['data'][0]
            hb_pp[i + decoder_node_offset] = c_void_p(pointer)
    ...
    def get_output(self):
        ...
        self.run_hpu_q.pop(0)
        self.run_hpu_q.pop(0)
        if(len(self.run_hpu_q) > (3*self.max_queue_size)):
