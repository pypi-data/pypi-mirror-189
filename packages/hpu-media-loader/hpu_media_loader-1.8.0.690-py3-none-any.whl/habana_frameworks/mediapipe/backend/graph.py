...
class graph_processor(object):
    ...
    def process_and_validate_graph(self, batch_size, queue_depth):
        """
        ...
        ops = self._ops_
        params = opnode_params(batch_size, queue_depth)
        for op in ops:
    ...
    def segment_graph(self):
        ...
        for o in self._output_tensors_:
    ...
    def process_recipe(self):
        if (self._hpu_tensor_info_ != None):
        ...
        input_tensors, output_tensors = self._mm_.get_hpu_tensor_info(self.get_recipe(),
                                                                      num_outputs,
                                                                      num_inputs)
        ...
        for ot in output_tensors:
            for hot in hpu_graph.output_unique_tensors:
                ...
                if(ot.name == hot.src_op.name+"_"+str(hot.src_port)):
                    hot.shape = ot.shape
        ...
        for o in self._output_tensors_:
            mapped = False
            for i in range(len(hpu_graph.output_tensors)):
                if(o.name == hpu_graph.output_tensors[i].name):
