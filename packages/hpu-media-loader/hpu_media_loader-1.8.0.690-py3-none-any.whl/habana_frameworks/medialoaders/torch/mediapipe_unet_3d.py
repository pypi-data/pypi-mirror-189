...
class Unet3dMediaPipe(MediaPipe):
    ...
    def __init__(self, a_device=None, a_batch_size=1, a_prefetch_count=1, a_num_instances=1, a_instance_id=0, a_pipeline=None, a_enable_zoom=False, **kwargs):
        """
        Constructor method.
        ...
        if (self.pipeName != "TrainPipeline") and (self.pipeName != "BenchmarkPipeline_Train"):
            raise ValueError(
        ...
        if(seed_mediapipe == None):
            if (self.num_instances > 1) and (self.pipeName == "TrainPipeline") and (train_shuffle_across_dataset == True):
                raise ValueError(
        ...
        if (self.pipeName == "TrainPipeline"):
            ...
            self.img_reshape_transpose_pre_blur = fn.Reshape(size=shape,
                                                             ...
                                                             dtype=dt.FLOAT32)
            # ------------conv1 along W --------------------------------
            self.img_transpose_pre_blur1 = fn.Transpose(permutation=[4, 3, 2, 0, 1],
                                                        tensorDim=5,
                                                        dtype=dt.FLOAT32)  # WHDCN -> NCDWH

            shape_patch = self.patch_size.copy()
            shape = []
            shape.append(self.batch_size * image_num_channel *
                         shape_patch[2])  # NCD
            shape.append(shape_patch[0])  # W
            shape.append(shape_patch[1])  # H
            shape.append(1)

            self.img_reshape_pre_blur1 = fn.Reshape(size=shape,
                                                    tensorDim=len(shape),
                                                    layout='',
                                                    dtype=dt.FLOAT32)
            ...
            padL1, padR1, padT1, padB1 = get_pad_params(shape_patch[0],
                                                        shape_patch[1],
                                                        kW,
                                                        1)
            self.pad1 = fn.Pad(mode=1,
            ...
            priv_params = {}
            priv_params['batch_size'] = self.batch_size
            priv_params['channels'] = image_num_channel
            priv_params['min_sigma'] = min_sigma
            priv_params['max_sigma'] = max_sigma

            priv_params['input_depth'] = shape_patch[2]
            self.gaussian_kernel1 = fn.MediaFunc(func=gaussian_kernel_func,
                                                 shape=[
                                                     (shape_patch[2] * image_num_channel * self.batch_size), 1, kW, 1],
                                                 dtype=dt.FLOAT32,
                                                 priv_params=priv_params)

            self.gaussian_blur1 = fn.GaussianBlur(maxSigma=max_sigma,
                                                  dtype=dt.FLOAT32)
            ...
            self.img_reshape_post_blur1 = fn.Reshape(size=shape,
                                                     tensorDim=len(
                                                         shape),
                                                     layout='',
                                                     dtype=dt.FLOAT32)

            self.img_transpose_post_blur1 = fn.Transpose(permutation=[3, 4, 2, 1, 0],
                                                         tensorDim=5,
                                                         dtype=dt.FLOAT32)  # NCDWH -> WHDCN

            # ------------conv2 along H --------------------------------
            self.img_transpose_pre_blur2 = fn.Transpose(permutation=[4, 3, 2, 1, 0],
                                                        tensorDim=5,
                                                        dtype=dt.FLOAT32)  # WHDCN -> NCDHW
            ...
            shape.append(shape_patch[0])  # W
            shape.append(1)
            self.img_reshape_pre_blur2 = fn.Reshape(size=shape,  # (NCD)HW1
                                                    tensorDim=len(shape),
                                                    layout='',
                                                    dtype=dt.FLOAT32)
            ...
            padL2, padR2, padT2, padB2 = get_pad_params(shape_patch[1],
                                                        shape_patch[0],
                                                        kW,
                                                        1)
            self.pad2 = fn.Pad(mode=1,
                               pads=[0, padL2, padT2, 0, 0, padR2, padB2, 0],
                               dtype=dt.FLOAT32)

            priv_params['input_depth'] = shape_patch[2]
            self.gaussian_kernel2 = fn.MediaFunc(func=gaussian_kernel_func,
                                                 shape=[
                                                     (shape_patch[2] * image_num_channel * self.batch_size), 1, kW, 1],
                                                 dtype=dt.FLOAT32,
                                                 priv_params=priv_params)

            self.gaussian_blur2 = fn.GaussianBlur(maxSigma=max_sigma,
                                                  dtype=dt.FLOAT32)  # (NCD),H,W,1
            ...
            shape.append(shape_patch[1])
            shape.append(shape_patch[0])
            self.img_reshape_post_blur2 = fn.Reshape(size=shape,
                                                     tensorDim=len(shape),
                                                     layout='',
                                                     dtype=dt.FLOAT32)  # N,C,D,H,W
            self.img_transpose_post_blur2 = fn.Transpose(permutation=[4, 3, 2, 1, 0],
                                                         tensorDim=5,
                                                         dtype=dt.FLOAT32)  # N,C,D,H,W -> WHDCN

            # -----------conv3 along D--------------------------------
            self.img_transpose_pre_blur3 = fn.Transpose(permutation=[4, 3, 1, 2, 0],
                                                        tensorDim=5,
                                                        dtype=dt.FLOAT32)  # WHDCN -> NCHDW

            shape_patch = self.patch_size.copy()
            shape = []
            shape.append(self.batch_size * image_num_channel *
                         shape_patch[1])  # NCH
            shape.append(shape_patch[2])  # D
            shape.append(shape_patch[0])  # W
            shape.append(1)
            self.img_reshape_pre_blur3 = fn.Reshape(size=shape,
                                                    tensorDim=len(shape),
                                                    layout='',
                                                    dtype=dt.FLOAT32)
            ...
            padL3, padR3, padT3, padB3 = get_pad_params(shape_patch[2],
                                                        shape_patch[0],
                                                        kW,
                                                        1)
            self.pad3 = fn.Pad(mode=1,
                               pads=[0, padL3, padT3, 0, 0, padR3, padB3, 0],
                               dtype=dt.FLOAT32)

            priv_params['input_depth'] = shape_patch[1]
            self.gaussian_kernel3 = fn.MediaFunc(func=gaussian_kernel_func,
                                                 shape=[
                                                     (shape_patch[1] * image_num_channel * self.batch_size), 1, kW, 1],
                                                 dtype=dt.FLOAT32,
                                                 priv_params=priv_params)
            self.gaussian_blur3 = fn.GaussianBlur(maxSigma=max_sigma,
                                                  dtype=dt.FLOAT32)  # (NCH)DW1
            ...
            shape.append(shape_patch[2])
            shape.append(shape_patch[0])
            self.img_reshape_post_blur3 = fn.Reshape(size=shape,
                                                     tensorDim=len(shape),
                                                     layout='',
                                                     dtype=dt.FLOAT32)  # N,C,H,D,W
            self.img_transpose_post_blur3 = fn.Transpose(permutation=[4, 2, 3, 1, 0],
                                                         tensorDim=5,
                                                         dtype=dt.FLOAT32)  # N,C,H,D,W -> WHDCN
            ...
            self.scale = fn.MediaFunc(func=random_scale_func,
                                      ...
                                      seed=seed_augment,
                                      priv_params=priv_params)
            self.min0 = fn.ReduceMin(reductionDimension=0, dtype=dt.FLOAT32)
            self.max0 = fn.ReduceMax(reductionDimension=0, dtype=dt.FLOAT32)
            self.min1 = fn.ReduceMin(reductionDimension=1, dtype=dt.FLOAT32)
            self.max1 = fn.ReduceMax(reductionDimension=1, dtype=dt.FLOAT32)
            self.min2 = fn.ReduceMin(reductionDimension=2, dtype=dt.FLOAT32)
            self.max2 = fn.ReduceMax(reductionDimension=2, dtype=dt.FLOAT32)
            self.min3 = fn.ReduceMin(reductionDimension=3, dtype=dt.FLOAT32)
            self.max3 = fn.ReduceMax(reductionDimension=3, dtype=dt.FLOAT32)
            self.mul = fn.Mult(dtype=dt.FLOAT32)
            self.img_reshape_to_scalar = fn.Reshape(size=[1],
                                                    tensorDim=1,
                                                    layout='',
                                                    dtype=dt.FLOAT32)
            ...
            if(self.batch_size == 2):
                self.img_reshape_post_reduce = fn.Reshape(size=[1, self.batch_size],
                                                          tensorDim=2,
                                                          layout='',
                                                          dtype=dt.FLOAT32)
                self.split = fn.Split(axis=1, dtype=dt.FLOAT32)
                shape = []
                shape.append(
    ...
    def definegraph(self):
        ...
        if (self.pipeName == "TrainPipeline"):
            ...
            if self.augment == True:
                ...
                apply_blur = self.gaussian_kernel_apply_blur()
                # ------------conv1 along W --------------------------------
                img = self.img_transpose_pre_blur1(img)  # WHDCN -> NCDWH
                img = self.img_reshape_pre_blur1(img)  # (NCD),W,H,1
                img = self.pad1(img)
                gaussian_k1 = self.gaussian_kernel1(sigmas, apply_blur)
                img = self.gaussian_blur1(img, gaussian_k1)  # (NCD),W,H,1
                img = self.img_reshape_post_blur1(img)  # N,C,D,W,H
                img = self.img_transpose_post_blur1(img)  # NCDWH -> WHDCN

                # ------------conv2 along H --------------------------------
                img = self.img_transpose_pre_blur2(img)  # WHDCN -> NCDHW
                img = self.img_reshape_pre_blur2(img)  # (NCD)HW1
                img = self.pad2(img)
                gaussian_k2 = self.gaussian_kernel2(sigmas, apply_blur)
                img = self.gaussian_blur2(img, gaussian_k2)  # (NCD),H,W,1
                img = self.img_reshape_post_blur2(img)  # N,C,D,H,W
                img = self.img_transpose_post_blur2(img)  # N,C,D,H,W -> WHDCN

                # -----------conv3 along D--------------------------------
                img = self.img_transpose_pre_blur3(img)  # WHDCN -> NCHDW
                img = self.img_reshape_pre_blur3(img)  # (NCH)DW1
                img = self.pad3(img)
                gaussian_k3 = self.gaussian_kernel3(sigmas, apply_blur)
                img = self.gaussian_blur3(img, gaussian_k3)  # (NCH)DW1
                img = self.img_reshape_post_blur3(img)  # N,C,H,D,W
                img = self.img_transpose_post_blur3(img)  # N,C,H,D,W -> WHDCN
                ...
                scale = self.scale()
                min3, min3_i = self.min3(img)
                max3, max3_i = self.max3(img)
                min2, min2_i = self.min2(min3)
                max2, max2_i = self.max2(max3)
                min1, min1_i = self.min1(min2)
                max1, max1_i = self.max1(max2)
                min, min_i = self.min0(min1)
                max, max_i = self.max0(max1)
                if (self.batch_size == 1):
                    min = self.img_reshape_to_scalar(min)
                    max = self.img_reshape_to_scalar(max)
                    img = self.mul(img, scale)
                    img = self.clamp(img, min, max)
                elif (self.batch_size == 2):
                    min = self.img_reshape_post_reduce(min)
                    max = self.img_reshape_post_reduce(max)
                    min0, min1 = self.split(min)
                    max0, max1 = self.split(max)
                    img = self.img_reshape_before_mul(img)
                    img = self.mul(img, scale)
                    img0, img1 = self.split(img)
                    min0 = self.img_reshape_to_scalar(min0)
                    min1 = self.img_reshape_to_scalar(min1)
                    max0 = self.img_reshape_to_scalar(max0)
                    max1 = self.img_reshape_to_scalar(max1)
                    img0 = self.clamp(img0, min0, max0)
                    img1 = self.clamp(img1, min1, max1)
                    img = self.concat(img0, img1)
                else:
                    raise ValueError(
                        "Unet3dMediaPipe: Unsupported batchsize {} in definegraph!".format(self.batch_size))
