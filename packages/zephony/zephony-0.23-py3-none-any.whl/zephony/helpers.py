...
def is_valid_date(string, allowed_format='%Y-%m-%d'):
    try:
...
def is_valid_datetime(string, allowed_format='%Y-%m-%d %H:%M:%S.%f'):
    try:
        datetime.strptime(string, allowed_format)
        return True
    except ValueError:
...
def serialize_datetime(date_or_time_or_dt_object, format):
    ...
    if format == '%Y-%m-%d':
        return datetime.strftime(date_or_time_or_dt_object, format)
...
def add_pagination(query_params, default_page=1, default_page_size=100):
    """
    This function adds pagination related params to the passes query
    parameters and return the new dictionary.

    The pagination related parameters are:
        - page
        - page_size
    """
