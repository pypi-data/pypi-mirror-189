...
class ActivityPub:
    ...
    async def generate_authorization_url(
        ...
        :return: String containing URL to visit to get access token interactively from instance.
        """
        logger.debug(
    ...
    async def post_status(  # noqa: PLR0913  - No way around needing all this parameters
        ...
        if self.instance_type == INSTANCE_TYPE_TAKAHE:
            json_data = {
                "status": status,
    ...
    def __update_ratelimit(self: ActivityPubClass, headers: Any) -> None:
        ...
        if not self.instance_type == INSTANCE_TYPE_PLEROMA:
