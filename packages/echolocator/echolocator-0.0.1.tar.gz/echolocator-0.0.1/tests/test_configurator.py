...
# Configurator.
from echolocator_lib.configurators.configurators import Configurators
...
class GoodConfiguratorDirectTester(Base):
    ...
    async def _main_coroutine(self, constants, specification, output_directory):
        ...
        echolocator_configurator = Configurators().build_object(specification)

        # Perform symbol replacement.
        echolocator_configurator.substitute({"output_directory": output_directory})

        # Remove top level key of this name.
        echolocator_configurator.remove(["key_to_remove"])

        # Get the current dict representation of the configuration.
        current = await echolocator_configurator.load()
        ...
        value = echolocator_configurator.require("multi.level1.level2")
        assert value == "value_at_level2"

        # Check a good error on non-existence of key.
        with pytest.raises(NotFound):
            value = echolocator_configurator.require("multi.level1.level3")

        # Check a good error on non-existence of key.
        with pytest.raises(NotFound):
            value = echolocator_configurator.require("multi.level1.level2.level3")

        # Substitute in token.
        value = echolocator_configurator.resolve(
...
class BadConfiguratorDirectTester(Base):
    ...
    async def _main_coroutine(self, constants, specification, output_directory):
        ...
        try:
            ...
            await echolocator_configurator.load()

            logger.info(describe("loaded yaml", echolocator_configurator.get_current()))
        except Exception as exception:
