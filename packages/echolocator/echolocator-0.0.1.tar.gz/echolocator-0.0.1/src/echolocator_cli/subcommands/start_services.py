...
from echolocator_cli.subcommands.base import Base
...
services = {
    ...
    "echolocator_dataface_specification": "dataface",
    "echolocator_collector_specification": "collector",
    "echolocator_gui_specification": "gui",
}
...
class StartServices(Base):
    ...
    async def __run_coro(self):
        ...
        # echolocator_configurator.set_logging_mpqueue(self.__mainiac.mpqueue)
        ...
        async with context:
            if "gui" in selected_service_names:
                logger.info(
                    f"starting gui {callsign(echolocator_guis_get_default())}/index.html"
                )
