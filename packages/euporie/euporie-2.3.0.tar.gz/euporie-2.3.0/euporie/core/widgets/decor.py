...
class FocusedStyle(Container):
    ...
    def write_to_screen(
        ...
        # log.debug(
        # mouse_handlers.mouse_handlers[write_position.ypos][write_position.xpos]
        # )
        # mouse_handlers = MouseHandlers()
        # a = {
        # (x, y): h
        # for y, row in mouse_handlers.mouse_handlers.items()
        # for x, h in row.items()
        # }
        ...
        log.debug("x=%s y=%s", write_position.xpos, write_position.ypos)
        for i in range(40):
            mouse_handlers.mouse_handlers[i][i] = _wrap_mouse_handler(
                mouse_handlers.mouse_handlers[i][i]
            )
        ...
        # log.debug([(x, y, h) for (x, y), h in b.items() if a[x, y] != h])

        # for y in range(ypos := write_position.ypos, ypos + write_position.height):
        #    row = mouse_handlers.mouse_handlers[y]
        #    for x in range(xpos := write_position.xpos, xpos + write_position.width):
