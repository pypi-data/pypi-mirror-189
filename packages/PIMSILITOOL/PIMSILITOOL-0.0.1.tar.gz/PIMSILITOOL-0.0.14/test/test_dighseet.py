...
try:
    ...
    in_fc = r"C:\Surendra\From_Marissa\tbPODS7gdb.gdb (2)\tbPODS7gdb.gdb\ANOMALY_ILI"
    in_fld = "AnomalyPriority"
    # unique_values = set(row[0] for row in arcpy.da.SearchCursor(in_fc, in_fld))

    uniq_values = list(set(row[0] for row in arcpy.da.SearchCursor(in_fc, in_fld)))
    unique_values = [i for i in uniq_values if i]
    values = unique_values.sort()

    # values =  unique_values(in_fc, in_fld)
    # data = arcpy.da.TableToNumPyArray(in_fc, [in_fld])
    # uniq_data =  numpy.unique(data[in_fld])
