...
class MyApplication(Stage):
    def __init__(self, scope, id, *, env=None, outdir=None, stageName=None, permissionsBoundary=None):
        super().__init__(scope, id, env=env, outdir=outdir, stageName=stageName, permissionsBoundary=permissionsBoundary)
...
```python
# Example automatically generated from non-compiling source. May contain errors.
class MyLambdaStep(pipelines.Steppipelines.ICodePipelineActionFactory):
    ...
    return {"run_orders_consumed": 1}getconsumed_stack_outputs()pipelines.StackOutputReference[]
    return [self.stack_output_reference]
```
