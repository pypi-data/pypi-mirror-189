...
def Minecraft():
    ...
    try:
        ...
        log.info(
            f">>> 服务器IP: {Config.get('minecraft/host')} 端口号: {Config.get('minecraft/post')}")
...
def Serial():
    ...
    try:
        ...
        if Config.get('serial/com'):
            log.info(
                f">>> 串口号: {Config.get('serial/com')} 波特率: {Config.get('serial/bps')}")
            return MXSerial(Config.get('serial/com'), Config.get('serial/bps'))
        else:
            log.info(f">>> 串口列表: {serialUtils.getCom()}")
            log.info(
                f">>> 串口号: {serialUtils.getCom(-1)} 波特率: {Config.get('serial/bps')}")
