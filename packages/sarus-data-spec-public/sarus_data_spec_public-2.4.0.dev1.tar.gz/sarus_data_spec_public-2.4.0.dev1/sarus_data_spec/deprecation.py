...
class DeprecatedWarning(DeprecationWarning):
    ...
    def __init__(
        ...
        # near the class name, not here inside __init__
    ...
    def __str__(self) -> str:
        ...
        parts: t.DefaultDict[str, t.Any] = collections.defaultdict(str)
        parts["function"] = self.function

        if self.deprecated_in:
            parts["deprecated"] = " as of %s" % self.deprecated_in
        if self.removed_in:
            parts["removed"] = " and will be removed in {}".format(
                self.removed_in
            )
...
def deprecated(
    ...
    :param removed_in: The default is **None**,
    ...
    def _function_wrapper(function: t.Callable) -> t.Callable:
        ...
        def _inner(*args: t.Any, **kwargs: t.Any) -> t.Any:
            the_warning = DeprecatedWarning(
                function.__name__, deprecated_in, removed_in, details
            )
            warnings.warn(
...
def fail_if_not_removed(method: t.Callable) -> t.Callable:
    ...
    """
    # NOTE: Unless this is named test_inner, nose won't work
    # properly.See
    @functools.wraps(method)
    def test_inner(*args: t.Any, **kwargs: t.Any) -> t.Any:
        with warnings.catch_warnings(record=True) as caught_warnings:
            warnings.simplefilter("always")
            rv = method(*args, **kwargs)
        for warning in caught_warnings:
            if warning.category == DeprecatedWarning:
