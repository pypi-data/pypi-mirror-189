...
class Sample(StandardDatasetOp):
    ...
    async def size(self) -> st.Size:
        ...
        if (
            ...
            parent_size = await self.parent_size()
            size_ratio = (
                self.dataset.transform().protobuf().spec.sample.size
                / parent_size.statistics().size()
            )
        return size_builder(
    ...
    async def marginals(self) -> st.Marginals:
        ...
        if (
            ...
            size_ratio = (
                self.dataset.transform().protobuf().spec.sample.size
                / parent_marginals.statistics().size()
            )
    ...
    async def _arrow_to_arrow(
        ...
        seed = Scalar(
            self.dataset.transform().protobuf().spec.sample.seed
        ).value()
