...
class Terminal:
    ...
    def convert(
        ...
        while status != "completed":
            ...
            if sleep_step < len(sleep_secs) - 1:
                sleep_step += 1
            status = cls._task_status(convert_connector)
    ...
    def _convert(
        ...
        if json["entity"]["status"] == "running":
            credits = json["entity"]["credits"]
            cls._info("Convert credits used: {}".format(credits))
    ...
    @classmethod
    def _task_status(cls, connector: str) -> str:
        ...
        return json["result"]["output"]["entity"]["status"]
