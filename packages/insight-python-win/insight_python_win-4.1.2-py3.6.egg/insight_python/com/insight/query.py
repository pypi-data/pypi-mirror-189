from ..interface.mdc_query_handledata_class import *
...
# 查询股票每日衍生指标信息
def get_derived(htsc_code=None, trading_date=None, derived=None):
    '''
    :param htsc_code:华泰证券代码，支持多个code查询，列表类型
    :param trading_date: 时间范围 list类型 开始结束时间为datetime
    :param derived:衍生指标类型，可选成本均价线amv，人气和买卖医院指标明细ar_br，乖离率明细bias，布林线明细boll，中间意愿指标明细cr，成交量平均线和移动平均线vma_ma，成交量变异率vr，威廉指标明细wr
    :return: pandas.DataFrame
    '''
    ...
    if trading_date:
        if isinstance(trading_date, list):
            if len(trading_date) > 2:
                return "trading_date format is not [start_date, end_date]"
            if len(trading_date) == 1:
                trading_date = [trading_date[0], trading_date[0]]
            if not isinstance(trading_date[0], datetime) or not isinstance(trading_date[1], datetime):
                return "start_date,end_date format is not datetime"
        else:
            return "trading_date format is not list"
    else:
        return 'trading_date is null'

    if derived:
        if not isinstance(derived, str):
            return "derived format is not str"
    else:
        return 'derived is null'

    result = get_derived_handle(htsc_code, trading_date, derived)
    return result
...
def get_change_summary(market=None, trading_day=None):
    '''
    :param market_code: 证券市场代码，支持多个市场查询，列表类型
    :param trading_day: 时间范围，list类型，开始结束时间为datetime
    :return: pandas.DataFrame
    '''
...
def get_stock_info(htsc_codes=None, listing_date=None, listing_state=None):
    '''
    :param htsc_codes: 华泰证券代码，支持多标的查询，列表类型
    ...
    result = get_stock_info_handle(htsc_codes, listing_date, listing_state)
    return result
...
# 行业分类2
def get_industry(htsc_code, classified='sw'):
    '''
...
# 行业分类3
def get_industry_stocks(industry_code, classified='sw'):
    '''
    :param common_code: 行业代码
    :param classified: 行业分类 申万行业划分“sw”，证监会行业划分“zjh”，默认为申万行业划分
    :return: pandas.DataFrame
    '''
...
def get_new_share(htsc_code=None, book_start_date_online=None, listing_date=None):
    ...
    if book_start_date_online:
        if isinstance(book_start_date_online, list):
            ...
            if not isinstance(book_start_date_online[0], datetime) or not isinstance(book_start_date_online[1], datetime):
                return "start_date,end_date format is not datetime"
        else:
...
# 债券基础信息
def get_bond_info(htsc_codes=None, secu_category_code=None, listing_date=None, issue_start_date=None, end_date=None):
    '''
    :param htsc_codes: 华泰证券ID
    ...
    result = get_bond_info_handle(htsc_codes, secu_category_code, listing_date, issue_start_date, end_date)
    return result
...
# 可转债发行列表
def get_new_con_bond(htsc_code=None, exchange=None, book_start_date_online=None, listing_date=None, issue_date=None, convert_code=None):
    '''
    :param htsc_code: 华泰证券ID
    :param exchange: 交易市场
    :param book_start_date_online: 网上申购开始日期时间范围
    ...
    :param exchange_code_detail: 细分交易市场代码
    :return: pandas.DataFrame
    '''
    ...
    result = get_new_con_bond_handle(htsc_code, exchange, book_start_date_online, listing_date, issue_date, convert_code)
    return result
...
# 可转债转股结果
def get_con_bond_2_shares(htsc_code=None, pub_date=None, exer_begin_date=None, exer_end_date=None, convert_code=None, exchange=None):
    '''
    :param htsc_code: 华泰证券ID
...
# 股票配售
def get_allotment_share(htsc_code=None, ini_pub_date=None, ex_divi_date=None, is_allot_half_year=None):
    '''
    :param htsc_code: 华泰证券ID
    :param Ini_pub_date: 首次公告日期范围
    :param ex_divi_date:  除权除息日
    ...
    if ex_divi_date:
        if isinstance(ex_divi_date, list):
            if len(ex_divi_date) > 2:
                return "ex_divi_date format is not [start_date, end_date]"
    ...
    result = get_allotment_share_handle(htsc_code, ini_pub_date, ex_divi_date, is_allot_half_year)
    return result
...
# 指数成分股
def get_index_component(htsc_code=None, name=None, exchange=None, stock_code=None, trading_day=None):
    '''
    :param htsc_code: 华泰证券ID
    :param name: 指数简称
    :param exchange: 指数交易市场
    ...
    result = get_index_component_handle(htsc_code, name, exchange, stock_code, trading_day)
    return result
...
# 根据标的查询今日最新的指定证券的基础信息
def get_basic_info(htsc_code):
    '''
    ...
    data_handle.get_interface().queryLastMdContantCallback(listmarketdatatype, htsc_code)


# 根据市场查询今日最新的指定证券的基础信息
def get_all_basic_info(query):
    '''
    :param query: 交易市场，支持多市场查询，list类型 [(exchange1,security_type1),(exchange2,security_type2)]
    '''

    exchange_map = {'XSHG': 101, 'XSHE': 102, 'CSI': 703, 'CNI': 702, 'XBSE': 106, 'HKSC': 204, 'HGHQ': 205}
    security_type_map = {'index': 1, 'stock': 2, 'fund': 3, 'bond': 4}
    marketdatatype_list = []
    ...
    for marketdatatype in query:
        try:
            ...
            print("Input parameter error:'query'")

    security_id_list = []  # 置空表示不额外查询某些标的
    data_handle.get_interface().queryLastMdContantCallback(marketdatatype_list, security_id_list)
