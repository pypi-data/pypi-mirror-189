...
class Tremolo(TremoloProtocol):
    ...
    async def _handle_response(self, func, options={}):
        ...
        connection = self._server['request'].headers.get(b'connection', b'keep-alive')

        no_content = status[0] in (204, 304) or status[0] // 100 == 1
        chunked = version == b'1.1' and connection.find(b'close') == -1 and no_content is False
        ...
        try:
            ...
            if no_content or data == b'':
                ...
                if chunked:
                    ...
                    await self._server['response'].write(b'Content-Type: %s\r\nContent-Length: %d\r\nConnection: close\r\n\r\n'
                                                         % (content_type, len(data)), throttle=False)
