...
from typing import TYPE_CHECKING, Dict, FrozenSet, List, Set, Tuple, Type, TypedDict, cast
...
def rich_to_pyarrow(python_type: Type, name: str) -> pa.DataType:
    # Polars seems to allow optional for any dtype, so we ignore it when computing dtypes
    python_type = unwrap_optional_if_needed(python_type)
    ...
    if origin is not None:
        ...
        if origin == Annotated:
            ...
            return rich_to_pyarrow(args[0], name)
        if origin in (list, List, set, Set, frozenset, FrozenSet):
            ...
            arg = args[0]
            return pa.list_(rich_to_pyarrow(arg, name=f"{name}[]"))
    ...
    if issubclass(python_type, enum.Enum):
        ...
        return rich_to_pyarrow(get_enum_value_type(python_type), name)
    ...
    if (
        ...
        for field_name, type_annotation in annotations.items():
            underlying_dtype = rich_to_pyarrow(type_annotation, name=f"{name}.{field_name}")
    ...
    if issubclass(python_type, date):
        # Treating dates as timestamps because polars does not support pa.Date
...
def pyarrow_to_polars(pa_type: pa.DataType, name: str) -> pl.PolarsDataType:
    ...
    try:
        ...
        raise ValueError(
            f"Failed to convert pyarrow dtype `{pa_type}` for feature '{name}' to a Polars DataType."
        ) from exc
    assert isinstance(df, pl.DataFrame)
    return df.dtypes[0]


def serialize_pyarrow_type(pa_dtype: pa.DataType):
    """Serialize a PyArrow datatype into"""
