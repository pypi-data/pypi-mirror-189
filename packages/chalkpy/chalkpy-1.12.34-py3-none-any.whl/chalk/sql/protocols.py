...
from chalk.features import DataFrame, Feature, FeatureConverter, Features, FeatureWrapper
...
class BaseSQLSourceProtocol(Protocol):
    ...
    def execute_query(
        ...
        Args:
            finalized_query: The query to execute
            columns_to_converters: A function that, given the list of column names returned from the database,
                produces a mapping of columns to converters. Columns not in the resulting dictionary should be ignored.
            connection: A connection to execute the query under. If not specified, then a new connection is acquired.
        """
        ...
