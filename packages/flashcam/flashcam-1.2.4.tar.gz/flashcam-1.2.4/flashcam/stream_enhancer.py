...
# ------------------------------------------------- also ZMQ
    def save_avi(self, seconds =  1, name = "" ):
        """
        seconds>0 for TIMELAPS  ;   -1 with name="dm" for detectmotion
        """
        ...
        if seconds == 0:
            ...
            if not self.aviopened:
                ...
                filename = os.path.expanduser("~/DATA/"+filename)
                for i in range(3): print(filename)


                #--- what happens if not 640x480 and xvid?
                ...
                me = socket.gethostname()
                filename = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = os.path.expanduser("~/DATA/"+f"{me}{name}_{filename}.avi")
                for i in range(3): print(filename)
                self.aviopened = True
        ...
        # -------------- timelapse if seconds>0: *******************************
            ...
            if not self.aviopened_laps:
                me = socket.gethostname()
                filename = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = os.path.expanduser("~/DATA/"+ f"{me}_laps{seconds:04d}_{filename}.avi" )
                for i in range(3): print(filename)
