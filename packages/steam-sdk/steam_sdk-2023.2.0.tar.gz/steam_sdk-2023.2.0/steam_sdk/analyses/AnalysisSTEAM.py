...
from steam_sdk.data.DataAnalysis import DataAnalysis, ModifyModel, ModifyModelMultipleVariables
...
class AnalysisSTEAM:
    ...
    def __init__(self,
        ...
        if file_name_analysis:
            self.data_analysis = yaml_to_data(file_name_analysis,
    ...
    def load_circuit_parameters(self, step, verbose: bool = False):
        ...
        BM.load_circuit_parameters_from_csv(input_file_name=path_file_circuit_parameters,
                                            selected_circuit_name=selected_circuit_name)
    ...
    def run_parsim_event(self, step, verbose: bool = False):
        ...
        flag_set_up_viewer = step.flag_set_up_viewer
        path_viewer_csv = step.path_viewer_csv
        default_keys = step.default_keys

        # Check inputs
        if type(path_viewer_csv) == str:
            path_viewer_csv = [path_viewer_csv]  # Make sure this variable is always a list
        if path_viewer_csv and (len(path_viewer_csv) != len(software)):
            raise Exception(f'The length of path_viewer_csv ({len(path_viewer_csv)}) differs from the length of software({len(software)}). This is not allowed.')

        # path to ideal Quench dict
        outputfile_event_csv = os.path.join(os.getcwd(), 'output', 'run_parsim_event', 'events.csv')  # TODO avoid hard-coding this file name?
        ...
        if case_model == 'magnet':
            ...
            pem.set_up_analysis(model_name=model_name, simulation_name=simulation_name,
                                simulation_numbers=simulation_numbers, software=software,
                                t_PC_off=t_PC_off,
                                current_polarities_CLIQ=current_polarities_CLIQ,
                                dict_QH_circuits_to_QH_strips=dict_QH_circuits_to_QH_strips,
                                path_outputfile_event_csv=outputfile_event_csv)
            # copy original model
            ...
            for step_name in pem.list_AnalysisStepSequence:
                ...
                self.step_modify_model_multiple_variables(step, verbose=verbose)  # TODO: when a value is empty it will lead to no error and no outputs are created!
            ...
            # pem.list_AnalysisStepSequence
            # pem.dict_AnalysisStepDefinition
            # TODO: add flag_show_parsim_output ?
            # TODO: add flag to write yaml analysis with all steps

            # Optionally, write a .csv file that can be used to run a STEAM Viewer analysis
            if flag_set_up_viewer:
                for current_software, current_path_viewer_csv in zip(software, path_viewer_csv):
                    pem.set_up_viewer(current_path_viewer_csv, default_keys, simulation_numbers, simulation_name,
                                      current_software)
                    if verbose: print(
                        f'File {current_path_viewer_csv} written. It can be used to run a STEAM Viewer analysis.')
        else:
    ...
    def run_parsim_sweep(self, step, verbose: bool = False):
        ...
        for i, row in input_parsim_sweep_df.iterrows():
            ...
            def parse_string(s):  # TODO: better location for the function?
                '''
                    this function turns:
            ...
            for j, (var_name, var_value) in enumerate(row.items()):
                ...
                if 'Conductors[' in var_name:
                    try:
                        current_value = eval('model_data.' + var_name) # TODO literal_eval no working
                    except:
                        current_value = None
                        print(f'WARNING: Columnname "{var_name}" with value "{var_value}" in csv file {input_sweep_file} is skipped.')
                    if current_value:
                        new_vars_dict[var_name] = var_value
                # Check if the current variable is present in the model data structure & value in csv is not empty
                    ...
                    if type(var_value) == int or type(var_value) == float:
                        ...
                        new_vars_dict[var_name] = parse_string(var_value)
                    else:
                ...
                elif not rhasattr(model_data, var_name) and var_name is not 'simulation_number':
                    print(f'WARNING: Columnname "{var_name}" with value "{var_value}" in csv file {input_sweep_file} is skipped.')

            # if there are no variables to change found, add an entry that changes nothing to force to save the new simulation file
            if not new_vars_dict:
                new_vars_dict['GeneralParameters.magnet_name'] = rgetattr(model_data, 'GeneralParameters.magnet_name')  # this line is not changing the variable value
            ...
            next_step.variables_value = [[val] for val in new_vars_dict.values()]
            next_step.variables_to_change = list(new_vars_dict.keys())
            ...
            self.list_models[model_name] = deepcopy(local_model_copy)
            # TODO add functionality to change Conductors with Conductors[0].cable.material_insulation
        del local_model_copy
