...
class RoxieList:
    def __init__(self, roxie_data):
        ...
        for eo in roxie_data.coil.electrical_order:
            ...
            for halfTurn_nr, halfTurn in block.half_turns.items():
                ...
                self.x_insulated.append([insulated.iL.x, insulated.oL.x, insulated.oR.x, insulated.iR.x])
                self.y_insulated.append([insulated.iL.y, insulated.oL.y, insulated.oR.y, insulated.iR.y])
                self.x_bare.append([bare.iL.x, bare.oL.x, bare.oR.x, bare.iR.x])
                self.y_bare.append([bare.iL.y, bare.oL.y, bare.oR.y, bare.iR.y])
...
class ParserRoxie:
    ...
    def findConductorPositions(self, block: pd.Block = None, conductor: pd.CondPar = None, verbose: bool = False):
        ...
        for c in range(self.block.nco):
            ...
            if self.block.type == 2:  # block-coil
                ...
                for i, arg in enumerate(xBareCable):
                    (xBareCable[i], yBareCable[i]) = gf.rotatePoint((xBareCable[i], yBareCable[i]),
                                                                    (radius, y0Cond), alpha)
            ...
            for i, arg in enumerate(xBareCable):
                ...
                (xBareCable[i], yBareCable[i]) = gf.rotatePoint((xBareCable[i], yBareCable[i]), (x0, y0), turn)
                (xCable[i], yCable[i]) = gf.rotatePoint((xCable[i], yCable[i]), (x0, y0), turn)
        ...
        if self.block.type == 2 or self.block.nco == 1:
            block.block_corners.iL = pd.Coord(x=sigDig(corner0[0]), y=sigDig(corner0[1]))  # 1
            block.block_corners.oL = pd.Coord(x=sigDig(corner3[0]), y=sigDig(corner3[1]))  # 4
            ...
            if corner0[0] < corner1[0]:
                block.block_corners.iL = pd.Coord(x=sigDig(mid_inner_end[0]), y=sigDig(mid_inner_end[1]))  # 1
                block.block_corners.oL = pd.Coord(x=sigDig(mid_outer_end[0]), y=sigDig(mid_outer_end[1]))  # 4
                block.block_corners.iR = pd.Coord(x=sigDig(mid_inner_beg[0]), y=sigDig(mid_inner_beg[1]))  # 2
                block.block_corners.oR = pd.Coord(x=sigDig(mid_outer_beg[0]), y=sigDig(mid_outer_beg[1]))  # 3
            else:
                block.block_corners.iR = pd.Coord(x=sigDig(mid_inner_end[0]), y=sigDig(mid_inner_end[1]))
                block.block_corners.oR = pd.Coord(x=sigDig(mid_outer_end[0]), y=sigDig(mid_outer_end[1]))
                block.block_corners.iL = pd.Coord(x=sigDig(mid_inner_beg[0]), y=sigDig(mid_inner_beg[1]))
                block.block_corners.oL = pd.Coord(x=sigDig(mid_outer_beg[0]), y=sigDig(mid_outer_beg[1]))
        ...
        return self.data.coil.coils if block else self.data.coil
    ...
    def getCablePositions(self, blocks: Dict[str, pd.Block] = None, cadata: pd.Cadata = None, verbose: bool = False):
        ...
        for no in self.roxieData.coil.blocks:
            ...
            self.data.coil.electrical_order.append(
