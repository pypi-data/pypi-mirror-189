...
def change_role(subject, resource, role_name):
    ...
    return json_error_response(409, description="Error changing role.")
...
def get_selected_resources(
    ...
    resources via dyamic selection fields. Only the resources the given user has read
    ...
    :param filters: (optional) Additional filter expressions to apply to the respective
        resource query.
...
def get_resource_user_roles(
    ...
    items = UserRoleSchema(obj=resource).dump_from_iterable(paginated_user_roles)
...
def get_resource_group_roles(resource, page=1, per_page=10, filter_term="", user=None):
    ...
    of the user allow them to. Since such groups should still be listed, they are
    included using a limited subset of group attributes.
    ...
    items = GroupRoleSchema(obj=resource).dump_from_iterable(paginated_group_roles)
...
def get_internal_resource_export(
    ...
    Only used internally, as the preview functionality is only useful in a browser
    context.

    :param resource: The resource to export. An instance of :class:`.Record` or
        :class:`.Collection`.
    :param export_type: A valid export type as defined in
    ...
    if export_type not in const.EXPORT_TYPES[object_name]:
    ...
    if export_type in {"pdf", "qr"}:
        if preview or download:
            file_ext = "png" if export_type == "qr" else "pdf"

            return download_bytes(
                export_func(resource, export_type, export_filter=export_filter),
                f"{resource.identifier}.{file_ext}",
                as_attachment=download,
            )
