    ...
    <div v-for="(query, index) in queries" :key="query.id">
      <div class="form-row mb-4 mb-xl-2">
        <div class="col-xl-1 mb-1 mb-xl-0 d-flex justify-content-center">
          <popover-toggle class="w-100"
                          toggle-class="btn btn-sm btn-block btn-light text-muted"
                          width="400px"
                          placement="bottom"
                          v-if="index === 0">
            <template #toggle>
            ...
            <template #content>
              {{ $t(`This menu allows searching the generic extra metadata of records, including keys, types and \
different kinds of values based on the selected types. Multiple such queries can be combined with an 'AND' or an 'OR' \
operation in the form of '(Q1 AND Q2) OR (Q3 AND Q4)'.`) }}
              <hr class="my-1">
              {{ $t(`Note that keys inside of nested metadata entries are indexed in the form of 'key_1.key_2'. In \
case of list entries, keys are replaced by the corresponding index in the list instead, starting at 1.`) }}
            </template>
          </popover-toggle>
        ...
        <div class="col-xl-5 mb-1 mb-xl-0" v-if="query.type === 'str'">
          <div class="input-group input-group-sm">
            <div class="input-group-prepend">
        ...
        <div class="col-xl-3 mb-1 mb-xl-0" v-if="query.type === 'numeric'">
          <div class="d-flex justify-content-between">
            <div class="input-group input-group-sm mr-1">
              <div class="input-group-prepend">
                <span class="input-group-text">&ge;</span>
              </div>
              <input class="form-control" v-model="query.numeric.min" @keydown.enter="search">
            </div>
            <div class="input-group input-group-sm ml-1">
              <div class="input-group-prepend">
                <span class="input-group-text">&le;</span>
              </div>
              <input class="form-control" v-model="query.numeric.max" @keydown.enter="search">
            </div>
          </div>
        </div>
        <div class="col-xl-2 mb-1 mb-xl-0" v-if="query.type === 'numeric'">
          <div class="input-group input-group-sm">
            ...
            <input class="form-control" v-model="query.numeric.unit" @keydown.enter="search">
          </div>
        </div>
        <div class="col-xl-5 mb-1 mb-xl-0" v-if="query.type === 'bool'">
          <div class="input-group input-group-sm">
            <div class="input-group-prepend">
              <span class="input-group-text">{{ $t('Value') }}</span>
            </div>
        ...
        <div class="col-xl-5 mb-1 mb-xl-0" v-if="query.type === 'date'">
          <div class="d-flex justify-content-between">
            <div class="input-group input-group-sm mr-1">
              <div class="input-group-prepend">
                <span class="input-group-text">&ge;</span>
