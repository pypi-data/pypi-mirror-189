...
from sktmls.models import MLSTrainableCustom
from sktmls.utils import LogicProcessor, LogicChecker
import imp
import sys
...
class EurekaTestModel(MLSGenericModel, MLSTrainableCustom):
    """
    ...
    def __init__(
        ...
        handler_code: Dict[str, str] = {"dummy_Handler": "def dummy_Handler(data = None): return None"},
    ):
        ...
        if handler_code:
            assert isinstance(list(handler_code.keys())[0], str), "`handler_code`는 string 타입이어야 합니다."
            assert isinstance(list(handler_code.values())[0], str), "`handler_func_nm`는 string 타입이어야 합니다."

        self.handler_code = list(handler_code.keys())[0]
        self.handler_func_nm = list(handler_code.values())[0]
    ...
    def _postprocess(
            ...
            if self.postprocessing_item_profile:
                ...
                for item_profile in postprocessing_item_profile_preprocessed:
                    for item_id, keys in postprocessing_item_profile_preprocessed[item_profile].items():
                        item_values = pf_client.get_item_profile(profile_id=item_profile, item_id=item_id, keys=keys)
        ...
        try:
            try:
                ...
                self.handler_func = getattr(module, self.handler_func_nm)
                assert hasattr(self.handler_func, "__call__") is True, "후처리 로직은 함수이어야 합니다."
                result = self.handler_func(data)
            except (NameError, AttributeError, ModuleNotFoundError):
                self.handler_code = LogicChecker(self.handler_code)
                handler = imp.new_module("Handler")
                exec(self.handler_code, handler.__dict__)
