...
from wiliot.wiliot_testers.tester_utils import open_json, open_json_cache
...
def open_session(inlay_type_list):
    ...
    print_tab = [[SimGUI.Text('To print?', size=(40, 1)),
                 ...
                 [SimGUI.Text('Max QR wrong readouts in row:', size=(40, 1)),
                  SimGUI.Spin(values=[i for i in range(1, 10)], initial_value=int(gui_inputs_values['maxQRWrongTags']),
                              size=(6, 1), key='maxQRWrongTags')],
                 [SimGUI.Text('QR ComPort: ', size=(40, 1), visible=True),
    ...
    stop_conditiona_tab = [[SimGUI.Text('Max missing labels in a row:', size=(40, 1)),
                            SimGUI.Spin(values=[i for i in range(1, 6000)],
                                        initial_value=int(
                                            gui_inputs_values['maxMissingLabels']),
                                        size=(6, 1), key='maxMissingLabels')],
...
def save_screen(tested, passed, yield_, missing_labels, problem_in_locations_hist_val, ttfgp_avg,
    ...
    window = SimGUI.Window('Offline Tester - Run highlights (Shrunk)', layout, force_toplevel=True, keep_on_top=True)
    expand = False
    while True:
        event, values = window.read()
        # See if user wants to quit or window was closed
    ...
    # sys.exit(0)
    if expand:
        values = expand_save_screen(tested, passed, yield_, missing_labels, problem_in_locations_hist_val, ttfgp_avg,
...
def expand_save_screen(tested, passed, yield_, missing_labels, problem_in_locations_hist_val, ttfgp_avg,
    ...
    counter = 0
    line = []
    if problem_in_locations_hist_val is not None:
        for key in problem_in_locations_hist_val.keys():
            counter += 1
            line.append(SimGUI.Text(str(key) + ' = ' + str(problem_in_locations_hist_val[key]), size=(30, 1)))
            if counter % 3 == 0:
                layout.append(line)
                line = []
    if counter % 3 != 0:
    ...
    window.close()
    if shrink:
        values = save_screen(tested, passed, yield_, missing_labels, problem_in_locations_hist_val, ttfgp_avg,
                             default_upload_value=values['upload'])
    return values
