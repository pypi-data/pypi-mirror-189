...
from pyjamas.external.pascal_voc_io import PascalVocReader
...
@pytest.mark.usefixtures("image_fixture")
@pytest.mark.usefixtures("pjsannotationspath_fixture")
@pytest.mark.usefixtures("xmlroispath_fixture")
def test_cbExportAnnotationsXML(image_fixture, pjsannotationspath_fixture, xmlroispath_fixture):
    PyJAMAS_FIXTURE.io.cbLoadArray(image_fixture)
    PyJAMAS_FIXTURE.io.cbLoadAnnotations([pjsannotationspath_fixture])
    PyJAMAS_FIXTURE.io.cbExportAnnotationsXML(os.path.join(pjsfixtures.TMP_DIR, pjsfixtures.TMP_XML_FILE))

    xmlreader_pjs = PascalVocReader(os.path.join(pjsfixtures.TMP_DIR, pjsfixtures.TMP_XML_FILE))
    xmlreader_fixture = PascalVocReader(xmlroispath_fixture)

    assert xmlreader_pjs.getShapes() == xmlreader_fixture.getShapes()
