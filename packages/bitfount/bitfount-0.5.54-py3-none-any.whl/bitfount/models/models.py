...
import pickle  # nosec
from typing import Any, List, Literal, Mapping, Optional, Tuple, Union, cast
    ...
    def fit(
        ...
        **kwargs: Any,
        ...
        model.fit(
            ...
            self.train_dl.get_y_dataframe()[self.databunch.target].to_numpy(),
        )
        self._model = model
...
class RegBoostRegressor(RegressorMixIn, _BaseModel):
    ...
    def evaluate(
        ...
        test_target = (
            test_dl.get_y_dataframe()[self.databunch.target]
            .to_numpy()
            .astype(np.float32)
        )
