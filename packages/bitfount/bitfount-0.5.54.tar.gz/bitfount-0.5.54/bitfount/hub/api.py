...
import time
from typing import Any, Final, List, Mapping, Optional, Tuple, Type, cast
import warnings
...
class BitfountHub:
    ...
    def send_model(self, model_code_path: Path) -> None:
    ...
    def _get_model_response(
        self, username: str, model_name: str, model_version: Optional[int] = None
    ...
    def get_model(
        ...
        """
        attempts_to_get_model = 5
        response_json = self._get_model_response(
            username=username, model_name=model_name, model_version=model_version
        )
        while response_json is None and attempts_to_get_model > 0:
            time.sleep(5)  # give an extra 5 seconds in between retries
            response_json = self._get_model_response(
                username=username, model_name=model_name, model_version=model_version
            )
            attempts_to_get_model -= 1
        if response_json is None or len(response_json) == 0:  # i.e. empty JSON response
            logger.warning(
    ...
    def register_user_public_key(self, key: RSAPublicKey) -> None:
        ...
        key_json: _RegisterUserPublicKeyPOSTJSON = {
            "publicKey": _RSAEncryption.serialize_public_key(key, form="SSH").decode(
                "utf-8"
            ),
        }
        try:
            response = self.session.post(
                f"{self.url}/api/{self.username}/keys", json=key_json, timeout=10
            )
            response.raise_for_status()
        except RequestException as err:
            try:
    ...
    def get_user_public_key(
        ...
        """Retrieve public keys for the given username.

        Args:
            username: The user to retrieve the public keys of. If not provided,
              will use the signed-in user.

        Returns:
            The active public key of the given user or None if no key is registered
            for the user.
        """
        if username is None:
        ...
        try:
            response = self.session.get(
                f"{self.url}/api/identity-verifiers",
                params={
        ...
        response_json: _UserIdentityVerifiersResponseJSON = _extract_response_json(
            response
        )
        try:
            key = response_json["publicKey"]
            if key:
                # "publicKey" might be None or other nullable value
                try:
                    return _RSAEncryption.load_public_key(key.encode())
                except ValueError as ve:
                    raise RSAKeyError(
                        f"Unable to decrypt registered public key for user {username}:"
                        f" {str(ve)}."
                    )
            else:
                return None
        except KeyError:
            # "publicKey" was not in JSON, means no public key set
            return None
