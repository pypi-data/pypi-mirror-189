...
from bitfount.hub.types import (
    ...
    _UserIdentityVerifiersResponseJSON,
)
...
from bitfount.utils import _get_mb_from_bytes, web_utils
...
class TestBitfountHub:
    ...
    def test_send_model_failure(
        ...
        with pytest.raises(
            ModelUploadError, match="Failed to upload model details to hub"
        ):
            hub.send_model(model_file)
    ...
    def test_get_model_success(
        ...
        if model_version:
            good_model_response.update({"modelVersion": model_version})
    ...
    def test_register_public_key(
        ...
        responses.add(
            ...
            match=[
                json_params_matcher(
                    {
                        "publicKey": _RSAEncryption.serialize_public_key(
    ...
    def user_identity_verifiers_response_json(
        ...
        `public_key_registered` will control whether or not the public key is included
        ...
        response_json: _UserIdentityVerifiersResponseJSON = {
            "username": username,
            "samlUserID": "someSamlUserID",
            "oidcUserID": "someOIDCUserID",
        }
        if public_key_registered is not False:
            response_json["publicKey"] = public_key
        return response_json

    @pytest.mark.parametrize(
        "public_key_registered",
        (True, False, None, "undefined"),
        ids=lambda x: f"public_key_registered={x}",
    )
    @responses.activate
    def test_get_user_public_key(
        self,
        hub: BitfountHub,
        hub_url: str,
        public_key_registered: Optional[bool],
        registration_public_key: RSAPublicKey,
        user_identity_verifiers_response_json: _UserIdentityVerifiersResponseJSON,
        username: str,
    ) -> None:
        """Tests get_user_public_key() handles registered key retrieval."""
        responses.add(
            responses.GET,
            f"{hub_url}/api/identity-verifiers",
            json=user_identity_verifiers_response_json,
            status=200,
            match=[query_param_matcher({"usernameOrEmail": username})],
        )

        # If endpoint returns undefined key, will not be able to parse
        if public_key_registered == "undefined":
            with pytest.raises(
                RSAKeyError,
                match=f"Unable to decrypt registered public key for user {username}",
            ):
                hub.get_user_public_key()
        else:
            registered_key = hub.get_user_public_key()

            if public_key_registered:
                assert registered_key is not None
                assert _RSAEncryption.public_keys_equal(
                    registered_key, registration_public_key
                )
            else:  # key not present or is "null"
    ...
    @responses.activate
    def test_get_user_public_key_http_error(
        self,
        caplog: LogCaptureFixture,
        hub: BitfountHub,
        hub_url: str,
        ...
        """Tests get_user_public_key() handles HTTP error."""
        responses.add(
            responses.GET,
            f"{hub_url}/api/identity-verifiers",
            json={"success": False},
    ...
    @responses.activate
    def test_get_user_public_key_request_exception(
        self,
        caplog: LogCaptureFixture,
        hub: BitfountHub,
        hub_url: str,
        ...
        """Tests get_user_public_key() handles request error."""
        responses.add(
            responses.GET,
            f"{hub_url}/api/identity-verifiers",
            body=RequestException("TEST"),
