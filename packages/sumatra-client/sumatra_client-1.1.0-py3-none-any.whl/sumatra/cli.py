...
def table_list(args):
    tables = Client().get_tables().sort_index()
    if args.show:
        print(tables)
    else:
        print("\n".join(tables.index.to_list()))
...
def model_list(args):
    models = Client().get_models().sort_index()
    if args.show:
        print(models)
    else:
        print("\n".join(models.index.to_list()))
...
def model_put(args):
    print(Client().create_model_from_pmml(args.model, args.filename, comment=args.comment))
        ...
        if "subcommands" in cmd:
            ...
            for subcmd in sorted(cmd["subcommands"], key=lambda c: c["name"]):
                ...
                if cmd["name"] == "model":
                    ...
                    if subcmd["name"] == "put":
                        p.add_argument(
                            "filename", help="specify model PMML file"
                        )
                        p.add_argument(
