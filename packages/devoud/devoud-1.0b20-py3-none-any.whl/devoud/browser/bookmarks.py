...
class Bookmarks(QObject):
    ...
    def __init__(self, parent):
        ...
        with Path(self.parent().FS.config_dir(), 'bookmarks.json').open() as bookmarks_file:
            try:
                self._dict = json.load(bookmarks_file)
            except json.decoder.JSONDecodeError:
                print(f'[Закладки]: Произошла ошибка при чтении файла, ошибка: {json.decoder.JSONDecodeError}')
                self.remove_all()
    ...
    def append(self, data: dict) -> None:
        ...
        with Path(self.parent().FS.config_dir(), 'bookmarks.json').open('w') as bookmarks_file:
