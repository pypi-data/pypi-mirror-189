...
class History(QObject):
    ...
    def __init__(self, parent):
        ...
        with Path(self.parent().FS.config_dir(), 'history.csv').open(newline='') as history_file:
            try:
                self._list = list(csv.reader(history_file))
            except Exception as read_error:
                print(f'[История]: Произошла ошибка при чтении файла, ошибка: {read_error}')
                self.remove_all()
            self._list.reverse()
    ...
    def append(self, data: dict) -> None:
        ...
        with Path(self.parent().FS.config_dir(), 'history.csv').open('a', newline='') as history_file:
            writer = csv.writer(history_file)
            writer.writerow(data)

    def remove_(self, rows: list) -> None:
        """Список строк для удаления: [0, 1, 2...]"""
        with Path(self.parent().FS.config_dir(), 'history.csv').open('w', newline='') as history_file:
            for row in rows:
                del self._list[row]
            writer = csv.writer(history_file)
