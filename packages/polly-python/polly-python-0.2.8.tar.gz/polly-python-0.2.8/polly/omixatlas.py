...
from polly.constants import DATA_TYPES, REPORT_GENERATION_SUPPORTED_REPOS
...
class OmixAtlas:
    ...
    def validate_schema(self, body: dict) -> dict:
        ...
        Returns:
            dict: Dictionary having all the errors in the schema
        """
        try:
            # validate repo key
            if "data" not in body:
    ...
    def insert_schema(self, body: dict, repo_key=""):
        """
        ...
        Args:
            ...
            get this by runningget_all_omixatlas function. If not passed, taken from payload.
            body (dict):  The payload should be a JSON file for a specific table as per the structure defined for \
                schema. Only data-admin at organisation level will have the authentication to insert the schema.
        Raises:
        ...
        if body and isinstance(body, dict):
            try:
                ...
                if repo_key:
                    # if repo_id is passed as integer, change to str
    ...
    def update_schema(self, body: dict, repo_key=""):
        """
        ...
        Args:
            ...
            get this by runningget_all_omixatlas function. If not passed, taken from payload.
            body (dict): The payload should be a JSON file for a specific table as per the structure defined for \
                schema. Only data-admin at organisation level will have the authentication to update the schema.
        Raises:
            apiErrorException: Some Issue in Inserting the Schema for the OmixAtlas.
        ...
        if body and isinstance(body, dict):
            try:
                ...
                if resp.status_code == http_codes.OK:
                    print(const.SCHEMA_WRITE_SUCCESS)
            except Exception as err:
                raise err
        else:
            raise paramException(
    ...
    def replace_schema(self, body: dict, repo_key=""):
        """
    ...
    def create(
        ...
        try:
            omix_hlpr.check_create_omixatlas_parameters(
                display_name, description, repo_name, image_url, components, category
            )
    ...
    def add_datasets(
        ...
        Args:
            ...
            source_folder_path (dict): source folder paths from data and metadata files are fetched.
            In this dictionary, there should be two keys called "data" and "metadata"
            with value consisting of folders where data and metadata is stored respectively.
            destination_folder_path (str, optional): Destination folder structure in s3. Users should use this only when
            they want to manage the folder structure in the backend.
            ...
            validation(bool, optional): Users was to activate validation. By Default False.
    ...
    def update_datasets(
        ...
        """
        This function is used to update an existing data into an OmixAtlas.
        Once user runs this function successfully,
        ...
        Args:
            ...
            source_folder_path (dict): source folder paths from data and metadata files are fetched.
            In this dictionary, there should be two keys called "data" and "metadata" with value
            consisting of folders where data and metadata is stored respectively.
            ...
            for items in report_list:
                ...
                if "url" in items:
                    ...
                    file_url = self._return_workspace_file_url(
                        shared_id, access_url, workspace_id, workspace_path
                    )
                    report_id = items.get("report_id")
                    details_list.append(added_by)
                    details_list.append(added_time)
    ...
    def _return_workspace_file_url(
        self, shared_id, access_url: str, workspace_id: str, workspace_path: str
    ) -> str:
        if shared_id is None:
            # return private url
    ...
    def download_metadata(
        ...
        Args:
            ...
            metadata_key(str): optional parameter metadata_key, by default takes value as "field_name", /
            takes "original_name" as value when the field names are to be replaced with original name.
        Raises:
              InvalidParameterException:
              InvalidPathException:
              InvalidDirectoryPathException:
