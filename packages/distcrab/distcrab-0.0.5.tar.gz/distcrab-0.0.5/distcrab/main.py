...
class Tar():
    def __init__(self, **kwargs):
        ...
        if kwargs.get('version'):
            ...
            src = BytesIO(get(f'''http://192.168.21.1:5080/APP/develop/develop/update/industry/crab/dists/crab-{version}.tar.xz''').content)
        if kwargs.get('branch'):
            version = BytesIO(get(f'''http://192.168.21.1:5080/APP/develop/develop/update/industry/crab/heads/{kwargs.get('branch')}.txt''').content).read().decode()
            src = BytesIO(get(f'''http://192.168.21.1:5080/APP/develop/develop/update/industry/crab/dists/crab-{version}.tar.xz''').content)
        else:
    ...
    def __iter__(self):
        yield (PurePosixPath(f'''/tmp/crab.tar.xz'''), self.src)
...
def distcrab(ip='192.168.1.200', port=22, username='root', password=None, version=None, branch=None, directory='/usr/local/crab/'):
    ...
    ]).writeArchive(Tar(version=version, branch=branch, directory=directory)).exec([
        '/bin/sync',
        f'/bin/tar -xJf /tmp/crab.tar.xz -C {directory}',
        '/bin/sync',
