...
IInternalHotUpgrade_VERSION = "4.1.65022"
...
class IInternalHotUpgrade(BaseEndpoint):
    ...
    def running(self):
        ...
        response = requests.get(self.base + postUri, params=queryParams, verify=False, headers={
                                'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IInternalHotUpgrade_VERSION}, data=__encoded__)
    ...
    def start(self, onlyReady, mode):
        ...
        __data__ = __HotUpgradeTaskExecutionModeSerDer__().encode(mode)
        __encoded__ = json.dumps(__data__)
        queryParams = {'onlyReady': onlyReady}
