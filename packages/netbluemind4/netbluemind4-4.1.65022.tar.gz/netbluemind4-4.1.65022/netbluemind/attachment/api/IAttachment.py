...
class IAttachment(BaseEndpoint):
    ...
    def getConfiguration(self):
        ...
        response = requests.get(self.base + postUri, params=queryParams, verify=False, headers={
                                'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IAttachment_VERSION}, data=__encoded__)
    ...
    def shareDedup(self, extension, document):
        postUri = "/{extension}/share_dedup"
        __data__ = None
        __encoded__ = None
        postUri = postUri.replace("{extension}", extension)
