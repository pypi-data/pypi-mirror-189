...
IMailboxFoldersByContainer_VERSION = "4.1.65022"
...
class IMailboxFoldersByContainer(BaseEndpoint):
    def __init__(self, apiKey, url, container):
        ...
        self.base = self.base.replace('{container}', container)
    ...
    def getVersion(self):
        ...
        response = requests.get(self.base + postUri, params=queryParams, verify=False, headers={
                                'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IMailboxFoldersByContainer_VERSION}, data=__encoded__)
    ...
    def removeMessages(self, id):
        postUri = "/removeMessages/{id}"
        __data__ = None
        __encoded__ = None
        postUri = postUri.replace("{id}", id)
        queryParams = {}
