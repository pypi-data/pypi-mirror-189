...
class ITask(BaseEndpoint):
    ...
    def log(self):
        ...
        response = requests.get(self.base + postUri, params=queryParams, verify=False, headers={
                                'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': ITask_VERSION}, data=__encoded__)
        return response.content

    def getCurrentLogs(self, offset):
