...
IUserSettings_VERSION = "4.1.65022"
...
class IUserSettings(BaseEndpoint):
    ...
    def set(self, uid, settings):
        ...
        response = requests.post(self.base + postUri, params=queryParams, verify=False, headers={
                                 'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IUserSettings_VERSION}, data=__encoded__)
        return self.handleResult__(None, response)

    def setOne(self, uid, name, value):
        postUri = "/{uid}/_setting/{name}"
        __data__ = None
        __encoded__ = None
        postUri = postUri.replace("{uid}", uid)
        postUri = postUri.replace("{name}", name)
        __data__ = serder.STRING.encode(value)
        __encoded__ = json.dumps(__data__)
        queryParams = {}
