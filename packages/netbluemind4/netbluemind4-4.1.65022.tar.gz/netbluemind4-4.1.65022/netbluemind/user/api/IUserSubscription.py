...
IUserSubscription_VERSION = "4.1.65022"
...
class IUserSubscription(BaseEndpoint):
    ...
    def subscribe(self, subject, subscriptions):
        ...
        __data__ = serder.ListSerDer(
            __ContainerSubscriptionSerDer__()).encode(subscriptions)
        __encoded__ = json.dumps(__data__)
        queryParams = {}

        response = requests.post(self.base + postUri, params=queryParams, verify=False, headers={
                                 'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IUserSubscription_VERSION}, data=__encoded__)
