...
IPublishCalendar_VERSION = "4.1.65022"
...
class IPublishCalendar(BaseEndpoint):
    ...
    def generateUrl(self, mode):
        ...
        response = requests.put(self.base + postUri, params=queryParams, verify=False, headers={
                                'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IPublishCalendar_VERSION}, data=__encoded__)
    ...
    def createUrl(self, mode, token):
        postUri = "/_create/{mode}"
        __data__ = None
        __encoded__ = None
        postUri = postUri.replace("{mode}", mode)
