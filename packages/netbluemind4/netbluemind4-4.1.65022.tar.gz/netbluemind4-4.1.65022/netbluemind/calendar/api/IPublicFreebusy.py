...
IPublicFreebusy_VERSION = "4.1.65022"
...
class IPublicFreebusy(BaseEndpoint):
    ...
    def get(self, email, callerUserUid, callerDomain, query):
        ...
        queryParams = {'callerUserUid': callerUserUid,
                       'callerDomain': callerDomain}

        response = requests.post(self.base + postUri, params=queryParams, verify=False, headers={
                                 'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IPublicFreebusy_VERSION}, data=__encoded__)
