...
IResourceTypes_VERSION = "4.1.65022"
...
class IResourceTypes(BaseEndpoint):
    ...
    def update(self, uid, resourceTypeDescriptor):
        postUri = "/{uid}"
        __data__ = None
        __encoded__ = None
        ...
        __data__ = __ResourceTypeDescriptorSerDer__().encode(resourceTypeDescriptor)
        __encoded__ = json.dumps(__data__)
        queryParams = {}

        response = requests.post(self.base + postUri, params=queryParams, verify=False, headers={
                                 'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IResourceTypes_VERSION}, data=__encoded__)
    ...
    def create(self, identifier, resourceTypeDescriptor):
        postUri = "/{identifier}"
        __data__ = None
        __encoded__ = None
