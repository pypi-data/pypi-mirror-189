...
class IDomains(BaseEndpoint):
    ...
    def all(self):
        ...
        response = requests.get(self.base + postUri, params=queryParams, verify=False, headers={
                                'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IDomains_VERSION}, data=__encoded__)
    ...
    def setDefaultAlias(self, uid, defaultAlias):
        postUri = "/{uid}/_default_alias"
        __data__ = None
        __encoded__ = None
        postUri = postUri.replace("{uid}", uid)
        __data__ = serder.STRING.encode(defaultAlias)
        __encoded__ = json.dumps(__data__)
        queryParams = {}
