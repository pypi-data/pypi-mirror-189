...
class PhenonautPPTX():
    ...
    """
    def __init__(self, cover_title:str="Phenonaut", cover_subtitle:str="",template_file:Optional[Union[Path, str]]=None, experiment_hash:Optional[str]=None):
        ...
        if template_file is None:
            template_file = pkg_resources.resource_filename('phenonaut', 'output/ph0_pptx_template.pptx')
        ...
        slide.placeholders[13].text = f"Generated: {datetime.datetime.now().strftime('%d/%m/%Y, %H:%M:%S')}"
        slide.placeholders[14].text = experiment_hash if experiment_hash is not None else "Hash: NA"

    def add_image_slide(self, title:str, img_file:Union[Path, str],width:Optional[Union[int, float]]=None, height:Optional[Union[int, float]]=None):
        ...
        slide.shapes.add_picture(str(img_file), *self.top_left_content_loc, width=width, height=height)
