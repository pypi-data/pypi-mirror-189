...
''',
    ...
    parser.add_argument('-v', "--version",  action="version", version="pbandplot 0.1.0.2")
    ...
    if os.path.exists(args.input):
        ...
        if args.dos is None:
            ...
            if args.broken is None:
                plots.NobrokenWd(arr, fre, ticks, args.output, labels, args.size, args.vertical, darr, dele, elements, width_ratios, legend, color)
            else:
                broken = args.broken
                if broken[2] >= 1 or broken[2] <= 0:
                    broken[2] = 0.2

                plots.BrokenWd(arr, fre, ticks, args.output, labels, args.size, args.vertical, broken, darr, dele, elements, width_ratios, legend, color)
