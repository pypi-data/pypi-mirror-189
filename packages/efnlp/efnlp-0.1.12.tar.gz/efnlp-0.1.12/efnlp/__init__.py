...
#     def define(self, p: str, t: int) -> int:
#         if len(p) == 0:
#             self.token = t
#             return 0
#         if p[0] not in self.prefixes:
#             self.prefixes[p[0]] = Encoder()
#         self.depth = self.prefixes[p[0]].define(p[1:], t) + 1
#         return self.depth

#     def encode(self, s: str, i: int) -> Optional[Tuple[int, int]]:
#         if i >= len(s):
#             return None
#         if s[i] not in self.prefixes:
#             return (self.token, i+1)
#         return self.prefixes[s[i]].encode(s, i+1)
#         #     return None
#         # result = self.prefixes[s[i]].encode(s, i+1)
#         # if result is None:
#         #     return (self.token, i+1) # leave it to caller to assess next step
#         # return result
...
#     def define(self, p: str, t: int) -> EncoderSet:
#         if len(p) == 0:
...
#         i, result = 0, []
#         while i < len(s):
#             t, i = self.prefixes[s[i]].encode(s, i+1)
#         return result
...
class Language:
    ...
    def encode(self, s: str) -> List[int]:
        ...
        # class Encoder:
        #   depth: int # max(prefixes[*].depth) + 1
        #   token: int # token to assign "here"
        #   prefixes: Dict[str, Encoder]
        #
        #   def encode(self, s: str, i: int) -> Optional[Tuple[int, int]]:
        #       if (i >= len(s)) or (s[i] not in self.prefixes): # second condition access ok
        #           return None
        #       # recursive encode; s[i] _is_ in prefixes
        #       result = self.prefixes[s[i]].encode(s, i+1)
        #       if result is None:
        #           return (self.token, i+1) # leave it to caller to assess next step
        #       # match found within prefixes
        #       return result
        #
        # class EncoderSet:
        #   depth: int # max(prefixes[*].depth) + 1
        ...
        #       return self.prefixes[s[i]].encode(s, i+1)
        #
        # What's a character encoder? Something like
        #
        #   EncoderSet(1, {c: Encoder(0, i, {}) for i, c in enumerate(ascii_letters)})
        #
