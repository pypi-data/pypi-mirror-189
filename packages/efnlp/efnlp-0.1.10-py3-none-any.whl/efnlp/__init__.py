...
class Language:
    ...
    def encode(self, s: str) -> List[int]:
        ...
        #           return None
        #       # recursive encode; s[i] is in prefixes
        #       result = self.prefixes[s[i]].encode(s, i+1)
        #       if result is not None: # match found within prefixes
        #           return result
        #       return (self.token, i+1) # leave it to caller to assess next step
        # 
        # What's a character encoder? Something like
        # 
        #   Encoder(1, ?, {c: Encoder(0, i, {}) for i, c in enumerate(ascii_letters)})
        # 
...
class _SuffixTreeMerge:  # (beam.CombineFn):
    ...
    def merge_accumulators(self, accumulators: List[Optional[AccmType]]) -> Optional[AccmType]:
        ...
        for accm in accumulators:
            if accm:
                assert isinstance(accm, tuple), (type(accm), accm)
