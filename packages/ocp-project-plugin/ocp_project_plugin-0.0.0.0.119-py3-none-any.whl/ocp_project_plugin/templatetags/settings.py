...
@register.simple_tag
def get_ocp_resource_quota_url(cluster_env, app_env):
...
@register.simple_tag
def get_url(app, cluster_env):
    if cluster_env == 'TST':
        if app == 'argocd':
            return ARGOCD_TST_URL
        if app == 'grafana':
...
def count_app_environments(object_id):
    return AppEnvironment.objects.filter(id=object_id).count()


@register.simple_tag
def get_all_app_environments(object_id):
    return AppEnvironment.objects.filter(id=object_id)
