...
class Selenium(Scraper):
    ...
    def on_hover(self, path: str, element: any) -> None:
        element = self.xPath(element=element, path=path)
    ...
    def on_click(self, path: str, element: any=None, timeout: int=0, pos: int=0) -> any:
        if element is None and path is not None:
            element = self.xPath(path=path, element=element, timeout=timeout, pos=pos)
