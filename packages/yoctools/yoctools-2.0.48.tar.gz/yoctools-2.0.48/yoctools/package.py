...
class HardwareInfo:
    def __init__(self, dic):
        ...
        for k, v in dic.items():
            if k in ['cpu_id', 'cpu_name', 'arch_name', 'chip_name', 'vendor_name', 'board_name',
                     'ld_script', 'flash_program', 'toolchain_prefix', 'toolchain_path'] and v:
                self.__dict__[k] = v
            else:
                if type(v) == dict:
                    self.cpu_list[k] = v
        ...
        if type == 'chip':
            if len(self.cpu_list) == 0:
                if not self.cpu_name:
                    return 'Chip component must set hw_info.cpu_name field'
                if not self.vendor_name:
                    return 'Chip component must set hw_info.vendor_name field'
                if not self.ld_script:
                    return 'Chip component must set hw_info.ld_script field'
                ...
                for k, v in self.cpu_list.items():
                    if 'cpu_name' in v and (not v['cpu_name']):
                        return 'Chip component must set hw_info.%s.cpu_name field' % k
                    if (not self.ld_script) and ('ld_script' in v and (not v['ld_script'])):
                        return 'Chip component must set hw_info.%s.ld_script field' % k

        elif type == 'board':
            if not self.chip_name:
                return 'Board component must set hw_info.chip_name field'
        # elif type == 'solution':
    ...
    def update_path(self, path):
        ...
        for k, v in self.cpu_list.items():
            ...
            for k1, v1 in v.items():
                if k1 == 'ld_script' or k1 == 'flash_program':
    ...
    def out(self, hw_key=None):
        ...
        if len(self.cpu_list) > 0:
            ...
            for k in ['cpu_id', 'arch_name', 'vendor_name', 'chip_name', 'cpu_name', 'board_name', 'ld_script', 'toolchain_prefix', 'toolchain_path']:
                if self.__dict__[k]:
...
#   toolchain_path: /home/xxx/csky-abiv2-elf/bin  # <可选项> 编译器的绝对路径
#   cpu0:
...
#   toolchain_path: /home/xxx/csky-abiv2-elf/bin
...
'''
        ...
        if text:
            if hw_key in ['board', 'chip', 'solution']:
                text = text_info + text_head + 'hw_info:\n' + text
...
class build_config:
    ...
    def out(self):
        text = ''
        for k in ['cflag', 'cxxflag', 'asmflag', 'ldflag', 'linktype', 'prebuild_script', 'postbuild_script']:
...
class Package(object):
    ...
    def check_name(self):
        # name
        if not self.name:
            put_string('%s: `name` field cannot be empty!' % self.filename, level='error')
            return False
        elif is_contain_chinese(self.name):
            put_string("%s: `name` field can't be chinese!" % self.filename, level='error')
            return False
        elif len(self.name) > 64:
            put_string("%s: `name` field is too long, max length is 64." % self.filename, level='error')
            return False
        elif not is_legal_name(self.name):
            put_string("%s: `name` field does not conform to naming conventions." % self.filename, level='error')
            return False
    ...
    def load(self, filename):
        ...
        if had_err:
            put_string("warning: '%s' config may be err in %s. You may be fix this by using command like `yoc format name of the component`!" % (conf_str, filename), level='warning')
        ...
        if not self.description:
            put_string('%s: `description` field cannot be empty!' % filename, level='error')
            exit(-1)
        if not (type(self.description) == str or (sys.version_info.major == 2 and type(self.description) == unicode)):
            put_string('%s: `description` field must be a string!' % filename, level='error')
            exit(-1)
        # version FIXME:
        if not self.version:
        ...
        if self.type not in ['solution', 'common', 'board', 'chip', 'sdk']:
            put_string(
                '%s: `type` field must be "solution" or "common" or "board" or "chip" or "sdk".' % filename, level='error')
            exit(-1)
    ...
    def dumps(self):
        text = '''
## 第一部分： 基础信息
{name}{version}{description}{type}{tag}{keywords}{homepage}{author}{license}
## 第二部分：依赖信息
#           指定该组件依赖的组件及版本
...
{export}'''
        ...
        return text.format(
            ...
            type=self.text_out(
                'type', self.type, '# <必选项> 组件类型，为：solution, chip, board, common, sdk'),
            sdk_chip=self.sdk_chip_out(),
            depends=self.depends_out(),
            # depends_forward=self.depends_forward_out(),
    ...
    def save(self, filename):
        text = self.dumps()
        try:
            with codecs.open(filename or self.filename, 'w') as f:
                f.write(text)
            return True
        except:
    ...
    def depends_forward_out(self):
        text = ''
        if len(self.depends_forward) > 0:
            text = 'depends_forward:\n'
            for d in self.depends_forward:
                for k, v in d.items():
                    if ">=" in v:
                        text += '  - {}: "{}"\n'.format(k, v)
                    else:
    ...
    def sdk_chip_out(self):
        text = ''
        if len(self.sdk_chip) > 0:
            text = 'sdk_chip:\n'
            for d in self.sdk_chip:
                for k, v in d.items():
                    text += '  - {}: {}\n'.format(k, v)
                    break
