...
from jhdata.filehelper import *
from jhdata.data_tools.database import create_table_if_not_exists, upsert, insert_one
...
class RequestQueue:
    ...
    def fetch_one(self, use_secrets=True):
        ...
        statement = select(table).order_by(table.c.priority.desc(), table.c.requested_at.desc())
    ...
    def insert_many(self, items):
        ...
        for item in items_parsed:
            insert_one(item, self.table_name, schema_name=self.schema, table_schema=self.request_queue_schema)
