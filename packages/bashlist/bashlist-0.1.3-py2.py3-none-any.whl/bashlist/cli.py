"""This module provides bashlist TO-DO cli"""
# bashlist/cli.py
...
from  bashlist import ( ERRORS, __app_name__, __version__, config, database, bashlist
)
...
def get_todoer() -> bashlist.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please, run "bashlist init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
        return bashlist.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "bashlist init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
