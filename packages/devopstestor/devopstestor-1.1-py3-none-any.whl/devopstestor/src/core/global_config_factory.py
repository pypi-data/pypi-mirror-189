        ...
        for path in [lib_path, client_path]:
            ...
            for file in os.listdir(path):
                ...
                if node_name in base_config.config:
                    ...
                    base_config.config[node_name] = yaml.load(open(file_path), Loader=yaml.Loader)
        ...
        for lpath in list_paths:
            if not os.path.isabs(lpath):
                ...
                if not os.path.exists(lpath):
                    lpath = os.path.join(base_config.config['client_path'], lpath)
                    if not os.path.exists(lpath):
                        lpath = os.path.join(base_config.config['lib_path'], lpath)
                        if not os.path.exists(lpath):
